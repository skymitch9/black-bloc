...
> BUILT on `worktree-agent-abf063b9177e02f17`, not merged, not deployed, not run against Discord)
> and the sweeps count (**95** rows, numbered to 102). Same day, for the EVENTS row (`/event` is
> one command that opens a panel and
...
| Test something | [`sweeps.md`](sweeps.md) — 95 rows in priority order, each with what to expect (+ the detailed phase 1–8a scripts) |
