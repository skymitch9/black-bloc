...
> | Registry keys | **293** on branch `golive-toggle` (`golive_live_author`, §C2 — NOT merged); **292** on `main` after the spotlight-pings and end-wording merges (`spotlight_bump_pings` in, `golive_end_suffix` + `golive_end_mode` out; 293 at v149 — two `posts_versions_*` + `golive_boot_sweep`; 290 at v148) — **25 namespaces** | `len(settings_store.KEY_TYPES)` |
