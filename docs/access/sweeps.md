...
| # | Do this | Expect |
|---|---|---|
| `FD-a` | From any account, run **/ask** in `#blackbloc-logs` | One window only you can see: the heading **Need something?**, the line under it, and three buttons — **Ask staff privately**, **Request something**, **Propose an event** |
| `FD-b` | Press each of the three in turn, backing out each time | **Ask staff privately** raises modmail's own two-field form. **Request something** raises the what / why / needed-by form. **Propose an event** turns the window itself into the event draft card (day, time, where) — the same one `/event` ▸ **Propose an event** opens |
| `FD-c` | Dashboard → https://blackbloc.heygabi.ai/modmail.html → **Doors** → **Front door** → pick `#blackbloc-logs` → **Post the front door** | The card's preview matches what lands: one message in the channel with the same heading, line and three buttons. The card now reads *"It is in #blackbloc-logs"* and offers **Move the front door** and **Take it down** |
| `FD-d` | ⚠️ **The row about the one door per channel.** Before `FD-c`, post the **ticket button** into the same channel (**Ticket button** card, or `/modmail` ▸ **Setup…** ▸ **Ticket button…**). Then do `FD-c` and watch that message | The Open-a-ticket message **disappears** — one door per channel. The Logs page shows `frontdoor.ticket_button_hidden`. The Ticket button card still names the channel, because `modmail_panel_channel_id` keeps its value |
| `FD-e` | Now press **Take it down** on the Front door card, and wait five minutes | The front door goes, and the **Open a ticket** message comes back by itself in the same channel (`modmail.panel_posted` on the Logs page). Nothing had to be re-pointed |
| `FD-f` | With the door posted, press **Ask staff privately** and then **Request something** on the POSTED message | Both raise their form, and every reply is ephemeral — nobody else in the channel learns who pressed. A ticket opened this way reads **the ticket button** in the *Came in by* column on the Modmail page |
...
| `FD-h` | Delete the posted front-door message by hand and wait five minutes | It comes back. The Logs page shows `frontdoor.gone` then `frontdoor.posted` |
| `FD-i` | Re-post the **welcome** post (`/posts` ▸ the welcome post ▸ **Post it**) into the door's channel | The front door is posted again UNDER it, so it still sits directly below the rules: `frontdoor.below_post` then `frontdoor.moved`. Set `frontdoor_follows_post` to `none` and repeat — nothing moves |
| `FD-j` | Set **frontdoor_mode** to `off` (Settings ▸ modmail, or `/settings` ▸ **A setting group…** ▸ modmail), wait five minutes, then run `/ask` | The posted door is taken down (`frontdoor.taken_down`), and `/ask` disappears from the picker while `hide_commands_when_off` is on. With hiding off, `/ask` answers in words, names `frontdoor_mode` and says both ways to turn it back on. Set it back to `on` and re-post from the card |
