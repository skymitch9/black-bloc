...
> temporarily) — secret NAMES only. Last verified: **2026-09-26 08:5x — the `deploy.md` ROW only**, at the v172 ritual: [`../deploys.log`](../deploys.log) is **171** lines (`grep -c .`), the last being **v172** `4b6ce946`. `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
> Before that, **2026-09-25 22:3x — the `deploy.md` ROW only**, at the v171 ritual: [`../deploys.log`](../deploys.log) is **170** lines (`grep -c .`), the last being **v171** `329ea08e`. `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
> Before that, **2026-09-25 19:3x — the `deploy.md` ROW only**, at the v166 ritual: [`../deploys.log`](../deploys.log) is **165** lines (`grep -c .`), the last being **v166** `b06d4f1d`; the row names `deploy.md`'s new flyctl-login gotcha. `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
> Before that, **2026-09-25 16:2x — the `deploy.md` ROW only**, at the v165 ritual: [`../deploys.log`](../deploys.log) is **164** lines (`grep -c .`), the last being **v165** `2351311a`. `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
> Before that, **2026-09-25 14:2x — the `deploy.md` ROW only**, at the v164 ritual: [`../deploys.log`](../deploys.log) is **163** lines (`grep -c .`), the last being **v164** `7787b10c`; the row names `deploy.md`'s *`git pull --rebase` flattens a local merge commit* gotcha (`a460e9a8`). `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
> Before that, **2026-09-23 18:3x — the `deploy.md` ROW only**, at the v163 ritual: [`../deploys.log`](../deploys.log) is **162** lines (`grep -c .`), the last being **v163** `8acfcde2`. `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
...
| [`deploy.md`](deploy.md) | Hosting on Fly.io: first launch, secrets, volume, redeploy, logs. The redeploy path is heavily exercised — **169** lines in [`../deploys.log`](../deploys.log), the last being **v170** on 2026-09-25 22:07 (counted 2026-09-25 19:3x at the v166 ritual; it said 164 / v165). It carries the *flyctl can lose its login mid-day* gotcha (2026-09-25, the v166 launch). It carries the *`git pull --rebase` flattens a local merge commit* gotcha (2026-09-25, `a460e9a8` — why spotlight-ping-windows landed linear; push with `git fetch` + `git merge --ff-only` instead). Its check-clean step carries the *do not touch the tree between launching `deploy.ps1` and its check-clean* gotcha (v162). Its *gate's pytest step* section is the `-n 16` / `-rfE` / junit / 120 s timeout gate, first run for real by v157 |
