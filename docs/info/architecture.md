...
>
> | What | v152 (`main`, 2026-09-21 — re-measured by import, 2026-09-21 10:38) | Where it is measured |
> |---|---|---|
...
> | Schema version | **52** on `main` at v152 — unchanged by v152, which added no column (measured 2026-09-21 10:38); **52** since v151 (measured 2026-09-21: `spotlight_channels.spotlight` / `.announce` / `.youtube_channel_id` / `.youtube_handle`, all additive — the spotlight row became the channel record; ✅ the migration RAN at the v151 boot, four ADDED columns, **no rebuild**); 51 at v150 (`raid_trains.event_id`; **50** in the same release — `golive_fan_roles.spotlight_id` with `user_id` relaxed to NULL, which REBUILT the table at the boot; 49 at v149 `post_versions`; 48 at v148 the `spotlight_*` tables; 47 at v146) | `storage/db.py:SCHEMA_VERSION` |
> | Registry keys | **297** on `main` at v152 (measured by import 2026-09-21 10:38: `len(settings_store.KEY_TYPES)`) — `golive_channel_optout_post` (297, the channel opt-out follow-up; the `golive` + `pings` + `youtube` join set it was filed against reads **52**) in; 296 at v151 — `golive_channel_spotlight_default` (295, channel streamers) and `golive_autolink_presence` (296, auto-link) in; 294 at v150 — `spotlight_bump_pings`, `golive_live_author` and `raidtrain_event_default` in, `golive_end_suffix` and `golive_end_mode` out (293 at v149; 290 at v148) — **24 namespaces** (measured 2026-09-21) | `len(settings_store.KEY_TYPES)` |
...
> | Mock contract | **21 pages / 198 routes / 24 core settings** at v152 — unchanged by v152, which added no page and no route — and at v151 (measured 2026-09-21: `POST /api/golive/links/sweep` is the 198th) — 21/197 at v150 when raid trains became page 21 (the previews were never in `contract.pages`, and they are deleted now); 20/186 at v139, 17/150 at v108, 149 routes at v92. `ls site/public/*.html` re-counted off disk 2026-09-21 09:55: **21** | `site/mock/contract.json`, read 2026-09-21 09:55. ⚠️ **`node site/mock/check.mjs` has NOT been re-run since the `raidtrain-page` landing** (`check: ok - 21 pages, 197 routes, 24 core settings, all keys present`), so 198 is read off `contract.json`, not off a live check |
> | Tests | **7189** (+3 skipped where the receive extension is absent — **KI-31**); 7178 at v151, 7118 at v150 | the v152 deploy gate, **both orders**, read off the v152 [`../deploys.log`](../deploys.log) line (which does carry its gate figure; v151's did not). ⚠️ One of six runs in that gate went red on **2 tests whose names were lost** and five later runs were clean — **KI-35**, `WATCHING` |
...
> | Deploys | **151** lines (counted 2026-09-21 10:38), last `32d3c0b` (v152) at 2026-09-21 10:35 | `../deploys.log` |
>
