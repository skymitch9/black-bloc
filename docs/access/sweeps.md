...
| **339** | Beside a **voice channel**, put `https://twitch.tv/blackbloc` in the box (row 336's gesture), then look at the draft panel's bottom button row before and after the draft is complete | While the draft is still missing something, the bottom row reads **Title & details · Where · Time zone · Back** and an **Open link** button is squeezed in at the end. Once the draft is submittable, **Submit** appears and the **Open link** button quietly goes — ⚠️ five is Discord's hard cap for one row and Submit wins. Nothing is lost: the **Where** line in the card above is still a clickable link. On the review card there is room, so the button stays there |
