...
CLOSE_WOULD_DELETE = (
    ...
    "nothing was closed. Close it with `/modmail close` from the test channel, or wait until the "
    "owner turns test mode off."
)
SNIPPET_NEEDS_BOTH = (
...
def build_router(bot: Any) -> APIRouter:
    ...
    async def modmail_reply(
        ...
        why_not = await send_reply(
            bot, guild, row, actor_for(bot, who, guild), text, anonymous=anonymous
        )
        await note(
            bot,
            guild,
            ...
            who,
            target=row["user_id"],
            details={"ticket_id": ticket_id, "anonymous": anonymous, "delivered": why_not is None},
        )
        return {
    ...
    async def modmail_snippet_save(
        ...
        await save_snippet(bot.db, name, content, int(who["id"]))
        await note(bot, guild, "web.modmail.snippet", who, details={"name": name})
    ...
    async def modmail_snippet_remove(request: Request, name: str) -> dict[str, Any]:
        ...
        if not await remove_snippet(bot.db, wanted):
            raise Refused(404, "no_such_snippet", NO_SUCH_SNIPPET.format(name=wanted))
        await note(bot, guild, "web.modmail.snippet_remove", who, details={"name": wanted})
    ...
    async def modmail_block(request: Request, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        await add_block(bot.db, user_id, int(who["id"]), reason)
        await note(bot, guild, "web.modmail.block", who, target=user_id, reason=reason)
    ...
    async def modmail_unblock(request: Request, user_id: str) -> dict[str, Any]:
        ...
        await remove_block(bot.db, wanted)
        await note(bot, guild, "web.modmail.unblock", who, target=wanted)
