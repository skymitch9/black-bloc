...
- **Wave-1 review findings — what is still open after ENGINEERING SWEEP 3 (v93, 2026-09-05; the
  ...
  **Q2 (create-recurrence web route, "2. A") and Q3 (Logs button `count`/`important_only`, "3. B")
  ...
  to `DONE.md` ("Logs buttons … and Create-a-recurring-poll from the website"). **Open from their
  reports:** 🔴 `POST /api/polls` (the one-off create) does not check `poll_mode` — a staffer can make
  a poll from the website while `/poll` says polls are off; the new recurrence route gates it with
  `409 polls_off`, so the fix is the same three lines in `poll_create` (own build, small); the `logs`
  namespace makes **23** settings groups against `/settings`' 25-option select — two more namespaces
  and the picker needs a `Find…` path like `chat` has; mock `state.pollRecurrences` seeds ids 4/5
  inside `state.polls`' id space (tidy with the `'draft'` one above);
