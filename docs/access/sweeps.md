...
> **2026-09-03** — rows **80–86** added by the polls panel build (`/poll` becomes ONE command that opens a panel; the eleven subcommands go), and rows **6, 8, 9 and 27** were rewritten in place for it rather than added. ⚠️ Rows 80–86 are **BUILT, not yet live** and were not run against Discord (this build cannot reach it). Rows 73–79 belong to the events build in flight; 66–72 to the two before it. Before that — rows **69–72** added by the applications no-role build (a form may keep a
...
> **79** un-exercised rows; none of 66–72 or 80–86 has been run against Discord or the live site.
