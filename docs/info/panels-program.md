...
- **F2 — modmail's in-thread `/reply` `/areply` `/note` `/close`.** ✅ **Decided
  ...
  ticket. **Build A (2026-09-05) leaves all four alone**; the card and the three
  retirements are Build B ([`modmail-panel-design.md`](modmail-panel-design.md) §J).
