...
| Temp voice | `community/tempvoice.py` | `/voice` | 0 | 1 | member + staff | ✅ **BUILT — wave 2**, 2026-09-03, branch `worktree-agent-a386d425f5527fe95`; `/voice` opens the panel and BOTH groups and all twenty-two subcommands are retired, so the top-level count drops by one (**38, measured** through `test_the_command_tree_stays_inside_discords_limits`). Owner fork **F1 = (a)**: the per-channel control post is left exactly as it is, a second door onto the same `do_*` functions. Staff bypass the allowed-role gate and can hand ANY open channel to somebody else, with a DM to the displaced owner. [`voice-panel-design.md`](voice-panel-design.md) |
