...
| `site/public/assets/page-requests.js:1126` | `flashLinked` opens every enclosing `<details>` first: a card inside a shut foldout (Done and Declined both are) has no box to scroll to, so the scroll silently does nothing. Found by rendering, not by reading. |
| `site/public/assets/page-requests.js:1145` | The off-page case: staff fetch `/api/requests/N`, a member scans their own `/mine`, and the result is PINNED above the sections with Back to all rather than paging the reader around. ⚠️ A 404 or a 403 here is "that number is not one you can read", **not** an outage — the two get different sentences, because the fixes are different. `pinned` is set before the fetch so a refresh mid-flight cannot pin twice. |
