...
| Key | Note |
|---|---|
| `black_bloc/requests.py:42` | ⚠️ **A LOOK is not a STATUS.** `LOOKS` has seven entries; `STATUSES` has seven too and they are **different sets**: `filed` and `sent_back` are looks with no state behind them, and `open` and `withdrawn` are states with no card. Everything downstream — the log kind, the colour, the fields, who is DMed, whether a card posts at all — reads the look, never the status. |
| `black_bloc/requests.py:319` | `look_of` is the whole of that distinction in three lines: `in_progress` arrived at **from `review`** is `sent_back`. Storing a `sent_back` flag would have been a second home for a fact the `was:` already carries, and it would drift the first time somebody moved a row by hand. |
| `black_bloc/requests.py:325` | `checked_move` grew two keyword-only text arguments rather than two new functions, because a move is judged in ONE place and the site, the slash commands and the tests all ask this. `TEXT_NEEDED` is keyed by LOOK (`review` needs `built`, `sent_back` needs `note`), which is why the required-text rule fires on a send-back and not on an ordinary pick-up — same target state, different look. |
| `black_bloc/requests.py:141` | `DONE_NEEDS_A_CHECK` fires only from `OPEN_STATUSES`. From `declined` or `withdrawn` the generic "is where a request finishes" sentence is still right, and pointing somebody at `/request ready` there would be a lie. The refusal names both the slash path AND the site button, because the person reading it may be on either. |
| `black_bloc/requests.py:191` | The colours are DATA (`EMBED_COLOURS`), and so are the titles, the field lists and the inline flags. Seven looks by four properties as literals inside one builder is how the sixth one quietly gets the fifth one's colour. |
| `black_bloc/requests.py:455` | `moment` reads `created_at` for the filed card and `decided_at` for every other, so a card is stamped **when the move happened**. A row read back before `decided_at` is written (or a naive ISO string from an older row) falls back to now rather than raising — a card with a slightly wrong timestamp beats no card. |
| `black_bloc/requests.py:465` | ⚠️ **`field_value` returning `""` is what leaves a field OFF the card.** Discord refuses an empty field value outright, so "How to test" on a review with none, "Assignee" on an unassigned row and "Due" with no date are absent rather than blank. Every value goes through `clamp(…, 1024)` here and nowhere else — §J measured the worst look at 2004 of Discord's 6000, entirely because of this line. |
| `black_bloc/requests.py:510` | `site_view` answers `None` when there is no origin, and `post_line`/`dm` pass `view=None` straight through. A link button with a broken URL is worse than no button; `bot.settings.origin` is always set in practice, so this is the belt to the braces. |
| `black_bloc/requests.py:525` | `move_line` is the plain one-liner that REPLACED the sent message — it is now only ever written to the server log, in `post_line`'s two `log.warning` calls, so a card the guard skipped still says in the log what it would have said. |
| `black_bloc/requests.py:666` | `set_status` builds its `UPDATE` from a dict because five columns now have per-move rules and "leave this one alone" was already being faked with `COALESCE`. **A column with no rule for this move is not in the statement at all** — which is how `ready_by` survives a hold-and-resume and how `decided_by` survives a withdrawal. |
| `black_bloc/requests.py:420` | `may_accept` answers True when nothing is recorded in `ready_by`. A row moved to `review` by the landing one-off, or by a `/status` write before this shipped, must not become unacceptable by anybody. |
| `black_bloc/cogs/community/requests.py:316` | ⚠️ **`kind_via(f"request.{look}", via)` — LOOK, not status.** This is the line that makes `request.sent_back` exist at all; the row's status is `in_progress`. `tests/test_logkinds.py:KNOWN_DYNAMIC` is keyed on the source of that expression, so renaming the variable breaks the build loudly rather than silently dropping six kinds out of the classification check. |
| `black_bloc/cogs/community/requests.py:149` | `person_told` is the ONE place that decides who gets a DM, and it is the reason `tell_requester` was renamed: on `sent_back` the person told is the staffer in `ready_by`, not the requester. Nobody is DMed on `review` — it is staff-facing and the channel card is its notice. |
| `black_bloc/cogs/community/requests.py:165` | The `request_dm_on_decision` gate applies to `DM_LOOKS` only, so the send-back DM to `ready_by` is NOT gated by it. That key is documented as the **requester's** switch; widening it to staff notices would give one key two meanings and there would be no way to want one without the other. |
| `black_bloc/cogs/community/requests.py:185` | `post_line` keeps its name and its `line` argument even though the line is never sent — the embed is the message and `content` is `None`. ⚠️ **The guard check is unmoved and unchanged**: TEST_MODE still refuses every channel but the test one and logs `request.notify_skipped_test_mode`, and a mutation that removes the check turns `test_the_review_card_is_skipped_and_logged_while_test_mode_holds_the_channel` red. |
| `black_bloc/cogs/community/requests.py:266` | `apply_decision` passes `ready_by` and `sent_back_reason` on EVERY call and lets `set_status` decide whether this move writes them — the alternative, each caller working out which columns its move touches, is the same rule in five places. The `request_review_by_other` check lives here and not in `accept`, so `POST /{id}/status {"status":"done"}` cannot walk around it. |
| `black_bloc/cogs/community/requests.py:359` | `send_back` is the only one of the three wrappers with a guard of its own: `accept` and `mark_ready` are refused by `TRANSITIONS` when the row is in the wrong state, but `review` → `in_progress` is legal from `open` too, so without this a `/request sendback` on an untouched request would quietly pick it up. |
| `black_bloc/cogs/community/requests.py:449` | `ReadyModal` prefills from the row, so a request sent back and marked ready again does not need retyping. `built` is `required=True` at the Discord level AND refused in words by `checked_move` — the client-side one is a courtesy, the server-side one is the rule. |
...
> NEW; the older `Requests, fourth pass` rows above are deliberately **not** re-keyed here
> (the standing rule: the conductor re-keys the whole file at merge). ⚠️ Several of those
> rows now point at code that MOVED to `black_bloc/panels.py` — `retire`, `on_timeout`,
> `interaction_check`/`went_quiet`, `still_staff`, `db_ready` and `NoteModal.note.label` —
> so trust each note's anchor text over its number until the re-key. Nothing in this wave
