...
| P5a | **Sync the Join To Create A Channel lobby's permissions with its category** — Discord ▸ the channel ▸ Permissions ▸ **Sync now** — so the lobby and the rooms it makes become visible to whoever the *Voice Channels* category shows | owner | immediately after P5. The lobby was moved up into the main voice area at v117 and kept **staff-only** on purpose (owner, 2026-09-17: *"keep its visibility staff only, when we swap off shadow mode it should become visible to whatever permissions inherit from the voice group"*). ⚠️ **The rooms follow the LOBBY, not the category** — `tempvoice_room_overwrites` is `lobby` (v117), so syncing the lobby is the whole job and no setting has to change. Leave that key alone unless you want rooms to ignore the lobby; `category` is the pre-v117 behaviour. ⚠️ Do **not** press **Setup** on `/voice` to do this — `repair_creator_channel` rewrites the lobby's overwrites from its category and would undo a deliberate difference |
