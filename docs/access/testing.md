...
|---|---|---|---|
| Hermetic | `pytest -q -n auto` | every module, every route shape, every panel card, against fakes — **5,889 tests, ~30 s** (2026-09-16, v115 gate) | nothing but the venv — ⚠️ and a CLEAN environment: a shell that exports the real `.env` names (`POLL_VOTE_SECRET`, `DEV_GUILD_ID`, `TWITCH_*`, `GROQ_API_KEY`, `ANTHROPIC_API_KEY`…) turns **nine** "no key" tests red; see `../info/gotchas.md` |
