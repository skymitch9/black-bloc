...
| `black_bloc/raidtrain.py:84` | `_value` reads a sqlite `Row`, a plain dict or an object with attributes. That is what keeps every helper in this file testable with dict fixtures while the cog hands it real rows — the same trick `render_lineup` needs to be a pure function. |
| `black_bloc/raidtrain.py:96` | ⚠️ **Times belong to the POSITION, never to the person.** Slots are created eagerly at `create` with their times already in them, so `swap` moves five holder columns and touches no timestamp. A design that stored times on the claim would have to rewrite them on every reorder, and a half-finished reorder would leave a lineup with two 8pm slots. |
| `black_bloc/raidtrain.py:107` | `ends_at` prefers the SLOT rows' own `ends_at` over `starts_at + minutes × count`. The arithmetic is the fallback for a train whose rows are unreadable; the rows are the truth, because they are what a swap and a future per-slot length would move. |
| `black_bloc/raidtrain.py:148` | `caps_ok` treats **0 as no ceiling**, which is why the settings validator lets the key go to 0 and its max-reason sentence says so. An organizer's `assign` never calls this — D10 says the cap is for self-service claims only. |
| `black_bloc/raidtrain.py:190` | A holder is written `<@id>`, and every send/edit of this text goes out under `AllowedMentions(users=False)` (see the cog's `_mentions`). The mention RENDERS as their name and pings nobody — which matters because the lineup is edited on every claim and a pinging edit would notify twenty people twenty times. Checklist 11. |
| `black_bloc/raidtrain.py:211` | ⚠️ **`render_lineup` is the whole post, and it is re-rendered from the rows every time rather than patched.** One function means the Discord post, `/raidtrain status` and the dashboard's `lineup` field can never disagree about what the lineup says. |
| `black_bloc/raidtrain.py:253` | The reminder names BOTH neighbours with their twitch.tv links, because the thing a slot holder actually needs at T-30 is not "your slot is soon" — it is who to watch for the incoming raid and who to send the audience to. That is the request's own wording ("who they will be raided by and the person they raid next"). |
| `black_bloc/raidtrain.py:279` | ⚠️ **`due_reminders` deliberately excludes a slot whose start has already gone by** (`start <= floor`), and `missed_reminders` picks exactly those up. They are two functions rather than one flag because the two outcomes are different actions — send, versus stamp and stay quiet — and the cog must be able to do the second without the first. |
| `black_bloc/raidtrain.py:315` | `due_checkins` takes the live user ids as an ARGUMENT rather than reaching for `golive.open_sessions` itself. That keeps this module Discord-free and database-free, and it is what makes the check-in window testable without a session table. |
...
| `black_bloc/cogs/content/raidtrain.py:250` | `swap_holders` moves five columns and no time (see `raidtrain.py:96`). `reminded_at` travels WITH the person, so somebody already reminded for 8pm who is moved to 9pm is not reminded again — the alternative, clearing it, would DM them twice about one train. |
...
| `black_bloc/cogs/content/raidtrain.py:622` | ⚠️ **The only role that may be mentioned is the configured ping role.** `users=False` is what makes the in-place edits safe (see `raidtrain.py:190`), and `everyone=False` is what stops a train titled `@everyone` from doing anything. Checklist 11. |
...
| Key | Note |
|---|---|
| `black_bloc/api/tools/raidtrain.py:102` | Every id leaves as a STRING. A Discord snowflake is bigger than a JavaScript safe integer, and a number that quietly loses its last digits on the page is the worst kind of bug — it looks like it worked. |
| `black_bloc/api/tools/raidtrain.py:144` | ⚠️ **`/status` is declared BEFORE `/{train_id}`.** FastAPI takes the first route that matches and `{train_id}` is typed `int`, so the other order would answer `/api/raidtrains/status` with a 422 about a path parameter — a bare status code in front of a person, which the estate rule forbids. The mock's matcher has the same first-match rule and the same declaration order. |
| `black_bloc/api/tools/raidtrain.py:188` | The create route does its own worded 400s and then calls the COG for the lineup post, so one implementation of "post the lineup" serves both surfaces and each audience gets its own words. Same split as `/api/youtube/links`. |
| `black_bloc/api/tools/raidtrain.py:253` | ⚠️ **One route fills and empties a seat**, keyed on `member_id` being null. Two routes would be two places to forget the Twitch-link check, and the page's slot grid has one control per row either way. |
