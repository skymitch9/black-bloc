...
| Row | Do | Expect |
|---|---|---|
| **594** (was `EF-a`) | On the dashboard's **Events** page, press **Make the forum** (or `/event` ▸ **Settings** ▸ **Rooms…** ▸ **Forum…** ▸ **Make the forum**) | `#events` appears as a FORUM under the BlackMail category with six tags — 🟡 pending, 🟢 approved, 📣 live, 🔴 denied, ✅ done, ⚫ cancelled. `events_forum_channel_id` fills in. The reply names test mode and says the forum was claimed. ⚠️ This is the row that proves a forum can be made in this guild at all |
| **595** (was `EF-b`) | Set `events_review_mode` to **forum**, then propose an event with `/event` | A POST appears in `#events` named *Title · YYYY-MM-DD*. Its FIRST message carries the review card, **Approve**, **Deny** and **Not an event — make it a request**, and the post wears 🟡 **pending**. NO channel is made under `events_category_id`. A second message carries **Delete this post**. The proposer is NOT added to the post — they hear by DM |
| **596** (was `EF-c`) | Press **Approve** on the post | The tag becomes 🟢 **approved**, the post stays open, the proposer is DMed, and the announcement goes wherever `events_posts_where` says — into the POST when that is `room` |
| **597** (was `EF-d`) | Propose a second one and press **Deny** with a reason | The tag becomes 🔴 **denied** and the post is **archived** (it drops off the active list, and it is still readable). Nothing is deleted. The proposer is DMed the reason |
| **598** (was `EF-e`) | On a third post, press **Delete this post** as staff and type a line | A modal titled **Remove this post?**. The post goes for good, the event is cancelled, and the DM reads *"staff removed its post."* plus your line. Pressing it as the host instead answers *"Only staff can remove this post"* and nothing happens |
| **599** (was `EF-f`) | With an event still open in a room from before the flip, approve it | The ROOM is renamed `approved-…` exactly as it always was, and no post is made for it. A mode flip reaches new events only |
| **600** (was `EF-g`) | On the dashboard's **Events** page, open the event from row 595 | Its detail card reads **Review post** (not *Review channel*) and its button reads **Remove its post**. The link opens the post |
