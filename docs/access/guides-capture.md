...
| 2026-09-17 11:48–12:0x | ✅ **THE FIRST RE-SHOOT SESSION, end to end at v121** — the hub's **Mark every screenshot stale…** had been pressed (the homes were set and the log channel renamed), so §1 answered **17** and the rows WERE the job for the first time. Purge window 1 → 30, self-test **run #39** (11:49:02, `114 OK · 0 FAILED`, 24 cards, *"they go after 30 minute(s)"*, *"Started from website"*), **14 root cards shot**, **16 guides uploaded** (`media/19`–`media/34`), window → 1, `golive-announce` seen rendering | ✅ **16 of 17 landed; stale 17 → 1.** 🔴 **The one left is `modmail-ticket`, and it is reason 2 of §5, newly true:** the `/modmail` card now prints `Your ticket is open: !Sky · #2` and `#2 @!Sky — forum · !Sky · #2`, because practice ticket #2 is open. It was clean at v111 only because nothing was open; it is a **list of a real person** now and was NOT captured. `media/11` (v111) stays on the guide, still stale. The owner's call: close/withdraw #2 and re-shoot, or draw a mock. ⚠️ Two other cards gained live rows and were judged: `/request` now lists four request TITLES (`Sample request`, `TEST 2`, `TEST`, `Do`) with **no author and no name** — uploaded and flagged, the way `/golive` was in the 09-16 drill; `/golive` itself is now CLEANER than then (`live now — 0`, so no streamer's name). ⚠️ **Two §2 needles had gone stale and cost a pass:** `Raid trains are on` is now **`Raid trains are shadow`**, and `Black Bloc’s settings…` uses a **straight** apostrophe (`Black Bloc's settings for this server`) — match on `settings for this server`. Also found: `find`'s ref for a hidden file input is easy to get by un-hiding just the FIRST step's picker from the page-script tool, which makes the query return exactly one element; and the guide page's `loading="lazy"` picture never loads at all in a BACKGROUND tab — `img.loading='eager'` + re-setting `src` + `img.decode()` forced it (`naturalWidth` 0 → 800) where scrolling could not. §4's refusal table still never fired |
| 2026-09-17 15:03–15:20 | ✅ **THE SECOND RE-SHOOT SESSION, end to end at v127** — §1 answered **5** (the v125–v127 deploys marked them through `release.json`). Purge window 1 → 30 (read back off `/api/settings`, not the page), self-test **run #47** (22:05:29Z, `116 OK · 0 FAILED`, **25 cards**, `via: website`), **3 root cards shot**, **4 guides uploaded** (`media/37`–`media/40`), window → 1, `feature-modes` seen rendering | ✅ **4 of 5 landed; stale 5 → 1.** `request-file` → **`media/37`** (791×527), `voice-room` → **`media/38`** (800×612), `feature-modes` → **`media/39`** (1055×1059), `modmail-ticket` → **`media/40`** (1080×834, the mock redrawn). 🔴 **The one left is `golive-announce`, and it is reason 2 of §5 — the roles have swapped since 09-17 11:48.** `/golive` now reads `live now — 2` and lists **two real streamers by name** (one of them not the owner), so it was NOT captured; `media/19` (v121) stays on the guide, still stale. Re-shoot it when nobody is live, or draw a mock. ⚠️ **And `/modmail` has gone the other way: its card is CLEAN again** (`No ticket is open.`, only role NAMES), so reason 2 no longer applies there — the mock was kept because this session was briefed to keep it, but a real capture is now possible and is the owner's call. The mock was redrawn first: v127 renamed one field, `ticket forum — # modmail` → **`thread parent — # the-main-table`**, so `modmail-mock.html` was edited, re-served on 8799 and re-shot. `/request` was judged clean again (five request TITLES, no author, no name) and shot. ⚠️ **The new trap, and it cost the first zoom of the session: Discord RE-FLOWS between the measurement and the zoom.** A `scrollIntoView` → screenshot → `zoom` sequence put the settings card's region over a *different* card entirely (the crop came back showing `/request` and the front door). Measure the box and fire the `zoom` **back to back, in consecutive calls**, and re-measure after anything that scrolls; older messages were still streaming in from the earlier scroll-up and moving everything. Frame **1543×784** against `innerWidth` **2498** / `innerHeight` **1269** at dpr 1.5 → `k` **0.6177**, `dy` **0.15**. ⚠️ Clamping the crop to the painted union or to `li.bottom - 4` **shaves the last button row's rounded corners**; +3 frame px on the BOTTOM edge (never the top) fixes it. ✅ A page-script `PUT /api/settings/<key>` from `settings.html` is the page's OWN per-key write path (`assets/api.js:154`), and using it instead of **Save Changes** sidesteps the false `1 change pending` (`chat_memory_model`) defect this file records — one key touched, nothing else. One `Runtime.evaluate` timed out at 45 s on the guide page and one call hit an extension disconnect; both worked on the retry, the same way `zoom` does. §4's refusal table still never fired |
| — | 🔴 §4's refusal table | still **never run** — nothing has ever been refused, so it remains written from the code |
...
- **2026-09-17 12:12–12:15 Phoenix — the `/modmail` mock (conductor).** The agent's v121 session withheld `modmail-ticket` because the real card named the owner's own open test ticket; the owner chose a mock. Drawn as `scripts/scan/shots/modmail-mock.html` from the `birthday-mock.html` template (same sampled colours), the v121 forum-mode inbox with two invented tickets; served on 8799, `zoom` to disk — ⚠️ **the first `zoom` timed out twice ("renderer may be frozen") and the retry worked each time**, and the CSS→frame factor was **0.605** here (frame 1512 over `innerWidth` 2498 at dpr 1.5), not the number §3 measured, so measure it every session; the band under the card trimmed with `System.Drawing`; uploaded through the editor with the fields set by page-script (`input` + `change` events, read back) and `file_upload` on the first step's input → **media/35**, `source: mock`, stale **0**.

- **2026-09-17 15:2x Phoenix — the `/golive` mock (conductor).** The v127 re-shoot withheld `golive-announce` because two real streamers were live and named in the card; the owner chose a mock. `scripts/scan/shots/golive-mock.html` from the v121 capture's layout with invented streamers; served on 8799, `zoom` with +35 frame px of bottom slack (the CSS→frame factor under-reads the bottom; §3's `dy`), the band trimmed with `System.Drawing`; uploaded through the editor as `a drawn illustration`, v127. Two mocks now: `/birthday` and `/golive` (and `/modmail`, which could go back to a capture — its card is clean again).
