...
> (a worktree holds no token). 323–326 are done in `#mute-me-bot-test-spam`; **327 needs TEST_MODE lifted**
...
| # | Do this | Expect |
|---|---|---|
| 252 | `/settings` as a Lead in `#mute-me-bot-test-spam` → **Self-test…** | a card titled **The self-test** saying what it will do, that it runs at every boot, where the cards go (`#mute-me-bot-test-spam`) and that they are deleted again after 1 minute(s) (`selftest_purge_minutes` — was 5 until v103, 2026-09-10). Buttons: **Run the self-test · Logs · Back**. ⚠️ **Purge now is NOT there** — nothing has been posted yet, so there is nothing to purge, and a button that answers "nothing to do" is not drawn |
...
| # | Do this | Expect |
|---|---|---|
| 280 | As a **Lead**: `/chat` in `#mute-me-bot-test-spam` → **Personality…** | the same eleven moods in the same order as before this build, each with its label; the footer still says where they came from, now as **Ported from catalog-platform@03dcb91.** Switch one OFF |
...
| 291 | In `#mute-me-bot-test-spam`, ask the bot in chat: *"can i file a request"*, then *"how do i set my birthday"* | it names **`/request` ▸ File a request** and **`/birthday` ▸ Set my birthday** — commands and buttons that exist. ⚠️ It used to say `/request create` and `/birthday set`, both retired by the panel waves, so anybody who followed the advice got Discord's own *"unknown command"*. Same for `/help`'s features list, which also said `/poll create` |
...
⚠️ **Rows 323–326 are done in `#mute-me-bot-test-spam`** (TEST_MODE), as everything else is.
...
| **350** | In `#mute-me-bot-test-spam`, do row 343's gesture beside a **voice channel** — pick the channel first, then press **`Link or place (optional)…`** and type `ttv/skyaiva` | Both halves are there: the **Where** line reads **`#raid-night · twitch.tv/skyaiva`** with the second half in blue, and the review card carries the **Open link** button (row **338**) pointing at `https://twitch.tv/skyaiva`. ⚠️ The link that is STORED is the full `https://twitch.tv/skyaiva`, not the shorthand — so the announcement, the card and the scheduled event's description all show the same thing, and nothing has to know about the table a second time |
| **351** | `/event` ▸ **Propose an event** ▸ fill it in ▸ **Submit**, then open the room Black Bloc just made (`pending-…`) | The review card is **in the room**, not in `#mute-me-bot-test-spam`, and under it a second message: *"This room is Black Bloc's — it goes away on its own **5 minutes after it ends**. Staff can remove it sooner."* with a red **Delete this room** button. The spam channel gets nothing. Approve it from the card and the announcement lands **in the room too** |
| **352** | Leave that approved event until its start time passes (or edit its start on https://blackbloc.heygabi.ai/events.html to a minute from now) and watch the room | The go-live ping — *"**{title}** is starting now!"* — arrives in the room within a minute, pinging only the configured role. Nothing appears in `#mute-me-bot-test-spam` |
...
| **358** | https://blackbloc.heygabi.ai/settings.html ▸ the **Events** group — find **`events_posts_where`**, **`events_room_delete_who`**, **`events_approver_role_id`**, **`events_room_notice`**; set `events_posts_where` to **both** and approve a new event. Then `/event` ▸ **Settings** ▸ **Rooms…** and set it back to **room** | All four keys are on the page with a sentence each. On **both**, the announcement appears in the room AND in `#mute-me-bot-test-spam`. The **Rooms…** page shows the same four in words and writing from either side is one `event.settings` row |
...
| **364** (was `G-e`) | Run `/help` in `#mute-me-bot-test-spam` | Ten lines carry a small **guide** link — `/golive`, `/pings`, `/event`, `/birthday`, `/voice`, `/poll`, `/request`, `/memory`, `/raidtrain`, `/apply` — and the staff-only commands carry none. The last page has one **All the guides** button. Measured: two pages, the longest **1843** of the 1900-character limit |
