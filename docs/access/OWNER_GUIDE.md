...
| See what it has been doing | https://blackbloc.heygabi.ai → **Logs** (after Phase 12 lands: every feature has its own Logs section too; `/golive logs` etc. in Discord) |
| See the team's requests | https://blackbloc.heygabi.ai/requests.html (after Phase 13 lands); members file with `/request` in Discord or on that page |
...
| Quiet the Discord log channel | after Phase 12: Settings → `<feature>_log_level` (off / important / all) |
