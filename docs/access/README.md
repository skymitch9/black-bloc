...
> temporarily) — secret NAMES only. Last verified: **2026-09-25 16:2x — the `deploy.md` ROW only**, at the v165 ritual: [`../deploys.log`](../deploys.log) is **164** lines (`grep -c .`), the last being **v165** `2351311a`. `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
...
| [`deploy.md`](deploy.md) | Hosting on Fly.io: first launch, secrets, volume, redeploy, logs. The redeploy path is heavily exercised — **164** lines in [`../deploys.log`](../deploys.log), the last being **v165** on 2026-09-25 16:15 (counted 2026-09-25 16:2x at the v165 ritual; it said 163 / v164). It carries the *`git pull --rebase` flattens a local merge commit* gotcha (2026-09-25, `a460e9a8` — why spotlight-ping-windows landed linear; push with `git fetch` + `git merge --ff-only` instead). Its check-clean step carries the *do not touch the tree between launching `deploy.ps1` and its check-clean* gotcha (v162). Its *gate's pytest step* section is the `-n 16` / `-rfE` / junit / 120 s timeout gate, first run for real by v157 |
