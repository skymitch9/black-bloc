...
> | Schema version | **51** (v150, `raid_trains.event_id`; **50** in the same release — `golive_fan_roles.spotlight_id` with `user_id` relaxed to NULL, which REBUILT the table at the boot; 49 at v149 `post_versions`; 48 at v148 the `spotlight_*` tables; 47 at v146) | `storage/db.py:SCHEMA_VERSION` |
> | Registry keys | **294** on `main` at v150 — `spotlight_bump_pings`, `golive_live_author` and `raidtrain_event_default` in, `golive_end_suffix` and `golive_end_mode` out (293 at v149; 290 at v148) — **24 namespaces** | `len(settings_store.KEY_TYPES)` |
