...
| **P-f** | Leave the channel at `#welcome` and press **Post it** | It refuses with the test-mode sentence, nothing reaches `#welcome`, and the Logs page carries one `post.would_post` naming `#welcome` |
| **P-g** | Point the post at `#blackbloc-logs`, press **Save Changes**, then **Post it** | One message appears in `#blackbloc-logs` and is pinned. The card's button now reads **Update the post**, and the list wears **posted · pinned** |
...
| **P-n** | Run `/posts` in `#blackbloc-logs` | An ephemeral panel with one line per post, **New post…**, **Turn posts off**, **Logs**, **Open on the site**, and a **A post…** dropdown. Picking one opens a card with the first 300 characters and the same moves |
...
| **P-p** | On the panel press **Turn posts off**, then wait a minute and type `/` | `/posts` is gone from the list, the Posts page says in words that posts are off, and the Logs page carries `post.mode`. Turn it back on from `/settings` ▸ **Turn a feature back on…** |
| **P-q** | ⚠️ The cutover, once the owner says so: point `welcome` at `#welcome`, press **Post it**, then delete Carl-bot's `1285806434050768927` by hand and turn Carl's welcome off | Black Bloc's message is the only welcome post in the channel and is pinned. 🔴 **Never run — this is the one row that touches a real channel, and it needs the test guard lifted first** |
