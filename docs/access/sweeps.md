...
| **P-a** | Open `https://blackbloc.heygabi.ai/posts.html` as staff | A **Posts** row sits under **Runs the server** after Modmail, and the page lists **Welcome and rules** with a **not posted** pill and a **ships with the bot** pill. An amber line at the top says Black Bloc is in test mode and that **Post it** on anything but `#blackbloc-logs` writes down what it would have sent |
| **P-b** | Press **Welcome and rules** and read the box against `#welcome` in Discord | Every word is Carl's, including the `Welcome to** Black` spacing. The counter under the box reads `1 461 / 2 000` |
| **P-c** | Type into the box until the counter passes 1 800, then past 2 000 | The counter turns amber at 1 800 and red past 2 000, and **Save Changes** refuses in a sentence naming both numbers. Nothing is saved |
| **P-d** | Set **Style** to an embed and watch the preview | The cap becomes `4 096`, the preview draws the embed box with the title at the top, and the `#` header line renders as literal `# Familiarize…` — Discord does not draw a header inside an embed |
| **P-e** | Type `<#…>` for a channel, `<@&…>` for a role and `<img src=x onerror=alert(1)>` into the box | The first two draw as `#channel` and `@Role`; the third appears as WORDS. ⚠️ **Nothing pops up** — if a dialog ever appears here, stop and tell a Lead: the preview escapes before it renders anything |
| **P-f** ⚠️ *rewritten 2026-09-16 by the shadow build; the old row assumed `on`* | With the mode left at its default **shadow**, leave the channel at `#welcome` and press **Post it** | It POSTS — into `#blackbloc-logs`, not `#welcome`. Nothing reaches `#welcome`, the pill reads **posted (shadow)**, and the Logs page carries `post.shadow_posted`. ⚠️ To see the old `post.would_post` refusal, set the mode to **on** first (that is PS-e) |
| **P-g** ⚠️ *rewritten 2026-09-16; the shadow default makes the old workaround unnecessary* | Set the mode to **on**, point the post at `#blackbloc-logs`, press **Save Changes**, then **Post it** | One message appears in `#blackbloc-logs` and is pinned. The card's button now reads **Update the post**, and the list wears **posted · pinned**. ⚠️ If a shadow copy was already up, it is DELETED by this press and the Logs page carries `post.shadow_taken_down` |
| **P-h** | Change one word, press **Save Changes**, and read the list | The post wears **changes not yet posted**. Press **Update the post**: the SAME message in `#blackbloc-logs` changes and ⚠️ **no second copy appears**. The pill goes |
| **P-i** | Unpin that message by hand in Discord and wait five minutes | Black Bloc pins it again and the Logs page carries a second `post.pinned` |
| **P-j** | Delete that message by hand in Discord and wait five minutes | The list goes back to **not posted**, the words are all still in the box, and the Logs page carries `post.message_gone` |
| **P-k** | Press **Take it down** | The message goes from `#blackbloc-logs`, every word stays on the page, and the button reads **Post it** again |
| **P-l** | Press **Put the original back**, confirm, and read the box | Carl's text is back byte for byte; the channel you chose is kept |
| **P-m** | Press **Delete this post** on **Welcome and rules** | It refuses in a sentence saying it is the one Black Bloc ships with and to press **Take it down** instead. Make a post with **A new post**, then delete that one: it goes |
| **P-n** ⚠️ *rewritten 2026-09-16: the mode button is now a select* | Run `/posts` in `#blackbloc-logs` | An ephemeral panel with one line per post, **New post…**, **Logs**, **Open on the site**, a **Posts are: off / shadow / on** dropdown showing the stored value, and a **A post…** dropdown. Picking a post opens a card with the first 300 characters and the same moves |
| **P-o** | On a card, press **Edit…** | A modal opens with the title and the whole message already in it. Paste 3 000 characters into a plain post and submit: it is refused in words naming 3 000 and 2 000, and nothing is saved |
| **P-p** ⚠️ *rewritten 2026-09-16: the mode button is now a select* | On the panel pick **Posts are: off**, then wait a minute and type `/` | `/posts` is gone from the list, the Posts page says in words that posts are off, and the Logs page carries `post.mode`. Put it back to **shadow** from `/settings` ▸ **Turn a feature back on…** — ⚠️ that path sets it to **on**, so set it back to shadow from the Posts page's own switch if you are not ready to go live |
| **P-q** | ⚠️ The cutover, once the owner says so: point `welcome` at `#welcome`, set the mode to **on**, press **Post it**, then delete Carl-bot's `1285806434050768927` by hand and turn Carl's welcome off | Black Bloc's message is the only welcome post in the channel and is pinned, and the shadow copy in `#blackbloc-logs` is gone. 🔴 **Never run — this is the one row that touches a real channel, and it needs the test guard lifted first** |

| **PS-a** ✅ *exercised against the MOCK in a browser, 2026-09-16* | Open the Posts page as staff with nothing changed since the deploy | The switch top-right reads **ON · SHADOW · OFF** with **SHADOW** lit, and an amber line says *Posts are in **shadow**: Post it sends the real message to #blackbloc-logs … Turning posts **on** is the go-live* |
| **PS-b** ✅ *mock* | Open **Welcome and rules** and read the line under the box | `shadow — this goes to #blackbloc-logs, not #welcome, until posts are on.` ⚠️ Change **Channel** to `#blackbloc-logs` and it changes to *…which is where it was going anyway* — it must never read `not #blackbloc-logs` about `#blackbloc-logs` |
| **PS-c** ✅ *mock* | Press **Post it** | The message appears in `#blackbloc-logs` and is pinned. The header pills read **posted (shadow) · pinned**, and the list's line reads *The shadow copy is in #blackbloc-logs. It is pinned.* The Logs page carries `post.shadow_posted` |
| **PS-d** ✅ *mock* | Change one word, **Save Changes**, then **Post it** again | The SAME message in `#blackbloc-logs` changes. ⚠️ **No second copy appears**, the **changes not yet posted** pill goes, and the Logs page carries `post.shadow_updated` — not a second `post.shadow_posted` |
| **PS-e** ✅ *mock* | Set the switch to **ON** while the post still names `#welcome`, and press **Post it** | It refuses with the test-mode sentence and the Logs page carries `post.would_post` naming `#welcome`. ⚠️ **The shadow copy is still up** — a refused real post must never drop the rehearsal |
| **PS-f** ✅ *mock* | Set the switch to **OFF** and press **Post it** | It refuses in words naming the Settings page, nothing is sent, and **no** log line is written at all |
| **PS-g** ✅ *mock* | Point the post at `#blackbloc-logs`, set the switch to **ON**, and press **Post it** | A real message goes up, the pill reads **posted** (not *posted (shadow)*), ⚠️ **the shadow copy is deleted**, and the Logs page carries `post.posted` followed by `post.shadow_taken_down` |
| **PS-h** | Run `/posts`, pick **Welcome and rules**, and read the card while the mode is **shadow** | The card carries the same sentence the page does: *shadow — this goes to #blackbloc-logs, not #welcome, until posts are on.* |
| **PS-i** | On the `/posts` panel pick **Posts are: shadow**, then re-run `/posts` | The reply names `#blackbloc-logs` and says nothing reaches members until posts are on; the select comes back with **shadow** already chosen, and the panel's own text repeats it |
