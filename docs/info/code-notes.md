...
| `black_bloc/cogs/content/raidtrain.py:1001` | The order of the refusals is the order they cost: the train's status, then the per-member ceiling, then whether the slot exists, then the race. Each returns a sentence, and the last one is the only one that can be reached by two people at once. |
| `black_bloc/cogs/content/raidtrain.py:1120` | `submit_train` reuses `events.py`'s `start_error` and `START_IN_THE_PAST` rather than writing a second pair of date sentences (checklist 15). The zone is the member's own `/timezone`, so nothing new about time was invented for this phase (D15). |
| `black_bloc/cogs/content/raidtrain.py:1182` | `_numbers` refuses out-of-range sizes BEFORE a row is written, and its sentence names Discord's one-message limit as the reason rather than just quoting a number. |
