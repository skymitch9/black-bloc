...
| `site/public/assets/page-requests.js:236` | ⚠️ **`anchored()` is what makes every Discord card's link button work.** Each card carries `id="r-{id}"`, and `{origin}/requests#r-{id}` is built in `black_bloc/requests.py:request_url`. The two halves are in different languages and nothing type-checks between them — `tests/test_requests.py::test_every_card_carries_one_link_button_to_the_request_on_the_site` pins the URL shape, and this is the other end of it. |
