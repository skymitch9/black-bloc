...
- **Owner 2026-09-01 ~15:2x: the bot invented `#black-support-hub` in a live reply** ("make sure it references real channels, this was a great call by the bot to do this though" — the redirect behaviour is praised, the invented channel is the bug). ⚠️ Also the first confirmed LIVE model conversations. Fix (queued behind the in-flight who-has builder — same files): (1) persona-core rule: only name channels/roles/members present in the grounding or the conversation; (2) deterministic post-check on every model reply — unknown `#channel` tokens swapped for the configured destination and logged (`chat.reply_channel_fixed`, routine) so hallucination frequency is measurable; (3) new key `chat_home_channel_id` (channel, blank default = de-channel the sentence) — both doors. Status: **queued.**
