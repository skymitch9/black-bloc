...
> | Schema version | **50** on branch `spotlight-pings` (`golive_fan_roles.spotlight_id`, and `user_id` relaxed to NULL so a channel can hold a role — NOT merged; `main` is **49**, v149 `post_versions`; 48 at v148 the `spotlight_*` tables, 47 at v146) | `storage/db.py:SCHEMA_VERSION` |
> | Registry keys | **294** on branch `spotlight-pings` (`spotlight_bump_pings`, group `golive`; NOT merged, `main` is **293** at v149 — two `posts_versions_*` + `golive_boot_sweep`; 290 at v148) — **25 namespaces** | `len(settings_store.KEY_TYPES)` |
