...
## Guides (G1) — lettered until the merge numbers them
...
| Row | Do this | Expect |
|---|---|---|
| **G-a** | Deploy, wait for the boot, then open the dashboard's **Logs** page and filter to **Guides** | One **`guide.seeded`** line saying **17**, and no second one on the next boot. A later deploy whose seed wording changed leaves **`guide.seed_refreshed`** with a count instead — never a second `guide.seeded` |
| **G-b** | Signed in as staff, open `https://blackbloc.heygabi.ai/api/guides` in a browser tab | **17** guides, `audience: "staff"`, `may_edit: true`. Ten of them say `audience: "member"`; the seven staff ones carry `/mod`, `/modmail`, `/automod`, `/honeypot`, `/rolemenu`, `/settings` and the staff `/event` |
| **G-c** | Sign out, open the same address. Then sign in with an account that is in the server but holds no staff role | Signed out: **401** and the sentence about signing in with the Discord account you moderate with — never a bare status. As a plain member: **200** with **10** guides, every one `audience: "member"`, and `may_edit: false`. A staff guide's own address (`/api/guides/mod-case`) answers **404** in words |
...
| **G-e** | Run `/help` in `#mute-me-bot-test-spam` | Ten lines carry a small **guide** link — `/golive`, `/pings`, `/event`, `/birthday`, `/voice`, `/poll`, `/request`, `/memory`, `/raidtrain`, `/apply` — and the staff-only commands carry none. The last page has one **All the guides** button. Measured: two pages, the longest **1843** of the 1900-character limit |
| **G-f** | `/settings` ▸ **A setting group…** ▸ **guides** ▸ `guides_help_links` ▸ set it to **false**, then run `/help` again | Not one line carries a guide link and no button is drawn. Set `guides_mode` to **off** instead and the same thing happens, and a member opening the guides page is told in a sentence that guides are turned off |
| **G-g** | Open a guide as staff, change one step's wording, press **Save Changes**, then read the Logs page | Exactly **ONE** `web.guide.edited` line — never a `web.` row and a bare one together — and its detail reads `steps +0 −0 ~1, faults +0 −0 ~0, facts +0 −0 ~0`. Press **Put the original back** on that step and the shipped wording returns |
| **G-h** | Type `Simply press the thing` into a step and save it | The step **saves**, and an amber note beside it says the step opens with **Simply** and that no control is named. ⚠️ The linter warns and never refuses (§D.4) |
| **G-i** | Press **Delete this guide** on one of the seventeen, then make a new one with **New guide** and delete that | The shipped guide refuses in a sentence that tells you to press **Unpublish** instead and says nothing is lost. The one you made deletes, and the Logs page carries `web.guide.created` then `web.guide.deleted` |
| **G-j** | `/settings` ▸ **guides** ▸ `guides_who_edits` ▸ **manage_guild**, then try to save a guide as a staffer without Manage Server | The save is refused in words naming Manage Server and naming the key; nothing is written. A Lead saves the same change normally. ⚠️ The check is made when Save is pressed, so it bites even on an editor that was already open |
