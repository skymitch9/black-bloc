...
LINKED = "**{name}** is linked to {title}. Black Bloc posts when that channel goes live."
...
def build_router(bot: Any) -> APIRouter:
    ...
    async def youtube_links() -> list[dict[str, Any]]:
        ...
        db = require_db(bot)
        return [link_row(guild, row) for row in await all_links(db)]
    ...
    async def youtube_link(request: Request, payload: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            _said, stored = await link_channel(
    ...
    async def youtube_status() -> dict[str, Any]:
        """The key, how many channels are linked, and everything the live probe is doing."""
        db = require_db(bot)
        guild = require_guild(bot)
