...
> | Registry keys | **293** on `main` at v149 (two `posts_versions_*` + `golive_boot_sweep`; 290 at v148) — **25 namespaces**. ⚠️ **291 on branch `end-wording`** (2026-09-20): `golive_end_suffix` and `golive_end_mode` retired into `golive_end_template`'s `{live}` (`info/end-wording-design.md`); NOT merged, so `main` still reads 293 | `len(settings_store.KEY_TYPES)` |
