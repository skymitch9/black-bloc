...
| `black_bloc/cogs/content/raidtrain.py:491` | The clock does all three moves and the poller owns all of them: auto-lock at the start (D11 — fixed, deliberately not a key: a train that keeps taking sign-ups while it runs cannot have a raid order), `→ live`, then `→ done` once the last slot's own `ends_at` has passed. |
| `black_bloc/cogs/content/raidtrain.py:519` | ⚠️ **`reminded_at` is stamped BEFORE the DM is attempted, and that order is the whole point.** A crash between the two loses ONE reminder; the other order would re-send on every sweep for as long as the failure lasted. The same rule as `youtube.record_video` returning False for a duplicate. |
| `black_bloc/cogs/content/raidtrain.py:536` | Checklist 2: `raidtrain.remind` (sent), `raidtrain.would_remind` (shadow) and `raidtrain.dm_failed` (tried and refused) are three different kinds, because the owner's verification method is reading the log and those three must be distinguishable at a glance. |
| `black_bloc/cogs/content/raidtrain.py:570` | Check-in reads `golive.open_sessions` — the one signal, not a second detector. It is also why **KI-15** exists: a holder whose presence is hidden and who is not Twitch-linked is never seen. Accepted, same limit as F1. |
| `black_bloc/cogs/content/raidtrain.py:612` | D7: blank `raidtrain_channel_id` borrows `events_announce_channel_id`, so a server sets up ONE place rather than two. Same pattern as `youtube_channel_id` borrowing `golive_channel_id`. |
| `black_bloc/cogs/content/raidtrain.py:622` | ⚠️ **The only role that may be mentioned is the configured ping role.** `users=False` is what makes the in-place edits safe (see `raidtrain.py:189`), and `everyone=False` is what stops a train titled `@everyone` from doing anything. Checklist 11. |
| `black_bloc/cogs/content/raidtrain.py:638` | The one guarded path to a channel. A channel TEST_MODE refuses is logged as `raidtrain.post_skipped_test_mode` and returns None — never raised, so a refused post can never abort the state change that preceded it (checklist 12: the important thing first, cosmetics last). `no_channel_configured` and `channel_not_visible` are separate named reasons because the fixes differ. |
| `black_bloc/cogs/content/raidtrain.py:715` | The thread is opened on the message that was actually posted, and the guard is told it owns the thread (`own_channel`) so the "train moves" lines may be sent into it while test mode is on — the same allowance temp voice uses for the rooms it makes itself. |
| `black_bloc/cogs/content/raidtrain.py:741` | See §J above: this is the raid-train-owned scheduled-event creator. It refuses under TEST_MODE with `raidtrain.would_create_scheduled` (checklist 1: a side effect the HTTP guard cannot see checks `bot.guard` itself), and it never writes outside `raid_trains`. |
| `black_bloc/cogs/content/raidtrain.py:812` | ⚠️ **The lineup is EDITED, never reposted.** One message is the whole public record of a train, so a member who bookmarked it keeps a live view; a repost would leave a trail of stale lineups and would ping the ping role again each time. A failed edit is a logged `raidtrain.post_failed`, not an exception — the claim that caused it has already been committed. |
| `black_bloc/cogs/content/raidtrain.py:853` | ⚠️ **Mode `off` refuses EVERY member command in words**, naming `/raidtrains mode` — not a hidden command and not a dead one. The staff group deliberately keeps working while off, because `mode` is how you turn it on. |
| `black_bloc/cogs/content/raidtrain.py:872` | D2: organizer = the configured role OR staff. It is not `default_permissions`, because Discord's permission lock cannot express "or this role"; the gate is at runtime and the refusal NAMES who may do it. |
| `black_bloc/cogs/content/raidtrain.py:983` | ⚠️ **Every command that ends up editing the lineup DEFERS first, and every refusal that can be decided without touching Discord answers BEFORE the defer.** Checklist 8 and 24: `_refresh_lineup` fetches and edits a message, message edits are rate-limited, and discord.py sleeps through a 429 — so the 3-second interaction window would expire and the member would see "The application did not respond". `tests/cogs/content/test_raidtrain.py` asserts `response.deferred` on all seven, so removing a defer fails by name. |
| `black_bloc/cogs/content/raidtrain.py:1002` | The order of the refusals is the order they cost: the train's status, then the per-member ceiling, then whether the slot exists, then the race. Each returns a sentence, and the last one is the only one that can be reached by two people at once. |
| `black_bloc/cogs/content/raidtrain.py:1122` | `submit_train` reuses `events.py`'s `start_error` and `START_IN_THE_PAST` rather than writing a second pair of date sentences (checklist 15). The zone is the member's own `/timezone`, so nothing new about time was invented for this phase (D15). |
| `black_bloc/cogs/content/raidtrain.py:1185` | `_numbers` refuses out-of-range sizes BEFORE a row is written, and its sentence names Discord's one-message limit as the reason rather than just quoting a number. |
