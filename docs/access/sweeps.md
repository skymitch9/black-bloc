...
> test can click a Discord button; the deploy booted (selftest 107/0) but nobody has pressed **Where**. 329–334 are done in `#black_bloc-logs` or on the
...
> (a worktree holds no token). 323–326 are done in `#black_bloc-logs`; **327 needs TEST_MODE lifted**
...
All of this happens in **`#black_bloc-logs`** (test mode) or on **https://blackbloc.heygabi.ai**.
...
- **Phase 1 (live; rewritten 2026-09-04 for the panel — every subcommand below is
  gone):** in `#black_bloc-logs` run `/settings` (expect the root panel, and the three core keys
...
- **Phase 3 (live; honeypot `shadow`; rewritten 2026-09-03 for the panel — every
  ...
  to it and you moved in; the control panel goes into `#black_bloc-logs` with a
...
| 55 | Applications — applying | in `#black_bloc-logs`: `/apply` → **Apply for…** → **Twitch Team** (or press **Apply** on the posted panel) | a modal with your five questions; on submit an ephemeral "Sent to staff…", a DM "…is with staff now", and a card with **Approve** / **Deny** in the test channel (test mode redirects it there and the reply says so) |
...
| 101 | Roster, opening and closing, and the Apply button | **A form…** → a no-role form → **Roster** → **Take somebody off…** → a reason; **Back** → **Close it**, then **Open it**; then **Post the Apply button** → pick a channel | the roster is one shorter and they are DMed the reason; the card's "Taking applications" line flips no → yes; the Apply button appears in the channel you picked (test mode refuses any channel but `#black_bloc-logs`, in words). ⚠️ **Delete** is only drawn when nobody is waiting on the form — with somebody waiting it is absent and the card says how many |
...
| # | What | Do this | Expect |
|---|---|---|---|
| 118 | The member panel with the mode off | `/youtube` in `#black_bloc-logs` as a plain member with nothing linked | ONE ephemeral panel titled *Your YouTube channel*: an intro line, a line saying announcements are **off** **and that a Lead changes it with Announcements are… on this panel**, then **Link my channel · Refresh · Open on the site**. **No** picker, **no** Setup, **no** Logs. Nothing anywhere says `/uploads` or `/youtube link` |
...
| # | What | Do this | Expect |
|---|---|---|---|
| 126 | The member panel with the mode ON | `/pings` in `#black_bloc-logs` as a plain member | ONE ephemeral panel titled *Your pings*: your pings written out, **Follow a streamer…**, **Stop following…** if you follow one, the Events toggle, the fan button and **Refresh** — and **no** Streamers…, **no** Settings, **no** Logs, **no** site link (every `/api/pings/*` route is staff-only, so a link would be a sign-in wall). Nothing anywhere says `/pingroles` or `/pings follow` |
...
| # | What | Do this | Expect |
|---|---|---|---|
| 135 | The panel with no channel of your own | `/voice` in `#black_bloc-logs` with no temp channel | ONE ephemeral panel titled *Your voice channel* saying you don't own one and naming the lobby to join — and **no** Rename, **no** Lock, **no** Claim anywhere. Nothing says `/voice rename` or `/tempvoice setup` |
...
⚠️ **Run every row in `#black_bloc-logs`.** While the guard is installed the engine
...
|---|---|---|---|
| 155 | The panel itself | `/chat` in `#black_bloc-logs` as a Lead | ONE ephemeral panel: the mode and tier lines, the turns and money, the notes count, then **Personality… · Knowledge… · Settings**, the two mode toggles, and **Logs · Refresh · Open on the site**. A line points at `/memory` for what the bot remembers about a person — a line, never a button. Nothing anywhere says `/chat status`, `/chat knowledge`, `/chat personality` or `/chat settings` |
...
must be run in `#black_bloc-logs`.
...
| 178 | on `pronouns`: **Post it** → the test channel; then **Move it…** → the same channel; then **Take it down** | one panel, moved not duplicated (it edits the message it already has); taking it down leaves the menu and everybody's roles alone and **Post it** comes back on the card. ⚠️ Any channel but `#black_bloc-logs` is refused in words while test mode is on |
...
⚠️ Every row runs in `#black_bloc-logs`; while test mode is on the trap is created inside
...
card appears in **`#black_bloc-logs`**, not in the ticket channel, because `speak`
...
| 241 | **Roles & channels…** as a staff member who does **not** have Manage Server | the button is **not drawn** and the panel says in one line that re-pointing the staff, log, moderation-log and role-menu channels is for somebody with Manage Server. It is not offered-and-refused; it is not offered. As a Lead it is there, each picker labelled in words (*Where staff talk — and who counts as staff*) and never with the raw key name. ⚠️ Set `settings_core_keys_admin_only` to `false` and plain staff get it. Then open `staff_channel_id`'s own card as a Lead and press **Put the default back**: it asks first — *Are you sure?* naming **#black_bloc-logs** and saying that automod and the honeypot both stop arming, quietly. **Leave it as it is** changes nothing. It is the ONLY key that asks; every other reset is one press |
...
| # | Do this | Expect |
|---|---|---|
| 280 | As a **Lead**: `/chat` in `#black_bloc-logs` → **Personality…** | the same eleven moods in the same order as before this build, each with its label; the footer still says where they came from, now as **Ported from catalog-platform@03dcb91.** Switch one OFF |
...
| 291 | In `#black_bloc-logs`, ask the bot in chat: *"can i file a request"*, then *"how do i set my birthday"* | it names **`/request` ▸ File a request** and **`/birthday` ▸ Set my birthday** — commands and buttons that exist. ⚠️ It used to say `/request create` and `/birthday set`, both retired by the panel waves, so anybody who followed the advice got Discord's own *"unknown command"*. Same for `/help`'s features list, which also said `/poll create` |
...
| # | Do this | Expect |
|---|---|---|
| **300** (was `PD-a`) | `/poll` in `#black_bloc-logs` ▸ **Create** ▸ fill the modal ▸ **Save for later** (bottom row) | the panel comes back with *"Saved. Resume it from this panel any time."*, and the embed now carries a line *"You have a saved draft: **your question** — saved a few seconds ago"* with a blue **Resume draft** button under it. ⚠️ Nothing is posted and no poll number is used up — the draft lives in its own table, never in `polls` |
...
| # | Do this | Expect |
|---|---|---|
| **305** (was `LB-a`) | `/poll` in `#black_bloc-logs` ▸ **Logs** | the log list as before, and now **two buttons under it**: **Show more** and **Important only**. ⚠️ If the log holds fewer than ten lines, **Show more is not there at all** — there is nothing more to show, and a button that would do nothing is not drawn |
...
| # | Do this | Expect |
|---|---|---|
| **310** (was `RW-a`) | Dashboard → https://blackbloc.heygabi.ai/polls.html → **Create a poll** → write a question and two options, pick **#black_bloc-logs**, then the **Repeat** select (directly under the Channel / Ping / Voters / Results / Thread row) → **Every week** | a row of day, time and timezone fields appears, the sentence under the form changes to *"Saves “…” as a template that opens in #black_bloc-logs every Saturday at 19:00 …"*, and the button relabels itself from **Create the poll** to **Save the repeating poll**. ⚠️ Only the field that applies is drawn — **Which day** for weekly, **Day of the month** for monthly, neither for daily |
...
| **317** (was `LG-c`) | `/poll` in `#black_bloc-logs` ▸ **Settings** ▸ turn polls **off**. Then, on the dashboard, https://blackbloc.heygabi.ai/polls.html ▸ **Create a poll** ▸ write a question and two options ▸ **Create the poll** | a red sentence in the form's notice line saying polls are turned off and how to turn them back on — **not** a bare 409, and **not** a poll. ⚠️ Then look at the **Polls** list on that same page: **no new row**, because the refusal happens before anything is written. Before this branch the website made the poll anyway while `/poll` refused |
...
| **319** (was `LG-e`) | Turn polls back **on** from `/poll` ▸ **Settings**, then create a poll from the dashboard again | it posts to `#black_bloc-logs` as it always did, and the **Logs** section at the foot of the polls page shows **one** `poll.created` line carrying `via: website`. Nothing about the ordinary path changed — the gate only bites when polls are off |
...
⚠️ **Rows 323–326 are done in `#black_bloc-logs`** (TEST_MODE), as everything else is.
...
⚠️ **Rows 329–334 are done in `#black_bloc-logs`** (TEST_MODE) or on the website. **Row 335
...
| **350** | In `#black_bloc-logs`, do row 343's gesture beside a **voice channel** — pick the channel first, then press **`Link or place (optional)…`** and type `ttv/skyaiva` | Both halves are there: the **Where** line reads **`#raid-night · twitch.tv/skyaiva`** with the second half in blue, and the review card carries the **Open link** button (row **338**) pointing at `https://twitch.tv/skyaiva`. ⚠️ The link that is STORED is the full `https://twitch.tv/skyaiva`, not the shorthand — so the announcement, the card and the scheduled event's description all show the same thing, and nothing has to know about the table a second time |
| **351** | `/event` ▸ **Propose an event** ▸ fill it in ▸ **Submit**, then open the room Black Bloc just made (`pending-…`) | The review card is **in the room**, not in `#black_bloc-logs`, and under it a second message: *"This room is Black Bloc's — it goes away on its own **5 minutes after it ends**. Staff can remove it sooner."* with a red **Delete this room** button. The spam channel gets nothing. Approve it from the card and the announcement lands **in the room too** |
| **352** | Leave that approved event until its start time passes (or edit its start on https://blackbloc.heygabi.ai/events.html to a minute from now) and watch the room | The go-live ping — *"**{title}** is starting now!"* — arrives in the room within a minute, pinging only the configured role. Nothing appears in `#black_bloc-logs` |
...
| **358** | https://blackbloc.heygabi.ai/settings.html ▸ the **Events** group — find **`events_posts_where`**, **`events_room_delete_who`**, **`events_approver_role_id`**, **`events_room_notice`**; set `events_posts_where` to **both** and approve a new event. Then `/event` ▸ **Settings** ▸ **Rooms…** and set it back to **room** | All four keys are on the page with a sentence each. On **both**, the announcement appears in the room AND in `#black_bloc-logs`. The **Rooms…** page shows the same four in words and writing from either side is one `event.settings` row |
...
| **364** (was `G-e`) | Run `/help` in `#black_bloc-logs` | Ten lines carry a small **guide** link — `/golive`, `/pings`, `/event`, `/birthday`, `/voice`, `/poll`, `/request`, `/memory`, `/raidtrain`, `/apply` — and the staff-only commands carry none. The last page has one **All the guides** button. Measured: two pages, the longest **1843** of the 1900-character limit |
