...
| `black_bloc/settings_store.py:778` | A list key defaults to a **fresh** `[]` on every call, never a shared module-level list — a cog that appended to the default would otherwise change the default for every guild that has not set one. |
...
| `tests/test_settings_store.py:75` | Three tiny fakes stand in for `Role`, `Permissions` and `Member`. They work because the staff derivation duck-types (`black_bloc/settings_store.py:576`) — a test that had to build real Discord objects would be a test nobody writes. |
...
| `black_bloc/settings_store.py:827` | ⚠️ **`chat_mode` defaults to `on`**, which is the opposite of every mode key that can act on a person. The owner asked for the bot to answer when pinged; a feature that ships off is a feature nobody sees. Under `TEST_MODE` the guard already confines it to `#mute-me-bot-test-spam` and DMs, so "on by default" cannot reach the server until the owner lifts test mode. |
...
| `black_bloc/polls.py:293` | `NeedsPanel` is a `ValueError` **carrying the sentence**, not a flag. That is the whole shape of decisions 5 and 6: a poll asking for anonymity or hidden results is **refused in words**, never quietly downgraded to a public one. Downgrading is the failure mode that matters here — "anonymous" that isn't is a lie to every voter. |
| `black_bloc/polls.py:519` | `surface_for` checks in one deliberate order: unknown kind → panel-owned kind → anonymous → hide-until-close → more than ten options. The first thing that needs the panel is what the person is told about, so a poll asking for two impossible things does not have to be submitted twice to learn both. Every branch ends with the same clause, `NEXT_UPDATE` (`:164`), so the promise reads identically wherever it surfaces. |
| `black_bloc/polls.py:319` | `validate` returns a **sentence or None**, never raises, because it is the fast path and every caller answers with it directly. It catches the four measured native limits (question 300, label 55, at least 2 answers, whole hours 1–768) plus case-folded duplicate labels — two identical answers split a vote and no reader can tell them apart, so it is refused rather than posted. |
| `black_bloc/polls.py:342` | `whole_hours` rejects `bool` before `int` on purpose: `True` is an `int` in Python and would otherwise be read as a one-hour poll. |
| `black_bloc/polls.py:566` | `bar` divides by the **total**, and a poll nobody has voted in has a total of 0 — hence the explicit zero branch rather than a try/except. The chart is 20 cells of `█`/`░`, computed with no Discord anywhere, which is what makes it unit-testable. |
| `black_bloc/polls.py:581` | `ranked` sorts by `(-votes, position)`, so a tie keeps the order the options were **written in** rather than whatever the dict happened to yield. |
| `black_bloc/polls.py:587` | `winners` returns a **list**, and `results_text` marks a winner only when that list has exactly one entry. `Poll.victor_answer` (`discord/poll.py:511`) is always `None` until the poll has finished, so the renderer never asks Discord who won — it counts. A genuine tie is named in a `Winner` field on the embed (`:335`) instead of two rows both claiming it. |
| `black_bloc/polls.py:614` | `average_rating` returns `None` — never 0 — the moment a label is not a number, so a `kind="rating"` poll whose options were overwritten does not print an invented mean. |
| `black_bloc/polls.py:895` | `counts_from_options` is the one shape (`position` / `label` / `votes`) that the embed, the API row, the CSV and the mock all read, so a fifth caller cannot invent a sixth spelling. |

### `black_bloc/cogs/community/polls.py` — everything that touches Discord

| Key | Note |
|---|---|
| `black_bloc/cogs/community/polls.py:531` | ⚠️ **`guard_allows` is the whole test-mode story for this feature, and it is asked BY HAND at every acting site.** `guard.py:101-142` patches exactly `http.send_message`, `http.edit_message` and `http.delete_channel`. `http.end_poll` is **not** among them (`discord/message.py:1897`), and neither is the voters endpoint — so `poll.end()` and `PollAnswer.voters()` would reach a real channel with the guard installed and silent. Create, end, cancel, remind, post-results and read-voters each ask first (`:531`, `:592`, `:696`, `:733`, `:1071`, `:1416`), exactly as `cogs/moderation/automod.py` does for delete and timeout. |
| `black_bloc/cogs/community/polls.py:881` | ⚠️ **The answer ids are read back off the posted message, not guessed.** `Poll.add_answer` numbers answers `len(self.answers) + 1` locally (`discord/poll.py:621`), but the ids that arrive on gateway vote events are Discord's. `_answer_ids` therefore reads `message.poll.answers` and falls back to `1..N` only when the count does not match — a guessed id would file every vote against the wrong option. |
| `black_bloc/cogs/community/polls.py:872` | `_expiry` prefers `message.poll.expires_at` over our own arithmetic, because Discord rounds a duration to whole hours when it reads one back (`discord/poll.py:452`) and the loop's close time must be **Discord's**, or the two drift by up to an hour. |
| `black_bloc/cogs/community/polls.py:382` | The reminder is claimed by a **conditional** `UPDATE … WHERE reminded_at IS NULL` whose `rowcount` decides whether to send. Claim-then-send, not send-then-mark: a restart between the two would otherwise double-ping the channel. `clear_reminder` (`:300`) releases the claim when the send fails, so a failure is not recorded as a last call — checklist item 2, with distinguishable kinds (`poll.reminded` vs `poll.remind_failed`). |
| `black_bloc/cogs/community/polls.py:3114` | `_reminder_horizon` takes the **widest** `poll_reminder_minutes` across every guild for the SQL window, then `_remind` re-checks each row against its own guild's setting (`:1051`). One query, no per-guild round trip, and a guild with a short window cannot be reminded early by a guild with a long one. |
| `black_bloc/cogs/community/polls.py:711` | `_write_results` calls `end_poll()` **only when Discord has not finalised the poll itself**. Native polls close on their own and Discord posts a `poll_result` system message; the loop's job is to read the final counts and archive them. Counts are approximate while open and exact once finalised (`discord/poll.py:206-211`), which is why the numbers are read *after* the end call, not before. |
| `black_bloc/cogs/community/polls.py:625` | ⚠️ **`refresh_voters` is why a missed gateway event is not a lost vote.** The raw listeners keep a running `poll_votes` tally, but they can miss (a disconnect, a restart); at close the voter list is read once from `PollAnswer.voters()` and upserted, which heals it. It returns on the first failure rather than half-filling a list, and it never runs for an `anonymous` row — a poll sold as anonymous must not leave names in our database either. |
| `black_bloc/cogs/community/polls.py:1207` | `_vote` listens to `on_raw_poll_vote_add` / `_remove` only, per the measurement above, and ignores any message that is not a Black Bloc poll, is `anonymous`, or is on a non-native surface. |
| `black_bloc/cogs/community/polls.py:778` | `cancel_poll` writes the status and the log line **inside** the lock and ends the vote at Discord **outside** it: the irreversible thing (the row) lands first, cosmetics after (checklist 12). A cancelled poll is still ended at Discord — leaving a vote running that Black Bloc has forgotten is worse than an unfinished cancel. |
| `black_bloc/cogs/community/polls.py:911` | `apply_decision` sets `open` before posting and **cancels the row when the post fails**. Without that the poll would sit `open` with no `message_id` and no `closes_at`, which the due-poll sweep (`:381`, `closes_at IS NOT NULL`) can never pick up — a permanently open row nobody can close. |
| `black_bloc/cogs/community/polls.py:887` | The auto-thread flag lives **on the poll row** (`polls.auto_thread`), not read from the setting at post time. A poll held for review can be approved days later, and the answer to "did this poll ask for a thread" must not change because a Lead flipped the setting in between. The thread is handed to `guard.own_channel` so Black Bloc may speak in it under test mode. |
| `black_bloc/cogs/community/polls.py:570` | `card_channel` sends the review card to `staff_channel_id`, or to the test channel while the guard is installed — the `events.py` shape, one line rather than a second copy of the rule. |
| `black_bloc/cogs/community/polls.py:1172` | The archive runs in the same 5-minute loop and logs **once per pass, only when something moved**, with the ids and the dropped-vote count. `poll_results` is never deleted: decision 14 is "archive, never delete"; only the per-voter rows go, and only when `poll_archive_drop_votes` says so. |
| `black_bloc/cogs/community/polls.py:397` | `save_results` writes `poll_options.final_votes`, one `poll_results` row and `polls.total_votes` together. The `poll_results` row is what survives the archive, which is why the totals are duplicated there rather than re-derived from options that may be gone. |
...
| `black_bloc/polls.py:346` | `parse_day` takes `2026-09-05` **and** `2026-09-05 19:00` and reads both in the server's zone (`America/Phoenix`, no DST, the value birthdays already uses), returning UTC. `timezones.parse_start` was not reused: it accepts only the long form, and a day poll whose slots are whole days should not have to type `00:00`. |
| `black_bloc/polls.py:359` | ⚠️ **`clock_label` builds the twelve-hour time by hand because Windows `strftime` has no `%-I`** — `%I` pads (`07 pm`) and the platform-specific unpadded flag is `%-I` on POSIX, `%#I` on Windows. Building it from `hour % 12 or 12` is the only form that reads the same on both. |
| `black_bloc/polls.py:393` | `date_slots` decides **whether the label carries a time** from the ask rather than from a flag: an hourly step always does, and a daily step does only when the start itself had a time on it. A seven-day poll of "Sat 05 Sep" reads better than seven copies of "· 12 am". |
| `black_bloc/polls.py:378` | `date_trouble` is checked **before** any slot is generated and returns the sentence; `date_slots` returns `[]` for an unreadable start rather than raising, so a caller that skips the check gets nothing rather than a half-laid-out poll. |
| `black_bloc/polls.py:519` | ⚠️ **`surface_for` no longer refuses anonymity, hidden results or eleven options — it returns `panel`.** It still raises `NeedsPanel` for a kind no surface carries (free text, number, ranked) and for more than **25** options, which is the panel's own button ceiling. That is the whole shape of 10b: three of 10a's four refusals became a surface. |
| `black_bloc/polls.py:532` | `panel_reason` returns the **first** thing that rules native out, in a fixed order (anonymous → hidden → too long), so a poll asking for two impossible things names the one the person most likely meant rather than listing both. `panel_note` (`:527`) wraps it in the sentence the creation flow shows. |
...
| `black_bloc/polls.py:426` | `cadence_token` is one string (`daily`, `weekly:sat`, `monthly:12`) so the row carries a cadence in one column and the loop parses it in one place. |
| `black_bloc/polls.py:459` | ⚠️ **`MAX_MONTH_DAY` is 28 and the refusal says why: February.** A monthly poll set for the 31st would silently skip four months a year, and clamping it to "the last day" quietly changes what somebody asked for. Refusing with the reason is the only honest option. December rolls into January by `divmod(month, 12)` rather than by date arithmetic that would throw on the 13th month. |
| `black_bloc/polls.py:513` | `whole` replaced the bool-before-int guard `whole_hours` had, and `whole_hours` (`:326`) now calls it. `True` is an `int` in Python and would otherwise be read as one hour, one slot or a one-day step. |

### `black_bloc/cogs/community/polls.py` — the panel, and everything it touches

| Key | Note |
|---|---|
| `black_bloc/cogs/community/polls.py:677` | ⚠️ **`voter_key` is how an anonymous poll counts one vote each without keeping who cast it.** A panel MUST store a row per voter or a person could vote ten times; a poll sold as anonymous must not leave member ids in the database. So for `anonymous` rows the stored `user_id` is the top 63 bits of `sha256("<poll_id>:<user_id>")` — per-poll salted, so the same person is a different number in every poll, deterministic enough for "clear my vote" to find their row, and useless to anybody reading the table. `poll_votes.user_id` stays an INTEGER; nothing else changed. |
| `black_bloc/cogs/community/polls.py:729` | `set_panel_vote` writes a person's **whole answer as a replacement** — delete every row of theirs, insert what they now hold. An append-with-toggle would drift the moment two clicks raced; a replacement is idempotent and the button computes the new set from `my_positions` (`:626`) first. |
| `black_bloc/cogs/community/polls.py:956` | `cast_vote` takes the per-poll `asyncio.Lock` **around the write only**, then repaints outside it: two people pressing at once serialise on the row, and the message edit (which can be rate-limited) never holds the lock. Checklist 6, with the DB's own unique index as the second guard. |
| `black_bloc/cogs/community/polls.py:897` | `panel_view` gives a button per option up to **five** and one "Vote" button past that. Five is not Discord's cap (25 over five rows) — it is where a row of buttons stops being readable, and past it the modal shows the same options with a scrollbar instead. |
| `black_bloc/cogs/community/polls.py:1056` | ⚠️ **`vote_picker` is where 2.7.1's typed modal fields earn their place, and where their real cap bites: both `RadioGroup` and `CheckboxGroup` are capped at TEN options in the library** (`discord/ui/radio.py:63`, `discord/ui/checkbox.py:71-73` — "between 2 and 10", "up to 10 items"). So the picker is a `RadioGroup` for one choice, a `CheckboxGroup` for many, and a plain `Select` (25 options, `required=False`, only valid in a modal) once there are more than ten. The research's §4.4 typed-modal design is real, but only for short polls. |
| `black_bloc/cogs/community/polls.py:1093` | `picked_values` reads `.values` when the component has one and `.value` when it does not, so the three pickers above are interchangeable to the caller. A `RadioGroup` has `value`; `CheckboxGroup` and `Select` have `values`. |
| `black_bloc/cogs/community/polls.py:1101` | The modal opens **pre-filled with what that person already picked** (`default=` per option), because a modal that forgets your answer makes "change one thing" mean "type it all again". Submitting it with nothing picked says `PICK_SOMETHING` and changes no vote — clearing is a separate button, so an empty submit is a slip rather than an instruction. |
| `black_bloc/cogs/community/polls.py:879` | `panel_card` is the one home for the panel's embed, so the post, every vote's repaint and the close all render the same card from the same row. The alternative — three call sites each passing its own arguments to `panel_embed` — is how a poll ends up saying `open` after it closed. |
| `black_bloc/cogs/community/polls.py:911` | `repaint_panel` asks `guard_allows` **and** swallows its own errors: a failed repaint must never fail the vote that caused it. The vote is the irreversible thing and the picture is cosmetics (checklist 12). |
| `black_bloc/cogs/community/polls.py:1251` | ⚠️ **`_write_panel_results` never calls `end_poll`, because a panel has no poll at Discord to end.** The whole native close path — `end_poll`, `counts_from_message`, `refresh_voters` — is skipped; the counts come from `poll_votes`, which is the authority for this surface. `_shut_panel` (`:1172`) then takes the buttons off and puts the final bars on, including for a poll that had been hiding them. |
| `black_bloc/cogs/community/polls.py:349` | ⚠️ **`poll_plan` is the one place a poll's arguments become a poll**, and it is why `POST /api/polls` cannot accept something the `/poll` panel would refuse. Validation, slot generation, surface derivation and the panel sentence all happen here; the two callers only decide where it goes and who is told. |
| `black_bloc/cogs/community/polls.py:401` | `store_poll` writes the row, its options and the `poll.created` line together and answers *(the row, is it held)*. `status=` overrides the review switch, which is how the recurrence loop opens an occurrence without sending it back for approval — a Lead approved the recurrence once, not every Tuesday. |
| `black_bloc/cogs/community/polls.py:338` | `add_options` gained `values`: `poll_options.value` carries the ISO instant a date slot stands for, so a slot survives its own label. If `poll_date_labels` is flipped later, the stored instants are still the truth about which evening won. |
| `black_bloc/cogs/community/polls.py:464` | ⚠️ **`polls_by_status` now excludes `recurrence IS NOT NULL`.** A template is not a poll: without this it would show up on the `/poll` panel, on the dashboard index and in the pending-review queue, as a poll nobody can vote in. `recurrences` (`:381`) is the query that wants them. |
| `black_bloc/cogs/community/polls.py:509` | ⚠️ **`claim_occurrence` is what makes the recurrence loop idempotent.** It moves `recur_next_at` on with `WHERE recur_next_at = <what we read>` and only opens the poll when `rowcount` says it got there first. Reading-then-opening-then-writing would post the same occurrence twice across a restart, exactly the way the reminder would have without `claim_reminder`. |
| `black_bloc/cogs/community/polls.py:3059` | `_recur` computes the NEXT time before claiming this one, and a cadence it cannot read **pauses the template** (`recur_next_at = NULL`) with a `poll.recur_failed` line rather than being retried every five minutes forever. Checklist 5: a bad row is finished, never left blocking. |
| `black_bloc/cogs/community/polls.py:1523` | Pause is `recur_next_at = NULL` — no new column, and `due_polls` already requires the column to be non-NULL, so a paused template is simply never due. **Resume** recomputes the next time rather than restoring an old one, so resuming after a fortnight does not fire immediately (`resume_recurrence`, `:1541`). |
| `black_bloc/cogs/community/polls.py:1568` | ⚠️ **Delete does not delete.** `delete_recurrence` sets the template to `cancelled` and clears its next time; the row stays, the occurrences it opened keep a `schedule_id` that still resolves, and the archive sweep tidies it away on the usual schedule. Decision 14 is "archive, never delete", and a dangling `schedule_id` is a worse answer than a cancelled row. |
| `black_bloc/cogs/community/polls.py:2717` | ⚠️ **A date poll is refused a recurrence, in words.** Its slots are fixed days; the second time round it would be asking about a day that has been and gone. `CadenceModal.on_submit` says so before anything is stored, and `RepeatButton` is not rendered for a date draft at all. |
| `black_bloc/cogs/community/polls.py:1727` | `send_review_card` was lifted out of the cog method so `POST /api/polls` can hold a poll for review too. It answers *(where it went, the card)* and distinguishes "no staff channel at all" (`None, None`) from "the send failed" (`target, None`) — the first cancels the poll, the second leaves it decidable by hand. |
...
- **Nothing has been deployed and nothing merged.** `poll_mode` ships `on`, so
  merging this makes `/poll` live for staff the moment the bot
...
| `black_bloc/requests.py:431` | ⚠️ **`ORDER BY (status <> 'open'), id DESC` — open first, then newest (it said `'pending'` until Phase 17 renamed the state) — is done in SQL, and `limit`/`offset` are passed straight through.** The first cut read every row and sliced in Python, which is fine at fifty requests and wrong at five thousand. `page_of` at `:241` still exists, but only for the SLASH command, where the rows are already in hand and a Discord message caps the list anyway. |
| `black_bloc/requests.py:491` | ⚠️ **`set_status` clears `decline_reason` on any move that is not a decline (or, since Phase 17, a hold — the hold reason lives in the same column, and `held_from` is written on the way into hold and cleared on the way out), and stamps `done_at` only on the move to `done`.** A request declined "we already have one" and later approved would otherwise still be carrying the sentence that says it was refused, and the page would show both. `decided_by`/`decided_at` use `COALESCE`, so a later move that names nobody (a withdrawal) does not erase who decided it first. Pinned by *moving to done stamps when and moving off declined forgets the reason*. |
| `black_bloc/requests.py:522` | `set_fields` takes `...` for "leave it alone" and `None` for "clear it", and returns the names it actually wrote. That is what lets `POST …/status` tell a save that changed nothing (refused) from one that cleared a field (allowed), and what keeps `web.request.updated`'s `changed` list honest. |

### What a person may type

| Key | Note |
|---|---|
| `black_bloc/requests.py:289` | `parse_due` uses `date.fromisoformat`, so `2026-02-30` and `2026-13-01` are refused as firmly as `next tuesday` — a shape check with a regex would have taken both. An empty box is `None`, never an error: the due date is optional and a member who leaves it blank has not made a mistake. |
| `black_bloc/requests.py:301` | ⚠️ **`due_on` is a plain `YYYY-MM-DD` STRING everywhere — stored, sent and taken — and only ever becomes an instant at render time.** `due_stamp` builds `<t:…:D>` from **local midnight in the server's zone** (`timezones.DEFAULT_TZ`, America/Phoenix, a fixed UTC-7), so the stamp reads as the day the person typed rather than the day before it. Midnight UTC would flip the date for every reader west of UTC, the owner included; an ISO instant on the wire would make the dashboard guess a zone. Pinned by *the due stamp is local midnight in the server's zone, not UTC*. |
| `black_bloc/requests.py:412` | `checked_fields` refuses What before Why, so the sentence a person gets names the box they left empty rather than "fill it in". Both are then cut to 1000, which is what the modal's `max_length` already enforces — the API has no such enforcement, so the cut is the one that matters. |
| `black_bloc/requests.py:67` | `STATUS_WORDS` is the vocabulary in one place, and the API sends it as `status_word` beside `status`. Without it the dashboard would carry a second copy of the mapping and the two would drift the first time a state was renamed. |
...
| `black_bloc/settings_store.py:810` | The default is `important` for every one, matched on the `_log_level` suffix rather than a twelve-way `if`. |
...
| `black_bloc/settings_store.py:960` | `chat_staff_can_ping_roles` defaults **on**, which is what the owner asked for, and is safe because the gate above it is `is_staff`. A server that wants a bot that can never ping anything flips one switch. |
...
| Key | Note |
|---|---|
| `black_bloc/requests.py:24` | ⚠️ **`TRANSITIONS` is the one table, and every path asks it** — `moves_from`, `can_move`, `checked_move`, the API's `moves` field and the page's buttons all read this and nothing else. `STAFF_STATUSES` and `FINAL_STATUSES` are DERIVED from it rather than written out again, so adding a state to the table is the whole change. |
| `black_bloc/requests.py:334` | A refused move says where it IS, where it cannot go, and where it CAN go, by name. `moves_sentence` is what makes the last part true from the table rather than from a hand-written list that would drift. |
| `black_bloc/requests.py:360` | `checked_move` is the whole gate in one place: same-state, illegal move, and reason-required. The cog, the API's `/status`, `/hold` and `/decline` all go through it, so a rule cannot be enforced on one surface and not the other. |
| `black_bloc/requests.py:732` | ⚠️ **`held_from` is written on the way INTO hold and cleared on the way out, in the same UPDATE.** Two statements would leave a window where a resumed request still claims to be parked. `decline_reason` carries the HOLD reason as well as the decline one — the column name is a persisted key and changing it would be a migration, so the note is here instead. (Third pass: this is now a dict-built statement covering five per-move columns — see the `Requests — third pass` section.) |
| `black_bloc/requests.py:846` | `resume_target` falls back to `in_progress` when `held_from` is `open`: `open → hold → open` would be a loop with no work in it, and the owner's machine has resume meaning "pick it back up". |
| `black_bloc/requests.py:470` | `row_value` exists so a schema-22 row read before the `held_from` column is added reads as nothing rather than raising. `sqlite3.Row` throws `IndexError` on an unknown column, not `None`. (It does the same job again for schema 26's four columns.) |
| `black_bloc/requests.py:435` | `status_channel_id` is the fallback rule in one place: `request_status_channel_id` if set, otherwise `request_notify_channel_id`, so one channel carries both kinds of line by default and nobody has to set two keys to get any output. |
| `black_bloc/requests.py:181` | ⚠️ **REWRITTEN by the third pass, not renumbered: the four `NOTIFY_*` constants this note described are GONE.** The channel message is an embed now, and `MOVE_LINE` is the seven-look plain-text table that survives only for the server log. The owner's original point still holds and is why it is a table of seven fixed shapes rather than one templated setting: the shapes differ, so a single `request_status_template` would be worse. |
...
| Key | Note |
|---|---|
| `black_bloc/requests.py:48` | ⚠️ **A LOOK is not a STATUS.** `LOOKS` has seven entries; `STATUSES` has seven too and they are **different sets**: `filed` and `sent_back` are looks with no state behind them, and `open` and `withdrawn` are states with no card. Everything downstream — the log kind, the colour, the fields, who is DMed, whether a card posts at all — reads the look, never the status. |
| `black_bloc/requests.py:372` | `look_of` is the whole of that distinction in three lines: `in_progress` arrived at **from `review`** is `sent_back`. Storing a `sent_back` flag would have been a second home for a fact the `was:` already carries, and it would drift the first time somebody moved a row by hand. |
| `black_bloc/requests.py:378` | `checked_move` grew two keyword-only text arguments rather than two new functions, because a move is judged in ONE place and the site, the slash commands and the tests all ask this. `TEXT_NEEDED` is keyed by LOOK (`review` needs `built`, `sent_back` needs `note`), which is why the required-text rule fires on a send-back and not on an ordinary pick-up — same target state, different look. |
| `black_bloc/requests.py:155` | `DONE_NEEDS_A_CHECK` fires only from `OPEN_STATUSES`. From `declined` or `withdrawn` the generic "is where a request finishes" sentence is still right, and pointing somebody at `/request ready` there would be a lie. The refusal names both the slash path AND the site button, because the person reading it may be on either. |
| `black_bloc/requests.py:229` | The colours are DATA (`EMBED_COLOURS`), and so are the titles, the field lists and the inline flags. Seven looks by four properties as literals inside one builder is how the sixth one quietly gets the fifth one's colour. |
| `black_bloc/requests.py:614` | `moment` reads `created_at` for the filed card and `decided_at` for every other, so a card is stamped **when the move happened**. A row read back before `decided_at` is written (or a naive ISO string from an older row) falls back to now rather than raising — a card with a slightly wrong timestamp beats no card. |
| `black_bloc/requests.py:639` | ⚠️ **`field_value` returning `""` is what leaves a field OFF the card.** Discord refuses an empty field value outright, so "How to test" on a review with none, "Assignee" on an unassigned row and "Due" with no date are absent rather than blank. Every value goes through `clamp(…, 1024)` here and nowhere else — §J measured the worst look at 2004 of Discord's 6000, entirely because of this line. |
| `black_bloc/requests.py:690` | `site_view` answers `None` when there is no origin, and `post_line`/`dm` pass `view=None` straight through. A link button with a broken URL is worse than no button; `bot.settings.origin` is always set in practice, so this is the belt to the braces. |
| `black_bloc/requests.py:705` | `move_line` is the plain one-liner that REPLACED the sent message — it is now only ever written to the server log, in `post_line`'s two `log.warning` calls, so a card the guard skipped still says in the log what it would have said. |
| `black_bloc/requests.py:846` | `set_status` builds its `UPDATE` from a dict because five columns now have per-move rules and "leave this one alone" was already being faked with `COALESCE`. **A column with no rule for this move is not in the statement at all** — which is how `ready_by` survives a hold-and-resume and how `decided_by` survives a withdrawal. |
| `black_bloc/requests.py:579` | `may_accept` answers True when nothing is recorded in `ready_by`. A row moved to `review` by the landing one-off, or by a `/status` write before this shipped, must not become unacceptable by anybody. |
...
| `black_bloc/panels.py:125` (`NoteModal.__init__`, the label mutation — was `cogs/community/requests.py:909` until wave 0; the cog's thin subclass is at `:916`) | `NoteModal.note.label` is mutated per-instance in `__init__`, the same way `ReadyModal.built.default` already was — one `discord.ui.TextInput` field, three labels (`hold`/`decline`/`sendback`), each naming who the note is sent to inside Discord's 45-character label cap. ⚠️ This discord.py version deprecates the `.label` setter in favour of `discord.ui.Label` (Components V2) — it still works, and nothing else in this codebase needs a per-instance label yet, so the migration is left for whoever adopts Components V2 wholesale rather than done piecemeal here. |
| `black_bloc/settings_store.py:1342` (`default`) | `request_panel_minutes` follows the exact shape `request_review_by_other` already set: `KEY_TYPES["request_panel_minutes"] = "int"`, a `KEY_HELP` sentence, and a `default()` branch returning **10** — no `KEY_MIN`/`KEY_MAX` bound, matching every other undecided-bound `int` key in this registry (`honeypot_purge_days`, `poll_reminder_minutes`) rather than inventing a ceiling nobody asked for. ⚠️ **10, not the 15 this was first built with**: review finding F2 measured that the "gone quiet" footer can only be written while Discord's 15-minute interaction token is still valid, so 15+ silently loses it. The value is still NOT clamped — the `KEY_HELP` sentence is the warning, and KI-20 carries the same one. |
...
# Polls panel (wave 1, 2026-09-03)

`/poll` is ONE command that opens an ephemeral panel; the `poll` and `poll recur` groups and
all eleven subcommands are gone. Design: [`polls-panel-design.md`](polls-panel-design.md);
the invariants it is reviewed against are [`panels-program.md`](panels-program.md) §2.

### `black_bloc/polls.py` — the button table and the lines, still with no gateway in it

| Key | Note |
|---|---|
| `black_bloc/polls.py:95` | ⚠️ **`DENIED: (OPEN,)` — a denied poll is no longer the end of the line.** Owner's standing rule (`CLAUDE.md`): never a terminal state staff cannot leave. `TERMINAL_STATUSES` is derived from this table, so it collapsed to `(ARCHIVED,)` on its own, and `apply_decision`'s `drop_lock` branch followed — a denied poll keeps its lock because it can still move. |
| `black_bloc/polls.py:787` | `MoveButton` mirrors the requests NamedTuple and adds ONE field: `staff_only`. `End` is the only move in this feature a non-staffer may make, so it is the only row that sets it False — the alternative was a second table for members, which is exactly the "two spellings of one move" P3 forbids. |
| `black_bloc/polls.py:817` | `card_buttons` takes `creator_may_end` as well as the design's `staff`/`is_creator`, so the `poll_creator_may_end` key is answered in the pure module rather than by each caller pre-computing a misleading `is_creator=False`. A move nobody present may make is never returned, so P9's "not rendered" falls out of the table instead of being remembered at every call site. |
| `black_bloc/polls.py:842` | `summary_line` and `recur_line` (`:851`) take the already-parsed datetime rather than calling `golive.parse_ts` themselves. ⚠️ **`polls.py` cannot import `golive`**: `golive` → `settings_store` → `polls` is a cycle, and `panels` → `settings_store` → `polls` is the same cycle. Leaving the parse with the caller is what keeps §H's promise that this module imports no database and no gateway. |
| `black_bloc/polls.py:860` | `recurrence_card` is the one builder a template gets; `review_card` would have shown a poll's hours and options but nothing about when it comes round. The paused case writes `RECURRENCE_PAUSED` where the clock would be rather than dropping the field, so the card never looks half-rendered. |

### `black_bloc/cogs/community/polls.py` — the panel, and five writes lifted out of it

| Key | Note |
|---|---|
| `black_bloc/cogs/community/polls.py:1523` | `pause_recurrence` / `resume_recurrence` (`:1541`) / `delete_recurrence` (`:1568`) / `save_recurrence` (`:1587`) are the four recurrence writes, each `(what to say, the row)` and each taking `via`. They were inline in four subcommand bodies AND inline again in `api/tools/polls.py`; both callers now go through one copy, so one write leaves one `action_log` row (checklist 34) and the routes' `note()` calls were deleted rather than kept. `resume_recurrence` answers `(sentence, None)` when the stored cadence no longer reads, which is what the route turns into its 409. |
| `black_bloc/cogs/community/polls.py:1627` | `apply_poll_settings` is the whole of the old `/poll settings` body: every `store.set`, every `store.clear`, and ONE `poll.settings` line per submit however many switches moved. It takes no `via` — no web route calls it, because the dashboard writes settings through the generic settings API, which logs `web.settings.set` itself. |
| `black_bloc/cogs/community/polls.py:1645` | `counts_for` moved from a cog method to module level because `build_card` needs it and a card is not a command. `Polls.counts_for` stays as a one-line delegate so nothing holding a cog had to change. |
| `black_bloc/cogs/community/polls.py:1659` | `post_now` and `send_for_review` (`:1676`) are the old `_post_now` / `_send_for_review` cog methods turned into `(said, row)` functions: the panel answers through a followup after its own defer, not through `answer()`, so the sentence has to come back rather than be sent. |
| `black_bloc/cogs/community/polls.py:1886` | ⚠️ **`PollDraft` holds the whole create flow and NOTHING is written until `Post it`** (design fork I-3). `draft_trouble` catches the one thing `poll_plan` cannot be handed — hours that are not a whole number, because `poll_plan` calls `int()` on them — while slots and step are passed through as typed text, so `date_trouble` can quote back what the person actually wrote. |
| `black_bloc/cogs/community/polls.py:2207` | `MOVE_FUNCS` and `RECUR_FUNCS` (`:2219`) are late-bound lambdas, so the shared function behind each button can be swapped in a test and every move is provably going through the one copy rather than a private path. |
| `black_bloc/cogs/community/polls.py:2258` | `may_end` is the panel's one non-staff gate and it re-asks at click time exactly as `still_staff` does (P8) — a `poll_creator_may_end` flipped off while a card is open takes the move away rather than letting a stale render through. |
| `black_bloc/cogs/community/polls.py:2274` | `write_draft` is the ONE write the panel makes: `store_poll`, then `save_recurrence` (repeating), `send_for_review` (review on) or `post_now`. Every refusal ahead of it re-renders the preview or the panel and answers in words, so a failed create never leaves half a poll behind. |
| `black_bloc/cogs/community/polls.py:2619` | The create modal uses Discord's five-component cap exactly: question, options, hours, a `RadioGroup` of kinds, a `CheckboxGroup` of the two switches. ⚠️ **All five are wrapped in `discord.ui.Label`.** A bare `TextInput` still works in 2.7.1, but mixing bare inputs with labels produces a payload carrying both an action row and label components, and `TextInput(label=…)` is deprecated in favour of exactly this shape. |
| `black_bloc/cogs/community/polls.py:2758` | `FindModal` is the panel's answer to the lost `poll_id` argument: the pick select carries `OPEN_STATUSES` only, so a closed, cancelled or archived poll is reachable by number and by nothing else. It is deliberately NOT the library's `NoteModal` — that is a paragraph box, and a paragraph box for `12` reads wrong. |
| `black_bloc/cogs/community/polls.py:2838` | The settings panel is five rows of controls over `settings_lines`, and every one of them goes through `save_settings` → `apply_poll_settings`, so "one write, one log row" holds however a switch was flipped. `SettingsToggle` reads the current value at render time and writes the NEXT one, which is why its label says `Polls: on` rather than `Turn polls off`. |
| `black_bloc/cogs/community/polls.py:1126` | The cog's own `answer()` was a byte-for-byte second copy of `panels.answer` and is gone (it used to sit just above `dm`). `option_label` came out of `requests.py` into `panels.py` in the same commit — wave 0 deviation 2 said the first feature wave needing a second copy should extract it, and this is that wave; `requests.option_label` now delegates, so its tests stayed green unchanged. |

### `black_bloc/panels.py` — what wave 1 added to the library

| Key | Note |
|---|---|
| `black_bloc/panels.py:64` | `option_label(ident, status, text, limit)` is the generic form of `requests.option_label`: the id, an optional status word, then as much text as fits. It clamps the WHOLE label rather than only the text, so a caller handing it a long status word still gets something Discord will accept. |
