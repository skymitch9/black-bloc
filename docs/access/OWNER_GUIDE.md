...
> Last verified: **2026-09-03** for the GO-LIVE row (`/golive` is ONE command that opens a panel; the `golive`
> and `twitch` groups are gone; not run against Discord by eye) and the sweeps count (**117** rows,
> numbered to 117 — 109–117 are the go-live panel's, 104–108 the MEMORY PANEL build's, and 103 the operator token's; the count read 95 here
> and was already two builds stale, counted against [`sweeps.md`](sweeps.md) itself).
...
| Test something | [`sweeps.md`](sweeps.md) — 117 rows in priority order, each with what to expect (+ the detailed phase 1–8a scripts) |
