...
> | Registry keys | **292** on `main` after the spotlight-pings and end-wording merges (`spotlight_bump_pings` in, `golive_end_suffix` + `golive_end_mode` out; 293 at v149 — two `posts_versions_*` + `golive_boot_sweep`; 290 at v148) — **25 namespaces** | `len(settings_store.KEY_TYPES)` |
...
> | Tests | **6967** (+3 skipped where the receive extension is absent — **KI-31**) | the v149 deploy gate |
> | The Discord mock | `black_bloc/preview.py` — **15 renderers** over **19 settings keys**, behind `GET /api/preview/features` and `POST /api/preview/message` (staff). Measured on branch `discord-mock` 2026-09-20; NOT on `main` and NOT deployed. The site draws them with `site/public/assets/discordmock.js` | `len(preview.RENDERERS)` / `len(preview.KEY_FEATURES)` |
