...
> **2026-09-03** — rows **130–137** added by the YOUTUBE PANEL build (`/youtube` becomes ONE
...
> and **47** were rewritten IN PLACE for it rather than added. ⚠️ Rows 130–137 have **not** been
> run against Discord and **nothing has been run against YouTube** — no channel resolved, no feed
> fetched, no panel opened. `youtube_mode` is still `off`. ⚠️ **The 130–137 block starts at 130
> because a sibling wave-2 GOLIVE branch holds 109–129; if that branch lands differently the
> conductor renumbers this block at merge** — nothing outside this file points at these numbers.
> Same day — rows **104–108** added by the MEMORY PANEL build (`/memory` becomes ONE
...
| 47 | YouTube uploads — the staff paths | `/youtube` as staff → **Setup** (pick a channel and a ping role), **Back** → **Link for somebody…** → pick a member → paste their channel | the panel's own embed is the old `/uploads list` header — the mode, the channel, the sweep health, **api key — not set (feed only)** and who is linked; Setup names where posts will go; linking for somebody counts their history the same way. Rows **130–137** walk the whole panel |
...
Rows **130–137**. `/youtube` is now ONE command that opens a panel; the `youtube` and
