...
| `black_bloc/cogs/content/raidtrain.py:572` | Check-in reads `golive.open_sessions` — the one signal, not a second detector. It is also why **KI-15** exists: a holder whose presence is hidden and who is not Twitch-linked is never seen. Accepted, same limit as F1. |
