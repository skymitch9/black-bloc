...
> temporarily) — secret NAMES only. Last verified: **2026-09-25 22:3x — the `deploy.md` ROW only**, at the v171 ritual: [`../deploys.log`](../deploys.log) is **170** lines (`grep -c .`), the last being **v171** `329ea08e`. `ls docs/access/*.md` still **12**. ⚠️ **NOT checked:** every other row's number or the facts inside any file.
