...
| `black_bloc/cogs/community/events.py:337` | Review finding F14: `_event_locks` grew one entry per event for the life of the process and never shrank. A settled event cannot be raced — `can_transition` (`events.py:39`) refuses every move out of a terminal status — so its lock is dropped. ⚠️ **Dropping it while it is held is safe and deliberate**: a waiter already holds a reference to the `Lock` object and is unaffected, and a click arriving after the pop makes a fresh lock that immediately finds a terminal status. |
...
| `black_bloc/cogs/community/events.py:1043` | Runs **inside** the caller's lock — it is only ever reached from `_go_live`, which holds it — so it must not take the lock again. `approved → done` is a legal transition (`events.py:39`) precisely so this can exist. |
...
| `black_bloc/timezones.py:11` | The one format the modal accepts, stated once and shown to the person in three places — the input's own label, its placeholder, and the refusal at `events.py:61`. A 24-hour clock on purpose: `7:30` with no am/pm is the ambiguity a bot must not resolve by guessing. |
...
| `black_bloc/modcases.py:112` | Clamps at the **act site** as well as at the validator (`automod.py:233`), for the same reason `do_ban` does (`cogs/moderation/honeypot.py:204`): a value written to the settings row before the validator existed is still on disk, and Discord's answer to 29 days is a 400, not a shorter timeout. |
...
| 🖤 ❤ ♥ 💜 💖 | `chat.py:23` (`LOVE_MARKS`), `chat.py:212` | unchanged — hearts take no modifier, and `LOVE_MARKS` is INPUT matching, not output |
...
| Key | Note |
|---|---|
| `black_bloc/chat.py:504` | ⚠️ **`reply_for` keeps its parameter list and became a COROUTINE, which is the one real deviation from the brief.** A data intent has to read the database (open go-live sessions, the next approved event, stored birthdays, the member's zone), and a synchronous seam cannot. `answer_for` is the new inner call and returns an `Answer(intent, kind, slot, text)` rather than a bare string, because the cog needs the intent for its action row and the kind to decide whether to route — `chat.py:574`'s `reply_for` is now a two-line wrapper over it and stays the documented swap point for a step-3 backend. The old cog called `classify` a second time to get the label; that second call is gone, and with it the note at `cogs/content/chat.py:91` from step 1. |
