...
| [`deploy.md`](deploy.md) | Hosting on Fly.io: first launch, secrets, volume, redeploy, logs. The redeploy path is heavily exercised — **163** lines in [`../deploys.log`](../deploys.log), the last being **v164** on 2026-09-25 14:16 (counted 2026-09-25 14:2x at the v164 ritual; it said 162 / v163). It carries the *`git pull --rebase` flattens a local merge commit* gotcha (2026-09-25, `a460e9a8` — why spotlight-ping-windows landed linear; push with `git fetch` + `git merge --ff-only` instead). Its check-clean step carries the *do not touch the tree between launching `deploy.ps1` and its check-clean* gotcha (v162). Its *gate's pytest step* section is the `-n 16` / `-rfE` / junit / 120 s timeout gate, first run for real by v157 |
