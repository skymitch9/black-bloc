...
`#mute-me-bot-test-spam`** — every sweep row below assumes that. The panel itself is ephemeral and
...
| Do this | Expect |
|---|---|
| `/chat` in `#mute-me-bot-test-spam` as a Lead | ONE ephemeral panel: the mode and tier lines, the notes count, then **Personality… · Knowledge… · Settings**, the two mode toggles, **Logs · Refresh · Open on the site**. Nothing says `/chat status`, `/chat knowledge` or `/chat personality` anywhere |
