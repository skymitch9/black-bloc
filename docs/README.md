...
> Last verified: **2026-09-25 22:3x — the bot's version and three counts only**, at the v171 docs ritual: the bot is at **v171** (deployed 2026-09-25 22:29 Phoenix, release commit `329ea08e`; [`deploys.log`](deploys.log) is the authority, **170** lines by `grep -c .`). `ls docs/info/*.md` still **126**, `ls docs/access/*.md` still **12**, the site **22** pages. One dated line added under *Where the bot stands*. ⚠️ **NOT re-checked:** that block's mode table and every other figure; the v167–v170 lines below were written at their own rituals; nothing met a browser or Discord.
> Before that, **2026-09-25 19:3x — the bot's version, four counts and one table cell only**, at the v166 docs ritual: the bot is at **v166** (deployed 2026-09-25 19:30 Phoenix, release commit `b06d4f1d`; [`deploys.log`](deploys.log) is the authority, **165** lines by `grep -c .`). `ls docs/info/*.md` now **126** (six new today: marathon-ux, shadow-home-per-feature, marathon-people, event-drawer, marathon-drawer-lite designs and the guides audit), `ls docs/access/*.md` still **12**, the site **22** pages (`ls site/public/*.html`). One dated line added under *Where the bot stands*; the mode table's `shadow` row now names the v166 rehearsal homes (the three keys the conductor set and read back at 19:3x — NOT re-read here). ⚠️ **NOT re-checked:** the rest of that block's mode table and every other figure (the tree paragraph's counts included); nothing met a browser or Discord.
> Before that, **2026-09-25 16:2x — the bot's version and four counts only**, at the v165 docs ritual: the bot is at **v165** (deployed 2026-09-25 16:15 Phoenix, release commit `2351311a`; [`deploys.log`](deploys.log) is the authority, **164** lines by `grep -c .`). `ls docs/info/*.md` now **120** (three new: marathon feeds, marathon event modes, the Linux CI hang), `ls docs/access/*.md` still **12**, the site **22** pages (`ls site/public/*.html`). One dated line added under *Where the bot stands*. ⚠️ **NOT re-checked:** that block's mode table and every other figure (the tree paragraph's counts included); nothing met a browser or Discord.
> Before that, **2026-09-25 14:2x — the bot's version and three counts only**, at the v164 docs ritual: the bot is at **v164** (deployed 2026-09-25 14:16 Phoenix, release commit `7787b10c`; [`deploys.log`](deploys.log) is the authority, 163 lines). `ls docs/info/*.md` now **117** (the four 2026-09-25 designs: spotlight ping windows, marathon schedule, marathon next event, marathon events page), `ls docs/access/*.md` still **12**, the site **22** pages (`ls site/public/*.html`; `marathons.html` gone, `channels.html` off the nav but on disk). One dated line added under *Where the bot stands*. ⚠️ **NOT re-checked:** that block's mode table and every other figure (the tree paragraph's counts included); nothing met a browser.
> Before that, **2026-09-23 18:3x — the bot's version and two counts only**, at the v163 docs ritual: the bot is at **v163** (deployed 2026-09-23 18:32 Phoenix, release commit `8acfcde2`; [`deploys.log`](deploys.log) is the authority, 162 lines). `ls docs/info/*.md` still **113**, `ls docs/access/*.md` still **12**, the site **22** pages. One dated line added under *Where the bot stands*. ⚠️ **NOT re-checked:** that block's mode table and every other figure; nothing met a browser.
...
**2026-09-25 22:3x:** v171 live (one merge: a feed-found marathon is pre-loaded quietly and the staff notice posts once its schedule is out — `marathon_feed_notice_when`, default `published`; schema 67, 492 keys; the four live GDQ marathons each notice when their schedule publishes; see [`DONE.md`](DONE.md) 2026-09-25 v171). Earlier tonight v170 (22:07, the mode switch atop Marathons) and v169 (22:00, the Event column says the mode). The mode table below was NOT re-read.
...
**2026-09-25 14:2x:** v164 live (four merges: a spotlit channel can ping always, never or only inside a ping window; GDQ marathon schedules — request #12 — read off the tracker every half hour with our members' runs boarded, reminded and shouted out, `marathon_mode` shadow; the next GDQ event suggested when a marathon ends; marathons live on the Events page and make an event, `/marathon` folded into `/event`; plus the Channels page off the main nav; schema 62, 469 keys; see [`DONE.md`](DONE.md) 2026-09-25 v164); the mode table below was NOT re-read.
...
| **`shadow`** — the rehearsal copy goes to the feature's own `<feature>_shadow_channel_id` when set, else `shadow_channel_id` (`#blackbloc-logs` since v166, 2026-09-25 19:3x; `frontdoor_` and `posts_shadow_channel_id` are `#welcome-test`), nothing to the real channel | `frontdoor_mode` (set 2026-09-18 16:57, aimed at `#welcome`), `golive_mode`, `poll_mode`, `birthday_mode`, `tempvoice_mode`, `honeypot_mode`, `automod_mode` |
