...
> Last verified: **2026-09-22 13:4x — the v155 docs ritual, the fact and releases tables
> re-measured by import.** Registry keys **299 → 307** (`len(settings_store.KEY_TYPES)`), schema
> **52 → 53** (`storage/db.py:SCHEMA_VERSION`; the one new column, `spotlight_channels.starts_at`,
> was confirmed applied on the live volume by the boot log at 20:35:23Z), cogs **23** (unchanged),
> setting groups **24** (unchanged), features **21** (unchanged), routes **198** / pages **21**
> (both unchanged, read off `contract.json`; `check.mjs` was NOT re-run live), tests **7316**
> (+3 skipped, the v155 gate's FIRST run), deploys log **154** lines, last `a285afc7` (v155) at
> 2026-09-22 13:35. ONE merge landed: `spotlight-dates` (`fad98c9e`). ⚠️ **NOT re-measured at that
> pass:** the command count (still the suite's `TOP_LEVEL_NOW`, which the v155 boot log corroborates
> at *synced 33 app commands*), the Discord-mock renderer counts, and anything below the two tables.
> Before that, **2026-09-21 20:1x — the v154 docs ritual, the fact and releases tables
> re-measured.** Registry keys **299** (unchanged, `len(settings_store.KEY_TYPES)`), schema **52**
> (unchanged — v154 added no column), routes **198** / pages **21** (both unchanged, read off
> `contract.json`; `check.mjs` was NOT re-run live), tests **7253** (+3 skipped), deploys log
> **153** lines, last `52c48235` (v154) at 2026-09-21 20:08. Five merges landed: `no-previews`
> (`4dd9f649`), `channel-kinds` (`6017db05`), `quiet-channel-kinds` (`78aed68a`), `mode-column`
> (`f3f16dd0`) and `golive-settings-help` (`ef7c5fd5`). ⚠️ **Nothing else here was re-measured at
> this pass**, and nothing met live Discord or a browser against production. Before that,
> **2026-09-21 17:3x — the `logkinds.py` line of the Shape tree only**:
...
>
> | What | v155 (`main`, 2026-09-22 — re-measured by import, 2026-09-22 13:4x) | Where it is measured |
> |---|---|---|
...
> | Schema version | **53** on `main` at v155 (measured by import 2026-09-22 13:4x) — v155 added **one** column, `spotlight_channels.starts_at` (nullable, through `ADDED_COLUMNS`), and the live volume took it on boot: `database: added spotlight_channels.starts_at` at 20:35:23Z. Was **52** at v154 — unchanged by v154, which added no column (measured 2026-09-21 20:1x); unchanged by v153, which added no column (measured 2026-09-21 12:2x); unchanged by v152 too (measured 2026-09-21 10:38); **52** since v151 (measured 2026-09-21: `spotlight_channels.spotlight` / `.announce` / `.youtube_channel_id` / `.youtube_handle`, all additive — the spotlight row became the channel record; ✅ the migration RAN at the v151 boot, four ADDED columns, **no rebuild**); 51 at v150 (`raid_trains.event_id`; **50** in the same release — `golive_fan_roles.spotlight_id` with `user_id` relaxed to NULL, which REBUILT the table at the boot; 49 at v149 `post_versions`; 48 at v148 the `spotlight_*` tables; 47 at v146) | `storage/db.py:SCHEMA_VERSION` |
> | Registry keys | **307** on `main` at v155 (re-measured by import 2026-09-22 13:4x: `len(settings_store.KEY_TYPES)`) — the eight new ones are v155's spotlight-dates words, all `golive`-namespaced, in their own Settings drawer. Was **299** at v154 (unchanged from v153 — neither `no-previews`, `channel-kinds`, `quiet-channel-kinds`, `mode-column` nor `golive-settings-help` added a key; re-measured by import 2026-09-21 20:1x: `len(settings_store.KEY_TYPES)`). 299 at v153 (measured by import 2026-09-21 12:2x) — `golive_member_optout_post` (298, the member opt-out follow-up) and `golive_autolink_youtube_video` (299, the YouTube video-to-channel lookup, **shipped false**) in; the `golive` + `pings` + `youtube` join set they were filed against reads **54** (measured 2026-09-21 12:2x; ⚠️ both branches bumped it 52 → 53 independently and the merged truth is 54 — **KI-36**). 297 at v152 — `golive_channel_optout_post` (297, the channel opt-out follow-up; that join set read **52**) in; 296 at v151 — `golive_channel_spotlight_default` (295, channel streamers) and `golive_autolink_presence` (296, auto-link) in; 294 at v150 — `spotlight_bump_pings`, `golive_live_author` and `raidtrain_event_default` in, `golive_end_suffix` and `golive_end_mode` out (293 at v149; 290 at v148) — **24 namespaces** (measured 2026-09-21) | `len(settings_store.KEY_TYPES)` |
...
> | Mock contract | **21 pages / 198 routes / 24 core settings** at v155 — unchanged by v155 too, which added no page and no route (the three `/api/golive/spotlight` routes gained FIELDS; re-read off `contract.json` 2026-09-22 13:4x, `check.mjs` ok in the v155 gate, NOT re-run live after the deploy). Was the same at v154 — unchanged by v154, v153 AND v152, none of which added a page or a route (re-read off `contract.json` 2026-09-21 20:1x; `check.mjs` ok in the v154 gate, NOT re-run live after the deploy) — at v151 (measured 2026-09-21: `POST /api/golive/links/sweep` is the 198th) — 21/197 at v150 when raid trains became page 21 (the previews were never in `contract.pages`, and they are deleted now); 20/186 at v139, 17/150 at v108, 149 routes at v92. `ls site/public/*.html` re-counted off disk 2026-09-21 09:55: **21** | `site/mock/contract.json`, read 2026-09-21 09:55. ⚠️ **`node site/mock/check.mjs` has NOT been re-run since the `raidtrain-page` landing** (`check: ok - 21 pages, 197 routes, 24 core settings, all keys present`), so 198 is read off `contract.json`, not off a live check |
> | Tests | **7316** (+3 skipped where the receive extension is absent — **KI-31**), the v155 gate's FIRST run, no failures — **+63** over v154, all from `spotlight-dates`; 7253 at v154, 7237 at v153 (third run), 7189 at v152, 7178 at v151, 7118 at v150 | the v154 deploy gate's first run, read off the v154 [`../deploys.log`](../deploys.log) line. At v153 the first two runs never reached Fly: run 1 was REFUSED on a guides picture test (**KI-32**, second sighting, 7236 passed beside it) and run 2 was **killed by the OS for low memory** at 99 % of pytest beside a 12 GB whisper job (**KI-37**). At v152, one of six runs went red on **2 tests whose names were lost** — **KI-35**, `WATCHING` |
...
> | Deploys | **154** lines (counted 2026-09-22 13:4x), last `a285afc7` (v155) at 2026-09-22 13:35 | `../deploys.log` |
>
...
> | v154 `52c48235`, 2026-09-21 | 23 | 33 | 52 | 198 | 7253 |
> | **v155 `a285afc7`, 2026-09-22** | **23** | **33** | **53** | **198** | **7316** |
> | **v153 `3d75254`, 2026-09-21** | **23** | **33** | **52** | **198** | **7237** |
> | **v152 `32d3c0b`, 2026-09-21** | **23** | **33** | **52** | **198** | **7189** |
