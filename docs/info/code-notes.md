...
> Keyed against `feat/requests-panel` at the commit this section was written in.
...
| Key | Note |
|---|---|
| `black_bloc/requests.py:389` | ⚠️ **`CARD_BUTTONS` is DATA, not a chain of `if status ==`.** Every earlier request state-machine bug in this file (`TRANSITIONS`, `EMBED_TITLES`, `MOVE_LINE`) was a table for the same reason: a table is one thing to get wrong, and `tests/test_requests.py::test_every_status_maps_to_the_move_buttons_the_panel_design_names` is parametrised over all seven statuses, so a status the table forgets fails by name instead of silently rendering no buttons. `card_buttons()` is the only reader; the cog never branches on status by hand. |
| `black_bloc/requests.py:50` | **`SELECT_CAP = 25` is Discord's own hard limit on a select's options, not a decision.** It caps BOTH selects — the staff "Pick a request…" (which also says "N of M — the rest are on the site" past the cap, per the design) and the member's own "Take one back…" (which silently truncates with no such message, because a member realistically never has 25 requests open at once; noted here since it is the one place the two selects behave differently for the same reason). |
| `black_bloc/requests.py:895` | ⚠️ **`withdraw_request` deliberately widens checklist item 34's own wording.** That item's own text lists "withdraw" among the routes where `note()` is the sole logger — true when it was written, that morning, before withdraw had a shared function to call. This build gives withdraw one (the panel's confirm and the site's `POST …/withdraw` both call it), so the route's `note("web.request.withdrawn", …)` became the SAME double-post shape checklist 34 exists to catch — `kind_via("request.withdrawn", via)` replaces it, and `tests/test_logkinds.py::test_a_route_never_notes_an_event_its_shared_path_already_logged` is what would have failed if the route had kept its own line. |
| `black_bloc/cogs/community/requests.py:672` | ⚠️ **Views are not persistent, and every render creates a FRESH one with the FULL `request_panel_minutes` timeout — the clock is inactivity, not wall time from when `/request` was first opened.** `RequestView(minutes)` is constructed by `build_panel`/`build_card`/`open_withdraw_confirm` on every single render; clicking anything resets the countdown rather than merely extending a single running one. ⚠️ **This note used to end "only the LAST view nobody touches ever reaches `on_timeout`", and that was WRONG** — see `retire` at `:167` and the design doc's deviation 9. A displaced view keeps its own timeout task; nothing in the library stops it. See KI-20 for why persistence (`bot.add_view`, the `TempVoicePanel` pattern) was not used instead. |
| `black_bloc/cogs/community/requests.py:167` (`retire`) | ⚠️ **Every re-render MUST stop the view it replaces, or that view's timeout will overwrite the card that replaced it with a stale disabled copy.** `ViewStore.add_view` (installed `discord/ui/view.py:940–968`) merely overwrites `_synced_message_views[message_id]`; it never touches the displaced view's `__timeout_task`, so that task keeps running and `_dispatch_timeout` (`view.py:611–620`) eventually calls the OLD view's `on_timeout`. `retire` is called at `:550` (`render_panel`), `:575` (`finish_card`), `:599` (`open_card`) and `:638` (`open_withdraw_confirm`), always immediately BEFORE the edit; every item callback passes its own `self.view` down as `previous`, and `CardMoveButton` (`:798`) hands it to `ReadyModal`/`NoteModal` so the modal's submit path retires it too. `RequestModal` (filing) never re-renders and carries nothing. |
| `black_bloc/cogs/community/requests.py:683` (`on_timeout`) | ⚠️ **The guard is `self.replaced`, NOT `self.is_finished()` — the obvious spelling would have disabled the footer entirely.** Measured against the installed library: `_dispatch_timeout` does `__stopped.set_result(True)` (`view.py:619`) and only THEN `create_task(self.on_timeout())` (`:620`), so `is_finished()` is **already True inside a genuine timeout** — proved by running a real `discord.ui.View` with a 0.05s timeout and reading the flag from inside `on_timeout`. `stop()` alone is what actually prevents the timeout in production (`_dispatch_timeout` returns early on a done future); the `replaced` flag is the belt-and-braces half, and it is the only one that can tell "replaced" apart from "expired". |
| `black_bloc/cogs/community/requests.py:679` (`interaction_check`) / `:694` (`went_quiet`) | ⚠️ **`view.message` is an `InteractionMessage` bound to ONE interaction token, and Discord kills that token 15 minutes after the interaction — so at the old 15-minute default the "gone quiet" footer essentially never got written.** `View._refresh_timeout` (`view.py:315–317`, from `_scheduled_task` at `:596–597`) extends the timeout on EVERY interaction with the view, including ones that do not re-render at all (`Logs`, opening a modal), so by the time it fires the message's token is at least `request_panel_minutes` old and usually older. `interaction_check` therefore records `self.last_interaction` (and always returns True — it gates nothing), and `went_quiet` tries `last_interaction.edit_original_response` first, falling back to `self.message.edit` when there is no recorded interaction OR when the recorded one is refused. Both are wrapped: an `HTTPException` is logged at info, never raised. The other half of the fix is the default dropping to **10** (`settings_store.py:1299`). |
| `black_bloc/cogs/community/requests.py:158` (`still_staff`) | ⚠️ **The panel checks `is_staff` when it RENDERS; a staffer demoted while their card is open would keep moving requests until the panel timed out — so every move re-asks.** `still_staff` runs in `CardMoveButton.callback` (`:798`, before any defer or modal) and at the top of `Requests.ready_submit` (`:1002`) and `Requests.note_submit` (`:1026`, before their defers), refusing in words with `store.staff_refusal(guild.id)` through `answer()`. ⚠️ **`settings_store.require_staff` is deliberately NOT reused**: it calls `interaction.response.send_message` directly, so it only works where nothing has been deferred — one shape (`still_staff` + `answer()`) at all three sites beats two. `LogsButton` needs nothing, because `send_logs` (`actionlog.py:295–297`) calls `require_staff` itself before it has responded to anything. |
| `black_bloc/cogs/community/requests.py:457` | ⚠️ **`MOVE_FUNCS`'s three entries are ALL lambdas, not two lambdas and one direct reference.** `{"accept": accept, "resume": resume_request}` would have baked in the function OBJECT at dict-creation time (module import), and `monkeypatch.setattr(requests_cog, "accept", fake)` afterward would silently not affect what `run_move` calls — a lambda body does a NAME lookup in the module's globals at call time, so it is the one shape both a test's mock and a future call-through actually see. Found by writing `test_a_direct_move_button_calls_its_shared_function_and_leaves_via_alone` and watching the "accept" case call the real function anyway. |
| `black_bloc/cogs/community/requests.py:464` | `db_ready` assumes the caller already `defer()`'d — every component/modal callback in this file does `interaction.response.defer()` first BECAUSE, per `discord/interactions.py`'s own `InteractionResponse.defer`, a component or modal-submit interaction's default (non-`thinking`) defer is `deferred_message_update` (type 6): an ack that edits the SAME message in place, never a new one. `interaction.edit_original_response(...)` is what actually changes it afterward, and `interaction.followup.send(...)` is what puts the returned sentence beside it — checked against the installed library source (checklist 29), not memory. |
| `black_bloc/cogs/community/requests.py:489` | `build_panel`/`build_card` are free functions taking `bot`/`guild`/`actor`, not `Requests` cog methods, so every `discord.ui.Item` subclass can call them via `interaction.client` without holding a `self.cog` reference. Only `submit`, `ready_submit` and `note_submit` stay cog methods, because `RequestModal`/`ReadyModal`/`NoteModal` already take a `cog` argument (existing shape for the first two; matched for the third) and fetch it with `interaction.client.get_cog("Requests")` at the point a modal is opened. |
| `black_bloc/cogs/community/requests.py:604` | `open_withdraw_confirm` reuses `look_for_status` (open → `filed`, everything else itself) for the PREVIEW card too, not only the staff card — one status→look mapping, one place it can drift. The confirm view carries only "Yes, take it back" / "Keep it", never a `Back`, because the design lists exactly those two. |
| `black_bloc/cogs/community/requests.py:907` | `NoteModal.note.label` is mutated per-instance in `__init__`, the same way `ReadyModal.built.default` already was — one `discord.ui.TextInput` field, three labels (`hold`/`decline`/`sendback`), each naming who the note is sent to inside Discord's 45-character label cap. ⚠️ This discord.py version deprecates the `.label` setter in favour of `discord.ui.Label` (Components V2) — it still works, and nothing else in this codebase needs a per-instance label yet, so the migration is left for whoever adopts Components V2 wholesale rather than done piecemeal here. |
| `black_bloc/settings_store.py:1299` (`default`) | `request_panel_minutes` follows the exact shape `request_review_by_other` already set: `KEY_TYPES["request_panel_minutes"] = "int"`, a `KEY_HELP` sentence, and a `default()` branch returning **10** — no `KEY_MIN`/`KEY_MAX` bound, matching every other undecided-bound `int` key in this registry (`honeypot_purge_days`, `poll_reminder_minutes`) rather than inventing a ceiling nobody asked for. ⚠️ **10, not the 15 this was first built with**: review finding F2 measured that the "gone quiet" footer can only be written while Discord's 15-minute interaction token is still valid, so 15+ silently loses it. The value is still NOT clamped — the `KEY_HELP` sentence is the warning, and KI-20 carries the same one. |
