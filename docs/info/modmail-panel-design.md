...
under `TEST_MODE` a real ticket's card appears in `#blackbloc-logs`, not in the ticket — and
...
in `#blackbloc-logs` instead. That difference is the practice ticket's entire justification,
