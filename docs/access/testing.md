...
|---|---|---|---|
| Hermetic | `pytest -q -n auto` | every module, every route shape, every panel card, against fakes — **5,703 tests, 28 s** (2026-09-16, v111 gate) | nothing but the venv — ⚠️ and a CLEAN environment: a shell that exports the real `.env` names (`POLL_VOTE_SECRET`, `DEV_GUILD_ID`, `TWITCH_*`, `GROQ_API_KEY`, `ANTHROPIC_API_KEY`…) turns **nine** "no key" tests red; see `../info/gotchas.md` |
