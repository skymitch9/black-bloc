...
| `black_bloc/cogs/community/requests.py:639` | ⚠️ **Views are not persistent, and every render creates a FRESH one with the FULL `request_panel_minutes` timeout — the clock is inactivity, not wall time from when `/request` was first opened.** `RequestView(minutes)` is constructed by `build_panel`/`build_card`/`open_withdraw_confirm` on every single render; clicking anything resets the countdown rather than merely extending a single running one. This is simpler than tracking one timeout across message identity, and it means only the LAST view nobody touches ever reaches `on_timeout` — see KI-20 for why persistence (`bot.add_view`, the `TempVoicePanel` pattern) was not used instead. |
| `black_bloc/cogs/community/requests.py:639` | `on_timeout` calls `self.message.edit(...)`, and `self.message` is set by every caller right after `send_message`/`edit_original_response` returns (`view.message = await interaction.original_response()` or the `InteractionMessage` the edit itself returns). ⚠️ Near the DEFAULT 15-minute `request_panel_minutes`, this edit can itself fail — Discord's interaction webhook token is only valid ~15 minutes — and that failure is caught and logged, never raised; a panel that goes fully silent past its token life still answered its purpose (KI-20). |
| `black_bloc/cogs/community/requests.py:440` | ⚠️ **`MOVE_FUNCS`'s three entries are ALL lambdas, not two lambdas and one direct reference.** `{"accept": accept, "resume": resume_request}` would have baked in the function OBJECT at dict-creation time (module import), and `monkeypatch.setattr(requests_cog, "accept", fake)` afterward would silently not affect what `run_move` calls — a lambda body does a NAME lookup in the module's globals at call time, so it is the one shape both a test's mock and a future call-through actually see. Found by writing `test_a_direct_move_button_calls_its_shared_function_and_leaves_via_alone` and watching the "accept" case call the real function anyway. |
| `black_bloc/cogs/community/requests.py:447` | `db_ready` assumes the caller already `defer()`'d — every component/modal callback in this file does `interaction.response.defer()` first BECAUSE, per `discord/interactions.py`'s own `InteractionResponse.defer`, a component or modal-submit interaction's default (non-`thinking`) defer is `deferred_message_update` (type 6): an ack that edits the SAME message in place, never a new one. `interaction.edit_original_response(...)` is what actually changes it afterward, and `interaction.followup.send(...)` is what puts the returned sentence beside it — checked against the installed library source (checklist 29), not memory. |
| `black_bloc/cogs/community/requests.py:472` | `build_panel`/`build_card` are free functions taking `bot`/`guild`/`actor`, not `Requests` cog methods, so every `discord.ui.Item` subclass can call them via `interaction.client` without holding a `self.cog` reference. Only `submit`, `ready_submit` and `note_submit` stay cog methods, because `RequestModal`/`ReadyModal`/`NoteModal` already take a `cog` argument (existing shape for the first two; matched for the third) and fetch it with `interaction.client.get_cog("Requests")` at the point a modal is opened. |
| `black_bloc/cogs/community/requests.py:578` | `open_withdraw_confirm` reuses `look_for_status` (open → `filed`, everything else itself) for the PREVIEW card too, not only the staff card — one status→look mapping, one place it can drift. The confirm view carries only "Yes, take it back" / "Keep it", never a `Back`, because the design lists exactly those two. |
| `black_bloc/cogs/community/requests.py:833` | `NoteModal.note.label` is mutated per-instance in `__init__`, the same way `ReadyModal.built.default` already was — one `discord.ui.TextInput` field, three labels (`hold`/`decline`/`sendback`), each naming who the note is sent to inside Discord's 45-character label cap. ⚠️ This discord.py version deprecates the `.label` setter in favour of `discord.ui.Label` (Components V2) — it still works, and nothing else in this codebase needs a per-instance label yet, so the migration is left for whoever adopts Components V2 wholesale rather than done piecemeal here. |
| `black_bloc/settings_store.py:1297` (`default`) | `request_panel_minutes` follows the exact shape `request_review_by_other` already set: `KEY_TYPES["request_panel_minutes"] = "int"`, a `KEY_HELP` sentence, and a `default()` branch returning **15** — no `KEY_MIN`/`KEY_MAX` bound, matching every other undecided-bound `int` key in this registry (`honeypot_purge_days`, `poll_reminder_minutes`) rather than inventing a ceiling nobody asked for. |
