...
| `site/mock/server.mjs:727` | The mock's feature list is now **derived** from its own `SETTING_SPECS` (every key ending `_mode`) instead of a hand-written array of seven names, which is `api/status.py:89`'s rule in the mock's own terms — adding a mode key to one half no longer needs a matching edit in the other. |
...
| `black_bloc/api/tools/rolemenus.py:438` | ⚠️ **`can_move` is asked BEFORE the edit is written.** The first cut applied the field edits, then moved, so a refused move left a menu whose title had changed under a sentence that said "nothing was changed". Caught in the browser, not by reading: role menus ship OFF in the mock, so the first channel change refused with `rolemenu_off` and the title had already moved. `site/mock/server.mjs:1225` splits `checkMove` out of `movePanel` for the same reason. |
...
| `site/mock/server.mjs:493` | The mock's menus grew an `id`, because a request row's `menu_id` has to point at something. `runner-status` is the approval-gated, seven-day menu the owner's sweep describes. |
...
| `black_bloc/logkinds.py:392` | ⚠️ **An unrecognised level is `all` — today's behaviour — never `off`.** A fake store returning `None`, a key that has not been written, a store that has not loaded: none of those is a decision to go quiet, and the failure mode of guessing `off` is a silently missing audit trail that nobody notices. Pinned by *an unknown level is today's behaviour*. |
| `black_bloc/logkinds.py:370` | `like_patterns` emits both `head.%` and `web.head.%` per head; `feature_clause` in `actionlog.py` asks `core` **backwards** (`NOT (…)` over every other feature's patterns) because `core` is defined as everything nobody else claims and SQL cannot express that forwards. |
...
| `black_bloc/cogs/community/requests.py:557` | `/request logs` is the thirteenth, and it is the whole body of `send_logs` like the other twelve — which is why `settings_store.is_staff_command` finds `require_staff` in its `co_names` and hides it from non-staff with no `extras`. |
...
| Key | Note |
|---|---|
| `black_bloc/logkinds.py:328 via_of` | Two words, `discord` and `website`. **What the writer recorded wins**; otherwise the kind's `web.` head decides. That second half is what makes every row written before tonight say something rather than nothing. |
...
| `black_bloc/settings_store.py:927` | `chat_staff_can_ping_roles` defaults **on**, which is what the owner asked for, and is safe because the gate above it is `is_staff`. A server that wants a bot that can never ping anything flips one switch. |
...
| `black_bloc/requests.py:299` | A refused move says where it IS, where it cannot go, and where it CAN go, by name. `moves_sentence` is what makes the last part true from the table rather than from a hand-written list that would drift. |
| `black_bloc/requests.py:325` | `checked_move` is the whole gate in one place: same-state, illegal move, and reason-required. The cog, the API's `/status`, `/hold` and `/decline` all go through it, so a rule cannot be enforced on one surface and not the other. |
| `black_bloc/requests.py:666` | ⚠️ **`held_from` is written on the way INTO hold and cleared on the way out, in the same UPDATE.** Two statements would leave a window where a resumed request still claims to be parked. `decline_reason` carries the HOLD reason as well as the decline one — the column name is a persisted key and changing it would be a migration, so the note is here instead. (Third pass: this is now a dict-built statement covering five per-move columns — see the `Requests — third pass` section.) |
| `black_bloc/requests.py:780` | `resume_target` falls back to `in_progress` when `held_from` is `open`: `open → hold → open` would be a loop with no work in it, and the owner's machine has resume meaning "pick it back up". |
| `black_bloc/requests.py:434` | `row_value` exists so a schema-22 row read before the `held_from` column is added reads as nothing rather than raising. `sqlite3.Row` throws `IndexError` on an unknown column, not `None`. (It does the same job again for schema 26's four columns.) |
| `black_bloc/requests.py:400` | `status_channel_id` is the fallback rule in one place: `request_status_channel_id` if set, otherwise `request_notify_channel_id`, so one channel carries both kinds of line by default and nobody has to set two keys to get any output. |
| `black_bloc/requests.py:177` | ⚠️ **REWRITTEN by the third pass, not renumbered: the four `NOTIFY_*` constants this note described are GONE.** The channel message is an embed now, and `MOVE_LINE` is the seven-look plain-text table that survives only for the server log. The owner's original point still holds and is why it is a table of seven fixed shapes rather than one templated setting: the shapes differ, so a single `request_status_template` would be worse. |

## `black_bloc/cogs/community/requests.py` and the API

| Key | Note |
|---|---|
| `black_bloc/cogs/community/requests.py:185` | `post_line` is the shared guarded post. A channel the guard refuses logs `request.notify_skipped_test_mode` and returns `None` — silence and refusal must be distinguishable in the log (checklist 2), and neither may raise into the move that already happened. |
| `black_bloc/cogs/community/requests.py:266` | ⚠️ **The state moves FIRST, then the DM, then the channel card.** Checklist 12: the irreversible thing goes first, and a failed cosmetic must not abort it. Both notifications are best-effort and both log their own failure. |
| `black_bloc/cogs/community/requests.py:378` | `resume_request` is its own path rather than a `set` with a computed target, because Resume is a BUTTON with no status to choose — the target comes off the row, and the log kind (`request.resumed`) is the one that says a human pressed resume rather than picked a state. |
| `black_bloc/api/tools/requests.py:137` | The row carries `moves` and `resume_to` so the page can draw only the controls that would be accepted. The page never re-derives the machine; it renders what the API says is legal. |
| `black_bloc/api/tools/requests.py:285` | `_may_file` no longer returns anything. `request_auto_approve_staff` is gone and every filing arrives `open` — owner, 2026-09-02: *"Even a staff request can be bad"*. |
| `site/public/assets/page-requests.js:119` | `MOVES` is a label/tone/prompt table keyed by the state NAME, so `moveButtons` can build a bar straight from `row.moves` — a control that would be refused is never drawn, and adding a state to the machine needs one entry here. ⚠️ The third pass DELETED its `done` entry: `done` is only reachable from `review`, and that move is the review card's own Accept button. |
| `site/public/assets/page-requests.js:682` | The hold card drops the resume target from its move list, so Resume and "Pick it up" are not two buttons doing the same thing. |

## Where else Phase 17 touched

| Key | Note |
|---|---|
| `black_bloc/storage/db.py:11` | Schema **23**: two new tables, `requests.held_from`, and the ONE data migration — `pending`/`approved`/`planned` → `open`. It is idempotent because after the first run no row matches; `tests/storage/test_db.py:817` boots the file three times to prove it. |
| `black_bloc/storage/db.py:742` | The migration runs AFTER `_add_missing_columns`, so a schema-22 file has grown `held_from` before any row is touched. |
...
| `black_bloc/logkinds.py:415` | ⚠️ **There is NO `chat_memory_log_level`, and the design's §E table is wrong to list one.** Log levels are per FEATURE and derived from the kind's dotted head; the memory kinds are `chat.memory_*`, so their head is `chat` and `chat_log_level` already governs them. A key that governed nothing would be worse than none. Listed in the design's Deviations. |
...
| `black_bloc/cogs/community/requests.py:266` | `via` is **keyword-only with a Discord default** so the four slash callers keep writing bare `request.*` kinds untouched, and only the dashboard's `_decide` opts into the `web.` head. (Third pass: `mark_ready`, `accept` and `send_back` take the same keyword and pass it straight through.) The same shape is on `resume_request`, `events.apply_decision`, `events.cancel_event`, `polls.store_poll`/`apply_decision`/`close_poll`/`cancel_poll`, `role_menus.apply_request_decision`, `modmail.close_ticket`, `honeypot.ban_hit`/`make_trap_channel`, `tempvoice.make_creator_channel`/`forget_creator`, `automod.save_rule`/`apply_case`/`punish` and the six `modcmds` punishers. |
| `black_bloc/cogs/community/requests.py:318` | `details["via"] = via` is written even though the kind already carries the head, because `via_of()` prefers what the writer recorded over what the kind says. Belt and braces: a row keeps saying "Website" even if somebody later renames the kind. |
| `black_bloc/api/tools/requests.py:292` | The route's whole job here is now `via=VIA_WEBSITE` — the deleted `note(f"web.request.{status}")` was the defect. `request_status` still notes `web.request.updated` beside it, because assignee/priority/notes are a **different** change from the status move and nothing else logs them. |
...
| Key | Note |
|---|---|
| `black_bloc/requests.py:46` | ⚠️ **A LOOK is not a STATUS.** `LOOKS` has seven entries; `STATUSES` has seven too and they are **different sets**: `filed` and `sent_back` are looks with no state behind them, and `open` and `withdrawn` are states with no card. Everything downstream — the log kind, the colour, the fields, who is DMed, whether a card posts at all — reads the look, never the status. |
| `black_bloc/requests.py:337` | `look_of` is the whole of that distinction in three lines: `in_progress` arrived at **from `review`** is `sent_back`. Storing a `sent_back` flag would have been a second home for a fact the `was:` already carries, and it would drift the first time somebody moved a row by hand. |
| `black_bloc/requests.py:343` | `checked_move` grew two keyword-only text arguments rather than two new functions, because a move is judged in ONE place and the site, the slash commands and the tests all ask this. `TEXT_NEEDED` is keyed by LOOK (`review` needs `built`, `sent_back` needs `note`), which is why the required-text rule fires on a send-back and not on an ordinary pick-up — same target state, different look. |
| `black_bloc/requests.py:151` | `DONE_NEEDS_A_CHECK` fires only from `OPEN_STATUSES`. From `declined` or `withdrawn` the generic "is where a request finishes" sentence is still right, and pointing somebody at `/request ready` there would be a lie. The refusal names both the slash path AND the site button, because the person reading it may be on either. |
| `black_bloc/requests.py:209` | The colours are DATA (`EMBED_COLOURS`), and so are the titles, the field lists and the inline flags. Seven looks by four properties as literals inside one builder is how the sixth one quietly gets the fifth one's colour. |
| `black_bloc/requests.py:569` | `moment` reads `created_at` for the filed card and `decided_at` for every other, so a card is stamped **when the move happened**. A row read back before `decided_at` is written (or a naive ISO string from an older row) falls back to now rather than raising — a card with a slightly wrong timestamp beats no card. |
| `black_bloc/requests.py:579` | ⚠️ **`field_value` returning `""` is what leaves a field OFF the card.** Discord refuses an empty field value outright, so "How to test" on a review with none, "Assignee" on an unassigned row and "Due" with no date are absent rather than blank. Every value goes through `clamp(…, 1024)` here and nowhere else — §J measured the worst look at 2004 of Discord's 6000, entirely because of this line. |
| `black_bloc/requests.py:624` | `site_view` answers `None` when there is no origin, and `post_line`/`dm` pass `view=None` straight through. A link button with a broken URL is worse than no button; `bot.settings.origin` is always set in practice, so this is the belt to the braces. |
| `black_bloc/requests.py:639` | `move_line` is the plain one-liner that REPLACED the sent message — it is now only ever written to the server log, in `post_line`'s two `log.warning` calls, so a card the guard skipped still says in the log what it would have said. |
| `black_bloc/requests.py:780` | `set_status` builds its `UPDATE` from a dict because five columns now have per-move rules and "leave this one alone" was already being faked with `COALESCE`. **A column with no rule for this move is not in the statement at all** — which is how `ready_by` survives a hold-and-resume and how `decided_by` survives a withdrawal. |
| `black_bloc/requests.py:534` | `may_accept` answers True when nothing is recorded in `ready_by`. A row moved to `review` by the landing one-off, or by a `/status` write before this shipped, must not become unacceptable by anybody. |
| `black_bloc/cogs/community/requests.py:330` | ⚠️ **`kind_via(f"request.{look}", via)` — LOOK, not status.** This is the line that makes `request.sent_back` exist at all; the row's status is `in_progress`. `tests/test_logkinds.py:KNOWN_DYNAMIC` is keyed on the source of that expression, so renaming the variable breaks the build loudly rather than silently dropping six kinds out of the classification check. |
| `black_bloc/cogs/community/requests.py:163` | `person_told` is the ONE place that decides who gets a DM, and it is the reason `tell_requester` was renamed: on `sent_back` the person told is the staffer in `ready_by`, not the requester. Nobody is DMed on `review` — it is staff-facing and the channel card is its notice. |
| `black_bloc/cogs/community/requests.py:184` | The `request_dm_on_decision` gate applies to `DM_LOOKS` only, so the send-back DM to `ready_by` is NOT gated by it. That key is documented as the **requester's** switch; widening it to staff notices would give one key two meanings and there would be no way to want one without the other. |
| `black_bloc/cogs/community/requests.py:199` | `post_line` keeps its name and its `line` argument even though the line is never sent — the embed is the message and `content` is `None`. ⚠️ **The guard check is unmoved and unchanged**: TEST_MODE still refuses every channel but the test one and logs `request.notify_skipped_test_mode`, and a mutation that removes the check turns `test_the_review_card_is_skipped_and_logged_while_test_mode_holds_the_channel` red. |
| `black_bloc/cogs/community/requests.py:280` | `apply_decision` passes `ready_by` and `sent_back_reason` on EVERY call and lets `set_status` decide whether this move writes them — the alternative, each caller working out which columns its move touches, is the same rule in five places. The `request_review_by_other` check lives here and not in `accept`, so `POST /{id}/status {"status":"done"}` cannot walk around it. |
| `black_bloc/cogs/community/requests.py:373` | `send_back` is the only one of the three wrappers with a guard of its own: `accept` and `mark_ready` are refused by `TRANSITIONS` when the row is in the wrong state, but `review` → `in_progress` is legal from `open` too, so without this a `/request sendback` on an untouched request would quietly pick it up. |
| `black_bloc/cogs/community/requests.py:792` | `ReadyModal` prefills from the row, so a request sent back and marked ready again does not need retyping. `built` is `required=True` at the Discord level AND refused in words by `checked_move` — the client-side one is a courtesy, the server-side one is the rule. |
| `black_bloc/cogs/community/requests.py:683` | `/request ready` is two interactions: the command opens the modal, the modal does the move. ⚠️ A modal can only be the FIRST response, so `_wanted_id` must not `defer()` — `require_staff` answers without deferring, which is why it can be called before `send_modal`. |
| `black_bloc/api/tools/requests.py:489` | The three routes' whole job is `via=VIA_WEBSITE` and **no `note()`** — checklist 34. `tests/api/conftest.py:one_web_row` asserts exactly one `web.*` row per write, which is the assertion the pre-fix tests were missing (they asserted presence, which is what a double post looks like). |
| `black_bloc/api/tools/requests.py:549` | `/status` takes `built` and `how_to_test` as plain fields through `set_fields`, so fixing a typo in how-to-test is a partial save that leaves one `web.request.updated` row and moves nothing. It ALSO forwards them to `apply_decision` when a status is present, so `{"status":"review","built":…}` is a legal, if unused, second path into review. |
...
| `site/public/assets/page-requests.js:1167` | The off-page case: staff fetch `/api/requests/N`, a member scans their own `/mine`, and the result is PINNED above the sections with Back to all rather than paging the reader around. ⚠️ A 404 or a 403 here is "that number is not one you can read", **not** an outage — the two get different sentences, because the fixes are different. `pinned` is set before the fetch so a refresh mid-flight cannot pin twice. |
| `site/public/assets/page-requests.js:646` | The review card filters `done` and `in_progress` out of `row.moves` and draws Accept and Send back as its own buttons — those two moves need a dialog and a distinct label, which a generic move button cannot carry. `MOVES` lost its `done` entry entirely, so no card anywhere can draw a Done button. |
| `site/public/assets/page-requests.js:594` | `writtenRow` is a partial `/status` save, not a move: the owner asked that a typo in "how to test" not need a state change to fix. It is on the review card and on the done card, which is why `shutCard` builds a `notice()` only when the row is `done`. |
...
> `requests.py:477`); none was rewritten, because nothing they describe changed.
...
| `black_bloc/cogs/community/requests.py:454` (`build_panel`) / `:439` (`build_card`) | `build_panel`/`build_card` are free functions taking `bot`/`guild`/`actor`, not `Requests` cog methods, so every `discord.ui.Item` subclass can call them via `interaction.client` without holding a `self.cog` reference. Only `submit`, `ready_submit` and `note_submit` stay cog methods, because `RequestModal`/`ReadyModal`/`NoteModal` already take a `cog` argument (existing shape for the first two; matched for the third) and fetch it with `interaction.client.get_cog("Requests")` at the point a modal is opened. |
| `black_bloc/cogs/community/requests.py:570` (`open_withdraw_confirm`) | `open_withdraw_confirm` reuses `look_for_status` (open → `filed`, everything else itself) for the PREVIEW card too, not only the staff card — one status→look mapping, one place it can drift. The confirm view carries only "Yes, take it back" / "Keep it", never a `Back`, because the design lists exactly those two. |
| `black_bloc/panels.py:112` (`NoteModal.__init__`, the label mutation — was `cogs/community/requests.py:909` until wave 0; the cog's thin subclass is at `:825`) | `NoteModal.note.label` is mutated per-instance in `__init__`, the same way `ReadyModal.built.default` already was — one `discord.ui.TextInput` field, three labels (`hold`/`decline`/`sendback`), each naming who the note is sent to inside Discord's 45-character label cap. ⚠️ This discord.py version deprecates the `.label` setter in favour of `discord.ui.Label` (Components V2) — it still works, and nothing else in this codebase needs a per-instance label yet, so the migration is left for whoever adopts Components V2 wholesale rather than done piecemeal here. |
| `black_bloc/settings_store.py:1305` (`default`) | `request_panel_minutes` follows the exact shape `request_review_by_other` already set: `KEY_TYPES["request_panel_minutes"] = "int"`, a `KEY_HELP` sentence, and a `default()` branch returning **10** — no `KEY_MIN`/`KEY_MAX` bound, matching every other undecided-bound `int` key in this registry (`honeypot_purge_days`, `poll_reminder_minutes`) rather than inventing a ceiling nobody asked for. ⚠️ **10, not the 15 this was first built with**: review finding F2 measured that the "gone quiet" footer can only be written while Discord's 15-minute interaction token is still valid, so 15+ silently loses it. The value is still NOT clamped — the `KEY_HELP` sentence is the warning, and KI-20 carries the same one. |
| `black_bloc/cogs/community/requests.py:475` (the own-list gate) | ⚠️ **Only the RENDERING of a member's own requests is gated — `own_rows` is still fetched for every caller, on purpose.** Owner, 2026-09-03: *"We need to make the view request thing staff only"* → *"Viewing requests on the panel"*. `if staff or panel_shows_own_list(store, guild.id):` wraps the `summary_line` block AND the `PANEL_EMPTY` line, and nothing else in `build_panel` moved. The obvious "optimisation" — skipping the `list_requests(user_id=actor.id)` query when the list is hidden — would BREAK the withdraw select, which reads the same rows to decide what is `WITHDRAWABLE`; a member who cannot see their list must still be offered "Take one back…". ⚠️ **No sentence says the list is hidden**: with the key off a member's embed is `PANEL_INTRO` alone (plus the requests-off / staff-only-filing line when one applies), and the intro's "see where what you already asked for has got to" is still honest — the site button and the withdraw select both still name their requests. See deviation 12 in [`requests-panel-design.md`](requests-panel-design.md). |
| `black_bloc/settings_store.py:870` (`REQUEST_CARD_DEFAULT`) / `:1302` (`default`) | ⚠️ **The DEFAULT for `request_channel_moves` is every move but `done`; the CHOICES stay all seven.** Owner, 2026-09-03 10:17, minutes after the panel landed: "suppress the request.done box in discord, it's redundant information. This should be in website logs only". The gate already existed (`requests.py:521` `posts_a_card`, checklist 33 both ways since the third pass), and the live DB held NO stored `request_*` row, so a default change is the whole fix and a server can tick `done` back on from the Settings page. Two tuples, not one edited: `KEY_CHOICES` must keep `done` or the Settings checkbox and `/settings set-value` would REFUSE it. The requester's DM on done (`DM_LOOKS`, `request_dm_on_decision`) is untouched — it goes to the person who asked, not to the channel, so it is not the duplicate the owner named. `test_every_card_but_done_posts_and_one_pair_of_eyes_is_enough_until_a_lead_says_otherwise` pins both tuples. |
| `black_bloc/requests.py:477` (`panel_shows_own_list`) / `:51` (`PANEL_OWN_LIST_KEY`) | The helper and its key sit beside `panel_minutes`/`PANEL_MINUTES_KEY` rather than in the cog, so the cog holds no `store.get(..., "request_panel_own_list")` string of its own — the same shape `channel_moves`, `review_by_other` and `panel_minutes` already use, and the reason `tests/test_requests.py` can pin the default (`Store()` with no value at all → `False`) without building a `SettingsStore`. `bool(store.get(...))` rather than `is True`, because the registry coerces the key to a real bool before it is ever stored and a `None` from an unconfigured guild must read as off. Registered in the three places `request_panel_minutes` is: `settings_store.py:877` (`KEY_TYPES`, `"bool"`), `:898` (`KEY_HELP`) and `:1307` (`default()` → `False`) — which is all checklist 33 needs, since `VALUE_KEYS` (`cogs/core.py:27`) derives the `/settings set-value` list from `KEY_TYPES` and the Settings page reads the same registry. |
| `black_bloc/api/tools/requests.py:618` (`request_withdraw`) | The route keeps its own `NOT_YOURS` check (whose row is this?) and hands the rest to `withdraw_request` — identity belongs to the caller, because the panel and the route each already know their caller a different way (a Discord `User` versus a signed-in session), and `withdraw_request` takes a `row` it trusts is already the actor's own. |
...
| `black_bloc/panels.py:67` (`Panel`) | **Was `RequestView` (`cogs/community/requests.py:674–713` before this wave), moved verbatim except that the gone-quiet sentence is now the `footer` constructor argument** instead of a module constant read out of `requests.py`. `AnswersErrors` stays FIRST in the bases (checklist 30 — a view that defers without an `on_error` leaves a permanent spinner). Invariant P7. The cog's `RequestView(Panel)` (`cogs/community/requests.py:638`) keeps its name and its `__init__(minutes)` so `build_card`, `build_panel`, `open_withdraw_confirm` and every requests test are untouched. |
...
| `black_bloc/panels.py:55` (`capped_placeholder`) / `:14` (`CAPPED_PLACEHOLDER`) | Invariant P10 — Discord's 25-option select cap, with a placeholder saying what was left off. `CAPPED_PLACEHOLDER` is the sentence's ONE home (checklist 15): `requests.py:192` `PICK_CAPPED = CAPPED_PLACEHOLDER` reads it rather than restating it, and `requests.pick_placeholder` (`:453`) is now a one-line call passing `PICK_A_REQUEST`. The `pick` word stays per-feature ("Pick a request…", "Pick a poll…"); only the capped wording is shared. `capped` is still a parameter so a feature whose list is NOT on the site can say something else. |
| `black_bloc/panels.py:63` (`panel_minutes`) | Takes the settings KEY, so each feature keeps its own `<feature>_panel_minutes` registry entry (checklist 33 / invariant P13) while the read is one function. `requests.panel_minutes(store, guild_id)` (`:473`) is now a one-liner over it with `PANEL_MINUTES_KEY`; it is imported into `requests.py` as `library_panel_minutes` purely so that module's own two-argument public name does not shadow it. |
