...
| `black_bloc/config.py:51` | Optional on purpose: with no Twitch app the go-live feed still works on Discord presence alone, and the cog says so once in the log (`cogs/content/golive.py:335`). They are `str \| None` rather than `str` so that "not configured" and "configured with an empty string" cannot be confused. |
...
| `black_bloc/config.py:98` | Both halves or nothing: a client id with no secret cannot mint a token, so it must not count as configured. This property is the only thing the cog checks (`cogs/content/golive.py:269`). |
...
| `black_bloc/cogs/core.py:92` | The always-loaded cog, and the **template every other cog copies**: a `commands.Cog` subclass in its own module plus a module-level `async def setup(bot)`. Cogs reach the DB via `bot.db` and config via `bot.settings`, and never import each other (`architecture.md`, rule 2). |
| `black_bloc/cogs/core.py:97` | `ephemeral=True` on both commands: liveness checks are for the caller, and an ephemeral reply cannot clutter a channel however often it is run. |
| `black_bloc/cogs/core.py:141` | `/settings` lives in **core**, not in its own cog, because it configures the bot rather than a feature (`phase1-design.md`, §1 — "keep it simpler"). Declaring the `app_commands.Group` as a class attribute is how `discord.py` 2.x binds a group to a cog instance; it needs no registration beyond the cog itself. |
| `black_bloc/cogs/core.py:151` | Rendering goes through `settings_store.display_value` rather than a `<#…>` template here, because the registry now holds roles, numbers, an enum and free text. One fact, one home: the type table decides how a value reads, and this loop does not know the difference. |
| `black_bloc/cogs/core.py:184` | The action log call comes **after** the reply, so a log-channel problem can never delay or break the answer the person is waiting for. `log_action` swallows its own Discord failures (`actionlog.py:272`). |
| `black_bloc/cogs/core.py:226` | ⚠️ **Beyond the letter of `phase2-design.md`, and deliberate.** The design's settings table adds role, text, int and enum keys but names no command that can set them, which would have left `golive_template` and the three role filters reachable only by editing SQLite. `parse_value` accepts a raw id or a `<@&…>`/`<#…>` mention so a pasted mention works; validation still happens in `coerce_value`, so this command adds a parser, not a second rule set. |

| `black_bloc/cogs/core.py:111` | ⚠️ **`/help` — the owner's ask, verbatim: "we also need a / command that vomits out every /command that can be run" (sweep batch 2, 2026-08-26).** It reads the **tree**, never a hand-written list, so a command added to any cog appears here the day it is registered and nothing can go stale. First page through `response.send_message`, the rest through `followup.send`, all ephemeral and all `allowed_mentions=none` — the same shape `/settings show` uses (`:152`), because a command list is already past Discord's 2000 characters at 30-odd commands. |
| `black_bloc/cogs/core.py:81` | Commands are **global** and copied into the dev guild by `command_sync.py:19`, so the tree answers two different questions depending on whether it is asked with a guild. Both are asked and merged by name: a guild-only command would otherwise be missing from the list of "every command that can be run **here**". |
| `black_bloc/cogs/core.py:60` | The rendering is a pure function over command objects — no `interaction`, no bot — so the walk through groups, subgroups and the `(staff)` marking is asserted without Discord. The filter keeps a group's heading when the group's own name matches, so `/help filter:tempvoice` still says what `/tempvoice` is before listing it. |
| `black_bloc/cogs/core.py:115` | ⚠️ **`/help` filters the tree through the visibility registry, because the tree alone would still list a hidden command.** `tree_commands` merges the global commands with the guild copies (`:79`), and hiding removes the command from the **guild** copy only — the global one is still there, so a name-merge would put `/rolemenu` back in the list while nobody can run it. Reading the stored mode instead (`command_visibility.py:38`) makes `/help` agree with reality during the five seconds the debounce is open, too. |
...
| `black_bloc/cogs/community/role_menus.py:2084` | `/rolemenu mode` is the slash half of the dashboard switch, logged as `role_menu.mode` so the audit tab shows who turned role selection on or off. Modelled on `/tempvoice mode` (`cogs/community/tempvoice.py:1766`); the reply says what happens next, because "now off" alone does not tell a Lead the panels are still up. |
...
| `site/mock/server.mjs:729` | The mock's feature list is now **derived** from its own `SETTING_SPECS` (every key ending `_mode`) instead of a hand-written array of seven names, which is `api/status.py:89`'s rule in the mock's own terms — adding a mode key to one half no longer needs a matching edit in the other. |
| `site/mock/check.mjs:164` | ⚠️ **The contract check has to turn role menus on before it can see the post route at all.** With the key off, `POST /api/rolemenus/{name}/post` answers 409 — which would have made the guard pass (`GUARDED`) go green for the wrong reason and the shape pass (`checkRoutes`) fail outright. `tests/api/test_contract.py`'s `seeded` fixture sets the same key for the same reason, beside `events_create_scheduled`. |

## `black_bloc/cogs/community/tempvoice.py` — F8

| Key | Note |
|---|---|
| `black_bloc/cogs/community/tempvoice.py:1199` | The panel, the member picker and both modals carry `AnswersErrors` (`command_errors.py:43`) as their FIRST base, so the mixin's `on_error` wins the MRO over `discord.ui`'s logging default. Review finding F6: a raise inside any of them used to be a library log line and a control that did nothing. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/community/tempvoice.py:1665` | ⚠️ **The panel is now POSTED while test mode is on, into the test channel, and the old `would_post_panel` is gone.** Not posting it at all was defensible for a side effect (checklist 1) but the panel is a *message*, and the whole point of the feature is that somebody presses the buttons — the owner could not try a single control until test mode was lifted. This is exactly the trade `cogs/community/events.py:486` already made for the review card: the send is still the **guarded** send, so nothing reaches a channel the policy forbids, and the first line says which voice channel it controls. Three outcomes, three log kinds: posted in the voice chat → nothing extra; posted in the test channel → `tempvoice.panel_elsewhere` carrying both ids; no test channel to post in → `tempvoice.panel_failed` with reason `no_test_channel` (checklist 2 and 10). |
| `black_bloc/cogs/community/tempvoice.py:821` | ⚠️ **The panel no longer lives in the channel it controls, so `interaction.channel_id` is no longer the answer to "which channel is this?"** Three sources, in order: the id a modal or a select was constructed with (deterministic — a modal-submit interaction is not guaranteed to carry the message it came from), the **panel message id** looked up against `tempvoice_channels.panel_message_id` (which is why `panel_channel_id` now sits beside it), and finally the chat the click came from, which is what a panel sitting in its own voice channel still resolves by. Get this wrong and a click in the test channel edits the test channel. |
| `black_bloc/cogs/community/tempvoice.py:834` | `panel_context` hands back the row **and the resolved channel object**, because every handler needs both and each one resolving it separately is the second implementation this file exists to avoid (checklist 15). A row whose channel has vanished is answered with a sentence rather than an `AttributeError` inside a button. |
| `black_bloc/cogs/community/tempvoice.py:907` | ⚠️ **One helper per action, called by the button AND by the slash command, returning the sentence rather than sending it.** The logic used to live inside the button callbacks; `/voice rename` would have been a second copy of the rename rule, and the two would have drifted the first time one of them was fixed. Each helper logs **before** it returns (checklist 12 — the action log must survive a failed reply), and the caller does the answering, which is what lets the panel answer through `followup` and a command answer through `response` with one body. |
| `black_bloc/cogs/community/tempvoice.py:886` | ⚠️ **Renaming a channel is capped by Discord at two edits per ten minutes** (checklist 24), and the refusal used to blame the bot's permissions for it — which sends somebody to a Lead to fix a permission that is already correct. A 429 now gets its own sentence naming the real limit. `discord.RateLimited` is caught alongside `HTTPException` because they are **not** related by inheritance: the library raises the former only when a `max_ratelimit_timeout` is set, and it would otherwise escape every `except HTTPException` in this file. |
| `black_bloc/cogs/community/tempvoice.py:928` | `do_privacy` takes an optional `want`, so the **button toggles** and `/voice lock` **states**. Asking for the state a channel is already in returns a sentence and spends **no** API call — a `/voice lock` on a locked channel is otherwise a silent no-op that looks identical to a failure. ⚠️ **This and `do_forget_member` (`:1001`) are the two READ-MODIFY-WRITE edits, so they take the per-channel lock (checklist 6) and the blind ones do not** — Lock followed quickly by Unlock could otherwise leave the channel locked while telling the owner it was unlocked, whereas rename, limit, bitrate, region, ban and permit are writes where last-one-wins is the right answer, and locking them would hold one owner's rename up behind another edit on the same channel. ⚠️ **The lock NARROWS this window; it does not close it:** `overwrites_for` reads discord.py's cached channel, which is refreshed by a gateway event and not by the edit call, so a second read can still be stale. The claim/transfer race is the one genuinely closed, because that one re-reads the DATABASE inside the lock (`:1180`). |
| `black_bloc/cogs/community/tempvoice.py:1017` | Unban and unpermit are one helper because they are one operation — clearing that member's own `connect`/`view_channel` back to "whatever the channel says" — and two log kinds and two sentences because they are two intentions. The overwrite is **removed entirely** only when nothing else is left on it (`PermissionOverwrite.is_empty`); one that also carries, say, a `speak` denial keeps it. Removing the whole overwrite unconditionally would quietly undo settings this feature never made. |
| `black_bloc/cogs/community/tempvoice.py:276` | ⚠️ **Which channel a `/voice` command acts on, and the order matters.** The channel you are **connected to** wins over the one you merely own, because somebody who owns two (one abandoned, one they are sitting in) means the one they are sitting in. `claim` passes `owner_only=False` and takes *only* the connected channel — claiming something you are not in is the one thing the vendor's own docs leave undefined, and our answer is no (the same rule the Claim button has always had). |
| `black_bloc/cogs/community/tempvoice.py:270` | The `/voice` group is gated on `tempvoice_allowed_role_id`, the same setting that decides who join-to-create makes a channel for — one fact, one home (checklist 15). It is deliberately not `require_staff`: these are the owner's controls for their own channel, and the panel has always let a plain member press them. |
| `black_bloc/cogs/community/tempvoice.py:1044` | ⚠️ **Discord's bitrate ceiling is per boost tier, and `guild.bitrate_limit` is a FLOAT in bits.** The command takes kbps (8–96) because that is what Discord's own UI shows, clamps to the guild's ceiling, and says so when the clamp bit — silently giving somebody 64 when they asked for 96 is the kind of thing they re-run four times. A guild object with no `bitrate_limit` (a stale cache, a fake) falls back to 96 kbps, which every server has. |
...
| `black_bloc/cogs/community/tempvoice.py:1074` | Roles and members share one overwrites dict and `discord.py` types the keys, but a **pure** function cannot ask `isinstance` about a fake — so the caller passes the guild's role ids and this skips them. That keeps "who did the owner let in by name" testable without a guild, the same reason the position maths (`:234`) is pure. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/community/tempvoice.py:480` | ⚠️ **`None` still means "leave this one alone", and that is why `region` stores the STRING `auto` rather than `None`.** Every field is optional and a `None` argument keeps whatever was there, so a knob whose "off" value is `None` could never be turned back off. `/voice region auto` therefore writes `"auto"`, and `_create_for` (`:1517`) reads that as "pass no `rtc_region`". |
| `black_bloc/cogs/community/tempvoice.py:536` | `/voice reset`'s whole implementation: the row is **deleted**, not blanked, so `get_prefs` goes back to `None` and every default applies again — the same reasoning as `SettingsStore.clear` (`settings_store.py:844`). It returns whether a row was really deleted, so the command can tell "forgotten" from "there was nothing to forget" instead of claiming a change that did not happen. |
| `black_bloc/cogs/community/tempvoice.py:545` | ⚠️ **The two id columns are JSON read by a function that cannot raise.** This is called from `_create_for`, inside a voice event, where a raise is invisible (the same rule as `channel_name` at `:208`) — so a hand-edited row, a truncated write or a value that is not a list at all yields an empty list and a channel with no remembered members, never a member sitting in a lobby nothing happened in. |
...
| `black_bloc/cogs/community/tempvoice.py:583` | ⚠️ **A remembered member is re-applied as an overwrite on the NEW channel, and a member who has left is skipped rather than guessed at.** `guild.get_member` is the only check: an id that no longer resolves is dropped silently, because a `discord.Object` key would be written as a *member* overwrite by `Guild._create_channel` anyway (`:1666` has the same trap for roles) and an id for somebody who left is not a permission decision worth keeping. The owner is skipped too — their own overwrite is already the full one `owner_overwrites` (`:344`) wrote, and a stale "banned" entry for the owner would lock them out of their own channel. |
| `black_bloc/cogs/community/tempvoice.py:1123` | `/voice info` now answers two questions, not one: **what this channel is** (read off Discord) and **what the next one will be** (read off `tempvoice_prefs`). They can differ — somebody else's `/voice transfer`, a manual edit in Discord, a permit that Discord refused — and a page that showed only the first would make "it forgot my settings" impossible to diagnose. The block ends by naming `/voice reset`, because a memory the owner cannot see the end of is a memory they cannot switch off. |
| `black_bloc/cogs/community/tempvoice.py:1806` | ⚠️ **Extracted from `_voice_target` so `/voice reset` can be gated without owning a channel.** Every other `/voice` command acts on a channel, so the guild check, the `tempvoice_allowed_role_id` check and the database check used to live inside the lookup; reset applies to a person, not a channel, and copying the three checks would have been the second implementation of the gate (checklist 15). `_voice_target` calls this first, so the two cannot disagree about who may use `/voice`. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/community/events.py:84` | Checklist 9 in two dicts rather than a health class: what each loop last finished, and what it last raised. `/event settings` prints both (`black_bloc/cogs/community/events.py:1608`), because `is_running()` is not health — a loop that died three hours ago and a loop that ran a second ago are both "running" until one of them is not. |
| `black_bloc/cogs/community/events.py:107` | Names **both** halves of what test mode needs — a visible test channel AND a category around it — because review finding F12 showed they fail together and the old sentence only mentioned one of them. |
| `black_bloc/cogs/community/events.py:113` | `clamp` strips, so a title of spaces arrives as `""` and used to become a channel called `pending` with an embed that has no heading. Refused instead (review finding F14). |
| `black_bloc/cogs/community/events.py:117` | The two DST refusals, kept separate because the fixes are different: a skipped hour needs a different time, a repeated hour needs the person to say which of the two they mean. The detection is in `timezones.py:57`, not here — this file only says it in words. |
| `black_bloc/cogs/community/events.py:127` | Review finding F14: the modal never said which zone it read. A `TextInput` has no description field, so the Start box's **placeholder** carries it, and it is set per instance in `EventModal.__init__` because the zone is the requester's. `discord.py` deep-copies a modal's children per instance (`ui/modal.py:123`), so writing to `self.start` cannot leak one person's zone into the next person's modal. Placeholders cap at 100 characters and the longest IANA name is 32, so this cannot overflow. |
| `black_bloc/cogs/community/events.py:128` | What a cancelled event's public announcement is edited to say (review finding F7). |
| `black_bloc/cogs/community/events.py:174` | ⚠️ **The reason codes are for the LOG; this maps them to sentences for the person** (review finding F5). Auto-cancels told nobody at all — a proposal simply stopped existing — and `cancelled_by_<id>` is not something to send anyone. The default covers every code with no entry, so a new reason is a missing sentence rather than a crash inside a background loop. |
| `black_bloc/cogs/community/events.py:195` | Review finding F2's apology. An event whose end time had already gone by was announced as starting now; it is marked done in silence instead, and the requester is told plainly that nobody was told. Saying nothing would leave them believing it ran. |
| `black_bloc/cogs/community/events.py:344` | `card_channel_id` is written by the same call that writes the other two, so the three can never disagree about where the card is (`storage/db.py:227`). |
| `black_bloc/cogs/community/events.py:413` | Review finding F14: `_event_locks` grew one entry per event for the life of the process and never shrank. A settled event cannot be raced — `can_transition` (`black_bloc/events.py:33`) refuses every move out of a terminal status — so its lock is dropped. ⚠️ **Dropping it while it is held is safe and deliberate**: a waiter already holds a reference to the `Lock` object and is unaffected, and a click arriving after the pop makes a fresh lock that immediately finds a terminal status. |
| `black_bloc/cogs/community/events.py:513` | Returns whether the person was actually told, because "best effort" and "nobody knows" are the same thing until somebody records the difference. |
| `black_bloc/cogs/community/events.py:526` | The DM a requester is **owed** — cancelled, missed — goes through here so a closed DM is `event.dm_failed` in the action log rather than an `INFO` line nobody reads (review finding F5). The DMs that are merely nice to have still call `dm` directly. |
| `black_bloc/cogs/community/events.py:616` | The cache is empty after a restart, and an event approved before the restart is exactly the one most likely to need cancelling. `fetch_scheduled_event` is a round trip and is therefore only reached when the cache misses. |
| `black_bloc/cogs/community/events.py:628` | ⚠️ **The bug that killed the reconcile loop for the life of the process** (review finding F1). `ScheduledEvent.cancel()` raises a bare `ValueError` when the event is not `scheduled` (discord.py 2.7.1, `scheduled_event.py:296`), and only `HTTPException` was caught — so one event that had already started took the loop with it and left `/event cancel` half done. A running event is **ended** rather than cancelled, because Discord models those as different transitions; anything else it refuses is logged `event.cancel_scheduled_failed`, and under the guard nothing is touched at all and the log says `event.would_cancel_scheduled` (review finding F8, which also found this path returning silently). |
| `black_bloc/cogs/community/events.py:702` | ⚠️ **`announce_message_id` was write-only** (review finding F7): a cancelled event left a public post still advertising it, with the card still saying `approved`. Editing is a second way to speak in a channel, so it asks the guard the same way the post did (`black_bloc/cogs/community/events.py:660`) and has the same three kinds — edited, `would_edit_announcement`, `edit_announcement_failed`. `get_partial_message` avoids a fetch when the library offers one. |
| `black_bloc/cogs/community/events.py:1038` | **Phase 8a merge.** `loop_health(name)` is how the status page reads this cog's loop health without knowing its attribute names: here they are ⚠️ **dicts keyed by loop name**, and the key is the coro name with its `_` prefix and `_loop` suffix stripped — `_golive_loop` → `golive`. Handing that raw dict to a reader that expected a string is the bug this method exists to prevent: a non-empty dict is truthy, so a healthy loop rendered an error. The page asks; it never guesses (`black_bloc/api/status.py:139`). |
| `black_bloc/cogs/community/events.py:1044` | ⚠️ **A `tasks.loop` that raises does not retry — it stops, for good, silently** (review finding F1; discord.py logs it and returns). Both loops therefore register an `@error` handler that records the failure where `/event settings` will show it and calls `restart()`. `restart()` is safe from inside the handler: it schedules the restart on the task's done callback rather than re-entering the loop. |
| `black_bloc/cogs/community/events.py:1097` | ⚠️ **Three outcomes now, not one** (review finding F2). An approved event whose `ends_at` has already passed is never announced — the bot was offline, and "starting now!" for something that finished two hours ago is worse than silence. One that merely started late is capped by `events_max_late_minutes` (`settings_store.py:42`) and goes `live` quietly past it. The status moves in every case, because the row must stay true whether or not anybody was told. |
| `black_bloc/cogs/community/events.py:1132` | Runs **inside** the caller's lock — it is only ever reached from `_go_live`, which holds it — so it must not take the lock again. `approved → done` is a legal transition (`black_bloc/events.py:33`) precisely so this can exist. |
| `black_bloc/cogs/community/events.py:1194` | ⚠️ **Two consecutive misses before a cancel** (review finding F9). One reconcile pass seeing no channel is not evidence the channel is gone: a dropped gateway event, a cold cache after a reconnect or a partial outage all look identical, and the old code settled somebody's proposal on that. The note lives in memory on the cog rather than in the database on purpose — a restart should re-start the count, because a restart is exactly when the cache is least trustworthy. ⚠️ An unreadable `starts_at` is cancelled outright (review finding F10): the row could never go live, never finish and never resolve, so it would have sat `pending` for ever. |
| `black_bloc/cogs/community/events.py:1219` | ⚠️ **The action log is written BEFORE Discord is asked to undo anything** (review finding F1, checklist 12): cancelling the scheduled event is a network call that can fail, and a failed one must not take the record of the cancellation with it. Then the announcement is edited (`black_bloc/cogs/community/events.py:702`) and the requester is told why (`black_bloc/cogs/community/events.py:174`) — unless they are the one who cancelled it, which is the only case where a DM would be telling somebody what they just did. |
| `black_bloc/cogs/community/events.py:1608` | Checklist 9: last success and last error per loop, never `is_running()`. It is a list of lines rather than a string so `/event settings` splices it into the same block as everything else. |
| `black_bloc/cogs/community/events.py:80` | ⚠️ **A persistence key, matched by `discord.py` as a regex.** Same mechanism as the honeypot's Ban-now button (`cogs/moderation/honeypot.py:32`): one `add_dynamic_items` call at `cog_load` (`:1358`) revives **every** review card ever posted, with the event id carried in the `custom_id` itself. A stored persistent view would have needed one registration per event and a table of message ids. Approve and Deny share one class and one template because they share the entire path up to a single branch; two classes would have been two places to forget the staff check. Change this string and every card already in the server goes dead with no error anywhere. |
| `black_bloc/cogs/community/events.py:85` | A pending row with no review channel is normally impossible — the channel is made seconds after the row — but a process killed between the two leaves one, and nothing else would ever clean it up. The grace exists so the five-minute reconcile running *during* a submission cannot cancel an event that is halfway through being created. |
| `black_bloc/cogs/community/events.py:335` | The column name is interpolated into the SQL, and it is safe because both callers pass a module constant (`:861`), never anything a person typed. ISO-8601 UTC strings compare correctly as text, which is what lets "which events have started" be one indexed query rather than a Python loop over every row. |
| `black_bloc/cogs/community/events.py:359` | ⚠️ **`COALESCE` on the three decision columns, so a later write cannot erase who decided and why.** Going live and finishing both call this with no `decided_by`, and without the coalesce each of those would blank the deny reason and the decider recorded at approval time. `decided_at` is set only when `decided_by` is, because "when" without "who" is not worth keeping. |
| `black_bloc/cogs/community/events.py:402` | ⚠️ **The per-event lock lives on the BOT**, for the same reason temp voice's does (`cogs/community/tempvoice.py:276`): a button click arrives with an `interaction` and never with a cog, so the lock has to be reachable from `interaction.client`, and a module-level dict outlives the event loop that made it. This is the whole defence against two mods deciding at once — there is no unique index to fall back on, because the row already exists and it is the *status* that is being raced (checklist 6). Every writer re-reads the row **inside** the lock and asks `can_transition` (`black_bloc/events.py:33`); the loser is told who got there first rather than silently overwriting them. |
| `black_bloc/cogs/community/events.py:432` | The one place a stored row becomes a rendered card. Duration is derived from the two timestamps rather than stored a second time — one fact, one home — and an unreadable pair falls back to the default rather than raising inside a background loop. |
| `black_bloc/cogs/community/events.py:448` | ⚠️ **The requester IS given their own review channel** — an explicit member overwrite (view, send, read history), which reverses the Phase 4 default recorded here before (`phase4-design.md` left them out and called adding them a later ask; the owner made that ask on 2026-08-26 and it landed 2026-09-01). It is a **member** overwrite, so it neither depends on nor collides with the staff **role** ones above it, and a requester who is also staff simply has both. `read_message_history` is spelled out rather than inherited because the whole point is reading what staff asked before they arrived. ⚠️ **The access is not taken away at `done-`/`denied-`** (owner choice: simplest — nothing needs to re-edit overwrites at a decision, and retention deletes the channel anyway at `:1214`); ⚠️ and a **rename does not touch overwrites**, because `rename_channel` passes `name` and `reason` and nothing else (`:538`) — verified by reading that call, never against live Discord. `@everyone` is denied outright rather than left to inherit, because a review channel inheriting a public category would put every proposal in front of the whole server. The staff roles come from `settings_store.py:575` — the same computed-permission derivation the honeypot and every staff command use, so "approvers = anyone who can see the staff channel" has exactly one implementation. |
| `black_bloc/cogs/community/events.py:467` | ⚠️ **Channel creation is invisible to `guard.py`, so the test policy is enforced here by PLACE.** While a guard is installed, review channels are made in the **test channel's own category** and nowhere else — same rule as temp voice (`cogs/community/tempvoice.py:1637`) and the honeypot (`cogs/moderation/honeypot.py:310`). A test channel the bot cannot see means it cannot prove where it is, and the answer is no. ⚠️ **A test channel with no category is the same answer** (review finding F12): `category` was read without checking, so `create_text_channel(category=None)` put the review channel at the TOP level of the server — outside the very category the rule exists to confine it to, and outside anything `allows_place` would later let the bot rename or delete. Do not remove this when test mode is lifted; it is a no-op then (`bot.guard is None`). |
| `black_bloc/cogs/community/events.py:487` | ⚠️ **The one place Phase 4 deviates from what temp voice did, and deliberately.** The review card is a *message*, and the review channel is not the test channel, so `guard.py` would refuse the send — temp voice answers that by not posting its panel at all (`cogs/community/tempvoice.py:1597`). Here that would make the whole feature untestable: the Approve and Deny buttons **are** what the owner has to click. So while the guard is on, the card goes to the test channel instead — the send is still the guarded send, nothing is written anywhere the policy forbids, and the reply says plainly where it went (`:1108`). |
| `black_bloc/cogs/community/events.py:539` | ⚠️ **A rename is a side effect `guard.py` cannot see, so it asks `allows_place` first** and logs `event.would_rename` rather than doing it (review finding F3b) — a `would_…` kind, never shared with the `rename_failed` a real refusal produces (checklist 2). Renaming a channel is also **2 edits per 10 minutes** and `discord.py` sleeps through the 429 (checklist 24) — every caller defers first. It is also the last thing a decision does, after the status, the log, the scheduled event and the DM, because a rename Discord refuses must not take an approval with it; it logs `event.rename_failed` and the next status change tries again. The no-op check exists so the reconciler re-running does not spend one of those two edits saying nothing. |
| `black_bloc/cogs/community/events.py:571` | It returns the **event object** rather than its id, because the announcement needs `ScheduledEvent.url` to link it and the caller must be able to tell "made" from "not made" without a second lookup (review finding F4). ⚠️ **A real Discord scheduled event is visible to the whole server and `guard.py` cannot see it being made**, so while test mode is on it is skipped and logged `event.would_create_scheduled` (owner rule, `phase4-design.md`, §Test mode). The three outcomes never share a kind (checklist 2): made → the id on the row, refused by policy → `would_create_scheduled`, refused by Discord → `create_scheduled_failed`. `entity_type=external` is what lets an event be somewhere other than a voice channel, and Discord **requires** both an end time and a location for that type — the end time is the start plus the duration, and a requester who gave no location gets a sentence rather than a 400. |
| `black_bloc/cogs/community/events.py:660` | ⚠️ **The single guarded door to a public channel, and every refusal has its own kind.** `shadow` suppresses the post and nothing else (owner decision: the flow is staff-gated, so there is nothing to punish) → `would_announce`; a missing channel → `announce_failed`; the guard → `would_announce` with `test_mode`. Asking the guard *first* rather than letting it raise from inside the HTTP layer is the same trick as `cogs/content/golive.py:458`, for the same reason: in a background loop a `TestModeViolation` is an ugly traceback and a half-finished state. |
| `black_bloc/cogs/community/events.py:843` | ⚠️ **The order inside a decision is the contract.** Status → action log → scheduled event → announcement → the requester's DM → the rename → closing the card → answering the mod. Everything irreversible or recorded happens before anything cosmetic (checklist 12), and the answer is last because a failed answer must not take a real decision with it. All of it runs **inside** the lock, so a second mod's click finds the finished status rather than a half-applied one. |
| `black_bloc/cogs/community/events.py:925` | Deny asks for its reason in a modal, which is why the deny path cannot pre-defer: `send_modal` must itself be the response. The status is checked once before the modal opens — so nobody types a reason for an event that was approved while they were reading it — and again inside the lock after it is submitted, which is the check that actually decides. |
| `black_bloc/cogs/community/events.py:978` | ⚠️ **Five inputs, because `discord.py`'s `Modal.add_item` raises at six** — verified against the installed library at build time rather than taken from the design note, which flagged the cap as ambiguous. That cap is the whole reason there is no stream or voice picker here and location is free text (`phase4-design.md`, §The flow). The title field is `event_title` and not `title` because `discord.ui.Modal.title` is the modal's own heading. |
| `black_bloc/cogs/community/events.py:1083` | Approved-and-started rows are swept **before** live-and-finished ones, and the second query runs after the first loop has committed. That ordering is what lets an event the bot was offline for go live and then finish inside a single tick, instead of sitting `approved` forever because its end time had already passed (checklist 5). |
| `black_bloc/cogs/community/events.py:1183` | Reconciliation runs at `cog_load`, at `on_ready` **and** on a five-minute loop (checklist 4 and 25) — `setup_hook` runs before the gateway connects, so a reconcile that iterates `bot.guilds` there alone is a no-op on every real start, and a long-lived process never restarts on its own. |
| `black_bloc/cogs/community/events.py:1222` | Retention, and it reads `ends_at` rather than the decision time on purpose: a channel is kept for N days after the event **happened**, not after somebody pressed Approve. ⚠️ **Three things review finding F3 changed here, and the last one reverses what this note used to say.** It sweeps `denied-` and `cancelled-` channels as well as `done-` ones, because those were the two outcomes that piled up for ever; it asks `guard.allows_place` and logs `event.would_delete_channel` rather than deleting, because a delete in a background loop is the one mistake nobody can undo; and an unreadable `ends_at` now **KEEPS** the channel. The old convention — an unreadable timestamp counts as old — is right everywhere it means "stop waiting" (`black_bloc/golive.py:145`, `cogs/community/tempvoice.py:31`) and wrong here, because the action it licenses is destruction rather than tidying up. The floor on the setting is 1 day (`settings_store.py:41`), so "delete it the moment it ends" is no longer expressible at all. |
| `black_bloc/cogs/community/events.py:1262` | Three jobs, all of them "forget something that no longer exists" (checklist 26). A deleted **category** or a deleted **announce channel** is cleared from the settings — `SettingsStore.clear` (`settings_store.py:642`) exists for this — so `/event settings` stops naming a dead channel. A deleted **review channel** cancels its event immediately rather than waiting up to five minutes for the reconciler, which is only the backstop for the case the bot was offline. |
| `black_bloc/cogs/community/events.py:1313` | The row is written **before** the channel, so a crash between the two leaves something the reconciler owns rather than an orphan channel nobody remembers. A channel Discord refuses cancels the row in the same breath (`:1081`) — a pending event with no channel and no grace left would otherwise be cancelled minutes later by the loop, which is a worse way for the requester to find out. |
| `black_bloc/cogs/community/events.py:1417` | Returns **where the card actually went**, so the reply can say so instead of claiming a channel that has nothing in it (checklist 10). Three answers, three sentences: it is in the review channel, it is in the test channel because test mode is on, or it is nowhere and the log says why. It also **stores** that channel as `card_channel_id` (review finding F14), because otherwise the row held a `review_message_id` that does not exist in the `review_channel_id` beside it — a pair of columns that read as a fact and are one in test mode only by accident. |

## `black_bloc/cogs/moderation/honeypot.py` — F9

| Key | Note |
|---|---|
| `black_bloc/cogs/moderation/honeypot.py:380` | `SafeDynamicItem` (`command_errors.py:52`) first in the bases, and the body is `on_click` rather than `callback`, because `discord.py` swallows a dynamic item's exception itself and never reaches a view. Review finding F6: the highest-consequence button in the project — it bans somebody — was the one that could fail silently. |
| `black_bloc/cogs/moderation/honeypot.py:39` | ⚠️ **A persistence key, matched by `discord.py` as a regex.** `BanNowButton` is a `DynamicItem`, not a member of a stored view: one `add_dynamic_items` call at `cog_load` (`:411`) makes *every* Ban-now button ever posted work again after a restart, with the hit id carried in the `custom_id` itself. A plain persistent view would have needed one registration per button and a table of message ids. Change this string and every button already sitting in the log channel goes dead with no error anywhere. |
| `black_bloc/cogs/moderation/honeypot.py:111` | ⚠️ **The exemption matrix, and the only pure function in the feature — read it before changing who can be banned.** Order matters only for the log line (a staff bot reads as `bot`), but the *set* is the safety property: bots, Manage Server, anyone who can see the staff channel (`settings_store.py:359`), and the configured exempt roles. The vendor docs state no exemptions at all, so this list is ours by design (`phase3-design.md`, Part B). |
| `black_bloc/cogs/moderation/honeypot.py:175` | The burst guard. A spam bot posts five times in a second; the per-author lock (`:433`) serialises those five, and this makes the last four cost one query instead of one ban attempt each. The hit is still recorded and still logged — silence would look exactly like a broken listener. |
| `black_bloc/cogs/moderation/honeypot.py:211` | ⚠️ **A ban is invisible to `guard.py`, so the guard is asked here, and the answer while test mode is on is always no** (owner rule, `phase3-design.md`, Part B, §Test mode). It returns the *reason* rather than a bool because the three outcomes must not share a log kind: banned → `honeypot.banned`, refused by policy → `honeypot.would_ban`, refused by Discord → `honeypot.ban_failed` (checklist 2). The DM goes out **before** the ban, because a banned account cannot be DMed. |
| `black_bloc/cogs/moderation/honeypot.py:448` | The order is deliberate: exempt check → **delete** → record → ban. Deleting first in every mode but `off` is housekeeping, not punishment — nobody reading the channel should find spam sitting in it while the feature is still in shadow, and a shadow hit that kept the post visible would make the trap advertise itself. |
| `black_bloc/cogs/moderation/honeypot.py:567` | The watch-mode affordance: a shadow hit posts the content and a **Ban now** button into the log channel, so the owner converts a real catch into a real ban in one click without leaving Discord. It is skipped when there is no log channel, and skipped when the guard would refuse that channel — the button is useless if the ban behind it is refused anyway. |
| `black_bloc/cogs/moderation/honeypot.py:647` | Same test-mode shape as temp voice (`cogs/community/tempvoice.py:1640`): while the guard is on, the trap channel is created **inside the test channel's category**, and its pinned notice is not posted at all, because the notice is a message and `guard.py` would refuse it. The reply says which of the two happened rather than claiming a notice that is not there (checklist 10). |
| `black_bloc/cogs/moderation/honeypot.py:33` | ⚠️ **Only a real, human post trips the trap.** `MessageType` also covers join announcements, pins, boosts and thread-created notices, and every one of those is a message Discord posts *on behalf of* a member in the channel they acted in — banning somebody for Discord's own "so-and-so joined" line is the worst failure this feature could have. `reply` is included because a reply to the pinned notice is a real post. |
| `black_bloc/cogs/moderation/honeypot.py:34` | The shadow-mode dedupe window. A spam bot posts repeatedly; the per-author lock serialises those, `banned_already` stops the second real ban, and this stops the second **button**. Every hit is still recorded and still logged — as `honeypot.hit_recorded` rather than `honeypot.would_ban`, so the two are distinguishable at a glance (checklist 2) and the log channel does not fill with identical Ban-now buttons that all do the same thing. |
| `black_bloc/cogs/moderation/honeypot.py:77` | ⚠️ **Turning the trap `on` is refused while no staff role resolves**, and `/honeypot status` says so loudly in that state. This is the user-facing half of the derivation fix (`settings_store.py:359`): with an empty staff set the only exempt humans are those with Manage Server, so one mistyped message from a moderator is a real ban. The refusal names the setting and the command that proves it is fixed. |
| `black_bloc/cogs/moderation/honeypot.py:211` | ⚠️ **`delete_message_seconds`, not the deprecated `delete_message_days`**, and the value is clamped to Discord's 7-day ceiling here as well as in the validator (`settings_store.py:532`) — a bigger number stored before that validator existed would make **every** ban a 400. |
| `black_bloc/cogs/moderation/honeypot.py:427` | The trap catches posts in a **thread of** the trap as well as in the trap itself: a thread inherits the channel's visibility, so a bot that opens one is in exactly the same place. `parent_id` is read with `getattr` because an ordinary channel has none, and a `None` parent can never match a list of ids. Setup also denies thread creation to `@everyone` (`:593`) — this is the belt to that braces. |
| `black_bloc/cogs/moderation/honeypot.py:537` | ⚠️ **Deleting a message is a side effect `guard.py` cannot see**, so the delete asks the same place question temp voice asks (`cogs/community/tempvoice.py:1637`): while a guard is installed, the trap must be in the test channel's own category. Otherwise the answer is no and the attempt is logged as `honeypot.would_delete` — a `would_…` kind, never shared with the `delete_failed` that a real refusal produces. |
| `black_bloc/cogs/moderation/honeypot.py:651` | `/honeypot status` prints the **resolved** staff roles, by name and count, because that is how the owner checks the exemption is real. A count of zero while the mode is `on` gets the warning line rather than a blank: silence there reads as "no exemptions configured", which is exactly the state that bans a moderator. |
| `black_bloc/cogs/moderation/honeypot.py:730` | A trap channel that is deleted is forgotten automatically, and `/honeypot forget <id>` covers the case the bot was offline for. `setup` refuses a second trap while the first still exists (`:593`), so without a way to forget a dead id the command would be blocked forever. |
...
| `black_bloc/events.py:42` | Derived from `TRANSITIONS` rather than typed out, so a status that gains an exit stops being terminal in both places at once. It is what tells the cog when an event's lock may be dropped (`cogs/community/events.py:412`). |
| `black_bloc/events.py:166` | ⚠️ **The announcement promised an Interested button whether or not there was anything to click** (review finding F4) — and in test mode there never is, because a real scheduled event is visible to the whole server and is skipped (`cogs/community/events.py:570`). Three wordings, and the caller picks by what Discord actually returned: a made event with a URL gets the link, a made event without one points at the server's Events list, and no event at all says to watch the channel. Claiming a control that does not exist is the same defect as claiming a check that did not run (checklist 10). |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/content/golive.py:61` | Logged **once, at cog load**, not per poll: with no Twitch app the feature is not broken, it is simply presence-only, and a per-minute warning would train the reader to ignore the log (`phase2-design.md`, §Detection). |
| `black_bloc/cogs/content/golive.py:114` | Reads a column that may not be on the row object at all. `sqlite3.Row` raises `IndexError` for an unknown key rather than returning `None`, so a row fetched by older code — or a `dict` fake in a test — would otherwise blow up in the middle of ending a session. |
| `black_bloc/cogs/content/golive.py:124` | ⚠️ **A post that did not happen has a REASON, and the reason decides what gets logged.** Returning `None` for all four failures made a broken channel id, a 403 and a test-mode refusal indistinguishable from a shadow-mode dry run — every one of them logged `golive.would_announce`, so the owner reading the log during the shadow rollout could not tell a working feature from a broken one. |
| `black_bloc/cogs/content/golive.py:152` | One Twitch login belongs to at most one member. Checked at `/twitch link` time with a sentence naming the clash (`cogs/content/golive.py:960`) rather than at poll time, because `INSERT OR REPLACE` is keyed on `user_id`: two members claiming one login both get a row, and the poller can only announce whichever it saw first. |
| `black_bloc/cogs/content/golive.py:203` | Sessions are rows, not memory. That is the entire restart story: a bot that dies mid-stream comes back, finds `ended_at IS NULL`, and stays quiet instead of re-announcing — the incumbent's second measured defect. `announced_message_id` is stored so the end edit can find the message after a restart too. ⚠️ The `IntegrityError` branch is the **partial unique index** (`storage/db.py:13`) speaking: it means another path opened a session for this member between the debounce check and this insert. Returning `None` rather than raising is what lets the caller treat "someone beat me to it" as a normal outcome. |
| `black_bloc/cogs/content/golive.py:210` | Used when the announcement could not be posted in mode `on`: the row is removed rather than closed, so it starts **no cooldown** and the next presence event or poll tick tries again. A closed row would look like a stream that ran and ended, and would suppress the retry for a full `golive_cooldown_minutes`. |
| `black_bloc/cogs/content/golive.py:215` | ⚠️ **Records what actually happened, not what the settings say should have happened** — and since 2026-09-01 it records *which role*, not merely that there was one. The removal path reads the row alone (`:613`), so a live role survives the mode being changed, the guard being installed, or the role setting being cleared mid-stream — all three of which used to strand the role on the member forever, because removal was gated on the *current* mode. ⚠️ The 0/1 flag left one hole those three did not cover: a staffer who **repointed `golive_live_role_id` at a different role mid-stream** had the old role taken off nobody and the new one taken off somebody who never had it. Storing the id closes it, exactly as `birthdays.role_added_id` does (`cogs/community/birthdays.py:153`). `live_role_added` is kept beside it rather than replaced by "id is not NULL" because a row written before the column existed has `NULL` there and did have the role — see the fallback at `:613`. |
| `black_bloc/cogs/content/golive.py:238` | The optional `source` is what keeps the two detection paths from fighting: the poller only ends sessions **it** opened (`source='twitch'`), while a presence go-live claims the user for the presence path. Without it a Twitch poll a minute after someone's presence went live would close a session the grace timer still owns. |
| `black_bloc/cogs/content/golive.py:297` | Twitch logins are 4–25 characters of letters, digits and underscore. Accepting a full URL as well means a member who pastes their channel address gets linked instead of a refusal — the commonest way this command is used wrong. |
| `black_bloc/cogs/content/golive.py:316` | ⚠️ Two `app_commands.Group` class attributes; the Twitch client is `self.helix` **on purpose**, because `self.twitch` is the group object `discord.py` binds here and assigning over it would unregister the commands. |
| `black_bloc/cogs/content/golive.py:319` | **Phase 8a merge.** `loop_health(name)` is how the status page reads this cog's loop health without knowing its attribute names: here they are `last_poll_ok_at` / `last_poll_error`, and only the `poller` loop has any. The page asks; it never guesses (`black_bloc/api/status.py:141`). |
| `black_bloc/cogs/content/golive.py:324` | Reconciliation runs **before** the poller starts and regardless of whether Twitch is configured — presence-only servers strand sessions too. ⚠️ **And since 2026-09-01 the poller itself starts regardless too**, because the sweep is not only a Twitch sweep: `poll_once` ages sessions out *before* it looks at Helix (`:753`), so gating the loop on credentials left a presence-only server with no age-out tick at all between restarts, which is the one thing a wedge fix must never do. The loop is a no-op past the age-out on such a server — `poll_once` returns at the `helix is None` check — so this costs one function call a minute and buys the backstop. `/golive status` says so in words rather than reporting a running loop as Twitch polling (`:651`). The `is_connected` check is the offline-construction contract (`architecture.md`, rule 6): `tests/test_bot.py` loads every cog with no `connect()`, and both a `tasks.loop` and a reconcile query started there would fail. |
| `black_bloc/cogs/content/golive.py:335` | ⚠️ **The fix for the wedge.** `should_announce` refuses forever while a row has `ended_at IS NULL` (`black_bloc/golive.py:285`), so a kill -9, a Fly redeploy or a crash mid-stream left that member permanently unannounceable — the failure was silent and looked exactly like a working feature. Every open row is now re-checked at start and closed unless it can be *shown* to still be live. |
| `black_bloc/cogs/content/golive.py:343` | Two independent proofs of life, in cost order: the cached presence (free, but only if the guild is chunked — an uncached member reads as "not streaming", and closing is the safe answer there) and, for a Twitch-sourced session, one Helix call. ⚠️ A Twitch call that **fails** leaves the session open on purpose: "I could not check" is not "they are offline", and the age-out at `:718` is the backstop that stops even that wedging forever. |
| `black_bloc/cogs/content/golive.py:364` | The order is the one the review fixed: close the row → move the role → write the action log → **then** the cosmetic message edit. The edit is a round trip to Discord that can 403, 404 on a deleted message, or be refused by the guard; putting it last means none of those can abort the three things that matter. Same order as `_end_live` (`:458`), and the two are separate only because reconciliation has no `Member` object to hand. |
| `black_bloc/cogs/content/golive.py:387` | ⚠️ **The transition matters, not the state.** `before` still streaming means this event is a category or title change, and Discord sends a *lot* of those — reacting to `after` alone would re-announce on every one. The `_cancel_end` on the live branch is the flap fix: presence drops for a few seconds surprisingly often, and a cancelled grace timer is a stream that never ended. |
| `black_bloc/cogs/content/golive.py:399` | ⚠️ **The read-decide-insert below is not atomic**, and the presence listener and the Twitch poller can both reach it for the same member within the same tick — `open_session_for` says "nothing open" to both, and both announce. The per-user lock closes the window inside one process; the unique index (`storage/db.py:13`) closes it in the database, which is what survives a second process. Both, deliberately: the lock keeps the second caller from doing the work at all, the index keeps a bug in the lock from reaching `#live-now`. |
| `black_bloc/cogs/content/golive.py:403` | The order of the gates is deliberate: mode → bot → database → opt-out → role filters → debounce. Each is cheaper than the next, and the opt-out check comes before anything is written so an opted-out member leaves **no** trace in the sessions table (`phase2-design.md`, §Detection). |
| `black_bloc/cogs/content/golive.py:555` | ⚠️ **The guard is asked before the send, not after.** `guard.py` would raise `TestModeViolation` from inside the HTTP layer, which in a background task means an ugly traceback and a half-written session; asking first turns the same rule into a logged `golive.would_announce`. ⚠️ **`test_mode` is deliberately NOT treated as a post failure** — unlike a missing channel or a 403 it is a policy decision that will never succeed on retry, and rolling the session back would make every poll tick re-attempt and re-log it once a minute. It reads as shadow, which is what it is. Do not remove this when test mode is lifted — it is a no-op then (`bot.guard is None`). |
| `black_bloc/cogs/content/golive.py:582` | ⚠️ **Role edits are invisible to `guard.py`** (same class of hole as the role-menu select, `cogs/community/role_menus.py:659`). So the live role is only *added* when the mode is `on` **and** no guard is installed; otherwise the intent is written to the action log as `would_add_role`. Owner rule, `phase2-design.md`, §Test mode. It returns **the id of the role that really moved** (`None` when none did), because that id is what gets written to the session row (`:457`) — returning a bare `True` made the caller re-read the setting, which is the very thing that can change mid-stream. |
| `black_bloc/cogs/content/golive.py:615` | ⚠️ **Removal is unconditional on the session row, never on the current mode.** Adding is a policy decision; removing is cleaning up after one that was already made, and the settings can have changed in between. ⚠️ **The row names the role, and the current setting is only the fallback** (`:613`): `live_role_id` is `NULL` on any session opened before schema 19, and for those — and only those — today's `golive_live_role_id` is the best guess available, which is exactly the behaviour they had when they were written. A row written since carries the id that actually went on, so repointing the setting mid-stream no longer strands the old role. Every way the removal can fail — role deleted, member gone, guard installed, Discord refusing — ends in `golive.role_stuck` naming the member and role ids, because a *Live* role stuck on someone who is not live is invisible to the log otherwise, and it is the failure a person actually notices. |
| `black_bloc/cogs/content/golive.py:661` | ⚠️ Without this a template a staffer edited to contain `@everyone` — or a member whose display name does — pings the server. Only the configured ping role is allowed through, by id, so the mention in the rendered text still works and nothing else does. Passed on the end edit too (`:558`): an edit can create a mention the original post did not have. |
| `black_bloc/cogs/content/golive.py:715` | The 120 s grace is a **task per user**, held in `_end_tasks`, so a reappearing stream just cancels it. The `current_task()` check in `finally` is not paranoia: `_cancel_end` replaces the entry, and without the check a cancelled task's cleanup would pop its successor and leak a timer nobody can cancel. |
| `black_bloc/cogs/content/golive.py:727` | ⚠️ A `tasks.loop` that raises **stops for good**, silently. The body is one call inside a `try` so a Twitch outage costs one log line and the poller lives; `poll_once` is the real work and is what the tests call. `last_poll_error` is set here as well as inside `poll_once` because the two failures are different: this one is a bug, that one is Twitch. |
| `black_bloc/cogs/content/golive.py:739` | The backstop behind reconciliation: a session opened *after* the last restart can still be stranded (a member who leaves the guild, a presence event that never arrives, a grace task killed with the process). Unparseable `started_at` counts as old — the one thing a wedge fix must never do is wedge. Runs before the Twitch sweep and independently of it, so it still happens when Helix is down **and on a server with no Twitch credentials at all** — the poller is started unconditionally for exactly this (`:322`). It used only to run where credentials existed, which meant the backstop behind the backstop was missing on the servers most likely to need it: presence-only ones, where a dropped presence event is the whole failure mode. |
| `black_bloc/cogs/content/golive.py:756` | ⚠️ **A Twitch outage used to be SILENT on the Logs page.** A `TwitchError` aborts the sweep and returns, so open `twitch` sessions stay open and nothing is announced or ended until Helix answers again — correct behaviour, invisibly performed: the only trace was a `WARNING` in the process log and a `/golive status` nobody was reading. After `POLL_FAILURES_BEFORE_DEGRADED` **consecutive** failures one `golive.poll_degraded` line is written, so the gap has a name on the Logs page. ⚠️ **It is a constant, not a setting** (checklist 33 is about DECISIONS a person would want to change; "how many misses before we admit it" is a tuning number with no operator meaning) and ⚠️ **nothing is closed**: a Twitch outage must not end everybody's announcements at once, and the age-out at `:739` is the backstop that stops even a permanent outage wedging a session for ever. `== N` rather than `>= N` is what makes it one line per outage instead of one a minute for as long as it lasts; the count resets on the first good sweep (`:751`). |
| `black_bloc/cogs/content/golive.py:58` | Three misses, chosen not tuned: at `POLL_SECONDS` a single failed sweep is ordinary Helix flakiness and a three-minute run is an outage. Deliberately small, because the line's job is to appear while somebody is still looking. |
| `black_bloc/cogs/content/golive.py:783` | A live login with **any** open session is left alone — that is what stops a Twitch poll from double-announcing a stream presence already caught (`phase2-design.md`, §Detection). Ending, by contrast, is scoped to `source='twitch'` so the poller cannot close a presence session behind the grace timer's back. The `by_login` map is built by hand rather than as a comprehension so a login claimed by two members is **logged** instead of one of them being silently dropped. |
| `black_bloc/cogs/content/golive.py:819` | `Database.conn` raises when nothing is connected (`storage/db.py:453`), and a command body that touches it during startup or after a database failure would hand the caller a traceback and no reply. Owner rule: a refusal is a sentence saying what happened, what it needs and how to get it. |
| `black_bloc/cogs/content/golive.py:653` | ⚠️ **Credentials are asked about BEFORE `is_running()`**, and the order is the honesty. The loop now ticks on a server with no Twitch app at all (`:322`), so "running" would read as "Twitch polling is working" when no Twitch call is being made — checklist 10: claiming a check that was skipped is a lie. With no credentials the line says so and names what the sweep still does; with credentials it is the ordinary running/stopped. |
| `black_bloc/cogs/content/golive.py:859` | ⚠️ **`is_running()` answers "is the timer ticking", which is not the same as "is polling working".** A Twitch outage leaves the loop running and every sweep failing, and the old `/golive status` reported that as healthy. The last good poll and the last error are what make the difference visible without reading the process log. |
...
| `black_bloc/cogs/content/golive.py:957` | ⚠️ **Owner sweep finding, and it is about fear, not accuracy: "for /twitch link it should say channel name not login, login sounds more concerning".** The parameter is `channel`, the describe text is *Your Twitch channel name (the part after twitch.tv/)*, and no sentence this command can show a member contains the word *login* any more — a member asked for a "login" reasonably reads it as being asked for a password. The **internal** names are deliberately unchanged: the column is still `twitch_login`, the helper is still `clean_login`, and the action-log detail key is still `login`, because those are Twitch's own term for the field and renaming them would be a migration. Only the two user-facing format placeholders moved to `{channel}`, since they sit inside sentences a member reads.
| `black_bloc/cogs/content/golive.py:979` | Refused before anything is written, and the sentence deliberately does **not** name the other member — `/twitch link` is runnable by anyone, and answering it with "that belongs to <@id>" turns the command into a lookup for who owns which channel. |
...
| `tests/cogs/content/test_golive.py:394` | ⚠️ **The restart test.** A second `GoLive` instance on the same database announces nothing, because the open row is the state — no in-memory set of "currently live" exists to be lost. This is the incumbent defect the feature promises to beat. Its counterpart is `:876`: the row must *also* be cleared when the stream is genuinely over, or "quiet across a restart" becomes "quiet forever". |
| `tests/cogs/content/test_golive.py:482` | ⚠️ **The test-policy proof for role changes.** With a guard installed, mode `on` still adds no role and logs `golive.would_add_role`. If this test is ever edited, the test policy changed (`phase2-design.md`, §Test mode). |
| `tests/cogs/content/test_golive.py:650` | The 120 s grace is monkeypatched to 0 on the **module** attribute, because `_end_after_grace` reads it at call time. The paired flap test keeps the real-ish delay and asserts the task was cancelled instead — between them they cover both branches without a test that sleeps. |
| `tests/cogs/content/test_golive.py:890` | ⚠️ **The wedge test.** A row left open by a stop is closed at the next `cog_load`, with `reason="reconciled_on_start"` in the action log; the two tests after it pin the exceptions (a member still streaming, a Twitch session Helix says is live) so that reconciliation cannot degrade into "close everything at start", which would re-announce every stream that survives a redeploy. |
| `tests/cogs/content/test_golive.py:960` | ⚠️ **The stranded-role test.** The role is added in mode `on`, the mode is then changed to `shadow`, and the role must still come off. Gating removal on the current mode is the bug; if this test is ever "fixed" by re-reading the mode, read `black_bloc/cogs/content/golive.py:622` first. |
| `tests/cogs/content/test_golive.py:1031` | Pins that a mode-`on` post that genuinely failed logs `golive.post_failed` and **not** `golive.would_announce`, and — the half that actually matters — that it leaves no session behind, so the very next attempt posts. A rolled-back session that still started a cooldown would look identical for sixty minutes. |
| `tests/cogs/content/test_golive.py:1242` | Asserts the **absence** of a word, which is unusual and is the point: the owner's objection to `login` was that it *sounds* like a password prompt, and a test that only checked the new wording would let the old word creep back into any one of the four sentences. It sweeps the command description, the parameter description and all four link constants in one string. `str()` around each because discord.py wraps descriptions in `locale_str`.
| `tests/cogs/content/test_golive.py:1085` | Drops the unique index first, so the assertion is about the **lock** and nothing else. Without it the index alone would make the test pass and the lock could be deleted unnoticed; `:1025` is the same scenario with both defences in place, which is what actually ships. |
...
| `tests/cogs/moderation/test_honeypot.py:482` | ⚠️ **The burst test.** Two posts from one account arriving together produce two hits and exactly **one** ban call — the per-author lock plus `banned_already` (`black_bloc/cogs/moderation/honeypot.py:175`). |
...
| Key | Note |
|---|---|
| `tests/cogs/community/test_events.py:121` | `FakeScheduledEvent` **raises `ValueError` the way discord.py does** — `cancel()` on a running event, `end()` on one that is not running — because that raise is the whole of review finding F1 and a fake that merely records the call would have passed against the broken code. |
| `tests/cogs/community/test_events.py:224` | `FakeGuard` mirrors the real one's two questions rather than just its channel id: `allows_channel` for speaking, `allows_place` for making, renaming and deleting. It takes the category as a constructor argument instead of walking a channel cache, so a test can say "the test channel is missing" and "there is no category" separately. |
| `tests/cogs/community/test_events.py:412` | Starts are built by formatting a real future instant **in the zone under test**, not by hard-coding a date. A literal would start failing the "already gone by" check on whatever day it passed, which is the kind of test failure nobody diagnoses and everybody deletes. |
| `tests/cogs/community/test_events.py:580` | ⚠️ **The test-policy proof for the review card.** With a guard installed the review channel is still made and still named, and the card lands in the **test channel** instead — the deviation explained at `black_bloc/cogs/community/events.py:409`. If this test is ever edited, the test policy changed. |
| `tests/cogs/community/test_events.py:653` | Its other half, and the one that matters most: a real Discord scheduled event is visible to the whole server, so under a guard none is made and the log says `event.would_create_scheduled` — never `event.create_scheduled_failed`, which is asserted in the same test because the owner's verification method is reading the log (checklist 2). |
| `tests/cogs/community/test_events.py:752` | ⚠️ **The burst test.** Two mods pressing Approve together produce exactly one approval and one action-log row, and the loser is told. Unlike go-live there is no unique index behind the lock — the lock plus the `can_transition` re-read inside it is the only defence (`black_bloc/cogs/community/events.py:330`). |
| `tests/cogs/community/test_events.py:802` | Pins that the go-live sweep picks the started-and-approved row and leaves the future one and the undecided one alone. A loop that took "everything due" would announce events nobody approved. |
...
| Key | Note |
|---|---|
| `black_bloc/birthdays.py:19` | ⚠️ **Addressed through `importlib.resources.files`, not `__file__`** (review finding 12). `pyproject.toml` ships `data/*.json` as package data, and a package installed as a zip or by a loader that does not unpack has no `__file__` directory to walk — the seed file would silently be "0 rows" and the import would report a packaging fault. The value is a `Traversable`, which answers `read_text` and `exists` the same way a `Path` does; the `path` argument both readers take is still an ordinary `Path` for tests. |
| `black_bloc/birthdays.py:20` | The last-resort zone is a fixed **UTC-7**, which is Phoenix all year because Arizona does not observe DST. It only comes up if `zoneinfo` cannot resolve `America/Phoenix` at all — a container built without `tzdata`, which `pyproject.toml` now pins for exactly this reason. A fixed offset is correct here and would be wrong for any other zone. |
| `black_bloc/birthdays.py:22` | Discord's embed description caps at 4096; 2000 is well under it and keeps a staff-edited template from ever producing a send that fails on length. Truncation, not refusal, because a birthday post half the length is better than none on the day. |
| `black_bloc/birthdays.py:92` | Only a **leading** bracket or paren group is stripped, and repeatedly, because Birthday Bot's export renders nickname prefixes verbatim: `[Straight Hands] ShinDarkShadow`, `(Umazing) nadia`. A trailing flag emoji (`PopNoTartsEh 🇨🇦`) is deliberately left on — it is part of the name people actually use, and the contains-scoring handles it. A name that is *entirely* a tag is returned untouched rather than emptied. |
| `black_bloc/birthdays.py:98` | 3 / 2 / 1 with a floor of 2 is the owner's rule from `phase5-design.md`: a bare "contains" hit (score 1) is never imported on its own, because `PT` is inside dozens of names. Scores are a deliberate ranking, not a confidence — two members scoring 3 is ambiguity, not a tie to break. |
| `black_bloc/birthdays.py:138` | Falls back rather than raising, because this runs inside the five-minute sweep: one member with a nonsense zone string must not stop everyone else's birthday. The fallback chain is member zone → server zone → fixed UTC-7. |
| `black_bloc/birthdays.py:150` | Clamping uses **2024** (a leap year) for the month lengths so February 29 survives storage. Every read of a stored row goes through this, so a hand-edited database row cannot crash the sweep. |
| `black_bloc/birthdays.py:162` | Clamping and refusing are two different jobs and both exist on purpose: a *command* refuses February 30 with a sentence (silently storing the 29th would be a lie about the person's birthday), while a *stored row* is clamped so a bad row is survivable. |
| `black_bloc/birthdays.py:195` | February 29 is observed on the 28th in ordinary years — the choice is the 28th rather than March 1 so the wish always lands in the birth month. |
| `black_bloc/birthdays.py:207` | Returns **local midnight in the member's own zone**, which is what makes `<t:…:D>` render correctly for every viewer. The loop over two years is what handles "already passed this year" and February 29 in one pass. |
| `black_bloc/birthdays.py:226` | The age *they turn in the year of the date handed in*. ⚠️ Which date that is matters and was a review finding (5): the sweep passes **today**, and `person_lines` (the panel's own block and the person card, once `/birthday show`) passes the **next occurrence** — asking "how old today" for a birthday four months away answers with last year's number. Anything under 0 or over 150 reads as no age rather than a wrong one. |
| `black_bloc/birthdays.py:246` | The colour is staff-editable text, so an unparseable value falls back to the incumbent's `#4eefff` with a warning instead of raising inside the sweep (checklist item 17). It matches with `settings_store.HEX_COLOR` — **one regex, two callers** (checklist 15): the validator refuses a bad colour at the boundary, and this survives one that was already on disk before the validator existed. |
| `black_bloc/birthdays.py:255` | `str.format` raises `KeyError` on an unknown field, which is exactly the wanted behaviour: a staff template with `{nickname}` in it falls back to the default wording and says so in the log, rather than posting `{nickname}` or nothing. |
| `black_bloc/birthdays.py:283` | ⚠️ **The scoring rewritten for review finding 4.** Comparison is casefolded for the "exact" rules too — Discord usernames are case-insensitive in practice, and the export's capitalisation is whatever Birthday Bot rendered on the day. The **tag-stripped display name** is compared as an exact rule, not a contains one, because that is the whole finding: a member whose nickname is `[Straight Hands] ShinDarkShadow` only ever scored 1 against the export's `ShinDarkShadow` and so could never clear the floor of 2. It sits in the *username* tier rather than the display tier on purpose, so a member called plainly `PT` still beats `[40] PT` instead of the pair reading as ambiguous. `global_name` is scored because Discord's new display names live there and are frequently what the export captured. |
| `black_bloc/birthdays.py:323` | The three outcomes are `matched` / `ambiguous` / `not_found` and every one of them is reported — nothing is ever guessed. `ambiguous` deliberately carries the candidates, because the owner resolving it by hand needs the names, not a count. |
| `black_bloc/birthdays.py:335` | ⚠️ **Kept although nothing in the bot calls it** (review finding 7 asked why): it is what **generated** the JSON seed from the archived export, and it is what a future export gets re-parsed with. That is the reason the 39 rows are data and not a Python literal in the cog. It is exercised by `tests/test_birthdays.py`. |
| `black_bloc/birthdays.py:358` | Reads both a bare list and the `{"exported": …, "rows": […]}` wrapper, and skips an unreadable row rather than failing the whole import — a packaging fault should cost one name, not the command. |
| `black_bloc/birthdays.py:384` | Ages in the export are as of the export date, so the year they imply can be a year out until the person corrects it. The date lives in the data file, not in code, and the import report says the caveat out loud. |
| `black_bloc/birthdays.py:394` | Sorting is on the **UTC instant** of each local midnight, not on the local wall clock, because entries in different zones are otherwise not comparable. ⚠️ `next_lines` (the panel's Coming up block, once `/birthday next`) calls this rather than sorting its own copy (review finding 7) — the ordering rule has one home. |
| `black_bloc/birthdays.py:586` | Any failure returns the server zone rather than raising: the sweep must produce *a* date for every member, and Phoenix is the same answer the incumbent Birthday Bot gives everyone today. The fallback chain is member zone → server zone → fixed UTC-7 (`:19`). ⚠️ **The `timezone_table_exists` probe that stood in front of this was DELETED when Phase 5 merged into Phase 4** (checklist 15). It existed because the birthdays branch was built beside Phase 4 and could not assume `user_timezones` was there; now `storage/db.py:163` creates the table on every `connect()`, so a `sqlite_master` lookup on each zone read was a **second opinion about the schema** — and the module that owns that fact is `storage/db.py`. The `except` here already answers a genuinely missing table with the server zone, so nothing was lost but the extra query. |

## `black_bloc/cogs/community/birthdays.py` — F6

| Key | Note |
|---|---|
| `black_bloc/cogs/community/birthdays.py:72` | Five minutes is chosen against the measurement: the incumbent posts at ≈00:03 local, so a five-minute tick lands 00:00–00:05 and matches it. A shorter tick buys nothing, because the check is "is it that member's calendar day yet", not "is it exactly midnight". |
| `black_bloc/cogs/community/birthdays.py:160` | The upsert **clears `last_announced_on`** so that setting a birthday to today still posts on the next tick — that is the owner's test sweep. It deliberately does **not** clear `role_added` / `role_added_id`: a role that is on somebody has to come off tomorrow whatever they did to their row in between. |
| `black_bloc/cogs/community/birthdays.py:200` | ⚠️ **The row records WHICH role went on, not just that one did** (review finding 2). `birthdays.role_added_id` is added by `storage/db.py:406`'s additive `ADDED_COLUMNS` step, not by the `CREATE TABLE`, so an existing database gains it on the next start — same pattern as `golive_sessions.live_role_added`. Without it the reversal read the *current* setting, and changing `birthday_role_id` during somebody's birthday stranded the old role on them for good. |
| `black_bloc/birthdays.py:443` | ⚠️ **Moved out of the cog into the pure module by the panel build (wave 1)** — the month list is now a select on the panel, and nothing outside this module needs it. Discord refuses a message over 2000 characters; 39 imported names or a full year's list go past that easily. Splitting is done on whole lines so a name is never cut in half across two messages. |
| `black_bloc/cogs/community/birthdays.py:298` | The counts line comes first because it is the answer; the lists follow for the owner to act on. The second line says **how many members were searched**, because a report of "12 not found" means something completely different against 40 cached members than against 900 (review finding 4, and checklist 10 — never claim a check that did not really happen). |
| `black_bloc/cogs/community/birthdays.py:981` | **Phase 8a merge.** `loop_health(name)` is how the status page reads this cog's loop health without knowing its attribute names: here the success time is `last_run_at`, not `last_ok_at` — the one naming difference that made a guessing reader silently report nothing at all. The page asks; it never guesses (`black_bloc/api/status.py:141`). |
| `black_bloc/cogs/community/birthdays.py:988` | `tasks.loop.start()` runs the body immediately after `before_loop`, so a restart re-checks every row at once — today's un-posted birthdays and yesterday's still-attached roles both (checklist items 4 and 25). ⚠️ When the database is not connected yet the loop is **not** started here; `on_ready` at `:340` starts it instead (review finding 9), otherwise a slow database meant no birthdays for the life of the process. |
| `black_bloc/cogs/community/birthdays.py:994` | Cancelled on unload so a reloaded cog does not leave a second sweep running and double-post. |
| `black_bloc/cogs/community/birthdays.py:1002` | The whole tick is wrapped: an exception inside `tasks.loop` kills the loop for the life of the process, and a dead birthday loop is invisible until a birthday is missed. The error is kept for `/birthday status` to show, which is what makes "is it healthy" answerable rather than "is it running" (checklist item 9). |
| `black_bloc/cogs/community/birthdays.py:1016` | ⚠️ **The `@loop.error` handler checklist 28 asks for** (review finding 9). The `try` at `:1002` covers the body; this covers everything else — a raise from `before_loop`, from the store, or from the wrapper itself — and `restart()` is the only thing that brings a stopped `tasks.loop` back, because discord.py does not retry it. The reason is kept in `last_error` so `/birthday status` shows it. |
| `black_bloc/cogs/community/birthdays.py:1072` | The retry for a database that was not connected at `cog_load`. Guarded by `is_running()` so a reconnect (`on_ready` fires again after a resume) does not start a second sweep. |
| `black_bloc/cogs/community/birthdays.py:1080` | ⚠️ **Everything hangs off `today` in the *member's* zone, not the server's.** A member at UTC-4 is wished during the evening before in Phoenix — that is not a bug, it is what the incumbent does and what the owner asked for. An **unavailable** guild is skipped entirely (checklist 32): its member cache is not to be trusted, and treating it as real would log every member as missing. |
| `black_bloc/cogs/community/birthdays.py:1096` | The role reversal is checked *before* the opt-in and mode gates, so turning the feature off never strands a role (checklist item 3). |
| `black_bloc/cogs/community/birthdays.py:1116` | ⚠️ **A cache miss is not a wish** (review finding 6). The row is *not* stamped `last_announced_on`, so once the member is visible again the wish still lands on the day; `birthday.member_missing` is logged at most once per member per local day, because the sweep would otherwise write 288 identical rows for anybody who has left. The old behaviour marked the day done and called it `birthday.skipped`, which silently ate the birthday of anyone the cache had not filled in yet. |
| `black_bloc/cogs/community/birthdays.py:1154` | Returns a reason string instead of raising, so the caller can tell the three cases apart: `shadow` and `test_mode` are dry runs that still mark the day done, anything else is a real failure that does not. |
| `black_bloc/cogs/community/birthdays.py:1188` | ⚠️ **The guard cannot see a role change, and neither can `shadow`** (review finding 1, checklist item 1). The role is only really added when the mode is `on` **and** no guard is installed; every other case logs `birthday.would_add_role` and does nothing. Before the fix, shadow mode — the mode the feature ships in — handed out a real role on the live server while claiming to be a dry run. `role_added` is only set once Discord has actually accepted the change. |
| `black_bloc/cogs/community/birthdays.py:1215` | The reversal is unconditional on the mode at reversal time — the row says the role went on, so it comes off, even if birthdays were switched `off` in between (checklist item 3). ⚠️ It removes **`discord.Object(id=role_added_id)`**, never `guild.get_role(...)`: a role missing from the cache, or a `birthday_role_id` that has since changed or been cleared, must not be a reason to leave it on somebody — and the flag is only cleared once Discord has accepted the removal. A removal Discord refuses keeps the flag and retries next tick; only the *log line* is rate-limited, so a permanent 403 cannot write 288 action rows a day. |
| `black_bloc/cogs/community/birthdays.py:1252` | ⚠️ **Forgetting a birthday and opting out both hand the role back first** (`forget_birthday` and `change_opt`, once `/birthday remove` and `/birthday optout`) (review finding 3). Deleting the row was previously enough to strand the role until somebody noticed — the row that says "take this off tomorrow" was the thing being deleted. Both run inside the same per-user lock as the sweep. |
| `black_bloc/cogs/community/birthdays.py:1257` | One log line per member per local day per kind. The sweep is idempotent by design, which means a persistent failure would otherwise repeat every five minutes — and the owner's verification method is reading the log. |
| `black_bloc/cogs/community/birthdays.py:352` | ⚠️ The write takes the **per-user lock** (review finding 8, checklist 6): it clears `last_announced_on` while a sweep tick may be deciding whether to post for that same member, and the two interleaved could post twice or not at all. |
| `black_bloc/birthdays.py:497` | ⚠️ **Moved into the pure module as `stored_sentence` by the panel build** — the modal and the staff card say the same thing, so it has one home. The confirmation quotes the zone by name and the next occurrence as a HammerTime stamp, because "midnight" is the question people actually get wrong — theirs or the server's. |
| `black_bloc/cogs/community/birthdays.py:433` | `person_lines` (what `/birthday show` was) works the age out at the **next occurrence**, not in the current calendar year (review finding 5): a birthday still four months away was reported a year young. |
| `black_bloc/cogs/community/birthdays.py:454` | `next_lines` (what `/birthday next` was): `<t:…:D>` and `<t:…:R>` render in each viewer's own zone, which is the point; `allowed_mentions` is `none()` so the `<@id>` list reads as names without pinging five people. The ordering comes from `birthdays.upcoming()` rather than a second sort here. |
| `black_bloc/cogs/community/birthdays.py:710` | `send_month` (what `/birthday list` was). Paginated by packing lines, not by a page number: the first page answers the interaction and the rest go as ephemeral follow-ups, so the command never fails on length. |
| `black_bloc/cogs/community/birthdays.py:686` | `run_mode` (what `/birthday mode` was) writes a setting, so it takes the same `_ready` / `is_connected` guard as every other command that touches the database (checklist 8) — without it a mode flip during a database outage answered "The application did not respond". |
| `black_bloc/cogs/community/birthdays.py:423` | ⚠️ **`birthday_role_id` had no way back to "none"** (review finding 10): `/settings set-role` can only point it somewhere. This and the general `/settings clear` at `cogs/core.py:260` are two front doors on one `SettingsStore.clear`. The sentence says out loud that a role already given still comes off tomorrow, because that is the surprising half. |
| `black_bloc/cogs/community/birthdays.py:730` | `send_status` (what `/birthday status` was). Health, not liveness: last successful sweep, last error, resolved staff roles, and how many rows are stored and opted in. It also says out loud that test mode gives no roles, because that is the fact most likely to look like a bug. |
| `black_bloc/cogs/community/birthdays.py:250` | ⚠️ **The `/birthday import` command this note was written for is gone** (2026-08-27; the daily loop and the website route do the work now), so there is no interaction to defer. The reason it deferred still holds for anything that calls `members_of`: filling the member cache is a gateway round trip that cannot fit in Discord's three-second interaction window (checklist item 24). The report goes back as ephemeral follow-ups to the person who ran it. |
| `black_bloc/cogs/community/birthdays.py:245` | ⚠️ **Scores against `guild.members`, with no network call at all** (review finding 4). The design premise in `phase5-design.md` — *"member lookup uses `guild.query_members(query=…)` (HTTP, no members intent needed)"* — **is wrong for this bot**: the members intent is on (`intents.py:8`), so the cache holds the whole server; and `query_members` matches **prefixes only**, so every `[Tag] Name` nickname in the export was unfindable, and its `limit=10` could silently drop the right member. The cache is chunked first when it is shorter than `member_count`, and the report says how many members were searched, because "not found" against a half-filled cache is a different claim from "nobody matched". |
| `black_bloc/cogs/community/birthdays.py:258` | Re-running imports nothing it has already stored — an existing row of *any* source is kept and reported, which is stricter than the spec's "never overwrite `source='self'`" and makes the command genuinely idempotent. Without that, a re-run on somebody's birthday would clear their `last_announced_on` and post twice. |
...
| `black_bloc/cogs/core.py:32` | Only **channel** and **role** keys are clearable: those are the ones whose absence is meaningful ("no ping role", "no birthday role"). An enum or an int has a default that is already a real value, and `set-value` can reach it. The list is 11 long — Discord refuses a choice list over 25. |
| `black_bloc/cogs/core.py:246` | Logged as `settings.clear` only when something was actually removed, so the action log does not fill with no-ops (same rule as the birthday panel's **Clear the birthday role**, `cogs/community/birthdays.py:423`). |
...
| `tests/cogs/community/test_birthdays.py:469` | Finding 3, both halves: forgetting a birthday and opting out on the day take the role back before they forget or mute the row. The two tests call `forget_birthday` and `change_opt` directly now — the panel's Remove and Opt out buttons call the same pair. |
...
| `black_bloc/modcases.py:365` | ⚠️ **Asks the guard first rather than letting it raise from inside the HTTP layer** — same trick as `cogs/content/golive.py:544`. This runs from a message listener and from a command body alike; a `TestModeViolation` surfacing here would be a traceback in the listener and a dead command in the other. The broad `except` is deliberate for the same reason `actionlog.py:154`'s is: the punishment has **already happened** by the time the card is posted, and a modlog that will not take it must not undo it. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/moderation/modcmds.py:50` | Every refusal names **what happened, what it needs and how to get it** (owner rule: a person never sees a bare status code). They are keyed by action because the permission differs — Moderate Members, Kick Members, Ban Members, Manage Messages — and a refusal that names the wrong permission sends somebody asking an admin for access they already have. The purge one also names Discord's 14-day bulk-delete floor, which is the most common cause and is not a permissions problem at all. |
| `black_bloc/cogs/moderation/modcmds.py:431` | ⚠️ **The owner's test policy for this phase in one predicate.** `guard.py` gates messages, not punishments, so timeout / kick / ban / purge check this explicitly and refuse. `/warn` is deliberately **not** gated — it only writes a row and sends a DM — and neither are `/untimeout` and `/unban`, which *lift* a punishment rather than making one. ⚠️ That last pair is worth knowing before test mode is lifted: they are the two commands that can change the live server while the policy is on. |
| `black_bloc/cogs/moderation/modcmds.py:446` | A refused command still writes its case, with `applied=0` and `mode='test_mode'`, and logs `mod.would_<action>` — so the owner's verification method ("read the log, read `/cases`") shows the whole intent, not a gap. `deferred` exists because `/purge` has already deferred by the time it gets here and an interaction that has been deferred can only take a `followup` (`command_errors.py:65`). |
| `black_bloc/cogs/moderation/modcmds.py:479` | The DM is sent **before** the row is written, and the row before the reply: the member is told by the thing that is about to be recorded, and a failed reply cannot take a real warning with it (checklist 12). `dm_member` swallows a closed DM, so a member with DMs off is still warned. |
| `black_bloc/cogs/moderation/modcmds.py:495` | ⚠️ Over 28 days is **refused with a sentence rather than silently clamped**, because somebody typing `30d` is asking for something Discord cannot do and needs to be told to ban instead; the clamp at `modcases.py:112` is the belt underneath that brace, for values that arrive from anywhere else. The audit reason names the moderator, so Discord's own audit log says who did it even though the row here is the record of truth. |
| `black_bloc/cogs/moderation/modcmds.py:566` | `delete_message_seconds`, not the deprecated `delete_message_days` (checklist 22), and the window is clamped to 0–7 days at the call. The DM goes out **before** the ban for the obvious reason: a banned account cannot be DMed by a bot that no longer shares a server with it. |
| `black_bloc/cogs/moderation/modcmds.py:617` | ⚠️ **Defers before it deletes** (checklist 24). A bulk delete of a hundred messages is well outside the three-second interaction window, and a purge that worked but missed the deadline shows the moderator "the application did not respond" — with no way to tell whether it happened. The count is bounded at 100 because that is Discord's own bulk-delete limit, and the refusal says so rather than reporting an API error. |
| `black_bloc/cogs/moderation/modcmds.py:713` | ⚠️ **Pagination is a `page` argument, not buttons — a deliberate, smaller deviation from `phase6-design.md`.** A paging *view* is a persistent component with a `custom_id` that has to survive restarts (`cogs/community/role_menus.py:134`), and this reply is **ephemeral**: nobody else can see it, it cannot be revived after a restart, and a dead button in an ephemeral message is exactly the failure the persistence machinery exists to prevent. The reply names the next page's command, so paging is one click of the command picker. |
| `black_bloc/cogs/moderation/modcmds.py:747` | The audit reason handed to Discord always names the moderator, because Discord attributes the action to **Black Bloc** in the server's own audit log. Without this the server's audit log says the bot banned somebody and gives no way to find out who asked it to. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/moderation/automod.py:63` | Staff resolution walks every role's computed permissions on the staff channel (`settings_store.py:359`); doing that per message put a permissions calculation in the hot path of every message in the server. Sixty seconds is the delay between a staff role changing and automod noticing — acceptable, because the cost of the miss is one moderator's message being read, not punished. |
| `black_bloc/cogs/moderation/automod.py:93` | ⚠️ **While `staff_channel_id` is still the test channel, the resolved staff set means nothing** — the default is `TEST_CHANNEL_ID` (`settings_store.py:478`), so arming automod against it arms it against whoever can see the test channel. The refusal is separate from the empty-staff one because the fix is different: set a real channel, rather than fix the one you set. |
| `black_bloc/cogs/moderation/automod.py:134` | ⚠️ **A delete is invisible to `guard.py`'s send and edit gates** (checklist 1), so it checks the guard itself and returns `test_mode`, exactly like `do_timeout` above it. Defence in depth: `_answer_for` already refuses to punish at all while a guard is installed. Do not remove it when test mode lifts — it is a no-op then. |
| `black_bloc/cogs/moderation/automod.py:147` | ⚠️ **Every message that fed the window is deleted, not just the last one.** Five messages of one mention each is the shape Carl's rule was written for; deleting only the fifth left the other four in the channel — the spam stayed and the member was punished for it anyway. Messages other than the current one are `PartialMessage`s, so no fetch is needed, and each failure is its own `automod.delete_failed` line. |
| `black_bloc/cogs/moderation/automod.py:177` | The DM is sent **after** the timeout attempt so it can say what actually happened. `warn` is what decides whether the member is told at all (it is the "tell them" action); whether the sentence says warned or timed out depends on what landed. |
| `black_bloc/cogs/moderation/automod.py:389` | ⚠️ `SafeDynamicItem` — a `DynamicItem`'s exceptions never reach a view's `on_error` (`command_errors.py:52`), so without it a failure inside Apply now was a button that spun forever. Subclasses implement `on_click`, never `callback`. |
| `black_bloc/cogs/moderation/automod.py:406` | Order in the click: staff → database → **the case lock** → read → test-mode refusal → defer → claim → punish → record → rewrite the card → answer. The defer comes before the DM and the timeout because both are round trips that outrun Discord's three-second interaction window (checklist 24). The lock is keyed on the **case**, not on the clicker: two different staffers pressing the same button was exactly the race, and a per-clicker lock is two locks that never meet. |
| `black_bloc/cogs/moderation/automod.py:314` | Apply-now can only delete when the row remembers where the message was; a verdict written before those columns existed skips the delete rather than guessing. |
| `black_bloc/cogs/moderation/automod.py:356` | The test-mode branch runs **before** the claim, so a refused apply leaves the case open for later — claiming first would mark it applied for something that never happened. |
| `black_bloc/cogs/moderation/automod.py:475` | An edit is a new fact: somebody posting "hi" and editing it into five mentions did nothing the listener would have seen. Only a **content** change re-evaluates, because Discord fires this event for embeds resolving, pins and attachment processing too, and each of those would re-count the same mentions. |
| `black_bloc/cogs/moderation/automod.py:497` | ⚠️ **A log-only verdict is a log line and nothing else** (`automod.observed`). The two rules Carl leaves on — slowmode 6/4s and linkspam 1/1s — fire constantly by design; each one used to write a `mod_cases` row and post a card, which buried the real cases and made `/cases` unreadable. Rules that punish still get a row and a card in every mode. |
| `black_bloc/cogs/moderation/automod.py:503` | ⚠️ **While a guard is installed, `on` behaves like shadow**: nothing is punished, everything is logged `would_…`. The `do_*` functions refuse anyway; deciding it here is what keeps the card, the row and the log telling one story instead of three. |
| `black_bloc/cogs/moderation/automod.py:551` | Apply now is offered only when **nothing** was done. A partly-applied verdict (warn landed, timeout refused) is not a dry run and must not be shown as one — the card names what was done and what Discord refused, and re-applying it would DM the member a second time. |

## `black_bloc/cogs/moderation/modcmds.py` — the mod commands

| Key | Note |
|---|---|
| `black_bloc/cogs/moderation/modcmds.py:495` | ⚠️ **Act first, tell them after** (review finding). The DM used to go out before `member.timeout`, so a timeout Discord refused left the member told they had been timed out when nothing had happened. ⚠️ **`/kick` and `/ban` are the deliberate exception and stay DM-first**: after either, the bot can no longer DM them at all, so the choice there is "tell them before, and risk telling them about a kick that failed" against "never tell them". A failed kick's DM is a confusing message; a successful ban's missing DM is a member who never learns why. |
| `black_bloc/cogs/moderation/modcmds.py:526` | ⚠️ **Owner decision, 2026-08-26: `/untimeout` and `/unban` are refused while `TEST_MODE`**, logged `mod.would_untimeout` / `mod.would_unban`. They are reversals, not punishments, and were allowed at first for that reason — but both change a **third party's** standing on the real server, which is what test mode exists to prevent, and neither is visible to `guard.py`. |
| `black_bloc/cogs/moderation/modcmds.py:617` | An untargeted purge is filed with `user_id = NULL` and the channel on the row (`modcases.py:154`). With a member named it is still their case. |
| `black_bloc/cogs/moderation/modcmds.py:738` | Every long answer goes out as several ephemeral messages: the first as the interaction response, the rest as followups. A single 2000-character overflow is a 400, and the person sees "the application did not respond". |
...
| `black_bloc/cogs/core.py:152` | `/settings show` prints every registered key with its help text; the registry passed 2000 characters in Phase 6 and the command started answering nothing at all. Owner decision: chunk it, the first chunk as the response and the rest as ephemeral followups (`modcases.py:402`). |
...
| `site/public/assets/ui.js:1269` | Every write in every page module goes through `run(say, work, ok)`: it says "Working…", then either the sentence or the success line, and returns `{ok}` so the caller only refreshes when something actually changed. |
...
| Key | Note |
|---|---|
| `black_bloc/api/tools/mod.py:151` | ⚠️ **The guard check comes before the member lookup**, so a ban aimed at somebody who has left still refuses with the test-mode sentence rather than a 404. It calls the cog's `refuse_in_test_mode`, which records the not-applied case **and** the `mod.would_<kind>` line before the 409 — the refusal is a logged event, not a silent no (checklist 1 and 2). |
| `black_bloc/api/tools/mod.py:196` | Discord refusing an action is a **502**, not a 200 with a sad message: the `do_*` helpers return the cog's `REFUSED[kind]` sentence, which is compared by identity here. The page must not be able to say "kicked" when nobody was kicked. |
| `black_bloc/api/tools/modmail.py:63` | ⚠️ **"Close-with-delete" is the destructive part of closing, not closing itself.** The route asks `may_remove(bot, place)` — the cog's own answer — and refuses with 409 only when the guard would keep the channel. A ticket in the test channel's own category still closes normally. |
...
| `black_bloc/api/tools/rolemenus.py:483` | Posting a panel outside the test channel is 409 with `guard.refusal_message()` — the same check `/rolemenu post` makes, in the same place, before anything is written. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/community/events.py:781` | `apply_decision` is `decide` without the interaction: same `event_lock`, same `can_transition`, same log lines. `decide` now closes the card and answers with what it returns. ⚠️ A refused transition comes back as `(sentence, None)` and the API turns that into a 409 — two mods clicking Approve still cannot decide twice. |
| `black_bloc/cogs/community/events.py:744` | `cancel_event` is `Events._cancel`'s body; the cog method delegates. It returns False when the row was past cancelling, which is what the API's 409 is built on. |
| `black_bloc/cogs/moderation/automod.py:341` | `apply_case` is `ApplyNowButton.on_click`'s body, `case_lock` and `claim_case` included, so the web and the button race each other correctly rather than both applying. ⚠️ The button now **always** defers before doing anything (it used to defer only on the working path) — that is the one behaviour difference, and it is invisible: every branch still answers with the same sentence. |
| `black_bloc/cogs/moderation/automod.py:288` | `save_rule` takes a dict of changes rather than one field, so the web's rule editor is one call and one `automod.rule` log line. `/automod rule set` passes a single-entry dict. |
| `black_bloc/cogs/moderation/modcmds.py:160` | `refuse_in_test_mode` returns the sentence instead of sending it, so both callers do the same recording. ⚠️ The `do_*` helpers (`warn_member`, `timeout_member`, `kick_member`, `ban_member`, `unban_member`) each return the sentence the slash command sends, which is why the API can compare against `REFUSED[kind]` to tell a refusal from a success. |
| `black_bloc/cogs/moderation/honeypot.py:246` | `ban_hit` is the Ban-now path, returning `(outcome, sentence)`; the button sends the sentence, the router maps the outcome onto a status. `make_trap_channel` (`:287`) is `/honeypot setup` the same way. |
| `black_bloc/cogs/moderation/modmail.py:557` | `send_reply`, `close_ticket`, `post_transcript` and `remove_place` are the cog methods' bodies at module level; the methods delegate. The `user_lock` is inside `close_ticket`, so a web close and a slash close cannot both file a transcript. |
...
| `site/mock/server.mjs:778` | ⚠️ **The mock refused two things the real bot allows.** A modmail reply DMs the member and a warn only writes a case and DMs them — neither touches a guarded channel, so `tools/modmail.py`'s reply route never asks the guard and `tools/mod.py` lets `warn` past it. Refusing them here taught the pages a rule the bot has not got, which is the wrong direction for a stand-in: where the two disagree, the real API wins. |
| `site/mock/server.mjs:921` | `POST /api/mock/guard` is the second non-contract route, beside `/api/mock/reset`. `check.mjs` asserts the six genuinely-guarded routes 409 and the two ungarded ones do not (`site/mock/check.mjs:216`), then turns the guard off to read the shapes of the routes that refuse (`:515`) and back on at the end. ⚠️ **Before this, `check.mjs` could only pass with `MOCK_TEST_MODE=0`** — a comment told you to set it, so the default run was seven failures and nobody could tell a real break from the expected noise. |
...
## birthdays — daily import loop (2026-08-27)

| Key | Note |
|---|---|
| `black_bloc/cogs/community/birthdays.py:73` | `IMPORT_HOURS = 24`, and the loop is `@tasks.loop(hours=24)` rather than `time=<a clock time>` on purpose. âš ï¸ **`hours=24` runs once IMMEDIATELY when the loop starts** — which is what makes a fresh deploy import inside a minute instead of sitting idle until the next midnight. A `time=` loop sleeps to the next occurrence, so a container restarted at 00:05 would not import for another 24 hours, and the whole point of taking the command away was that nobody has to remember to run it. The trade is that the run drifts with restarts; nothing here cares what hour it lands on, because the import is idempotent. |
| `black_bloc/cogs/community/birthdays.py:1023` | The loop body is a copy of `_sweep`'s shape (checklist 28): `is_connected` guard, `try` around the work, `last_import_error` set on failure and cleared on success, `last_import_at` stamped only on a clean pass, a `before_loop` that waits for the gateway, and an `@_import_loop.error` handler that restarts it — discord.py re-raises a non-HTTP exception out of a loop and the loop is then dead for the life of the process. The health pair is separate from the sweep's (`last_run_at` / `last_error`) because a broken import must not read as a broken sweep, and `/birthday status` prints both. |
| `black_bloc/cogs/community/birthdays.py:984` | `loop_health` answers for `_import_loop` by attribute name, so `/api/status` lists it with no change at the API end — `api/status.py:_loops` finds loops by type and asks the cog for the health beside each. `tests/api/test_status.py:REAL_COGS` has the row that proves it. |
| `black_bloc/cogs/community/birthdays.py:1060` | âš ï¸ **The action log only hears about a run that took something.** A daily `0 imported Â· 39 already stored` embed in the log channel is 365 lines a year that say nothing changed, so a run with an empty `imported` bucket goes to `log.info` and stops there; only a non-empty one calls `log_action`. The counts are still in the process log every day, and `/birthday status` shows when the loop last ran. |
| `black_bloc/cogs/community/birthdays.py:1066` | `actor=None` and `"trigger": "daily"` in the details are what tell a log reader this was the loop and not a person. The web route (`api/tools/birthdays.py:128`) still logs the same `birthday.import` kind with a real actor and no trigger, which is the distinction. |
| `black_bloc/cogs/community/birthdays.py:1046` | `import_once` walks `bot.guilds` and skips `guild.unavailable` (checklist 32) — the same shape `run_once` uses. An empty seed file is one `log.warning` for the whole pass and a return, not one per guild, and it is not an error: `last_import_at` still stamps. |
| `black_bloc/cogs/community/birthdays.py:99` | `NOBODY_YET` no longer names `/birthday import` — the command is gone, not hidden, so it is deliberately NOT in `command_visibility.HIDDEN_WHEN_OFF`. `import_rows`, `report_lines`, `members_of` and `_import` stayed exactly where they were: the web route at `api/tools/birthdays.py:117` imports all four and is the surviving on-demand path. The cog's own `IMPORT_EMPTY` sentence went with the command (the web route keeps its own). |
...
| `site/public/assets/ui.js:1099` | ⚠️ **ONE save mechanism per page (§5.2), and one implementation of it.** `settingsEditor` owns the dirty set for a group of rows and the docked bar; `settingsPanel` wraps it for the ten feature pages and `page-settings.js` wraps it for all 47 keys at once. The per-field Save/Clear buttons are gone estate-wide, not just on the Settings tab. |
| `site/public/assets/ui.js:1126` | ⚠️ **The per-row Clear button had nowhere to go, so "put it back to its default" became a gesture: empty the control.** A row whose new value is blank while its stored value was not calls `clearSetting` instead of storing `null`. Without this the docked bar would have quietly removed a capability the old page had. |
...
| `black_bloc/cogs/content/golive.py:542` | ⚠️ **The bug this whole change exists to fix.** `_enrich` fired for ANY presence stream whose member had a `/twitch link` row, so a Twitch-linked member streaming on YouTube got a Helix query and `enriched()` filled their YouTube announcement with Twitch game/title. Now a named non-Twitch platform never reaches Helix at all — the tests assert `stream_calls == []`, not just that the fields survived. |
| `black_bloc/cogs/content/golive.py:99` | `TEST_STREAMS` is keyed by platform name so `/golive test`'s choice list and its fake streams cannot drift apart — the `app_commands.Choice` list is built from the same dict. |
| `black_bloc/cogs/content/golive.py:953` | `/golive test` still prefers the caller's REAL stream when no platform is chosen; an explicit choice always wins, because picking "YouTube" while live on Twitch and being shown the Twitch one reads as a bug. |
| `black_bloc/cogs/content/golive.py:888` | `/golive status` gained a `• <name> on <platform>` line per open session, and the whole reply now carries `AllowedMentions.none()` — it interpolates display names, which are attacker-controlled (review checklist 11). |
...
| `black_bloc/guard.py:15` | The type check is a free function because `allows_interaction` is the only caller and the test fakes need to be able to *not* have a `.type`. Note what this gate never sees: `ConnectionState.parse_interaction_create` sends types 2 and 4 to the command tree and dispatches components (3) and **modal submits (5)** straight to the view store, so `tree.interaction_check` — and therefore this method — is never consulted for a button or a modal. The button path is really gated by `panel_context`'s own `allows_channel` call (`black_bloc/cogs/community/tempvoice.py:835`); this allowance is the belt to that braces, and is what stops the tree refusing a component if discord.py ever routes one through it. |
| `black_bloc/cogs/community/tempvoice.py:1611` | The cog talks to the guard through two module helpers rather than reaching into `bot.guard` at five call sites, because the guard is `None` whenever test mode is off and every one of those sites would otherwise need the same `getattr` dance. |
| `black_bloc/cogs/community/tempvoice.py:1533` | ⚠️ **Ownership is recorded BEFORE the panel is posted**, immediately after the row is written. Post first and the guard refuses the send — the channel would exist with no controls in it, which is the exact failure this change was made to remove. |
| `black_bloc/cogs/community/tempvoice.py:1402` | The restore path. Every row whose channel still exists is owned again; a row whose channel is gone is deleted and disowned in the same breath (`:1383`). It is **incremental, not a wholesale replace**, so a reconcile that runs while the gateway reports no guilds cannot blank the set and silence every live panel for five minutes. |
| `black_bloc/cogs/community/tempvoice.py:1449` | Disowned only **after** Discord accepted the delete. A delete Discord refuses keeps the row (checklist 3) and must keep the allowance with it, or the panel in a channel that still exists stops answering. |
| `black_bloc/cogs/community/tempvoice.py:331` | ⚠️ **The bot needs its own overwrite on a spawned channel now that the panel lives inside it.** `owner_overwrites` builds a **hidden** channel by denying `view_channel` to `@everyone`, and a member who hid their last channel gets the next one created hidden — at which point Black Bloc could not see the chat it was about to post the panel into, and the create would end in `tempvoice.panel_failed` with no controls anywhere. `me` is the same allowance the lobby has had since Phase 3 (`:311`), so the two paths now build overwrites the same way. |
...
| `black_bloc/cogs/core.py:247` | ⚠️ **`golive_embed` was the 26th value-typed setting, and `/settings set-value` listed them all as `app_commands.choices` — Discord takes at most 25.** Adding the key would have made the command fail to register against the live API, which is a failure nothing in the test suite could have seen before (`tests/cogs/test_core.py` asserted `<= 25` and was the only thing that caught it). The key is now a plain `str` with an autocomplete, which filters as the user types and cannot overflow however many settings arrive later. An unknown key still ends at `coerce_value`'s sentence, so a typo gets an explanation rather than a stack trace. |
...
| `site/public/assets/ui.js:1194` | `fillTemplate` is the ONE home for filling a wording sample, and it copies `black_bloc/golive.py:render` exactly: known tokens filled, unknown ones left standing, `{{`/`}}` unescaped, and **null** when a stray brace means Python's `format_map` would have raised — which the pages report as "Black Bloc would use its own default instead" rather than pretending. |
| `site/public/assets/ui.js:1209` | `templateEditor` wraps one settings row as a textarea (`ui.js:883`, the client-side `longtext` type — the registry still calls these keys `text`, so no validator changed) plus the docked bar, and calls `paint(filled)` on every keystroke. Used by go-live, birthdays and temp voice. |
| `site/public/assets/ui.js:1290` | ⚠️ **`keepSaying`/`sayAgain` exist because a write that calls `refresh()` used to throw away its own outcome sentence** — the reload replaces the notice it was just written into. The sentence is parked under a name and put back on the notice the reload builds. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/community/tempvoice.py:199` | `forget_creator` was the cog's private `_forget`; the API needed it, so it is a module-level function and `_forget` is a one-line delegation. One implementation, two callers — `/tempvoice forget` and `POST /api/tempvoice/forget`. |
...
| `black_bloc/cogs/community/role_menus.py:1147` | `RequestButton` is a `DynamicItem` on `rolereq:(id):(approve|deny)`, registered in `cog_load` with `add_dynamic_items` — that registration, and nothing else, is what makes a card posted before a restart still work after one. Same shape as `events.py:DecisionButton`. |
| `black_bloc/cogs/community/role_menus.py:1122` | The Approve modal only appears when the menu HAS `expires_days`; a menu with no clock approves in one click. Blank days = the menu's own number, `0` = no end date, anything else = refused by name. |
| `black_bloc/cogs/community/role_menus.py:1264` | ⚠️ **`ask_staff` deviates from one sentence of the design.** `phase9-design.md` §Approval flow 1 says "deselecting the pending option withdraws"; the decision table says "picking it again withdraws". The posted select is ONE persistent view shared by every member, so it cannot show a per-member option as selected — deselecting is not an action anybody can take. **Picking a pending role again withdraws it**; omitting it does nothing. Omission would silently bin a request every time somebody picked something else. |
| `black_bloc/cogs/community/role_menus.py:1286` | Dropping a role you already hold is applied straight away even on an approval menu — approval gates getting a role, never giving one back. It closes the grant with `given_up`. |
| `black_bloc/cogs/community/role_menus.py:1209` | The select now reads its own menu row on every click (for `approval` / `expires_days`), so a menu deleted under a live panel answers `MENU_IS_GONE` instead of a traceback. |
| `black_bloc/cogs/community/role_menus.py:1469` | One hourly loop does both halves — expiry and the record sweep — because the design says "same loop". `loop_health("_expiry_loop")` (`:1436`) is what the health tab reads; `api/status.py:_loops` finds it by type. |
| `black_bloc/cogs/community/role_menus.py:1491` | ⚠️ **Expiry removes the role regardless of TEST_MODE.** Checklist 3: a reversal must not depend on the mode at reversal time, and the guard only patches sends and edits anyway. A Discord refusal logs `role.expire_failed` and leaves the grant OPEN for the next hour; a grant whose member has left is closed rather than retried forever (checklist 5). |
| `black_bloc/cogs/community/role_menus.py:1514` | ⚠️ **`reconcile_records` corrects RECORDS ONLY.** It never adds or removes a role — a row disagreeing with Discord means the row is wrong, because Discord is the record. It skips `guild.unavailable` (checklist 32) and logs `role.reconciled` only when something actually moved. |
| `black_bloc/cogs/community/role_menus.py:1537` | `on_member_update` filters through `was_ours` FIRST, so Black Bloc's own edits never come back as `role.changed_by_hand`. The list comprehensions pop the ledger even when the result is discarded, which is what keeps the book from growing. |
| `black_bloc/cogs/community/role_menus.py:1573` | A by-hand add of a role some **timed** menu owns opens a `manual` grant with **no expiry** — Black Bloc starts tracking it but will not take back a role it never handed out. `timed_menu_owns` (`:450`) is the join that decides. |
...
| `black_bloc/cogs/community/role_menus.py:1433` | ⚠️ **`/role` is NOT in `command_visibility.HIDDEN_WHEN_OFF`.** `rolemenu_mode: off` stops members picking; staff handing a role out for a week has nothing to do with the panels and must keep working. |
| `black_bloc/cogs/community/role_menus.py:1717` | `/rolemenu edit` is an ADDITION to the design's slash surface (the design put the editor on the dashboard, which is 9b). Without it there is no way to turn approval on before 9b lands, and the owner's sweep starts with exactly that. |
...
| `black_bloc/api/tools/rolemenus.py:438` | ⚠️ **`can_move` is asked BEFORE the edit is written.** The first cut applied the field edits, then moved, so a refused move left a menu whose title had changed under a sentence that said "nothing was changed". Caught in the browser, not by reading: role menus ship OFF in the mock, so the first channel change refused with `rolemenu_off` and the title had already moved. `site/mock/server.mjs:1227` splits `checkMove` out of `movePanel` for the same reason. |
...
| `site/mock/server.mjs:495` | The mock's menus grew an `id`, because a request row's `menu_id` has to point at something. `runner-status` is the approval-gated, seven-day menu the owner's sweep describes. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/community/polls.py:446` | ⚠️ **`guard_allows` is the whole test-mode story for this feature, and it is asked BY HAND at every acting site.** `guard.py:101-142` patches exactly `http.send_message`, `http.edit_message` and `http.delete_channel`. `http.end_poll` is **not** among them (`discord/message.py:1897`), and neither is the voters endpoint — so `poll.end()` and `PollAnswer.voters()` would reach a real channel with the guard installed and silent. Create, end, cancel, remind, post-results and read-voters each ask first (`:531`, `:592`, `:696`, `:733`, `:1071`, `:1416`), exactly as `cogs/moderation/automod.py` does for delete and timeout. |
| `black_bloc/cogs/community/polls.py:796` | ⚠️ **The answer ids are read back off the posted message, not guessed.** `Poll.add_answer` numbers answers `len(self.answers) + 1` locally (`discord/poll.py:621`), but the ids that arrive on gateway vote events are Discord's. `_answer_ids` therefore reads `message.poll.answers` and falls back to `1..N` only when the count does not match — a guessed id would file every vote against the wrong option. |
| `black_bloc/cogs/community/polls.py:787` | `_expiry` prefers `message.poll.expires_at` over our own arithmetic, because Discord rounds a duration to whole hours when it reads one back (`discord/poll.py:452`) and the loop's close time must be **Discord's**, or the two drift by up to an hour. |
| `black_bloc/cogs/community/polls.py:297` | The reminder is claimed by a **conditional** `UPDATE … WHERE reminded_at IS NULL` whose `rowcount` decides whether to send. Claim-then-send, not send-then-mark: a restart between the two would otherwise double-ping the channel. `clear_reminder` (`:300`) releases the claim when the send fails, so a failure is not recorded as a last call — checklist item 2, with distinguishable kinds (`poll.reminded` vs `poll.remind_failed`). |
| `black_bloc/cogs/community/polls.py:1705` | `_reminder_horizon` takes the **widest** `poll_reminder_minutes` across every guild for the SQL window, then `_remind` re-checks each row against its own guild's setting (`:1051`). One query, no per-guild round trip, and a guild with a short window cannot be reminded early by a guild with a long one. |
| `black_bloc/cogs/community/polls.py:626` | `_write_results` calls `end_poll()` **only when Discord has not finalised the poll itself**. Native polls close on their own and Discord posts a `poll_result` system message; the loop's job is to read the final counts and archive them. Counts are approximate while open and exact once finalised (`discord/poll.py:206-211`), which is why the numbers are read *after* the end call, not before. |
| `black_bloc/cogs/community/polls.py:540` | ⚠️ **`refresh_voters` is why a missed gateway event is not a lost vote.** The raw listeners keep a running `poll_votes` tally, but they can miss (a disconnect, a restart); at close the voter list is read once from `PollAnswer.voters()` and upserted, which heals it. It returns on the first failure rather than half-filling a list, and it never runs for an `anonymous` row — a poll sold as anonymous must not leave names in our database either. |
| `black_bloc/cogs/community/polls.py:1133` | `_vote` listens to `on_raw_poll_vote_add` / `_remove` only, per the measurement above, and ignores any message that is not a Black Bloc poll, is `anonymous`, or is on a non-native surface. |
| `black_bloc/cogs/community/polls.py:693` | `cancel_poll` writes the status and the log line **inside** the lock and ends the vote at Discord **outside** it: the irreversible thing (the row) lands first, cosmetics after (checklist 12). A cancelled poll is still ended at Discord — leaving a vote running that Black Bloc has forgotten is worse than an unfinished cancel. |
| `black_bloc/cogs/community/polls.py:826` | `apply_decision` sets `open` before posting and **cancels the row when the post fails**. Without that the poll would sit `open` with no `message_id` and no `closes_at`, which the due-poll sweep (`:381`, `closes_at IS NOT NULL`) can never pick up — a permanently open row nobody can close. |
| `black_bloc/cogs/community/polls.py:802` | The auto-thread flag lives **on the poll row** (`polls.auto_thread`), not read from the setting at post time. A poll held for review can be approved days later, and the answer to "did this poll ask for a thread" must not change because a Lead flipped the setting in between. The thread is handed to `guard.own_channel` so Black Bloc may speak in it under test mode. |
| `black_bloc/cogs/community/polls.py:485` | `card_channel` sends the review card to `staff_channel_id`, or to the test channel while the guard is installed — the `events.py` shape, one line rather than a second copy of the rule. |
| `black_bloc/cogs/community/polls.py:1098` | The archive runs in the same 5-minute loop and logs **once per pass, only when something moved**, with the ids and the dropped-vote count. `poll_results` is never deleted: decision 14 is "archive, never delete"; only the per-voter rows go, and only when `poll_archive_drop_votes` says so. |
| `black_bloc/cogs/community/polls.py:312` | `save_results` writes `poll_options.final_votes`, one `poll_results` row and `polls.total_votes` together. The `poll_results` row is what survives the archive, which is why the totals are duplicated there rather than re-derived from options that may be gone. |
...
| Key | Note |
|---|---|
| `black_bloc/api/tools/polls.py:293` | `refuse_outside_the_test_channel` is the API's copy of the by-hand guard check, for the same reason the cog has one. It is a no-op for a poll with no channel yet, so a held poll can still be **denied** from the dashboard while test mode is on — denying posts nothing. |
| `black_bloc/api/tools/polls.py:341` | The index carries a `notes` line saying the create form arrives with 10b. ⚠️ **There is deliberately no `POST /api/polls`**: creating from the dashboard means a channel picker and the panel surface, and half of that shipping alone would be a form that silently refuses anonymous polls. A page that cannot create says so in words rather than offering a button that 409s. |
| `black_bloc/api/tools/polls.py:253` | The export writes the totals **always** and the per-voter rows only when the poll kept them, with a `note` row naming which of the two reasons applies (anonymous, or archived-and-dropped). An empty CSV with no explanation reads as a bug; a named reason does not. |
...
| Key | Note |
|---|---|
| `black_bloc/cogs/community/polls.py:592` | ⚠️ **`voter_key` is how an anonymous poll counts one vote each without keeping who cast it.** A panel MUST store a row per voter or a person could vote ten times; a poll sold as anonymous must not leave member ids in the database. So for `anonymous` rows the stored `user_id` is the top 63 bits of `sha256("<poll_id>:<user_id>")` — per-poll salted, so the same person is a different number in every poll, deterministic enough for "clear my vote" to find their row, and useless to anybody reading the table. `poll_votes.user_id` stays an INTEGER; nothing else changed. |
| `black_bloc/cogs/community/polls.py:644` | `set_panel_vote` writes a person's **whole answer as a replacement** — delete every row of theirs, insert what they now hold. An append-with-toggle would drift the moment two clicks raced; a replacement is idempotent and the button computes the new set from `my_positions` (`:626`) first. |
| `black_bloc/cogs/community/polls.py:871` | `cast_vote` takes the per-poll `asyncio.Lock` **around the write only**, then repaints outside it: two people pressing at once serialise on the row, and the message edit (which can be rate-limited) never holds the lock. Checklist 6, with the DB's own unique index as the second guard. |
| `black_bloc/cogs/community/polls.py:812` | `panel_view` gives a button per option up to **five** and one "Vote" button past that. Five is not Discord's cap (25 over five rows) — it is where a row of buttons stops being readable, and past it the modal shows the same options with a scrollbar instead. |
| `black_bloc/cogs/community/polls.py:971` | ⚠️ **`vote_picker` is where 2.7.1's typed modal fields earn their place, and where their real cap bites: both `RadioGroup` and `CheckboxGroup` are capped at TEN options in the library** (`discord/ui/radio.py:63`, `discord/ui/checkbox.py:71-73` — "between 2 and 10", "up to 10 items"). So the picker is a `RadioGroup` for one choice, a `CheckboxGroup` for many, and a plain `Select` (25 options, `required=False`, only valid in a modal) once there are more than ten. The research's §4.4 typed-modal design is real, but only for short polls. |
| `black_bloc/cogs/community/polls.py:1008` | `picked_values` reads `.values` when the component has one and `.value` when it does not, so the three pickers above are interchangeable to the caller. A `RadioGroup` has `value`; `CheckboxGroup` and `Select` have `values`. |
| `black_bloc/cogs/community/polls.py:1016` | The modal opens **pre-filled with what that person already picked** (`default=` per option), because a modal that forgets your answer makes "change one thing" mean "type it all again". Submitting it with nothing picked says `PICK_SOMETHING` and changes no vote — clearing is a separate button, so an empty submit is a slip rather than an instruction. |
| `black_bloc/cogs/community/polls.py:794` | `panel_card` is the one home for the panel's embed, so the post, every vote's repaint and the close all render the same card from the same row. The alternative — three call sites each passing its own arguments to `panel_embed` — is how a poll ends up saying `open` after it closed. |
| `black_bloc/cogs/community/polls.py:826` | `repaint_panel` asks `guard_allows` **and** swallows its own errors: a failed repaint must never fail the vote that caused it. The vote is the irreversible thing and the picture is cosmetics (checklist 12). |
| `black_bloc/cogs/community/polls.py:1177` | ⚠️ **`_write_panel_results` never calls `end_poll`, because a panel has no poll at Discord to end.** The whole native close path — `end_poll`, `counts_from_message`, `refresh_voters` — is skipped; the counts come from `poll_votes`, which is the authority for this surface. `_shut_panel` (`:1172`) then takes the buttons off and puts the final bars on, including for a poll that had been hiding them. |
| `black_bloc/cogs/community/polls.py:264` | ⚠️ **`poll_plan` is the one place a poll's arguments become a poll**, and it is why `POST /api/polls` cannot accept something `/poll create` would refuse. Validation, slot generation, surface derivation and the panel sentence all happen here; the two callers only decide where it goes and who is told. |
| `black_bloc/cogs/community/polls.py:316` | `store_poll` writes the row, its options and the `poll.created` line together and answers *(the row, is it held)*. `status=` overrides the review switch, which is how the recurrence loop opens an occurrence without sending it back for approval — a Lead approved the recurrence once, not every Tuesday. |
| `black_bloc/cogs/community/polls.py:253` | `add_options` gained `values`: `poll_options.value` carries the ISO instant a date slot stands for, so a slot survives its own label. If `poll_date_labels` is flipped later, the stored instants are still the truth about which evening won. |
| `black_bloc/cogs/community/polls.py:379` | ⚠️ **`polls_by_status` now excludes `recurrence IS NOT NULL`.** A template is not a poll: without this it would show up in `/poll list`, on the dashboard index and in the pending-review queue, as a poll nobody can vote in. `recurrences` (`:381`) is the query that wants them. |
| `black_bloc/cogs/community/polls.py:424` | ⚠️ **`claim_occurrence` is what makes the recurrence loop idempotent.** It moves `recur_next_at` on with `WHERE recur_next_at = <what we read>` and only opens the poll when `rowcount` says it got there first. Reading-then-opening-then-writing would post the same occurrence twice across a restart, exactly the way the reminder would have without `claim_reminder`. |
| `black_bloc/cogs/community/polls.py:1650` | `_recur` computes the NEXT time before claiming this one, and a cadence it cannot read **pauses the template** (`recur_next_at = NULL`) with a `poll.recur_failed` line rather than being retried every five minutes forever. Checklist 5: a bad row is finished, never left blocking. |
| `black_bloc/cogs/community/polls.py:2225` | Pause is `recur_next_at = NULL` — no new column, and `due_polls` already requires the column to be non-NULL, so a paused template is simply never due. `paused:false` recomputes the next time rather than restoring an old one, so resuming after a fortnight does not fire immediately. |
| `black_bloc/cogs/community/polls.py:2263` | ⚠️ **Delete does not delete.** `/poll recur delete` sets the template to `cancelled` and clears its next time; the row stays, the occurrences it opened keep a `schedule_id` that still resolves, and the archive sweep tidies it away on the usual schedule. Decision 14 is "archive, never delete", and a dangling `schedule_id` is a worse answer than a cancelled row. |
| `black_bloc/cogs/community/polls.py:2107` | ⚠️ **A date poll is refused a recurrence, in words.** Its slots are fixed days; the second time round it would be asking about a day that has been and gone. The sentence says so and points at `/poll create kind:date` or a checkbox poll with the days written on it. |
| `black_bloc/cogs/community/polls.py:1462` | `send_review_card` was lifted out of the cog method so `POST /api/polls` can hold a poll for review too. It answers *(where it went, the card)* and distinguishes "no staff channel at all" (`None, None`) from "the send failed" (`target, None`) — the first cancels the poll, the second leaves it decidable by hand. |

### `black_bloc/api/tools/polls.py` — the create form's route

| Key | Note |
|---|---|
| `black_bloc/api/tools/polls.py:344` | `POST /api/polls` calls `poll_plan` and `store_poll`, so the page cannot ask for anything the slash command would refuse and the refusal sentence is the same one Discord would have shown. 10a deliberately left this route out because half of it shipping alone would be a form that silently refused anonymous polls; with the panel built, that objection is gone. |
| `black_bloc/api/tools/polls.py:395` | `_open_or_hold` cancels the row when the post fails and raises a **409** with the reason, rather than answering 200 with a poll that is not anywhere. A created-but-not-posted row is the state `apply_decision` was already careful about. |
| `black_bloc/api/tools/polls.py:308` | ⚠️ **`_shown` reads an OPEN panel's counts from `poll_votes`, not from `poll_options.final_votes`.** `final_votes` is written once at close, so without this branch every open panel on the dashboard would read zero while its message showed real bars. |
| `black_bloc/api/tools/polls.py:178` | `recurrence_row` is a shape of its own rather than `poll_row` with holes: a template has no `status` a reader would understand, no votes and no close time, but it does have a cadence, a next time and a paused flag. `cadence_said` is rendered server-side by `describe_cadence` so the page and Discord say it identically. |
| `black_bloc/api/tools/polls.py:205` | `wanted_options` takes a list (what the form sends) or a `A \| B` string (what the command sends) and normalises to the string `poll_plan` reads, so one validator serves both. |
| `black_bloc/api/tools/polls.py:436` | Pause/resume and delete are the only recurrence writes the dashboard gets. **There is no create-a-recurrence route**: setting one up needs a cadence, a weekday-or-day-of-month and a timezone, and `/poll recur create` already asks for those with Discord's own choice pickers. The page says where to make one instead of offering a form that would be the worse of the two. |
...
| Key | Note |
|---|---|
| `site/mock/server.mjs:1097` | ⚠️ **`CANNED` carries `black_bloc/chat.py`'s `LINES` tables VERBATIM**, including the two `ATTENDEE_LINES` as extra lines on `greeting` and `how_are_you`. A fixture that merely resembled them would have let the page look right while editing wording nobody says. `sort` follows `chat.py:ORDER`, so the mock classifies in the bot's own order. |
| `server.mjs:600` | Each data intent keeps **exactly two** lines — the template and the one for when there is nothing to say. That pair is the whole shape of a data intent, and seeding both means the empty state is drawn without anybody having to break something first. |
| `server.mjs:2881` | `seedChat` returns `{intents, lines, nextIntent, nextLine}` — lines live in a flat list keyed by `intent_id` rather than nested in the intent, so `PUT /api/chat/lines/{id}` can find one without knowing its intent, exactly as the route contract does. |
| `server.mjs:2822` | `chatWords` is `black_bloc/chat.py:normalise` in JavaScript, and `chatHasPhrase` is `has_phrase` — ⚠️ **a second copy of a rule, on purpose**, because a mock that matched substrings would let "helping" hit `help` and the page would be tested against a bot that does not exist. Measured: `helping is hard` → `unknown`. |
| `server.mjs:1091` | `chatOrdered` sorts **custom first, then built-ins by `sort`** — the design's classification order — and it is the same list the page renders, so the card order on screen IS the order a message is matched in. |
| `server.mjs:3026` | ⚠️ **The dry run picks the FIRST enabled line, not a random one.** `chat.py:respond` uses `random.choice`; a Try-it box that answered differently every press could not be checked, and the thing being tested is *which line pool*, not which draw. A line pool with nothing enabled falls back to the first line, which is the design's "the code tables remain the fallback". |
...
| `server.mjs:2881` | `DELETE /api/chat/intents/{id}` refuses a built-in **in words** and names the alternative ("turn it off instead"). `DELETE /api/chat/lines/{id}` (`:2879`) refuses the **last** line for the same reason: removing it would leave the intent with nothing to say. |
...
| **`total` / `page` / `per_page` / `shown` beside today's `limit` and `notes`.** The brief's payload drops `limit`; three existing pages send `?limit=`. | The mock answers **both** — `limit` is an alias for `per_page` (`server.mjs:990`) — and `contract.json` requires all seven. If 12a drops `limit`, Overview and Health need editing too. |
| **`mod_log_level` would create a `mod` settings namespace**, and `contract.json`'s `/api/settings` entry asserts `no_namespaces: ["mod", "modlog"]`. | `server.mjs:667` overrides it into **`automod`**, exactly as `modlog_channel_id` and `mod_dm_on_action` already are. 12a must add the same line to `black_bloc/api/settings_api.py:NAMESPACE_OVERRIDE` or the contract test fails on both halves. |
...
| `server.mjs:1704` | `.would_` is checked FIRST and is always routine, before the explicit set and before the suffix backstop — otherwise a kind like `poll.would_ended` would be caught by the `.ended` suffix and marked important. |
...
| `server.mjs:722` | `seedActions` numbers the whole list descending from the newest so the hand-written head (`recentActions`, the 23 rows the older pages were built against) keeps its exact content and its order. `SEEDED_ACTIONS` (`:204`) is built once and **cloned** into each `seedState()`, because `logAction` unshifts into `state.actions`. |
| `server.mjs:962` | `since`/`until` are compared as **strings**: ISO-Z stamps sort lexicographically, so a `YYYY-MM-DD` from a date box is padded to `T00:00:00.000Z` / `T23:59:59.999Z` and no clock arithmetic is needed. |
| `server.mjs:893` | `export.csv` is registered **before** `/api/actions` but the two cannot collide anyway — `match()` requires equal segment counts. It is deliberately **not** in `contract.json` (the brief: "link only"), and it is staff-gated: measured **403** for a stranger on both routes. |
...
| `black_bloc/cogs/moderation/modcmds.py:416` | ⚠️ **`/mod` is a NEW top-level group and the moderation commands stay where they are.** `/warn`, `/timeout`, `/kick`, `/ban`, `/purge`, `/case`, `/cases` have always been top-level; moving them into `/mod` would have broken every muscle memory and every doc. `/mod` holds `logs` and nothing else for now. |
...
| `black_bloc/requests.py:430` | ⚠️ **`ORDER BY (status <> 'open'), id DESC` — open first, then newest (it said `'pending'` until Phase 17 renamed the state) — is done in SQL, and `limit`/`offset` are passed straight through.** The first cut read every row and sliced in Python, which is fine at fifty requests and wrong at five thousand. `page_of` at `:241` still exists, but only for the SLASH command, where the rows are already in hand and a Discord message caps the list anyway. |
| `black_bloc/requests.py:489` | ⚠️ **`set_status` clears `decline_reason` on any move that is not a decline (or, since Phase 17, a hold — the hold reason lives in the same column, and `held_from` is written on the way into hold and cleared on the way out), and stamps `done_at` only on the move to `done`.** A request declined "we already have one" and later approved would otherwise still be carrying the sentence that says it was refused, and the page would show both. `decided_by`/`decided_at` use `COALESCE`, so a later move that names nobody (a withdrawal) does not erase who decided it first. Pinned by *moving to done stamps when and moving off declined forgets the reason*. |
| `black_bloc/requests.py:520` | `set_fields` takes `...` for "leave it alone" and `None` for "clear it", and returns the names it actually wrote. That is what lets `POST …/status` tell a save that changed nothing (refused) from one that cleared a field (allowed), and what keeps `web.request.updated`'s `changed` list honest. |

### What a person may type

| Key | Note |
|---|---|
| `black_bloc/requests.py:288` | `parse_due` uses `date.fromisoformat`, so `2026-02-30` and `2026-13-01` are refused as firmly as `next tuesday` — a shape check with a regex would have taken both. An empty box is `None`, never an error: the due date is optional and a member who leaves it blank has not made a mistake. |
| `black_bloc/requests.py:300` | ⚠️ **`due_on` is a plain `YYYY-MM-DD` STRING everywhere — stored, sent and taken — and only ever becomes an instant at render time.** `due_stamp` builds `<t:…:D>` from **local midnight in the server's zone** (`timezones.DEFAULT_TZ`, America/Phoenix, a fixed UTC-7), so the stamp reads as the day the person typed rather than the day before it. Midnight UTC would flip the date for every reader west of UTC, the owner included; an ISO instant on the wire would make the dashboard guess a zone. Pinned by *the due stamp is local midnight in the server's zone, not UTC*. |
| `black_bloc/requests.py:411` | `checked_fields` refuses What before Why, so the sentence a person gets names the box they left empty rather than "fill it in". Both are then cut to 1000, which is what the modal's `max_length` already enforces — the API has no such enforcement, so the cut is the one that matters. |
| `black_bloc/requests.py:66` | `STATUS_WORDS` is the vocabulary in one place, and the API sends it as `status_word` beside `status`. Without it the dashboard would carry a second copy of the mapping and the two would drift the first time a state was renamed. |

### The cog

| Key | Note |
|---|---|
| `black_bloc/cogs/community/requests.py:123` | ⚠️ **`submit` asks the guard BY HAND (`guard_allows`, at the top of `submit`), because a modal submission never goes through `tree.interaction_check`.** `guard.install()` patches `http.send_message` and sets `tree.interaction_check`, and neither one sees a modal reply: the interaction response goes out on a different endpoint, and the check only wraps app commands. This is the same rule the polls cog writes down at `cogs/community/polls.py:1421`. Pinned by *a modal sent from outside the test channel is refused by hand*. |
| `black_bloc/cogs/community/requests.py:249` | ⚠️ **`notify` SKIPS a channel the guard refuses rather than raising, and the request is filed either way.** The notice is a courtesy; the row is the point. Raising would mean that setting `request_notify_channel_id` to anything but the test channel while `TEST_MODE` is on would break filing altogether. A channel that refuses the post for a real reason (permissions, an outage) is a logged `request.notify_failed`, so a silent nothing is still a visible fact. |
| `black_bloc/cogs/community/requests.py:284` | `apply_decision` is the one path a status moves by, and both the slash command and the API call it — so the DM, the `request.<look>` line and the "already **x**" refusal cannot differ between the two. It returns `(said, fresh)` with `fresh is None` meaning nothing moved, the shape `role_menus.apply_request_decision` already uses. |
| `black_bloc/cogs/community/requests.py:174` | A DM that bounces is a `request.dm_failed` row, never a swallowed exception — a member who blocks the bot or leaves the server must not make a staff decision look like it failed. `bot.get_user` is the fallback when `guild.get_member` has nothing, so somebody who has left is still tried once. |
| `black_bloc/cogs/community/requests.py:616` | ⚠️ **The staff test is `store.is_staff(interaction.user)`, which is `settings_store.resolved_staff_roles` — the SAME derivation the dashboard's own gate uses**, via `api/auth.py:is_admitted`. Anything else here would mean a mod is auto-approved on the site and held in Discord, or the other way round. |
...
| `black_bloc/api/tools/requests.py:295` | `_may_file` raises for both refusals (it returned the auto-approve answer until Phase 17 retired approval — every filing now arrives `open`), so the route reads as one line and the two refusals cannot get out of order — `request_mode = off` is a 409 about the feature, `request_who_can_file = staff` is a 403 about the caller, and a member meets them in that order. |
...
| `page-requests.js:877` | `load(me)` branches once: member → `loadMember()` (File a request + Your requests, `/api/requests/mine`, Withdraw on the pending ones); otherwise `loadStaff()`. **There is no half-staff render** — the decide controls, the settings panel and the filter toolbar are not built at all rather than built and hidden. |
...
| Key | Note |
|---|---|
| ⚠️ `server.mjs:660` | **`state.requests` was already taken by ROLE requests** (`server.mjs:485`, read at `:1490`), and a second `requests:` key in the same object literal silently won — duplicate keys in a JS object literal are not an error, the last one wins. `GET /api/rolemenus/requests` started answering **feature requests mapped through the role-menu row builder**, which is how `check.mjs` reported twelve rows "missing requested_at, deny_reason". Renamed to `featureRequests` / `featureComments` / `nextFeatureRequest` / `nextFeatureComment`, and the row builder to `featureRequestRow` (`:2344`) beside role menus' own `requestRow` (`:1245`). **The contract check found this; reading the diff did not.** |
...
| `server.mjs:72` | ⚠️ **`dayAhead` builds `YYYY-MM-DD` from the LOCAL clock, and `page-requests.js:126 dueOn` reads it back as LOCAL midnight.** `new Date('2026-09-10')` is UTC midnight, which prints as the **ninth** anywhere west of Greenwich — the page would name a different day from the one somebody typed, on the owner's own machine (Phoenix, UTC-7). This is why the due date does not go through `ui.js:untilWhen`. |
| `server.mjs:3240` | `POST /api/requests` is the **first route in the mock a non-staff session may call**. It checks `request_mode`, then `request_who_can_file`, then auto-approves when the filer is staff and `request_auto_approve_staff` is on — three separate refusals in words rather than one generic 403. |
| `server.mjs:3271` | `/status` refuses a move on a **pending** row ("Approve it first") and on a **declined or withdrawn** one, so the board's segment cannot silently resurrect a settled request. Priority, assignee and notes are settable on any row, which is what lets Approve and triage happen in either order. |
| `server.mjs:3043` | `withdraw` is the member's own: it checks the row is theirs AND still pending. Both refusals name which it was. |
...
| `page-requests.js:409` | ⚠️ **The board segment has FOUR steps, not three.** Approving a request makes it `approved`, and a three-step Planned → In progress → Done segment would render with **nothing pressed** on every freshly approved row, which reads as broken. Approved is the first step. |
...
| ⚠️ `page-requests.js:471` | **`replaceChildren` turns a `null` child into the WORD "null"** — `el()` filters them, `replaceChildren` does not, and the first browser pass had a literal `null` beside every Change button. The list is built then `.filter(Boolean)`ed. |
| `page-requests.js:219` | The two-line clamp's Read-the-rest button is **measured, three times**: on render, again on `document.fonts.ready`, on every `toggle` (capture — `toggle` does not bubble) and debounced on resize. ⚠️ **The first reading is a lie in two ways**: the self-hosted face has not loaded, so the fallback fits in two lines where Rajdhani needs three (measured: 6 whys clamp at 390, and the first pass showed **0** buttons); and a why inside a shut foldout has `clientHeight` 0, so it never overflows. |
| `page-requests.js:294` | The notes drawer **fetches on first open**, not on load — thirty requests would otherwise be thirty extra calls per load for a thread almost nobody opens. A failed fetch resets `filled` so the next open retries. |
| ⚠️ `page-requests.js:369` | **`emptySaid` distinguishes an empty list from a page past the end of one.** Found by measuring: after a pager test left `state.board` at 2 and the fixture was reset to 14 rows, the board said "Nothing is approved and unfinished" while its own count said 14. That is a lie about the list rather than about the page. Real readers hit it whenever somebody else decides the last request while they are on page three. |
| `page-requests.js:746` | The filter box and the three assignee chips are **server-side** (`q`, `assignee`; `On me` resolves to the signed-in id), and both reset `pending` and `board` to page 1. `keepTyping` (`:769`, Members' own trick) puts the caret back after the rebuild. |
| `page-requests.js:629` | The outcome sentence under File a request says whether **this person's** request skips the queue — `request_auto_approve_staff` AND `me.staff`, not one or the other. Measured through all four states (no what / no why / no due / due). |
| `page-requests.js:734` | ⚠️ **Why sits in its own one-field `.formrow`.** A bare `.field` lays its label out BESIDE the control; inside a formrow the label sits above it. The member view showed both shapes on one form until this was fixed. |
...
| `black_bloc/api/tools/requests.py:404` | ⚠️ **`GET /api/requests/mine` and `GET /api/requests/export.csv` are declared BEFORE `GET /api/requests/{request_id}`.** `request_id` is typed `int`, so FastAPI would answer `mine` with a 422 validation error rather than the list — a bare status with no sentence, which is exactly what the front-door rule forbids. Route order is the whole fix and it is load-bearing; the mock's own `match()` scans in registration order for the same reason. |
| `black_bloc/api/tools/requests.py:575` | ⚠️ **`POST …/status` writes the fields FIRST and moves the state LAST.** A bad `assignee_id` sent alongside `status: done` must not leave the row done and unassigned. `set_fields` is a single `UPDATE`, `wanted_assignee` raises before it, and `apply_decision` runs only once everything else has landed. Pinned by *the status route moves the state last so a bad field stops it*. |
| `black_bloc/api/tools/requests.py:138` | `requester` and `assignee` are objects (`{id, name, avatar}`) rather than a flat `assignee_id` / `assignee_name` pair, because the page draws an avatar beside each and a null assignee has to be one absent thing, not two. `decided_by` stays flat, matching the audit rows. |
| `black_bloc/api/tools/requests.py:323` | `open` rides along on the list response — the sidebar badge, counted over the guild rather than the page, so paging to page 3 does not make the badge say 0. |
| `black_bloc/api/tools/requests.py:233` | `wanted_filter` reads `assignee=none` as the board's **Unassigned** column and turns it into `assignee_id IS NULL`; anything else is an id. It is a separate function from `wanted_assignee` because a FILTER and a WRITE want opposite things from an empty value — the filter wants every row, the write wants the field cleared. |
| `black_bloc/api/tools/requests.py:241` | ⚠️ **`q` also matches the requester's NAME, and a name is not in SQL.** Names live in the gateway's member cache, so `members_matching` resolves the query to ids in Python and `_where` ORs `user_id IN (…)` onto the three `LIKE` clauses. Capped at `NAME_MATCH_LIMIT` = 200 ids so a one-letter query cannot build an unbounded `IN` list. |
| `black_bloc/api/tools/requests.py:263` | `wanted_assignee` takes `...` for absent and `None`/`""` for cleared, so `{"assignee_id": ""}` unassigns and a payload that never mentions it leaves it alone. Both were needed: the page's picker sends the empty string to clear. |
| `black_bloc/api/tools/requests.py:365` | ⚠️ **`POST /api/requests` is NOT guard-refused under `TEST_MODE`, and that is deliberate** — it is a database write, and the only thing this feature sends to a channel is the optional notice line, which asks the guard itself. Refusing the route would mean nobody could file from the site while the bot is in test mode, for no safety gain. The COG's modal reply *is* guard-checked, because that one really is a message. Pinned by *filing from the site is not guard refused while test mode is on*. |
| `black_bloc/api/tools/requests.py:302` | ⚠️ **SUPERSEDED and left here as history: this note used to say a web decision leaves TWO action rows** (`request.<status>` from `apply_decision` plus `web.request.<status>` from `note`). That was the 2026-09-03 double-post defect the owner reported. One write leaves ONE row now: the route passes `via=VIA_WEBSITE` and notes nothing — see the `One web write, one log row` section. |
...
| ⚠️ `server.mjs` `POST /api/requests` | 13a's mock never checked `request_who_can_file`; the router does (`api/tools/requests.py:223 _may_file`). Added, with the router's own `STAFF_ONLY_FILES` sentence, so a member meets the same three refusals in the same order on both halves. |
| `page-requests.js:507 pagerFor` | Sent `per_page` the router does not take and derived `hasMore` from a page size of its own. It now reads `payload.per_page` and `payload.pages`, which the router sends (`API_PAGE` is **20**, not the page's 25). |
| **not reconciled, deliberately** | The mock's `/status` route lets a **pending** row move straight to `planned`, because `apply_decision` (`cogs/community/requests.py:170`) does; 13b's mock refused it with "Approve it first". The page tolerates both and the router is the truth. |

### Phase 12 meets Phase 13 — the thirteenth feature

| Key | Note |
|---|---|
| `black_bloc/logkinds.py:19` | `request` joins `FEATURES`, so `settings_store.py:438` generates `request_log_level` with the other twelve and `logkinds.LOG_LEVEL_KEYS` is thirteen long. `HEADS` takes **both** `request` and `requests` as heads, because `feature_of` splits on the first dot and nothing stops a later kind being written either way. |
...
| `black_bloc/logkinds.py:155` | ROUTINE gets `filed`, `withdrawn`, `in_progress`, `resumed`, `updated`, `comment` and `notify_skipped_test_mode` — the owner's line is that a DECISION is loud and the rest of a request's life is not. `bare()` means every `web.request.*` twin classifies with its bare kind, so fourteen kinds are covered by seven entries. (Third pass: `request.review` and `request.sent_back` went to `IMPORTANT`, not here — staff have something to do about both.) |
| `tests/test_logkinds.py:42` | Two additions: the `f'request.{status}'` call site in the cog, enumerated to its five statuses, and the nine `web.request.*` kinds under `black_bloc/api/writes.py::kind` — those reach `log_action` through `note()`, so the scanner sees the variable, not the string. |
| `black_bloc/cogs/community/requests.py:648` | `/request logs` is the thirteenth, and it is the whole body of `send_logs` like the other twelve — which is why `settings_store.is_staff_command` finds `require_staff` in its `co_names` and hides it from non-staff with no `extras`. |
...
| `page-requests.js:871` | `await logsSection('request')` at the foot of the STAFF view only — a member cannot read `/api/actions`, and asking would be one 403 swallowed for nothing. `request_log_level` joins the page's `SETTING_KEYS`, `request` joins `logs.js:18 LOG_FEATURES` (so the Logs page gets its chip) and `logs.js:46 ROUTINE` gets its sentence. |
...
| Key | Note |
|---|---|
| `black_bloc/logkinds.py:329 via_of` | Two words, `discord` and `website`. **What the writer recorded wins**; otherwise the kind's `web.` head decides. That second half is what makes every row written before tonight say something rather than nothing. |
...
| `black_bloc/actionlog.py:35` | `SUMMARY_SKIPS` keeps `via` out of the flattened `key=value` summary — it is its own column and its own part of the line, so it must not appear twice. The mock mirrors the same list (`server.mjs:940`). |
...
| `black_bloc/cogs/community/role_menus.py:2138` | Seeding now leaves a `role_menu.seeded` line from BOTH doors; the slash command left none at all before, so a seeded server had no record of when its menus appeared. |
...
| Key | Note |
|---|---|
| ⚠️ `black_bloc/cogs/community/events.py:249 checked_fields` | **The modal's whole validation chain, lifted out and made callable.** Title, the `YYYY-MM-DD HH:MM` parse, the **DST gap and ambiguity** checks, "that has already gone by" and the `1h30m` duration parse, in that order, returning `(fields, why)`. `submit` now calls it and so does the PUT, so a dashboard edit cannot make an event `/event propose` would have refused — and the refusal sentence is the identical one. |
| ⚠️ `black_bloc/api/tools/events.py:192 event_edit` | **The zone is the caller's, and it is named on the wire.** The body carries `start` as a bare `YYYY-MM-DD HH:MM` plus a `tz`; the page sends the browser's own IANA zone (`Intl.DateTimeFormat().resolvedOptions().timeZone`) and shows it above the field, and an unknown zone falls back to the staffer's stored `/timezone set` one. A bare local time with no zone is the bug this avoids — the cog reads it in the *requester's* zone, which is not who is typing. |
| `black_bloc/api/tools/events.py:186` | Only `pending` and `approved` may be edited (`EDITABLE`); a denied, cancelled, live or done event answers `409 not_editable` naming its state. Nothing here calls `can_transition`, because an edit is not a transition — the status does not move. |
| ⚠️ `black_bloc/api/tools/events.py:232` | **What an edit does NOT do is in the answer, not left to be discovered.** `rename_channel` (the cog's own helper, guard-aware, `would_rename` in test mode) follows the title, so the review channel keeps up. An announcement already posted and a Discord scheduled event already made **keep the old details**, and `notes` says so in words for whichever applies — there is no existing helper that rewrites either, and claiming a change that did not happen is checklist 10. `announced` and `scheduled` are on every row so the page can warn before the save, not only after. |
| `black_bloc/api/tools/events.py:89` | `GET /api/events/{event_id}` exists so the page has a detail route to name, though the list already carried every field the card shows — the audit's finding was that the page *fetched* them and rendered none of them. |
...
| `server.mjs:2816`, `:2820` | The mock learns the name and the token list so the Chat page draws its chips and refuses `who_has` as a custom name. It is deliberately NOT added to the `chatIntents` fixture: `who_is_live` is the only data intent that fixture carries, and a second one would need its own branch in `chatFilled`/`chatTokens` to answer honestly. |
...
## 2. The channel list rides EVERY model call, in both tiers

| Key | Note |
|---|---|
| ⚠️ `black_bloc/personas.py:213` | **The list goes AFTER the cached core and BEFORE the mood, and both halves of that are load-bearing.** After the core, because the `cache_control` breakpoint sits on the core block and a directory placed in front of it would invalidate the cached prefix every time a topic was edited — the list changes daily, the rules never. Before the mood, because the mood is the volatile per-conversation block: everything from the breakpoint down is re-sent anyway, so the ordering costs nothing and keeps the stack readable as rules → facts → voice. |
| ⚠️ `black_bloc/chat_llm.py:509` | **Built fresh per call from the gateway cache, deliberately NOT from the daily ingest.** The ingest is up to 24 hours stale; a channel made this morning would be a channel the bot denies exists. The knowledge rows still carry the same channels (item 1) — they are what the lexical search scores against — but the model's *permission to name a channel* comes from the live list. |
| `black_bloc/chat_llm.py:509` | A directory that cannot be built at all answers `DIRECTORY_NONE`, which tells the model to name **no** channel, rather than dropping the block. A missing block would leave the core's "channels that are in the channel list below" pointing at nothing, and a model reading a rule about an absent list is exactly where an invention comes from. |
| ⚠️ `black_bloc/directory.py:130` | **Over the cap the TOPIC goes, never the name — the name only falls off once every topic is already gone.** A dropped name is a channel the model is then forbidden to mention (the core rule) *and* the post-check guard would rewrite (item 3), so trimming by whole lines would silently make real channels un-nameable. Longest topic first, because the longest topic is the one paying the most for the least. |
| `black_bloc/directory.py:88` | `channel_names` is deliberately built from `open_channels` and not from the capped directory: the guard's question is "is this a real public channel", which does not stop being true because a topic did not fit. |
| `black_bloc/personas.py:33` | The naming rule lives in CORE rather than in the directory block so a trope can never displace it — the same structural argument the file already makes for the mood being appended. It names channels, roles AND members together because the owner's two live failures were one of each: an invented `#black-support-hub`, and a suggested `@Admin` role that does not exist. |

## 3. The post-check guard — a prompt rule is not a guarantee

| Key | Note |
|---|---|
| ⚠️ `black_bloc/chat_check.py:107` | **This exists BECAUSE item 2 exists, not instead of it.** A system prompt is a request; the guard is the enforcement, and it runs on every model reply whether or not the prompt was obeyed. It is also the only way hallucination frequency becomes a NUMBER: every fix writes `chat.reply_reference_fixed` with what was invented, so `/chat logs` and the Logs page count the rate instead of somebody happening to notice one. |
| ⚠️ `black_bloc/chat_llm.py:590` | **The guard runs BEFORE `remember`, so the window stores what the member actually saw.** Storing the raw model text would feed the invention back into the next turn's conversation history — the model would see itself say `#black-support-hub`, and the rule "do not name what is not in the list" loses to its own transcript. |
| `black_bloc/chat_check.py:18` | The channel pattern refuses a token with no letter in it (`#1`) and any `#` preceded by a word character, `<`, `:`, `/` or `#` — so `<#800>` mentions, `https://x/y#top` fragments and Markdown headings are all left alone. Discord channel names are lowercase, but the model writes `#General`, so the lookup is case-folded while the substitution keeps whatever it wrote. |
| ⚠️ `black_bloc/chat_check.py:60` | **A role is known by its whole name OR its first word.** The server's staff role is `Aunties / Uncles`; a model writes `@Aunties`, which is neither a mention nor the full name, and rewriting it would delete a correct answer. `@everyone` and `@here` are always known — item 4's transport strips them anyway, and rewriting them mid-sentence would read as a bug. |
| `black_bloc/chat_check.py:86` | Smoothing drops at most ONE preposition before the hole, plus a dangling `or`/`and` before punctuation, then tidies spacing. It is deliberately modest: "Ask in #nowhere." → "Ask.", "Head over to #nowhere." → "Head over." The result is sometimes terse, and terse is the point — the alternative shapes are inventing a replacement channel or deleting the whole sentence, and both lose more than the article does. Setting `chat_home_channel_id` is what replaces terseness with a real destination. |
| `black_bloc/chat_check.py:74` | A blank `chat_home_channel_id` is the DEFAULT and is safe: no channel is named at all. The owner has not decided which channel it should point at, and a guard that needed a decision before it could protect anything would not have shipped today. |
| ⚠️ `black_bloc/chat_llm.py:509` | `made_real` swallows its own failures and returns the reply unchanged. That is the one place the posture is fail-OPEN rather than fail-closed, and on purpose: the guard is a second lock behind the prompt and the directory, and a crash in the rewriter must not turn a working answer into silence. The exception is logged. |
| `tests/test_logkinds.py:161` | `FIXED_KIND` is a constant rather than a literal at the `log_action` call, so the enumeration test needs it in `KNOWN_DYNAMIC` beside the two chat kinds already there. That test is what makes an unclassified kind impossible by accident. |

## 4. Nobody gets pinged by a conversation, except when staff started it

| Key | Note |
|---|---|
| ⚠️ **Measured before anything was built** | `black_bloc/cogs/content/chat.py` has exactly ONE conversational send — the `message.reply` in `on_message` — and it was already passing `discord.AllowedMentions.none()`. The other 20 sends in the file are ephemeral slash-command answers (all already `none()`, and none of them can carry a member's text) and the staff note in `tell_staff` (`none()`). `answer_for` is also reached from `api/tools/chat.py:542`, which is the dashboard's dry run and sends nothing to Discord. **So there was no hole to close, and the item is the staff exception rather than the strip.** |
| ⚠️ `black_bloc/cogs/content/chat.py:713` | **The exception is keyed on the INITIATOR, never on the reply's content.** `store.is_staff` is the one canonical check — a role that can see `staff_channel_id`, or Manage Server — the same rule `require_staff`, `need_a_mod` and `who_has`'s escalate sentence all read. Nothing here re-derives who staff are, which is the whole point of routing it through the store. |
| ⚠️ `black_bloc/cogs/content/chat.py:713` | **`everyone=False` and `users=False` stay false for EVERYONE, staff included.** The owner asked for role pings; `@everyone`/`@here` is the one thing a chat reply must never be able to do, whoever asked, and `users=False` means the guard's smoothed member names cannot become pings either. `replied_user=False` keeps the existing "no ping on the reply itself" behaviour, which `mention_author=False` also says. |
| `black_bloc/cogs/content/chat.py:713` | Every failure path returns `none()`: no guild, the key unreadable, `is_staff` raising. A ping is the irreversible half of this decision, so the quiet answer is the one that survives a fault. |
| `black_bloc/settings_store.py:942` | `chat_staff_can_ping_roles` defaults **on**, which is what the owner asked for, and is safe because the gate above it is `is_staff`. A server that wants a bot that can never ping anything flips one switch. |

## 5. Who somebody is, and who is about to help

| Key | Note |
|---|---|
| ⚠️ `black_bloc/chat.py:376` | **`about_member` only matches when the message points at somebody, and that flag is what makes its triggers safe.** Its phrases are things like "is a mod" and "is a lead" — which are also inside "who is a mod" and "who is a lead", the questions `who_has` owns. Ordering alone could not separate them: above `who_has` it would have swallowed the role question, below `need_a_mod` it would have lost the owner's actual sentence ("im looking for a mod can I trust @Pawpette", which contains BOTH). The mention is the discriminator, so `classify` takes `mentions_member` and `NEEDS_A_MEMBER` skips the intent without one. |
| `black_bloc/chat.py:346` | Black Bloc's own mention is excluded before the flag is computed — every message that reaches this code @-mentions the bot, so "somebody was mentioned" would otherwise always be true. `others_mentioned` also ignores `<@&id>` role pings; `USER_MENTION` cannot match them because a `&` follows the `@`. |
| ⚠️ `black_bloc/chat.py:640` | **`with_extra` appends a sentence instead of using a token, because `need_a_mod`'s row ALREADY EXISTS in every live guild.** `seed_defaults` creates rows it does not find and never rewrites the ones it does, so a `{online}` token added to the code table would render for a guild seeded tomorrow and be invisible in the guild that asked for the feature. The append is checked against the line first, so a line that DOES carry the sentence is not given it twice. |
| ⚠️ `black_bloc/chat.py:857` | **`top_up_triggers` is the same problem for TRIGGERS, and it is solved the other way — by editing the row.** A trigger has to be matched before any line is chosen, so an append at render time cannot help. It adds only code-table phrases the row is missing, never removes anything, and never touches an intent of the guild's own. ⚠️ **The accepted cost: a built-in phrase staff deliberately DELETED comes back at the next restart.** That trade is deliberate — the ways to silence a built-in are to turn the intent off or to claim the phrase with a custom intent (customs are matched first), and both survive the top-up. Without it, "looking for a mod" would have shipped to a code table nothing reads. |
| `black_bloc/chat_data.py:398` | `staff_roles_of` was pulled out of `escalation` so `need_a_mod`, `who_has` and the new online-names sentence all read the SAME list — roles that can see `staff_channel_id`. There is still exactly one staff-resolution implementation in the codebase and this is not a second one. |
| ⚠️ `black_bloc/chat_data.py:411` | **A member with no readable presence counts as OFFLINE.** `getattr(status, "name", status)` on a member the presence cache has not filled yields `""`, which is in `OFFLINE`. Naming somebody as "online right now" who is not there sends a member to knock on an empty door; saying "none of them are online" when two are is a smaller failure. |
| ⚠️ `black_bloc/chat_data.py:432` | **Display names, never mentions — the owner's whole point.** *"dont @ them though, let the user do that part"*. Bots are dropped, duplicates across two staff roles are dropped, and the list is sorted case-insensitively BEFORE the limit is applied, so the same question names the same two people twice running rather than rotating. |
| `black_bloc/chat_data.py:452` | `chat_escalation_names` at 0 means the sentence is not added at all, which is different from "nobody is online". Both are reachable and they say different things. |
| `black_bloc/chat_data.py:498` | `about_member` answers about the FIRST person named. A sentence pointing at three people asking "can I trust them" is not a shape anybody has typed; the grounding block (`member_notes`) carries all of them for the model, and the deterministic answer stays one sentence. |
| ⚠️ `black_bloc/chat_data.py:498` | **The verdict comes from `bot.store.is_staff`, the canonical check, and nothing else** — not from a role-name list, not from `manage_messages`. The owner's words were *"can we check by permission levels"*, and this is the one rule the whole bot already agrees on, so a Lead qualifies without Leads being named anywhere. |
| `black_bloc/chat_data.py:487` | A member who is not staff gets their roles named and one sentence pointing at `need_a_mod` — which is where the online names live. No judgement about the person, and the escalation the asker actually wanted is one message away. |
| ⚠️ `black_bloc/chat_llm.py:471` | **Every model call carries a line per person the message named, and the opener tells the model that line is the whole truth about them.** The CORE rule already forbids inventing facts about a member; this is what makes obeying it possible — before it, a model asked "can I trust @Pawpette" had no way to know who Pawpette was, which is exactly how it answered "I don't know, ping @Admin". Capped at five people so a mention-storm cannot fill the turn. |
| `black_bloc/chat_llm.py:590` | Those same display names are handed to the item-3 guard as `people`, so a reply that says "ask Pawpette" is not rewritten as an invention. The guard and the grounding read one list. |

## 6. Staff commands are invisible below Aunties / Uncles

| Key | Note |
|---|---|
| ⚠️ `black_bloc/command_visibility.py:16` | **`manage_messages` was MEASURED, not chosen.** Among the server's human roles exactly Leads (administrator, which implies everything) and Aunties / Uncles carry it, which is precisely the line the owner wanted — so `Permissions(manage_messages=True)` hides a command from every role below Aunties and from nobody above. One constant, imported by nine cogs, so the line moves in one place if the role set ever changes. |
| ⚠️ **This is UX, not enforcement** | Discord calls `default_member_permissions` a *hint*: a server admin can override it per command in Server Settings → Integrations, and it says nothing about who may actually run the thing. **Every `require_staff` gate is untouched.** The lock only stops a member from seeing a command they would be refused by. |
| ⚠️ **Only top-level granularity exists** | Discord applies the permission to a top-level command and every subcommand under it; there is no per-subcommand form. So a group with even ONE member-facing subcommand stays visible and leans on its runtime gates. `/golive`, `/event`, `/birthday`, `/request`, `/voice`, `/poll`, `/timezone`, `/twitch`, `/help`, `/ping` and `/about` are that case, and **the groups were deliberately NOT restructured to force the lock** — moving `/golive optin` out from under `/golive` to hide five staff subcommands would cost every member the command they actually use. |
| ⚠️ `black_bloc/cogs/content/chat.py:205` | **`/chat` IS locked, and the brief's example list said it should not be.** Measured: all nine of its subcommands go through `require_staff`, so there is nothing member-facing to lose. Members reach chat by @-mentioning the bot, which is not a command. |
...
# Phase 15 (F14) — ping roles

> Keyed against the Phase 15 branch (`37407bd` + this commit). Nothing below has
> met a real Discord role: role creation, panel refresh and the Manage Roles
> ordering are test-suite evidence only.

## `black_bloc/pings.py` — the decisions, with the Discord calls at the edge

| Key | Note |
|---|---|
| `black_bloc/pings.py:515` | ⚠️ **`kind_via(...)` is why one helper serves both doors.** `/pingroles streamer add` and `POST /api/pings/streamers` write the SAME row and the SAME log line; the only difference the log should show is where it came from, so the `web.` head is built from the caller's `via` instead of being a second literal in a second module. This file used to own a private `head(via)`; 2026-09-03 replaced it (and four other copies) with the one `logkinds.kind_via` — see `black_bloc/logkinds.py:355`. |
| `black_bloc/pings.py:143` | ⚠️ **Checklist 17.** `pings_fan_role_template` is staff-editable text going into `str.format`, so a stray `{game}` raises `KeyError` — the fallback catches every exception, logs it, and uses `{name} pings`. The result is then whitespace-collapsed and cut to **100** characters, which is Discord's role-name ceiling (checklist 22); a template that renders to nothing at all falls back rather than asking Discord to make a nameless role. |
| `black_bloc/pings.py:208` | ⚠️ **The `notice` flag exists so the missing-role line is logged ONCE per stream, not twice.** `_go_live_once` calls this with `notice=True` and `_mark_ended` with `notice=False`: an announcement and its end-of-stream edit are the same session, and a role somebody deleted by hand would otherwise leave two `pings.fan_role_missing` rows for one event. |
| `black_bloc/pings.py:208` | The whole read is skipped while `pings_mode` is off, so turning the feature off costs the announcement path one settings lookup and no query at all. |
| `black_bloc/pings.py:260` | ⚠️ **`wear` catches bare `Exception`, not `discord.Forbidden`.** discord.py raises `Forbidden`, `HTTPException` and plain `TypeError`/`ValueError` out of `add_roles` depending on what went wrong (checklist 29), and every one of them means the same thing to the member: the role did not move. The sentence names the permission AND the ordering, because "Manage Roles" alone is the half of the answer that does not fix it. |
| `black_bloc/pings.py:299` | **25 is Discord's cap on options in one select**, taken from `role_menus.py:OPTIONS_MAX` rather than written again here (checklist 15). Page 0 keeps the name `streamers` so the panel staff already posted survives a 26th streamer arriving. |
| `black_bloc/pings.py:306` | The label is the ROLE's name, not the member's, because that is what a member reads on the panel and it survives a nickname change. A row whose role has gone falls back to the member, and `sync_streamer_menus` drops those rows before they ever reach a menu. |
| `black_bloc/pings.py:330` | ⚠️ **Every option is removed before any is added, and that is deliberate twice over.** `add_option` keeps an existing row's `position`, so adding without clearing first leaves the menu in insertion order rather than sorted by label; and a full page of 25 replaced by a different 25 would trip `check_option_count` on the 26th insert. Removing first makes both problems impossible. |
| `black_bloc/pings.py:345` | Trailing menus are un-posted before they are deleted, so a `streamers-2` panel does not sit in a channel pointing at a menu row that no longer exists (`MENU_IS_GONE` is the sentence a click on one would get). |
| `black_bloc/pings.py:362` | ⚠️ **`staff=True` is the ONLY way past `pings_mode off`** — the design's D1 says staff can always do it for anyone, and the mode is a member-facing switch, not a lock on the console. Everything else — `/pings fans on`, the `auto` path on `/twitch link` — goes through the same helper with `staff=False` and gets the words. |
| `black_bloc/pings.py:362` | A row whose Discord role has been deleted by hand is treated as no row at all, so the next ask makes a fresh role and `INSERT OR REPLACE` overwrites it. The alternative — refusing because a row exists — would wedge a streamer whose role somebody tidied away. |
| `black_bloc/pings.py:446` | **D4 `keep` is the default and it says nothing.** `on_streamer_left` returns `None` unless the setting is `delete`, so `/twitch unlink` and `/golive optout` read exactly as they did before this phase for everybody who has not opted into a role. |
| `black_bloc/pings.py:457` | **D1 `auto` is the only setting that makes a role from a link** — and it still asks `pings_mode`, so a server that has not turned pings on cannot accumulate roles nobody can use. |
| `black_bloc/pings.py:489` | ⚠️ **Setup is allowed while `pings_mode` is off, and SAYS SO.** Staff have to be able to make the role and post the panel before anybody can opt in; refusing here would make the feature impossible to prepare. The reply ends with the sentence naming the switch rather than leaving the owner to wonder why nothing happens. |
| `black_bloc/pings.py:489` | **D3: one role, two keys.** `golive_ping_role_id` and `events_ping_role_id` are both pointed at it and both stay separate keys, so the owner can split the feeds later from the Go-live and Events pages without a migration. The panel is created but **never posted** — posting is `/rolemenu post notifications`, which is the path the TEST_MODE guard already refuses correctly. |

## `black_bloc/cogs/content/pings.py` — the two doors

| Key | Note |
|---|---|
| `black_bloc/cogs/content/pings.py:87` | `role.members` is Discord's own view of who wears a role; a role that has gone reads as the WORDS "the role is gone from the server", never 0. A zero would say "nobody follows them", which is a different and wrong fact. |
| `black_bloc/cogs/content/pings.py:106` | ⚠️ **The attribute is `ping`, the command is `pings`.** The module `black_bloc.pings` is imported as `pings` in this file, and a class attribute of that name would make `@pings.command` in the class body and `pings.is_on` in a method body mean two different objects — legal Python, and exactly the trap that costs an hour. `chat_knowledge = Group(name="knowledge", …)` is the same convention. |
| `black_bloc/cogs/content/pings.py:136` | `pages_under_limit` is shared with `/rolemenu showall` and `/help`: a member following 30 streamers would otherwise hit Discord's 2000-character message limit and get nothing. |
| `black_bloc/cogs/content/pings.py:159` | The autocomplete sends a member ID, but somebody can type anything; the display name is accepted as a second try so a typed name still works, and anything else is answered with the sentence that says to pick from the list. |
| `black_bloc/cogs/content/pings.py:331` | ⚠️ **"Is this person a streamer" has two answers, and both count.** A linked Twitch channel is the obvious one; a session Black Bloc has already announced covers the presence-only streamer who never ran `/twitch link` — which, per the design, is most of them. |

## `black_bloc/golive.py` and `cogs/content/golive.py` — what F14 changed

| Key | Note |
|---|---|
| `black_bloc/golive.py:247` | ⚠️ **`ping_prefix` deduplicates, and the order is load-bearing.** One role may legitimately be both the shared Events role and a streamer's own; mentioning it twice would ping everybody twice for one stream. The shared role goes first because that is the one most readers are in. |
| `black_bloc/cogs/content/golive.py:416` | ⚠️ **The fan role is read AFTER the cooldown check and BEFORE `start_session`.** After, so a suppressed announcement never queries and never pings — the test asserts the read count, not the message count. Before, so the id is in the `details` dict the `golive.announce` line carries whether the post lands or not. |
| `black_bloc/cogs/content/golive.py:669` | ⚠️ **`allowed_mentions` is the enforcement; the `<@&…>` text is only the display.** Both roles are listed, deduplicated with `dict.fromkeys`, and everything else stays `False` — a stream title containing `@everyone` still cannot ping (checklist 11). The end-of-stream edit passes the SAME list: an edit does not re-ping, and the guard is there so a mention can never be ADDED by one. |

## `black_bloc/api/tools/pings.py` and the Pings section

| Key | Note |
|---|---|
| `black_bloc/api/tools/pings.py:31` | ⚠️ **`followers` is `null`, never `0`, when the role has gone** — the same honesty rule as `open_counts` on the status route. The page draws a dash for null and the count for a number, so "nobody follows them" and "there is nothing to count" are never the same pixel. |
| `black_bloc/api/tools/pings.py` (the whole router) | Every route calls the `pings.py` helpers with `via=VIA_WEBSITE` and lets THEM write the log line, rather than calling `writes.note()` after the fact. One write, one line, one home — and a helper that refuses mid-way cannot leave a log row saying it worked. |
| `site/public/assets/page-golive.js:195` | The Set-up button sits in the Pings section rather than beside the `golive_ping_role_id` field, and that field is NOT duplicated: the design's "do not duplicate it" note is why the section shows the mode switch and the button and leaves the role picker where it already lived. |
| `site/public/assets/page-golive.js:245` | The section is on the **Go-live** page, not a page of its own — the ownership question ("which surface already owns this?") answers go-live, because a ping role only ever means anything in front of a go-live announcement. `logsSection('pings')` under it is filtered to `pings.*` alone, so the Go-live log above it stays about streams. |
| `site/mock/server.mjs` (the F14 block) | ⚠️ **A role the mock "makes" is remembered on the fan-role ROW, never pushed into the module-level `ROLES`.** `check.mjs` re-seeds `state` between every route, and `ROLES` is not part of `state` — a push there would outlive the reset and make the run order-dependent. Found by reasoning about the reset, not by a failing run. |

# Phase 16 (F3) — YouTube upload announcements

> Keyed against the Phase 16 branch tip (`worktree-agent-a625d41c3dd848a49`, the
> docs commit that carries this section). ⚠️ **Nothing below has met live
> Discord or a live YouTube API key.** The feed facts, on the other hand, ARE
> measured: every "measured 2026-09-02" note in this section came from real
> requests to `youtube.com/feeds/videos.xml` that day, and the feed those
> requests returned is kept verbatim as `tests/fixtures/youtube_feed.xml`.

## What section J actually measured, and what it changed

The design told the builder to measure the feed before writing the cog. Four
findings, three of which moved the design:

| Question | What the design assumed | What was measured |
|---|---|---|
| The entry fields | `yt:videoId`, `published`, `title`, `link href`, `yt:channelId`, `author/name` | **All present**, 15 entries per feed. Design confirmed. |
| ETag / `If-None-Match` → 304 | honoured, so an unchanged feed costs a 304 (D9) | ⚠️ **No `ETag` and no `Last-Modified` are sent at all.** There is no conditional GET to make. |
| Telling a Short apart | needs `YOUTUBE_API_KEY` and `contentDetails.duration` | ✅ **The feed does it for free**: a Short's `link href` is `youtube.com/shorts/<id>`, a video's is `watch?v=<id>`. D5 works keyless. |
| Availability | not considered | ⚠️ **~50%.** 30 requests, 2 s apart, one channel: **15 × 200, 12 × 404, 3 × 500**. |

The 404s are not the channel being wrong. Three other well-known channels took
1, 11 and 24 tries before their first 200, and throughout the whole run
`youtube.com/robots.txt`, the channel page itself and an unrelated Atom feed
answered 200 — while the failures carried YouTube's own
`Server: YouTube RSS Feeds server` header. It is their server, not the network,
and it is intermittent rather than per-channel. This became **KI-12**, and it is
the single fact that shaped the client most: **a 404 must never be read as "that
channel is gone".**

## `black_bloc/youtube.py` — the client and the parsing

| Key | Note |
|---|---|
| `black_bloc/youtube.py:19` | ⚠️ **`FEED_ATTEMPTS = 4` is a measurement, not a guess.** At the measured ~50% success rate, four independent tries miss about 1 sweep in 16; at three it is 1 in 8. Four also stays well inside one poll interval, so a retrying sweep never overlaps the next one. If the rate ever drops below ~25% this number stops being enough — that threshold is written into KI-12 as what would change the design. |
| `black_bloc/youtube.py:22` | A browser-ish `User-Agent` is sent because a default library UA is the classic WAF trip (the estate has been bitten before). ⚠️ Honest note: it was measured NOT to matter here — a bare `python-urllib` UA and a Chrome UA got the same 404/200 mix — so this is belt and braces, not a fix. |
| `black_bloc/youtube.py:37` | ⚠️ **The canonical link is the ONLY reliable way to turn a handle into a channel id**, and the design's suggested `"channelId":"UC…"` scrape is actively wrong. Measured 2026-09-02: on `youtube.com/@Kurzgesagt` that pattern matched **four ids, none of them the channel's own**; on `@GoogleDevelopers` it matched **nothing at all**. `<link rel="canonical">` was correct on both. |
| `black_bloc/youtube.py:44` | The refusal names the exact thing to paste (`youtube.com/channel/UC…`) AND the way a Lead can make handles work. A refusal that says only "I could not resolve that" leaves somebody with no next move. |
| `black_bloc/youtube.py:49` | ⚠️ **The wording deliberately blames the feed, not the member.** After four failures the likeliest explanation by far is KI-12, so the sentence says "usually the feed being flaky rather than the channel being wrong" and tells them to try again. Telling somebody their channel is wrong when it is not is the failure mode this sentence exists to prevent. |
| `black_bloc/youtube.py:80` | **`kind_of` reads the URL path, and that is the whole Shorts implementation.** Measured 2026-09-02 across two real feeds: every Short linked as `/shorts/<id>`, every full video as `watch?v=<id>` (Kurzgesagt 13/2, MrBeast 9/6). This is what lets D5 ship without an API key, which the design did not expect. |
| `black_bloc/youtube.py:91` | ⚠️ **The channel id comes from the ENTRY, never from the feed element.** Measured: the feed-level `<yt:channelId>` reads `sXVk37bltHxD1rDPwtNM8Q` — the `UC` prefix is **missing** — while every entry's carries the full `UCsXVk37bltHxD1rDPwtNM8Q`. Storing the feed-level one would write an id that can never be fetched again. `tests/test_youtube.py:66` pins it, fixture and all. |
| `black_bloc/youtube.py:91` | An entry with no `yt:videoId` is dropped rather than stored with a blank primary key; an entry with no `link` falls back to the `watch?v=` address, which then reads as a full video rather than a Short. Both are tolerance for a field the feed is not contractually obliged to send. |
| `black_bloc/youtube.py:108` | Unreadable XML raises `YouTubeError` with the parser's own message, while an EMPTY body is simply no videos. The difference matters: empty is what a 304 or a truncated response looks like and must not wake the degraded line; malformed is a real fault worth a sentence. |
| `black_bloc/youtube.py:142` | `classify_row` puts **live before short** because a broadcast can be under a minute and would otherwise be filed as a Short and silently skipped — the same silence, but for the wrong reason and with a misleading log line. A row whose length cannot be read is `unknown`, never guessed (checklist 10). |
| `black_bloc/youtube.py:175` | `render` mirrors `golive.py:render` exactly, including the broken-template fallback (checklist 17) and `ping_prefix` for the mentions, and it IMPORTS both from `golive.py` rather than copying them (checklist 15). An empty title reads "a new video" so the default template can never post `****`. |
| `black_bloc/youtube.py:235` | ⚠️ **`fetch_feed` still takes and returns an ETag even though YouTube sends none.** Measured: no `ETag`, no `Last-Modified`, only `Cache-Control: public, max-age=900`. The parameter is kept because the column, the store helper and the 304 branch are all cheap and already correct — if YouTube ever adds the header the feature starts saving bandwidth with no code change. Today `etag` is always `NULL` and the 304 branch is exercised only by `tests/test_youtube.py`. |
| `black_bloc/youtube.py:235` | The retry loop returns on 200 **or** 304 and otherwise keeps trying; it does not distinguish 404 from 500, because measurement says both mean the same thing here — try again. The status codes seen are logged as a list on the final failure, so a genuinely dead channel is diagnosable from one log line. |
| `black_bloc/youtube.py:273` | `resolve` answers a bare id or a `/channel/` address **without any network call at all** — checked by `tests/test_youtube.py`, which asserts no request was made. That is the path `/uploads link-for` and the dashboard take most often, and it costs nothing and cannot flake. |
| `black_bloc/youtube.py:286` | With a key, `forHandle` is tried, then `forUsername`, and **then it still falls back to the page scrape**. Google's own API does not resolve every legacy `/c/` name, so the keyless path is not dead code once a key exists — it is the last rung of the same ladder. |
| `black_bloc/youtube.py:310` | ⚠️ **A title that cannot be fetched is blank, and the link still succeeds.** The channel id is the thing that matters; the title is decoration. Refusing a link because the flaky feed would not hand over a display name would make KI-12 into a user-facing failure for no gain. |
| `black_bloc/youtube.py:322` | Without a key `classify` returns `{}` — it never claims a kind it did not check (checklist 10). The caller then keeps whatever the URL path said, which is right for Shorts and admits ignorance about live streams. |

## `black_bloc/cogs/content/youtube.py` — the sweep

| Key | Note |
|---|---|
| `black_bloc/cogs/content/youtube.py:141` | ⚠️ **`record_video` returns whether the row was NEW, and that boolean is the duplicate guard.** `INSERT OR IGNORE` on a `video_id` primary key means two overlapping sweeps cannot both announce: the second gets `rowcount == 0` and stops. This is the read-decide-write race of checklist 6 solved with the uniqueness constraint rather than a lock, because the key is already unique and global. |
| `black_bloc/cogs/content/youtube.py:160` | ⚠️ **The mode travels WITH the timestamp** (checklist 3 / "a status flag travels atomically with its value"). `announced_at`, `announced_message_id` and `mode` are set in one statement, so a later `/uploads mode on` cannot make a shadow-era row look like a real post. The dashboard's `state` column reads that stored mode, never the current one. |
| `black_bloc/cogs/content/youtube.py:253` | Checklist 28: discord.py re-raises a non-HTTP exception out of a loop and the loop then stays stopped for the life of the process. The handler records the error where `/uploads list` and the Health tab can see it, and restarts. |
| `black_bloc/cogs/content/youtube.py:260` | ⚠️ **The poll gap is re-read every tick, not frozen at boot.** `youtube_poll_minutes` is a settings key, so a change from the dashboard has to take effect without a redeploy; `change_interval` is only called when the number actually differs, because calling it every tick would reset the timer and the sweep would never fire. |
| `black_bloc/cogs/content/youtube.py:266` | The gap is read from the first guild the bot is in. Black Bloc is a single-server bot; this is the one place that assumption is written down rather than being implicit, and the fallback of 10 matches the registry default so the two can never silently disagree. |
| `black_bloc/cogs/content/youtube.py:272` | ⚠️ **A sweep counts as failed only when EVERY channel failed.** With KI-12's ~50% rate, one channel out of five failing is the normal state of the world, and letting that trip the degraded line would make the line meaningless within an hour. One log line per outage, at the point the sweep genuinely stops being trustworthy (mirrors `golive.py:_poll_failed`). |
| `black_bloc/cogs/content/youtube.py:295` | Two members who link the same channel: the first is polled, the second is warned about in the log and skipped — the same shape as the Twitch poller's double-link warning. The `/youtube link` command refuses this outright (checklist 16); `_pollable` is the belt for a row that got in some other way. |
| `black_bloc/cogs/content/youtube.py:311` | The ETag is written back on every 200 so a future YouTube that does send one starts working immediately; on the SEED path `mark_seeded` writes it instead, in the same statement that sets `seeded = 1`. |
| `black_bloc/cogs/content/youtube.py:332` | ⚠️ **D10 lives here: linking counts everything already published as history.** Every current entry is stored with `announced_at` NULL and `mode` NULL, and `seeded` flips to 1. Without this the first sweep after a link would announce fifteen old videos at once, which is the single most obvious way this feature could embarrass somebody. |
| `black_bloc/cogs/content/youtube.py:352` | A classification failure is not a sweep failure: the URL-derived kind stands and the video is still announced. Losing the ability to spot a live stream is KI-11's known gap, not a reason to go silent. |
| `black_bloc/cogs/content/youtube.py:364` | ⚠️ **The row is written BEFORE the post, deliberately.** A crash between the two loses ONE announcement; the other order would repeat one on every restart. The design says it and `tests/cogs/content/test_youtube.py:324` proves it by exploding the channel's `send` and asserting the row exists with `announced_at` still NULL. |
| `black_bloc/cogs/content/youtube.py:364` | A member the bot cannot see anywhere still gets the video RECORDED. Storing it means they are not announced retroactively the moment they rejoin — the alternative is a burst of old uploads the day somebody comes back. |
| `black_bloc/cogs/content/youtube.py:393` | ⚠️ **D6's fallback, and the one place KI-11 lives in code.** With a key, whatever `videos.list` said is final. Without one, the only live signal available is the member's own go-live session — and it must be a session on platform `youtube` specifically, or a member streaming on Twitch would have their YouTube uploads silently swallowed. `tests/cogs/content/test_youtube.py:483` pins both halves. |
| `black_bloc/cogs/content/youtube.py:414` | ⚠️ **`off` returns before anything is rendered or logged.** Not even a `would_` line: `off` means the feature is not running, and a log full of would-lines from a feature nobody turned on is noise that hides the real ones. `shadow` is the mode that writes them. |
| `black_bloc/cogs/content/youtube.py:414` | Checklist 2: a test-mode refusal is `youtube.would_announce` with `reason=test_mode`, and a genuine failure is `youtube.post_failed`. They must be distinguishable at a glance, because "read the log" is the owner's verification method. |
| `black_bloc/cogs/content/youtube.py:477` | **The ONE guarded path.** Every upload post goes through here, and the `bot.guard` check happens before the channel is even fetched. Nothing else in this cog calls `channel.send`. |
| `black_bloc/cogs/content/youtube.py:500` | Checklist 11: `allowed_mentions` is the enforcement and the `<@&…>` text is only the display. A YouTube video TITLE is attacker-controlled text going straight into a Discord message, so `everyone=False` here is what stops a video called `@everyone free nitro` from pinging the server. `tests/cogs/content/test_youtube.py` asserts it with a display name of `@everyone` too. |
| `black_bloc/cogs/content/youtube.py:512` | ⚠️ **D3 in one line: blank `youtube_channel_id` means the go-live channel.** The owner sets ONE place up and both feeds use it; pointing uploads elsewhere is opting out, not opting in. It also means the feature inherits whatever the owner already proved works for go-live. |
| `black_bloc/cogs/content/youtube.py:522` | A member asking `/youtube status` must never see a bare "no". The three reasons uploads are not being posted — the mode, no channel, or it is genuinely on — read as three different sentences, and the mode one names the mode so they can quote it to a Lead. |
| `black_bloc/cogs/content/youtube.py:597` | ⚠️ **`link_and_seed` is public because the WEB calls it too.** `/youtube link`, `/uploads link-for` and `POST /api/youtube/links` all land here, so the seed can never be done one way in Discord and another way on the site (checklist 15). |
| `black_bloc/cogs/content/youtube.py:597` | ⚠️ **A link SUCCEEDS even when the feed will not answer** — it returns 0 seeded and the poller seeds later. Refusing the link would hand KI-12 straight to the member as "your channel does not work". The reply and the API message both say the count, and the dashboard shows a "not yet" badge, so nobody is told a check happened that did not (checklist 10). |
| `black_bloc/cogs/content/youtube.py:640` | `/youtube status` defers to `_where_words` rather than printing the mode raw, and says what happens to Shorts, because those are the two things a streamer actually wants to know and both are settings they cannot see. |
| `black_bloc/cogs/content/youtube.py:850` | `_handle_of` stores the handle ONLY when the input was a handle — a `UC…` id is not a handle, and writing one into that column would make the dashboard show a fake `@name`. |

## `black_bloc/api/tools/youtube.py` and the Go-live page

| Key | Note |
|---|---|
| `black_bloc/api/tools/youtube.py:85` | ⚠️ **`state_of` reads the mode STORED on the row.** A row announced during a shadow week still reads `would` after the owner flips to `on`, which is the honest answer to "what happened to this video". Deriving it from the current mode would silently rewrite history every flip. |
| `black_bloc/api/tools/youtube.py:101` | The GET carries `seeded` and `last_video` per row so the page can say "the feed has not answered for this one yet" — the API's job is to make KI-12 visible rather than to smooth it over. |
| `black_bloc/api/tools/youtube.py:109` | The POST does its own resolve and its own owner check so it can raise its own worded 400/409, then calls the COG for the store-and-seed. One implementation of the rule, two sets of words for two audiences. |
| `black_bloc/api/tools/youtube.py:162` | `unchanged_ratio` is `null`, not `0`, before any fetch has happened — the same honesty rule as `open_counts` and `followers`. "Nothing was unchanged" and "nothing has been counted" are different facts and must never render as the same number. |
| `black_bloc/api/tools/youtube.py:162` | The status route answers even with the cog unloaded (`api_key_set: false`, `running: false`) rather than 503ing, because a Health-style panel that vanishes when the thing it monitors is down is the opposite of useful. The WRITE route does refuse in that state, through `require_cog`. |
| `site/public/assets/page-golive.js:317` | The three states get three tones — announced `ok`, would `warn`, skipped `quiet` — so the table answers "did this actually go out?" at a glance without reading the Mode column. |
| `site/public/assets/page-golive.js:320` | The sweep card is FIRST in the section, above the tables. With KI-12 in play, "is the sweep even working" is the question that decides how to read everything below it; the missing-API-key sentence sits here too, worded as a fact rather than a warning because unset is the intended state. |
| `site/public/assets/page-golive.js:370` | ⚠️ **The section is on the Go-live page, not a page of its own** — the same ownership answer as F14's Pings section. Uploads share the go-live channel by default, share the fan roles, and are about the same people; a second page would split one question across two surfaces. |

## Where else Phase 16 touched

| Key | Note |
|---|---|
| `black_bloc/storage/db.py:138` | Schema **22**, additive only. `youtube_videos.video_id` is the primary key rather than an autoincrement id — the duplicate guard in `record_video` depends on it, and a YouTube video id is already globally unique. |
| `black_bloc/settings_store.py:42` | The default template lives beside `GOLIVE_TEMPLATE` so `youtube.py` can import it rather than declaring a second copy (checklist 15) — the same reason `golive.py` imports its own. |
| `black_bloc/settings_store.py:44` | ⚠️ **The floor of 5 minutes has a measured reason, and the validator says it.** The feed answers `Cache-Control: public, max-age=900` — 15 minutes — so polling faster than the cache cannot find anything sooner. The refusal explains that rather than just naming a number. |
| `black_bloc/cogs/core.py:35` | ⚠️ **`/settings clear` moved from static choices to an autocomplete, and Phase 16 is why.** Two new channel/role keys took `CLEARABLE_KEYS` to **27**, past Discord's hard ceiling of 25 choices on a command option. The alternatives were dropping keys from the menu — which would break checklist 33, since a key you cannot clear from Discord is not configurable both ways — or doing what `/settings set-value` already did. `tests/cogs/test_core.py` asserts the list is now over 25, so the reason cannot be forgotten. |
| `black_bloc/cogs/core.py:287` | The autocomplete filters on a substring of the KEY, so typing `youtube` narrows to the namespace. The 25-item slice is Discord's response cap, not a design choice. |
| `tests/test_youtube.py:74` | This test is the record that Shorts detection is free. If YouTube ever stops using `/shorts/` links it fails here first, loudly, against the captured fixture — and the fix is then `YOUTUBE_API_KEY` plus `classify`, which is already written and already tested. |
| `tests/test_youtube.py:225` | ⚠️ **A test that asserts an ABSENCE.** It exists so that a future session reading `fetch_feed`'s etag plumbing does not "fix" the missing 304 handling — the measurement, not the code, is what says YouTube sends no validator. |
...
| `black_bloc/command_visibility.py:16` | ⚠️ **`/memory` is HIDDEN while `chat_memory_mode` is off**, the same way `/rolemenu` and `/request` follow their own modes. The feature ships off, so at deploy the command is not in the tree at all — a member never meets a command whose only answer is "that is turned off". `MEMORY_IS_OFF` still exists for the window between a mode flip and the next command sync. |
...
| Key | Note |
|---|---|
| `black_bloc/requests.py:23` | ⚠️ **`TRANSITIONS` is the one table, and every path asks it** — `moves_from`, `can_move`, `checked_move`, the API's `moves` field and the page's buttons all read this and nothing else. `STAFF_STATUSES` and `FINAL_STATUSES` are DERIVED from it rather than written out again, so adding a state to the table is the whole change. |
| `black_bloc/requests.py:333` | A refused move says where it IS, where it cannot go, and where it CAN go, by name. `moves_sentence` is what makes the last part true from the table rather than from a hand-written list that would drift. |
| `black_bloc/requests.py:359` | `checked_move` is the whole gate in one place: same-state, illegal move, and reason-required. The cog, the API's `/status`, `/hold` and `/decline` all go through it, so a rule cannot be enforced on one surface and not the other. |
| `black_bloc/requests.py:730` | ⚠️ **`held_from` is written on the way INTO hold and cleared on the way out, in the same UPDATE.** Two statements would leave a window where a resumed request still claims to be parked. `decline_reason` carries the HOLD reason as well as the decline one — the column name is a persisted key and changing it would be a migration, so the note is here instead. (Third pass: this is now a dict-built statement covering five per-move columns — see the `Requests — third pass` section.) |
| `black_bloc/requests.py:844` | `resume_target` falls back to `in_progress` when `held_from` is `open`: `open → hold → open` would be a loop with no work in it, and the owner's machine has resume meaning "pick it back up". |
| `black_bloc/requests.py:469` | `row_value` exists so a schema-22 row read before the `held_from` column is added reads as nothing rather than raising. `sqlite3.Row` throws `IndexError` on an unknown column, not `None`. (It does the same job again for schema 26's four columns.) |
| `black_bloc/requests.py:434` | `status_channel_id` is the fallback rule in one place: `request_status_channel_id` if set, otherwise `request_notify_channel_id`, so one channel carries both kinds of line by default and nobody has to set two keys to get any output. |
| `black_bloc/requests.py:180` | ⚠️ **REWRITTEN by the third pass, not renumbered: the four `NOTIFY_*` constants this note described are GONE.** The channel message is an embed now, and `MOVE_LINE` is the seven-look plain-text table that survives only for the server log. The owner's original point still holds and is why it is a table of seven fixed shapes rather than one templated setting: the shapes differ, so a single `request_status_template` would be worse. |

## `black_bloc/cogs/community/requests.py` and the API

| Key | Note |
|---|---|
| `black_bloc/cogs/community/requests.py:194` | `post_line` is the shared guarded post. A channel the guard refuses logs `request.notify_skipped_test_mode` and returns `None` — silence and refusal must be distinguishable in the log (checklist 2), and neither may raise into the move that already happened. |
| `black_bloc/cogs/community/requests.py:284` | ⚠️ **The state moves FIRST, then the DM, then the channel card.** Checklist 12: the irreversible thing goes first, and a failed cosmetic must not abort it. Both notifications are best-effort and both log their own failure. |
| `black_bloc/cogs/community/requests.py:400` | `resume_request` is its own path rather than a `set` with a computed target, because Resume is a BUTTON with no status to choose — the target comes off the row, and the log kind (`request.resumed`) is the one that says a human pressed resume rather than picked a state. |
| `black_bloc/api/tools/requests.py:138` | The row carries `moves` and `resume_to` so the page can draw only the controls that would be accepted. The page never re-derives the machine; it renders what the API says is legal. |
| `black_bloc/api/tools/requests.py:295` | `_may_file` no longer returns anything. `request_auto_approve_staff` is gone and every filing arrives `open` — owner, 2026-09-02: *"Even a staff request can be bad"*. |
| `site/public/assets/page-requests.js:121` | `MOVES` is a label/tone/prompt table keyed by the state NAME, so `moveButtons` can build a bar straight from `row.moves` — a control that would be refused is never drawn, and adding a state to the machine needs one entry here. ⚠️ The third pass DELETED its `done` entry: `done` is only reachable from `review`, and that move is the review card's own Accept button. |
| `site/public/assets/page-requests.js:717` | The hold card drops the resume target from its move list, so Resume and "Pick it up" are not two buttons doing the same thing. |

## Where else Phase 17 touched

| Key | Note |
|---|---|
| `black_bloc/storage/db.py:11` | Schema **23**: two new tables, `requests.held_from`, and the ONE data migration — `pending`/`approved`/`planned` → `open`. It is idempotent because after the first run no row matches; `tests/storage/test_db.py:862` boots the file three times to prove it. |
| `black_bloc/storage/db.py:746` | The migration runs AFTER `_add_missing_columns`, so a schema-22 file has grown `held_from` before any row is touched. |
...
| `black_bloc/logkinds.py:416` | ⚠️ **There is NO `chat_memory_log_level`, and the design's §E table is wrong to list one.** Log levels are per FEATURE and derived from the kind's dotted head; the memory kinds are `chat.memory_*`, so their head is `chat` and `chat_log_level` already governs them. A key that governed nothing would be worse than none. Listed in the design's Deviations. |
...
# Phase 18 (F19) — raid trains

> Keyed against the Phase 18 branch tip (`worktree-agent-a1c38e3df0f71fa7e`, the
> docs commit that carries this section). ⚠️ **Nothing below has met live
> Discord.** No lineup has been posted, no reminder DM has ever arrived, and no
> member has claimed an hour. Everything here is test-suite evidence.

## What section J actually measured, and what it changed

The design told the builder to read `events.py`'s scheduled-event helper FIRST
and only ship `raidtrain_scheduled_event` on if it was reusable **without
editing `events.py`** (which is read-only in a parallel build).

**Finding: it is importable, and it is still not reusable.**
`cogs/community/events.py:570 create_scheduled_event(bot, guild, row)` is a
module-level function, so `from ..community.events import create_scheduled_event`
would compile. It is disqualified by what it does, not by where it lives:

1. It ends with `set_scheduled(bot.db, row["id"], made.id)`, which runs
   `UPDATE events SET scheduled_event_id = ? WHERE id = ?`. Passing it a raid
   train would stamp a Discord event id onto **whatever `events` row happens to
   share that id** — silent cross-table corruption, and exactly the class of bug
   this file exists to stop somebody re-introducing.
2. It gates on the `events_create_scheduled` setting, so raid trains would
   inherit the events feature's switch and D13's own key would be dead.
3. Every line it logs is `event.*`, so a raid train's calendar entry would never
   appear under `/raidtrains logs` or the Raid trains section's log filter.

**So D13 ships `raidtrain_scheduled_event = false`**, per §J's instruction, and
the key is NOT a dead switch: `_maybe_scheduled_event` is twenty lines of this
cog's own that call `guild.create_scheduled_event` directly, with raid-train log
kinds and no writes outside `raid_trains`. `find_scheduled_event` — the one
genuinely reusable piece, a pure cache-then-fetch lookup with no table in it —
IS imported from `events.py` for the cancel path (checklist 15).

⚠️ A second reason the default would be academic today: `create_scheduled_event`
refuses outright while `bot.guard` exists, and so does ours. Under TEST_MODE no
scheduled event can be made at all, whichever way the key is set.

## `black_bloc/raidtrain.py` — the pure half

| Key | Note |
|---|---|
| `black_bloc/raidtrain.py:18` | ⚠️ **`SLOT_COUNT_MAX = 24` is a Discord limit, not a taste.** A slot line renders at roughly 85 characters (`#12 <t:…:t>–<t:…:t> — <@700000000000000002> (https://twitch.tv/somelogin)`), and a message is capped at 2000, so 24 lines plus the header is the last size that certainly fits in ONE message. It has to be one message because the lineup is edited in place rather than reposted. r3dlabs runs 50-slot trains; ours refuses in words and says why. |
| `black_bloc/raidtrain.py:20` | `REMINDER_LATE_MINUTES` is checklist 31: a due-row sweep must age rows out. A slot whose start is more than this far past does not get a late DM telling somebody to go live for an hour that has gone — `missed_reminders` stamps it instead, so it also stops being carried by every future sweep. |
| `black_bloc/raidtrain.py:31` | The transition table is the ONE place a status move is decided; `may_move` and `allowed_moves` are both derived from it, and `allowed_moves` exists so a refusal can say what IS possible in words rather than printing a status list at somebody. `cancelled` is reachable only from `open`/`locked` — the design's rule, so a train that has already started is finished rather than un-run. |
| `black_bloc/raidtrain.py:83` | `_value` reads a sqlite `Row`, a plain dict or an object with attributes. That is what keeps every helper in this file testable with dict fixtures while the cog hands it real rows — the same trick `render_lineup` needs to be a pure function. |
| `black_bloc/raidtrain.py:95` | ⚠️ **Times belong to the POSITION, never to the person.** Slots are created eagerly at `create` with their times already in them, so `swap` moves five holder columns and touches no timestamp. A design that stored times on the claim would have to rewrite them on every reorder, and a half-finished reorder would leave a lineup with two 8pm slots. |
| `black_bloc/raidtrain.py:106` | `ends_at` prefers the SLOT rows' own `ends_at` over `starts_at + minutes × count`. The arithmetic is the fallback for a train whose rows are unreadable; the rows are the truth, because they are what a swap and a future per-slot length would move. |
| `black_bloc/raidtrain.py:147` | `caps_ok` treats **0 as no ceiling**, which is why the settings validator lets the key go to 0 and its max-reason sentence says so. An organizer's `assign` never calls this — D10 says the cap is for self-service claims only. |
| `black_bloc/raidtrain.py:189` | A holder is written `<@id>`, and every send/edit of this text goes out under `AllowedMentions(users=False)` (see the cog's `_mentions`). The mention RENDERS as their name and pings nobody — which matters because the lineup is edited on every claim and a pinging edit would notify twenty people twenty times. Checklist 11. |
| `black_bloc/raidtrain.py:210` | ⚠️ **`render_lineup` is the whole post, and it is re-rendered from the rows every time rather than patched.** One function means the Discord post, `/raidtrain status` and the dashboard's `lineup` field can never disagree about what the lineup says. |
| `black_bloc/raidtrain.py:252` | The reminder names BOTH neighbours with their twitch.tv links, because the thing a slot holder actually needs at T-30 is not "your slot is soon" — it is who to watch for the incoming raid and who to send the audience to. That is the request's own wording ("who they will be raided by and the person they raid next"). |
| `black_bloc/raidtrain.py:278` | ⚠️ **`due_reminders` deliberately excludes a slot whose start has already gone by** (`start <= floor`), and `missed_reminders` picks exactly those up. They are two functions rather than one flag because the two outcomes are different actions — send, versus stamp and stay quiet — and the cog must be able to do the second without the first. |
| `black_bloc/raidtrain.py:314` | `due_checkins` takes the live user ids as an ARGUMENT rather than reaching for `golive.open_sessions` itself. That keeps this module Discord-free and database-free, and it is what makes the check-in window testable without a session table. |

## `black_bloc/cogs/content/raidtrain.py` — the cog

| Key | Note |
|---|---|
| `black_bloc/cogs/content/raidtrain.py:146` | `create_train` writes the train AND every slot in one commit. Slots-on-create is what makes `position` meaningful from the first second: `/raidtrain list` can say which hours are free before anybody has claimed anything, which is half of what the request asked for ("slot availability"). |
| `black_bloc/cogs/content/raidtrain.py:228` | ⚠️ **`take_slot`'s `WHERE id = ? AND user_id IS NULL` is the race guard, and `rowcount` is how the caller learns it lost.** Checklist 6 wants a lock AND a database constraint; the lock is `_lock(train_id)`, and this conditional UPDATE is the constraint — a partial unique index could not be used here because the per-member ceiling is a SETTING (0 = unlimited), and a schema cannot encode a value the owner may change. |
| `black_bloc/cogs/content/raidtrain.py:250` | `swap_holders` moves five columns and no time (see `raidtrain.py:95`). `reminded_at` travels WITH the person, so somebody already reminded for 8pm who is moved to 9pm is not reminded again — the alternative, clearing it, would DM them twice about one train. |
| `black_bloc/cogs/content/raidtrain.py:290` | `stamp_slot` interpolates a column name into SQL, so it validates against a three-item allowlist first and raises otherwise. The allowlist form is deliberate (default-deny, per the estate's export rule); an exclusion list would leak the moment a column was added. |
| `black_bloc/cogs/content/raidtrain.py:335` | The DM helper is this cog's own rather than an import from `cogs/community/requests.py`, which was being rewritten in parallel by Phase 17. It is six lines and it catches everything, because a member with DMs closed is a normal state, not an error — the CALLER turns that into a `raidtrain.dm_failed` line so silence is never silent. |
| `black_bloc/cogs/content/raidtrain.py:409` | One `asyncio.Lock` per train. The contended path is two members claiming the same open slot in the same second, and the second half of that guard is the conditional UPDATE above. The sweep takes the same lock, so a claim cannot land between "read the slots" and "promote the train to live". |
| `black_bloc/cogs/content/raidtrain.py:436` | Checklist 28: discord.py re-raises a non-HTTP exception out of a `tasks.loop` and the loop then stays dead for the life of the process. `@sweep.error` restarts it and records the failure where `/api/raidtrains/status` and the Health tab can see it. |
| `black_bloc/cogs/content/raidtrain.py:442` | `_retime` re-reads `raidtrain_poll_minutes` every tick instead of freezing the boot value, so changing the gap on the dashboard takes effect without a restart. Same shape as the YouTube poller. |
| `black_bloc/cogs/content/raidtrain.py:477` | ⚠️ **Checklist 32: `guild.unavailable` is skipped.** An outage that hides a guild must not be read as "this train has no slots" — nothing is promoted, nothing is finished, nothing is stamped for a guild Discord is not currently telling us about. |
| `black_bloc/cogs/content/raidtrain.py:493` | The clock does all three moves and the poller owns all of them: auto-lock at the start (D11 — fixed, deliberately not a key: a train that keeps taking sign-ups while it runs cannot have a raid order), `→ live`, then `→ done` once the last slot's own `ends_at` has passed. |
| `black_bloc/cogs/content/raidtrain.py:521` | ⚠️ **`reminded_at` is stamped BEFORE the DM is attempted, and that order is the whole point.** A crash between the two loses ONE reminder; the other order would re-send on every sweep for as long as the failure lasted. The same rule as `youtube.record_video` returning False for a duplicate. |
| `black_bloc/cogs/content/raidtrain.py:538` | Checklist 2: `raidtrain.remind` (sent), `raidtrain.would_remind` (shadow) and `raidtrain.dm_failed` (tried and refused) are three different kinds, because the owner's verification method is reading the log and those three must be distinguishable at a glance. |
| `black_bloc/cogs/content/raidtrain.py:572` | Check-in reads `golive.open_sessions` — the one signal, not a second detector. It is also why **KI-16** exists: a holder whose presence is hidden and who is not Twitch-linked is never seen. Accepted, same limit as F1. |
| `black_bloc/cogs/content/raidtrain.py:617` | D7: blank `raidtrain_channel_id` borrows `events_announce_channel_id`, so a server sets up ONE place rather than two. Same pattern as `youtube_channel_id` borrowing `golive_channel_id`. |
| `black_bloc/cogs/content/raidtrain.py:627` | ⚠️ **The only role that may be mentioned is the configured ping role.** `users=False` is what makes the in-place edits safe (see `raidtrain.py:189`), and `everyone=False` is what stops a train titled `@everyone` from doing anything. Checklist 11. |
| `black_bloc/cogs/content/raidtrain.py:643` | The one guarded path to a channel. A channel TEST_MODE refuses is logged as `raidtrain.post_skipped_test_mode` and returns None — never raised, so a refused post can never abort the state change that preceded it (checklist 12: the important thing first, cosmetics last). `no_channel_configured` and `channel_not_visible` are separate named reasons because the fixes differ. |
| `black_bloc/cogs/content/raidtrain.py:720` | The thread is opened on the message that was actually posted, and the guard is told it owns the thread (`own_channel`) so the "train moves" lines may be sent into it while test mode is on — the same allowance temp voice uses for the rooms it makes itself. |
| `black_bloc/cogs/content/raidtrain.py:746` | See §J above: this is the raid-train-owned scheduled-event creator. It refuses under TEST_MODE with `raidtrain.would_create_scheduled` (checklist 1: a side effect the HTTP guard cannot see checks `bot.guard` itself), and it never writes outside `raid_trains`. |
| `black_bloc/cogs/content/raidtrain.py:817` | ⚠️ **The lineup is EDITED, never reposted.** One message is the whole public record of a train, so a member who bookmarked it keeps a live view; a repost would leave a trail of stale lineups and would ping the ping role again each time. A failed edit is a logged `raidtrain.post_failed`, not an exception — the claim that caused it has already been committed. |
| `black_bloc/cogs/content/raidtrain.py:858` | ⚠️ **Mode `off` refuses EVERY member command in words**, naming `/raidtrains mode` — not a hidden command and not a dead one. The staff group deliberately keeps working while off, because `mode` is how you turn it on. |
| `black_bloc/cogs/content/raidtrain.py:877` | D2: organizer = the configured role OR staff. It is not `default_permissions`, because Discord's permission lock cannot express "or this role"; the gate is at runtime and the refusal NAMES who may do it. |
| `black_bloc/cogs/content/raidtrain.py:988` | ⚠️ **Every command that ends up editing the lineup DEFERS first, and every refusal that can be decided without touching Discord answers BEFORE the defer.** Checklist 8 and 24: `_refresh_lineup` fetches and edits a message, message edits are rate-limited, and discord.py sleeps through a 429 — so the 3-second interaction window would expire and the member would see "The application did not respond". `tests/cogs/content/test_raidtrain.py` asserts `response.deferred` on all seven, so removing a defer fails by name. |
| `black_bloc/cogs/content/raidtrain.py:1007` | The order of the refusals is the order they cost: the train's status, then the per-member ceiling, then whether the slot exists, then the race. Each returns a sentence, and the last one is the only one that can be reached by two people at once. |
| `black_bloc/cogs/content/raidtrain.py:1127` | `submit_train` reuses `events.py`'s `start_error` and `START_IN_THE_PAST` rather than writing a second pair of date sentences (checklist 15). The zone is the member's own `/timezone`, so nothing new about time was invented for this phase (D15). |
| `black_bloc/cogs/content/raidtrain.py:1190` | `_numbers` refuses out-of-range sizes BEFORE a row is written, and its sentence names Discord's one-message limit as the reason rather than just quoting a number. |

## `black_bloc/api/tools/raidtrain.py` and the Events page

| Key | Note |
|---|---|
| `black_bloc/api/tools/raidtrain.py:103` | Every id leaves as a STRING. A Discord snowflake is bigger than a JavaScript safe integer, and a number that quietly loses its last digits on the page is the worst kind of bug — it looks like it worked. |
| `black_bloc/api/tools/raidtrain.py:145` | ⚠️ **`/status` is declared BEFORE `/{train_id}`.** FastAPI takes the first route that matches and `{train_id}` is typed `int`, so the other order would answer `/api/raidtrains/status` with a 422 about a path parameter — a bare status code in front of a person, which the estate rule forbids. The mock's matcher has the same first-match rule and the same declaration order. |
| `black_bloc/api/tools/raidtrain.py:189` | The create route does its own worded 400s and then calls the COG for the lineup post, so one implementation of "post the lineup" serves both surfaces and each audience gets its own words. Same split as `/api/youtube/links`. |
| `black_bloc/api/tools/raidtrain.py:254` | ⚠️ **One route fills and empties a seat**, keyed on `member_id` being null. Two routes would be two places to forget the Twitch-link check, and the page's slot grid has one control per row either way. |
| `site/public/assets/page-events.js:180` | A control nobody may use is NOT RENDERED: "Take off" only appears on a taken slot, "Put somebody in" only on an open one, and neither appears at all once `editable` is false. The estate rule prefers a missing control to one that refuses. |
| `site/public/assets/page-events.js:280` | ⚠️ **Raid trains are a SECTION on the Events page, not a page of their own** — the Events page already owns "what is scheduled", and a second surface answering the same question is the duplicate-surface bug the docs standard names. The mode warning sits at the top of the section, above the table, for the same reason the YouTube sweep card does: it decides how to read everything below it. |
...
| `black_bloc/cogs/community/role_menus.py:1540` | The reconciler is `RoleMenus.on_member_update`, and it filters with `grants.was_ours(...)` — one entry per remembered change, expiring after ~30 s. `tests/cogs/community/test_applications.py::test_a_role_this_cog_adds_is_never_reported_as_a_change_made_by_hand` runs the approve path and then this listener and asserts NO `role.changed_by_hand` line; the test beside it does the same edit WITHOUT the ledger and asserts the line is there, so the proof cannot pass by accident. |
...
| `black_bloc/applications.py:create_application` | ⚠️ **The `open_application` check is the polite answer; the partial unique index is the guard.** Two clicks a millisecond apart both pass the read, and the second INSERT raises `sqlite3.IntegrityError` — caught here and answered as "you already have one waiting" rather than reaching the member as a broken modal. Checklist item 6. `tests/test_applications.py::test_the_unique_index_catches_a_second_application_the_check_did_not_see` stubs the read out so the INSERT really is the thing that loses; it fails if the `except` is removed. |
...
| `black_bloc/cogs/community/applications.py:nudge_mentions` | ⚠️ **The decided card pings EXACTLY one person: the `owner_user_id` the form names.** Checklist item 11 says allow only the configured mention, and D8's whole point is that somebody is told to go and click the twitch.tv invite — a card that renders `@owner` without notifying them would look like it did its job and quietly not have. With no owner set it is `AllowedMentions.none()`. |
| `black_bloc/command_visibility.py:16` | ⚠️ **`applications_mode: ("apply",)` — the MEMBER group is hidden while the feature is off, the staff group is not.** Otherwise `/apply` sits in the picker refusing everybody, which the process rule calls the worse half of "prefer not rendering a control someone cannot use". `/applications` stays, because a Lead has to be able to turn it back on from Discord. Side effect: `applications_mode` ships **off**, so every boot now spends KI-2's second guild sync — already an accepted cost there. |
...
| `site/public/assets/page-rolemenus.js:applicationsSection` | The section's DOM id is **`sect-applications`** (`ui.js:section` adds the prefix), so the review link is `/rolemenus.html#sect-applications`. Rendered against the mock on 2026-09-02: the off/shadow/on switch, two pending cards with their answers, the decided table, both forms with their question counts, an Apply-button card each and the settings panel — no console errors, and the Edit button opens the editor with the form's three questions in it. |
...
## One web write, one log row (owner report, 2026-09-03)

> Keyed against `df393ab` (the `fix/web-write-double-log` merge), then re-keyed by
> the requests third pass to `feat/requests-third-pass`. ⚠️ The original header
> line here said "commit **HEADSHA**" — a placeholder that was never filled in;
> `df393ab` is the merge and `19d2f7d` is the docs commit that carries this
> section. Owner: *"The app double posted all messages with a web.request and a
> request"*.

| Key | Note |
|---|---|
| `black_bloc/logkinds.py:357` | ⚠️ **`kind_via` is `bare()` read backwards, and the ONLY place the `web.` head is put on.** Six modules used to build `f"{WEB}." if via == VIA_WEBSITE else ""` for themselves (pings, rolemenu_panels, applications twice, role_menus, tempvoice); six copies of an inverse is how one of them drifts from `bare()` and a kind stops classifying. `test_no_module_builds_the_web_head_for_itself` fails if a seventh appears. |
| `black_bloc/api/writes.py:185` | `note()` survives, but only for events **the route is the sole logger of** — `web.request.filed`, `web.request.updated`, the comments, the withdraw, the raid-train and role-menu CRUD. Where a shared bot function already logs, the route passes `via=VIA_WEBSITE` and notes nothing: that pairing was the 2026-09-03 double post, two rows and two embeds for one click. |
| `black_bloc/cogs/community/requests.py:284` | `via` is **keyword-only with a Discord default** so the four slash callers keep writing bare `request.*` kinds untouched, and only the dashboard's `_decide` opts into the `web.` head. (Third pass: `mark_ready`, `accept` and `send_back` take the same keyword and pass it straight through.) The same shape is on `resume_request`, `events.apply_decision`, `events.cancel_event`, `polls.store_poll`/`apply_decision`/`close_poll`/`cancel_poll`, `role_menus.apply_request_decision`, `modmail.close_ticket`, `honeypot.ban_hit`/`make_trap_channel`, `tempvoice.make_creator_channel`/`forget_creator`, `automod.save_rule`/`apply_case`/`punish` and the six `modcmds` punishers. |
| `black_bloc/cogs/community/requests.py:336` | `details["via"] = via` is written even though the kind already carries the head, because `via_of()` prefers what the writer recorded over what the kind says. Belt and braces: a row keeps saying "Website" even if somebody later renames the kind. |
| `black_bloc/api/tools/requests.py:302` | The route's whole job here is now `via=VIA_WEBSITE` — the deleted `note(f"web.request.{status}")` was the defect. `request_status` still notes `web.request.updated` beside it, because assignee/priority/notes are a **different** change from the status move and nothing else logs them. |
| `black_bloc/cogs/moderation/modcmds.py:161` | `refuse_in_test_mode` takes `via` too, so a dashboard punish refused by TEST_MODE leaves `web.mod.would_ban` rather than a bare one that reads as a Discord action. It is the actor's row, not a bot-emitted consequence — the `mod.*_failed` and `request.dm_failed` family deliberately stay bare. |
| `black_bloc/cogs/moderation/automod.py:178` | `punish` heads every row it leaves, not just the successful one: delete/warn/timeout and their `would_`/`_failed` twins are all outcomes of one actor's click, so they belong to the same Via. This is why the dashboard's Apply-now now leaves `web.automod.*` and no `web.mod.apply` — the automod rows ARE the event, and `mod.apply` was a second framing of it. |
| `black_bloc/cogs/moderation/modmail.py:616` | `send_reply` was a **false positive** in the survey that found these: it logs only `modmail.dm_failed`, nothing on success, so `web.modmail.reply` is the route's own and stays. `close_ticket` next to it did double, and does not. |
| `tests/test_logkinds.py:272` | `_branches` unwraps `kind_via(...)` so the dynamic-kind table does not need forty new entries: a literal inside it yields BOTH the bare and the `web.` form. An f-string inside it still needs `KNOWN_DYNAMIC`, keyed on the INNER expression — which is why the pre-existing keys like `f'request.{wanted}'` kept working and just grew their `web.` twins. |
| `tests/test_logkinds.py:459` | The mechanical guard for checklist 34. It resolves each route's imports to real module files, asks what kinds those functions log (following helpers within their own module, and reading `KNOWN_DYNAMIC` for the f-strings), and fails on any `note()` whose `bare()` kind is in that set. ⚠️ Helpers under `api/` are excluded (`API`) — `note` itself logs whatever it is handed, so without that filter every route matches itself. |
| `tests/test_logkinds.py:343` | `DOUBLE_LOG_ALLOWED` is **empty and should stay that way**. It exists so a genuine exception is written down with its reason rather than the guard being loosened; an entry here means "these two rows are different events", never "this one is fine". |
| `tests/test_logkinds.py:516` | The default matters more than the call sites: one shared function defaulting to `VIA_WEBSITE` would silently turn every slash command's kind into a `web.` one. A private helper with a REQUIRED `via` is fine — it is always handed one. |
| `tests/api/conftest.py:405` | `one_web_row` is the per-route half of the same guard, and it is behavioural where the AST walk is structural: it asserts the write left exactly ONE `web.*` row, of the named kind, with `details.via == "website"`. Counting is the point — the old tests asserted both kinds were *present*, which is exactly what a double post looks like. |
| `black_bloc/logkinds.py:155` | Eight kinds left `ROUTINE` with this change (`event.cancel`, `honeypot.ban`, `mod.apply`, `mod.rule`, `modmail.close`, `poll.cancel`, `poll.end`, `tempvoice.forget`): each was only ever produced by a deleted `note()`, and `test_no_classification_entry_is_dead` refuses a table entry nothing emits. ⚠️ Legacy rows keep their kind and classify the same, with ONE exception — `honeypot.ban` ends in `.ban`, an `IMPORTANT_SUFFIXES` entry, so old `web.honeypot.ban` rows now read as important. That matches the new `honeypot.banned`, so it was left. |
| `site/mock/server.mjs:MOD_KIND_DONE` | The mock has to lie the same way the real API does: the punish helpers log what HAPPENED (`warned`, `timed_out`, `banned`), not what was asked for (`warn`, `timeout`, `ban`), so the mock maps the route name to the outcome word rather than logging the route name. `contract.json`'s `action_kinds` was updated in the same commit; `check.mjs` only checks emitted ⊆ listed, so a stale mock would have passed silently. |

# Requests — third pass: the review state, the embeds and the site link

> Keyed against `feat/requests-third-pass` at the commit named in the header
> line above. Owner, 2026-09-03: *"a standard appealing template … one of the
> discord info boxes"*, *"Do we need an acceptance pending so a staffer can check
> if something is done?"*. Design: [`requests-embeds-design.md`](requests-embeds-design.md).

| Key | Note |
|---|---|
| `black_bloc/requests.py:47` | ⚠️ **A LOOK is not a STATUS.** `LOOKS` has seven entries; `STATUSES` has seven too and they are **different sets**: `filed` and `sent_back` are looks with no state behind them, and `open` and `withdrawn` are states with no card. Everything downstream — the log kind, the colour, the fields, who is DMed, whether a card posts at all — reads the look, never the status. |
| `black_bloc/requests.py:371` | `look_of` is the whole of that distinction in three lines: `in_progress` arrived at **from `review`** is `sent_back`. Storing a `sent_back` flag would have been a second home for a fact the `was:` already carries, and it would drift the first time somebody moved a row by hand. |
| `black_bloc/requests.py:377` | `checked_move` grew two keyword-only text arguments rather than two new functions, because a move is judged in ONE place and the site, the slash commands and the tests all ask this. `TEXT_NEEDED` is keyed by LOOK (`review` needs `built`, `sent_back` needs `note`), which is why the required-text rule fires on a send-back and not on an ordinary pick-up — same target state, different look. |
| `black_bloc/requests.py:154` | `DONE_NEEDS_A_CHECK` fires only from `OPEN_STATUSES`. From `declined` or `withdrawn` the generic "is where a request finishes" sentence is still right, and pointing somebody at `/request ready` there would be a lie. The refusal names both the slash path AND the site button, because the person reading it may be on either. |
| `black_bloc/requests.py:228` | The colours are DATA (`EMBED_COLOURS`), and so are the titles, the field lists and the inline flags. Seven looks by four properties as literals inside one builder is how the sixth one quietly gets the fifth one's colour. |
| `black_bloc/requests.py:612` | `moment` reads `created_at` for the filed card and `decided_at` for every other, so a card is stamped **when the move happened**. A row read back before `decided_at` is written (or a naive ISO string from an older row) falls back to now rather than raising — a card with a slightly wrong timestamp beats no card. |
| `black_bloc/requests.py:637` | ⚠️ **`field_value` returning `""` is what leaves a field OFF the card.** Discord refuses an empty field value outright, so "How to test" on a review with none, "Assignee" on an unassigned row and "Due" with no date are absent rather than blank. Every value goes through `clamp(…, 1024)` here and nowhere else — §J measured the worst look at 2004 of Discord's 6000, entirely because of this line. |
| `black_bloc/requests.py:688` | `site_view` answers `None` when there is no origin, and `post_line`/`dm` pass `view=None` straight through. A link button with a broken URL is worse than no button; `bot.settings.origin` is always set in practice, so this is the belt to the braces. |
| `black_bloc/requests.py:703` | `move_line` is the plain one-liner that REPLACED the sent message — it is now only ever written to the server log, in `post_line`'s two `log.warning` calls, so a card the guard skipped still says in the log what it would have said. |
| `black_bloc/requests.py:844` | `set_status` builds its `UPDATE` from a dict because five columns now have per-move rules and "leave this one alone" was already being faked with `COALESCE`. **A column with no rule for this move is not in the statement at all** — which is how `ready_by` survives a hold-and-resume and how `decided_by` survives a withdrawal. |
| `black_bloc/requests.py:577` | `may_accept` answers True when nothing is recorded in `ready_by`. A row moved to `review` by the landing one-off, or by a `/status` write before this shipped, must not become unacceptable by anybody. |
| `black_bloc/cogs/community/requests.py:348` | ⚠️ **`kind_via(f"request.{look}", via)` — LOOK, not status.** This is the line that makes `request.sent_back` exist at all; the row's status is `in_progress`. `tests/test_logkinds.py:KNOWN_DYNAMIC` is keyed on the source of that expression, so renaming the variable breaks the build loudly rather than silently dropping six kinds out of the classification check. |
| `black_bloc/cogs/community/requests.py:172` | `person_told` is the ONE place that decides who gets a DM, and it is the reason `tell_requester` was renamed: on `sent_back` the person told is the staffer in `ready_by`, not the requester. Nobody is DMed on `review` — it is staff-facing and the channel card is its notice. |
| `black_bloc/cogs/community/requests.py:193` | The `request_dm_on_decision` gate applies to `DM_LOOKS` only, so the send-back DM to `ready_by` is NOT gated by it. That key is documented as the **requester's** switch; widening it to staff notices would give one key two meanings and there would be no way to want one without the other. |
| `black_bloc/cogs/community/requests.py:208` | `post_line` keeps its name and its `line` argument even though the line is never sent — the embed is the message and `content` is `None`. ⚠️ **The guard check is unmoved and unchanged**: TEST_MODE still refuses every channel but the test one and logs `request.notify_skipped_test_mode`, and a mutation that removes the check turns `test_the_review_card_is_skipped_and_logged_while_test_mode_holds_the_channel` red. |
| `black_bloc/cogs/community/requests.py:298` | `apply_decision` passes `ready_by` and `sent_back_reason` on EVERY call and lets `set_status` decide whether this move writes them — the alternative, each caller working out which columns its move touches, is the same rule in five places. The `request_review_by_other` check lives here and not in `accept`, so `POST /{id}/status {"status":"done"}` cannot walk around it. |
| `black_bloc/cogs/community/requests.py:395` | `send_back` is the only one of the three wrappers with a guard of its own: `accept` and `mark_ready` are refused by `TRANSITIONS` when the row is in the wrong state, but `review` → `in_progress` is legal from `open` too, so without this a `/request sendback` on an untouched request would quietly pick it up. |
| `black_bloc/cogs/community/requests.py:883` | `ReadyModal` prefills from the row, so a request sent back and marked ready again does not need retyping. `built` is `required=True` at the Discord level AND refused in words by `checked_move` — the client-side one is a courtesy, the server-side one is the rule. |
| `black_bloc/cogs/community/requests.py:774` | `/request ready` is two interactions: the command opens the modal, the modal does the move. ⚠️ A modal can only be the FIRST response, so `_wanted_id` must not `defer()` — `require_staff` answers without deferring, which is why it can be called before `send_modal`. |
| `black_bloc/api/tools/requests.py:499` | The three routes' whole job is `via=VIA_WEBSITE` and **no `note()`** — checklist 34. `tests/api/conftest.py:one_web_row` asserts exactly one `web.*` row per write, which is the assertion the pre-fix tests were missing (they asserted presence, which is what a double post looks like). |
| `black_bloc/api/tools/requests.py:575` | `/status` takes `built` and `how_to_test` as plain fields through `set_fields`, so fixing a typo in how-to-test is a partial save that leaves one `web.request.updated` row and moves nothing. It ALSO forwards them to `apply_decision` when a status is present, so `{"status":"review","built":…}` is a legal, if unused, second path into review. |
| `black_bloc/storage/db.py:11` | Schema **26**: four nullable columns on `requests` (`built`, `how_to_test`, `ready_by`, `sent_back_reason`), added to the `CREATE TABLE` and to `ADDED_COLUMNS`. No backfill — a row that was `done` before this stays `done` with four nulls, and `requests.py:row_value` reads a missing column as nothing on an older file. The landing move of #1 and #2 into `review` is a one-off in the design doc, **not** a migration: `done` → `review` is not a staff move and must not become one. |
| `black_bloc/settings_store.py:861` | `enums` is a NEW registry type — "any of these words", where `enum` is "one of" and `channels`/`roles` are lists of ids. `coerce_value` returns the picked set **in `KEY_CHOICES` order**, so the stored value is stable no matter what order the checkboxes were ticked in and two identical settings never diff. |
| `black_bloc/cogs/core.py:25` | ⚠️ `VALUE_KEYS` had to learn `enums` or `request_channel_moves` would have been a **dashboard-only** key — exactly half of checklist 33, and the half that is invisible until somebody tries `/settings set-value` and gets no autocomplete. `test_the_card_moves_key_is_reachable_from_slash_settings_as_well_as_the_dashboard` is the guard. |
| `site/public/assets/ui.js:928` | The `enums` control is a checkbox per choice, not a segment: a segment says "one of these", and the answer here is a SET. `read` walks the DOM rather than a captured array, so the value saved is what is actually ticked. |
| `black_bloc/requests.py:291` | `REQUEST_ANCHOR` takes its page from `logkinds.FEATURE_PAGES["request"]` — **`requests.html`, with the extension.** The design said `{origin}/requests#r-N` and the build shipped that; the site is a `StaticFiles(html=True)` mount (`api/assets.py`), which resolves `/requests` to a file called `requests` or `requests/index.html` and nothing else — live it answered `{"detail":"Not Found"}` (measured 2026-09-03 06:19, before the merge deployed). One home for the page name, so the log-embed links and the card button cannot drift apart. |
| `site/public/assets/page-requests.js:238` | ⚠️ **`anchored()` is what makes every Discord card's link button work.** Each card carries `id="r-{id}"`, and `{origin}/requests.html#r-{id}` is built in `black_bloc/requests.py:request_url`. The two halves are in different languages and nothing type-checks between them — `tests/test_requests.py::test_every_card_carries_one_link_button_to_the_request_on_the_site` pins the URL shape, and this is the other end of it. |
| `site/public/assets/page-requests.js:flashLinked` | ⚠️ **TWO things break a deep link into a shut section, and neither raises anything.** (1) A card inside a shut section or foldout — Done and Declined both are — sits in a `.card` with `overflow: hidden` and a **clientHeight of 32**, so `scrollIntoView` does nothing at all. (2) Opening the `<details>` is not enough on its own: the scroll must wait a **frame** for layout to settle, or it measures the collapsed height and stays put. Opening and scrolling in the same tick left the reader at the top of the page with nothing in the console. Measured against the mock: with the section open and one frame elapsed, `.content` (the real scroller — the body never scrolls) goes 0 → 3019 and the card lands mid-viewport. |
| `site/public/assets/page-requests.js:cardFor` | The pinned card is drawn as whatever the row actually IS. Drawing every one as a review card would put Accept and Send back on an open request, and the API would then refuse the click — the page's own rule is that a control which would be refused is never drawn. |
| `site/public/assets/page-requests.js:1202` | The off-page case: staff fetch `/api/requests/N`, a member scans their own `/mine`, and the result is PINNED above the sections with Back to all rather than paging the reader around. ⚠️ A 404 or a 403 here is "that number is not one you can read", **not** an outage — the two get different sentences, because the fixes are different. `pinned` is set before the fetch so a refresh mid-flight cannot pin twice. |
| `site/public/assets/page-requests.js:675` | The review card filters `done` and `in_progress` out of `row.moves` and draws Accept and Send back as its own buttons — those two moves need a dialog and a distinct label, which a generic move button cannot carry. `MOVES` lost its `done` entry entirely, so no card anywhere can draw a Done button. |
| `site/public/assets/page-requests.js:614` | `writtenRow` is a partial `/status` save, not a move: the owner asked that a typo in "how to test" not need a state change to fix. It is on the review card and on the done card, which is why `shutCard` builds a `notice()` only when the row is `done`. |
| `site/mock/server.mjs:askDecide` | The mock enforces the SAME three refusals the API does (no `built`, no send-back note, the second-pair-of-eyes rule) because `check.mjs` only checks that emitted kinds are a subset of the listed ones — a mock that accepted anything would pass while the real API refused. ⚠️ Its `/accept` and `/sendback` read the row AFTER the move; the pre-existing `/hold`, `/decline` and `/status` entries do not, and answer with the state the row was in. |
| `site/mock/check.mjs:IDS` | `review_request_id` (11) is a row the SEED already has ready to check, so `/accept` and `/sendback` reach it without walking the machine first — every contract entry runs against a fresh seed. `progress_request_id` (12) is where `/ready` is legal from. |
| `site/public/assets/labels.js:160` | ⚠️ **This file did not parse between `7b1c592` and the fix on this branch**, so `LABELS` never loaded and **every dashboard page rendered blank**. The Phase 19 merge pasted the applications block after the object's closing brace and after `NAMESPACES`, leaving seven orphaned properties and a second `};`. `node --check site/public/assets/labels.js` is the whole test, and it is worth running after any merge that touches this file — nothing else in the suite reads it. |

# Requests, fourth pass — `/request` is ONE command that opens a panel

> ⚠️ **Re-keyed again 2026-09-03 at the wave-0 merge (`1861923`)** — seven rows whose code moved to
> `black_bloc/panels.py` now carry that path with a "was … until wave 0" note, and the cog keys
> moved UP (the library imports replaced ~60 lines of the cog). Before that,
> ⚠️ **re-keyed 2026-09-03 against `feat/requests-panel-own-list` (`dd7c788`)**, which made the
> panel's own-request list staff-only: every key in THIS section was shifted by anchor — the
> `requests.py` ones by **+1** (`PANEL_OWN_LIST_KEY` above them) or **+5** (`withdraw_request`,
> below `panel_shows_own_list` too), and the `cogs/community/requests.py` ones by **+1** above
> `build_panel` and **+2** below it (one import line, one `if`). `settings_store.py` +5;
> `api/tools/requests.py` unmoved. Two rows were ADDED (the gate at `:511`, the helper at
> `requests.py:512`); none was rewritten, because nothing they describe changed.
> Before that, keyed against `feat/requests-panel` at the commit this section was written in.
> Owner, 2026-09-03: *"Let's also have /request open a menu maybe. Let's try and
> minimize slash commands and maximize interactive windows."* Design:
> [`requests-panel-design.md`](requests-panel-design.md). ⚠️ **Not re-keyed against
> earlier sections** — this build deleted the nine `request_*` slash commands and every
> earlier note that describes them (the two "Requests — third pass" rows about
> `/request ready`'s two-interaction shape and `ReadyModal` prefilling, the Phase 13 row
> about `/request create` needing the group) now describes code that is **GONE**, not
> moved; they are left as historical record rather than rewritten, per the standing rule
> that only the conductor re-keys the whole file at merge.

| Key | Note |
|---|---|
| `black_bloc/requests.py:426` | ⚠️ **`CARD_BUTTONS` is DATA, not a chain of `if status ==`.** Every earlier request state-machine bug in this file (`TRANSITIONS`, `EMBED_TITLES`, `MOVE_LINE`) was a table for the same reason: a table is one thing to get wrong, and `tests/test_requests.py::test_every_status_maps_to_the_move_buttons_the_panel_design_names` is parametrised over all seven statuses, so a status the table forgets fails by name instead of silently rendering no buttons. `card_buttons()` is the only reader; the cog never branches on status by hand. |
| `black_bloc/requests.py:56` | **`SELECT_CAP = 25` is Discord's own hard limit on a select's options, not a decision.** It caps BOTH selects — the staff "Pick a request…" (which also says "N of M — the rest are on the site" past the cap, per the design) and the member's own "Take one back…" (which silently truncates with no such message, because a member realistically never has 25 requests open at once; noted here since it is the one place the two selects behave differently for the same reason). |
| `black_bloc/requests.py:973` | ⚠️ **`withdraw_request` deliberately widens checklist item 34's own wording.** That item's own text lists "withdraw" among the routes where `note()` is the sole logger — true when it was written, that morning, before withdraw had a shared function to call. This build gives withdraw one (the panel's confirm and the site's `POST …/withdraw` both call it), so the route's `note("web.request.withdrawn", …)` became the SAME double-post shape checklist 34 exists to catch — `kind_via("request.withdrawn", via)` replaces it, and `tests/test_logkinds.py::test_a_route_never_notes_an_event_its_shared_path_already_logged` is what would have failed if the route had kept its own line. |
| `black_bloc/cogs/community/requests.py:729` (`RequestView(Panel)`) | ⚠️ **Views are not persistent, and every render creates a FRESH one with the FULL `request_panel_minutes` timeout — the clock is inactivity, not wall time from when `/request` was first opened.** `RequestView(minutes)` is constructed by `build_panel`/`build_card`/`open_withdraw_confirm` on every single render; clicking anything resets the countdown rather than merely extending a single running one. ⚠️ **This note used to end "only the LAST view nobody touches ever reaches `on_timeout`", and that was WRONG** — see `retire` (now `black_bloc/panels.py:37`) and the design doc's deviation 9. A displaced view keeps its own timeout task; nothing in the library stops it. See KI-20 for why persistence (`bot.add_view`, the `TempVoicePanel` pattern) was not used instead. |
| `black_bloc/panels.py:37` (`retire` — was `cogs/community/requests.py:168` until wave 0) | ⚠️ **Every re-render MUST stop the view it replaces, or that view's timeout will overwrite the card that replaced it with a stale disabled copy.** `ViewStore.add_view` (installed `discord/ui/view.py:940–968`) merely overwrites `_synced_message_views[message_id]`; it never touches the displaced view's `__timeout_task`, so that task keeps running and `_dispatch_timeout` (`view.py:611–620`) eventually calls the OLD view's `on_timeout`. `retire` is called in the cog at `:607` (`render_panel`), `:632` (`finish_card`), `:656` (`open_card`) and `:695` (`open_withdraw_confirm`), always immediately BEFORE the edit; every item callback passes its own `self.view` down as `previous`, and `CardMoveButton` (`:811`) hands it to `ReadyModal`/`NoteModal` so the modal's submit path retires it too. `RequestModal` (filing) never re-renders and carries nothing. |
| `black_bloc/panels.py:81` (`Panel.on_timeout` — was `cogs/community/requests.py:685` until wave 0) | ⚠️ **The guard is `self.replaced`, NOT `self.is_finished()` — the obvious spelling would have disabled the footer entirely.** Measured against the installed library: `_dispatch_timeout` does `__stopped.set_result(True)` (`view.py:619`) and only THEN `create_task(self.on_timeout())` (`:620`), so `is_finished()` is **already True inside a genuine timeout** — proved by running a real `discord.ui.View` with a 0.05s timeout and reading the flag from inside `on_timeout`. `stop()` alone is what actually prevents the timeout in production (`_dispatch_timeout` returns early on a done future); the `replaced` flag is the belt-and-braces half, and it is the only one that can tell "replaced" apart from "expired". |
| `black_bloc/panels.py:77` (`interaction_check`) / `:92` (`went_quiet`) — were `cogs/community/requests.py:681`/`:696` until wave 0 | ⚠️ **`view.message` is an `InteractionMessage` bound to ONE interaction token, and Discord kills that token 15 minutes after the interaction — so at the old 15-minute default the "gone quiet" footer essentially never got written.** `View._refresh_timeout` (`view.py:315–317`, from `_scheduled_task` at `:596–597`) extends the timeout on EVERY interaction with the view, including ones that do not re-render at all (`Logs`, opening a modal), so by the time it fires the message's token is at least `request_panel_minutes` old and usually older. `interaction_check` therefore records `self.last_interaction` (and always returns True — it gates nothing), and `went_quiet` tries `last_interaction.edit_original_response` first, falling back to `self.message.edit` when there is no recorded interaction OR when the recorded one is refused. Both are wrapped: an `HTTPException` is logged at info, never raised. The other half of the fix is the default dropping to **10** (`settings_store.py:1315`). |
| `black_bloc/panels.py:28` (`still_staff` — was `cogs/community/requests.py:159` until wave 0) | ⚠️ **The panel checks `is_staff` when it RENDERS; a staffer demoted while their card is open would keep moving requests until the panel timed out — so every move re-asks.** `still_staff` runs in the cog's `CardMoveButton.callback` (`:818`, before any defer or modal) and at the top of `Requests.ready_submit` (`:1021`) and `Requests.note_submit` (`:1045`, before their defers), refusing in words with `store.staff_refusal(guild.id)` through `answer()`. ⚠️ **`settings_store.require_staff` is deliberately NOT reused**: it calls `interaction.response.send_message` directly, so it only works where nothing has been deferred — one shape (`still_staff` + `answer()`) at all three sites beats two. `LogsButton` needs nothing, because `send_logs` (`actionlog.py:295–297`) calls `require_staff` itself before it has responded to anything. |
| `black_bloc/cogs/community/requests.py:522` (`MOVE_FUNCS`) | ⚠️ **`MOVE_FUNCS`'s three entries are ALL lambdas, not two lambdas and one direct reference.** `{"accept": accept, "resume": resume_request}` would have baked in the function OBJECT at dict-creation time (module import), and `monkeypatch.setattr(requests_cog, "accept", fake)` afterward would silently not affect what `run_move` calls — a lambda body does a NAME lookup in the module's globals at call time, so it is the one shape both a test's mock and a future call-through actually see. Found by writing `test_a_direct_move_button_calls_its_shared_function_and_leaves_via_alone` and watching the "accept" case call the real function anyway. |
| `black_bloc/panels.py:45` (`db_ready` — was `cogs/community/requests.py:465` until wave 0) | `db_ready` assumes the caller already `defer()`'d — every component/modal callback in this file does `interaction.response.defer()` first BECAUSE, per `discord/interactions.py`'s own `InteractionResponse.defer`, a component or modal-submit interaction's default (non-`thinking`) defer is `deferred_message_update` (type 6): an ack that edits the SAME message in place, never a new one. `interaction.edit_original_response(...)` is what actually changes it afterward, and `interaction.followup.send(...)` is what puts the returned sentence beside it — checked against the installed library source (checklist 29), not memory. |
| `black_bloc/cogs/community/requests.py:545` (`build_panel`) / `:530` (`build_card`) | `build_panel`/`build_card` are free functions taking `bot`/`guild`/`actor`, not `Requests` cog methods, so every `discord.ui.Item` subclass can call them via `interaction.client` without holding a `self.cog` reference. Only `submit`, `ready_submit` and `note_submit` stay cog methods, because `RequestModal`/`ReadyModal`/`NoteModal` already take a `cog` argument (existing shape for the first two; matched for the third) and fetch it with `interaction.client.get_cog("Requests")` at the point a modal is opened. |
| `black_bloc/cogs/community/requests.py:661` (`open_withdraw_confirm`) | `open_withdraw_confirm` reuses `look_for_status` (open → `filed`, everything else itself) for the PREVIEW card too, not only the staff card — one status→look mapping, one place it can drift. The confirm view carries only "Yes, take it back" / "Keep it", never a `Back`, because the design lists exactly those two. |
| `black_bloc/panels.py:112` (`NoteModal.__init__`, the label mutation — was `cogs/community/requests.py:909` until wave 0; the cog's thin subclass is at `:916`) | `NoteModal.note.label` is mutated per-instance in `__init__`, the same way `ReadyModal.built.default` already was — one `discord.ui.TextInput` field, three labels (`hold`/`decline`/`sendback`), each naming who the note is sent to inside Discord's 45-character label cap. ⚠️ This discord.py version deprecates the `.label` setter in favour of `discord.ui.Label` (Components V2) — it still works, and nothing else in this codebase needs a per-instance label yet, so the migration is left for whoever adopts Components V2 wholesale rather than done piecemeal here. |
| `black_bloc/settings_store.py:1320` (`default`) | `request_panel_minutes` follows the exact shape `request_review_by_other` already set: `KEY_TYPES["request_panel_minutes"] = "int"`, a `KEY_HELP` sentence, and a `default()` branch returning **10** — no `KEY_MIN`/`KEY_MAX` bound, matching every other undecided-bound `int` key in this registry (`honeypot_purge_days`, `poll_reminder_minutes`) rather than inventing a ceiling nobody asked for. ⚠️ **10, not the 15 this was first built with**: review finding F2 measured that the "gone quiet" footer can only be written while Discord's 15-minute interaction token is still valid, so 15+ silently loses it. The value is still NOT clamped — the `KEY_HELP` sentence is the warning, and KI-20 carries the same one. |
| `black_bloc/cogs/community/requests.py:566` (the own-list gate) | ⚠️ **Only the RENDERING of a member's own requests is gated — `own_rows` is still fetched for every caller, on purpose.** Owner, 2026-09-03: *"We need to make the view request thing staff only"* → *"Viewing requests on the panel"*. `if staff or panel_shows_own_list(store, guild.id):` wraps the `summary_line` block AND the `PANEL_EMPTY` line, and nothing else in `build_panel` moved. The obvious "optimisation" — skipping the `list_requests(user_id=actor.id)` query when the list is hidden — would BREAK the withdraw select, which reads the same rows to decide what is `WITHDRAWABLE`; a member who cannot see their list must still be offered "Take one back…". ⚠️ **No sentence says the list is hidden**: with the key off a member's embed is `PANEL_INTRO` alone (plus the requests-off / staff-only-filing line when one applies), and the intro's "see where what you already asked for has got to" is still honest — the site button and the withdraw select both still name their requests. See deviation 12 in [`requests-panel-design.md`](requests-panel-design.md). |
| `black_bloc/settings_store.py:871` (`REQUEST_CARD_DEFAULT`) / `:1317` (`default`) | ⚠️ **The DEFAULT for `request_channel_moves` is every move but `done`; the CHOICES stay all seven.** Owner, 2026-09-03 10:17, minutes after the panel landed: "suppress the request.done box in discord, it's redundant information. This should be in website logs only". The gate already existed (`requests.py:556` `posts_a_card`, checklist 33 both ways since the third pass), and the live DB held NO stored `request_*` row, so a default change is the whole fix and a server can tick `done` back on from the Settings page. Two tuples, not one edited: `KEY_CHOICES` must keep `done` or the Settings checkbox and `/settings set-value` would REFUSE it. The requester's DM on done (`DM_LOOKS`, `request_dm_on_decision`) is untouched — it goes to the person who asked, not to the channel, so it is not the duplicate the owner named. `test_every_card_but_done_posts_and_one_pair_of_eyes_is_enough_until_a_lead_says_otherwise` pins both tuples. |
| `black_bloc/requests.py:512` (`panel_shows_own_list`) / `:52` (`PANEL_OWN_LIST_KEY`) | The helper and its key sit beside `panel_minutes`/`PANEL_MINUTES_KEY` rather than in the cog, so the cog holds no `store.get(..., "request_panel_own_list")` string of its own — the same shape `channel_moves`, `review_by_other` and `panel_minutes` already use, and the reason `tests/test_requests.py` can pin the default (`Store()` with no value at all → `False`) without building a `SettingsStore`. `bool(store.get(...))` rather than `is True`, because the registry coerces the key to a real bool before it is ever stored and a `None` from an unconfigured guild must read as off. Registered in the three places `request_panel_minutes` is: `settings_store.py:880` (`KEY_TYPES`, `"bool"`), `:904` (`KEY_HELP`) and `:1322` (`default()` → `False`) — which is all checklist 33 needs, since `VALUE_KEYS` (`cogs/core.py:27`) derives the `/settings set-value` list from `KEY_TYPES` and the Settings page reads the same registry. |
| `black_bloc/api/tools/requests.py:644` (`request_withdraw`) | The route keeps its own `NOT_YOURS` check (whose row is this?) and hands the rest to `withdraw_request` — identity belongs to the caller, because the panel and the route each already know their caller a different way (a Discord `User` versus a signed-in session), and `withdraw_request` takes a `row` it trusts is already the actor's own. |

# Panels library (wave 0, 2026-09-03)

> Keyed against `feat/panels-library` at the wave-0 build commit `5da0081`. This section is
> NEW. The older `Requests, fourth pass` rows above WERE re-keyed at the merge (`1861923`,
> 2026-09-03): the seven whose code moved here — `retire`, `on_timeout`,
> `interaction_check`/`went_quiet`, `still_staff`, `db_ready` and `NoteModal.note.label` —
> now name their `panels.py` line with a "was … until wave 0" note. Nothing in this wave
> changed behaviour; the 163 requests tests passed **unchanged in assertion**.

| Key | Note |
|---|---|
| `black_bloc/panels.py` (the module) | ⚠️ **This file exists so seventeen feature panels do not re-implement three defects seventeen times.** `/request` took four passes and a review that found exactly three (a replaced view's timeout clock overwriting the live card; a "gone quiet" footer that could never be written at 15 minutes; a staff check that only ran at render time) — deviations 9, 10 and 11 in [`requests-panel-design.md`](requests-panel-design.md). Every one of them lives here now, fixed once. [`panels-program.md`](panels-program.md) §2 is the invariant list every later panel is reviewed against; its §4 is this wave. The module imports `command_errors` and `settings_store` only — it knows nothing about requests, and `tests/test_panels.py::test_the_library_says_what_it_offers_and_knows_nothing_about_requests` pins that. |
| `black_bloc/panels.py:67` (`Panel`) | **Was `RequestView` (`cogs/community/requests.py:674–713` before this wave), moved verbatim except that the gone-quiet sentence is now the `footer` constructor argument** instead of a module constant read out of `requests.py`. `AnswersErrors` stays FIRST in the bases (checklist 30 — a view that defers without an `on_error` leaves a permanent spinner). Invariant P7. The cog's `RequestView(Panel)` (`cogs/community/requests.py:729`) keeps its name and its `__init__(minutes)` so `build_card`, `build_panel`, `open_withdraw_confirm` and every requests test are untouched. |
| `black_bloc/panels.py:81` (`on_timeout`) | ⚠️ **The guard is `self.replaced`, NOT `self.is_finished()` — the obvious spelling would disable the footer entirely.** Measured against the installed library: `_dispatch_timeout` does `__stopped.set_result(True)` (`discord/ui/view.py:619`) and only THEN `create_task(self.on_timeout())` (`:620`), so `is_finished()` is **already True inside a genuine timeout**. Deviation 9; invariant P6. This is the single most important line in the file to leave alone. |
| `black_bloc/panels.py:37` (`retire`) | ⚠️ **Every re-render MUST stop the view it replaces**, or that view's timeout overwrites the card that replaced it with a stale disabled copy. `ViewStore.add_view` (`discord/ui/view.py:940–968`) merely overwrites `_synced_message_views[message_id]`; it never touches the displaced view's `__timeout_task`, so that task keeps running and `_dispatch_timeout` (`view.py:611–620`) eventually calls the OLD view's `on_timeout`. Deviation 9; invariant P6. Called immediately BEFORE every edit — in the requests cog at `render_panel`, `finish_card`, `open_card` and `open_withdraw_confirm`; every later panel does the same. |
| `black_bloc/panels.py:77` (`interaction_check`) / `:92` (`went_quiet`) | ⚠️ **`view.message` is an `InteractionMessage` bound to ONE interaction token, which Discord kills 15 minutes after that interaction — so a panel whose `<feature>_panel_minutes` is 15+ silently loses its footer.** `View._refresh_timeout` (`view.py:315–317`, from `_scheduled_task` at `:596–597`) extends the timeout on EVERY interaction with the view, including ones that do not re-render at all (a Logs button, opening a modal). So `interaction_check` records `self.last_interaction` (and always returns True — it gates nothing), and `went_quiet` tries `last_interaction.edit_original_response` first, falling back to `self.message.edit` when there is no recorded interaction OR when the recorded one is refused. Both wrapped: an `HTTPException` is logged at info, never raised. Deviation 10; invariant P7, whose "default 10, and the key's help text says why" half stays per-feature in each `settings_store` `default()` branch. ⚠️ The log line now reads `"panel: could not disable a timed-out panel: %s"` on `black_bloc.panels`'s own logger, not `"requests: …"` — a grep for the old sentence will miss it. |
| `black_bloc/panels.py:28` (`still_staff`) | ⚠️ **A panel checks `is_staff` when it RENDERS; a staffer demoted while their card is open would keep moving things until the panel timed out — so every staff move re-asks.** Refuses in words with `store.staff_refusal(guild.id)` through `answer()`. ⚠️ **`settings_store.require_staff` is deliberately NOT reused**: it calls `interaction.response.send_message` directly, so it only works where nothing has been deferred. Deviation 11; invariant P8. |
| `black_bloc/panels.py:17` (`answer`) | The one refusal shape that copes with responded-or-not: `followup.send` once `interaction.response.is_done()`, `response.send_message` otherwise, always `ephemeral=True` and `AllowedMentions.none()` (checklist 11). It is what makes `still_staff` usable after a defer, which is the whole reason deviation 11 did not reuse `require_staff`. ⚠️ Distinct from `command_errors.answer`, which is the generic unhandled-error sentence and swallows its own failure; this one is a deliberate reply and lets an `HTTPException` reach `AnswersErrors`. Two functions, one name, on purpose — they are NOT interchangeable. |
| `black_bloc/panels.py:45` (`db_ready`) | ⚠️ **Assumes the caller already `defer()`'d** — it answers a followup, never `response.send_message`. Every component/modal callback defers first BECAUSE, per `discord/interactions.py`'s own `InteractionResponse.defer`, a component or modal-submit interaction's default (non-`thinking`) defer is `deferred_message_update` (type 6): an ack that edits the SAME message in place. Deviations 1 and 6; invariant P5 ("every click re-checks `db_ready`"). It imports `DB_UNAVAILABLE` from `settings_store` at module level, which is the ONE new import edge this wave adds (`requests` → `panels` → `settings_store`); no cycle, because nothing under `black_bloc/*.py` or `black_bloc/storage/` imports `black_bloc.requests`. |
| `black_bloc/panels.py:55` (`capped_placeholder`) / `:14` (`CAPPED_PLACEHOLDER`) | Invariant P10 — Discord's 25-option select cap, with a placeholder saying what was left off. `CAPPED_PLACEHOLDER` is the sentence's ONE home (checklist 15): `requests.py:208` `PICK_CAPPED = CAPPED_PLACEHOLDER` reads it rather than restating it, and `requests.pick_placeholder` (`:488`) is now a one-line call passing `PICK_A_REQUEST`. The `pick` word stays per-feature ("Pick a request…", "Pick a poll…"); only the capped wording is shared. `capped` is still a parameter so a feature whose list is NOT on the site can say something else. |
| `black_bloc/panels.py:63` (`panel_minutes`) | Takes the settings KEY, so each feature keeps its own `<feature>_panel_minutes` registry entry (checklist 33 / invariant P13) while the read is one function. `requests.panel_minutes(store, guild_id)` (`:508`) is now a one-liner over it with `PANEL_MINUTES_KEY`; it is imported into `requests.py` as `library_panel_minutes` purely so that module's own two-argument public name does not shadow it. |
| `black_bloc/panels.py:112` (`NoteModal`) | Invariant P12 — one paragraph field whose label names what the note is for and who is sent it. Generic on `title`/`label`/`max_length`/`on_submit`. ⚠️ **The callback is stored as `self.takes_note`, NOT `self.on_submit`** — an instance attribute named `on_submit` would shadow `discord.ui.Modal.on_submit`, which Discord calls with `(interaction)` alone, and the arity mismatch would surface only at click time in production. ⚠️ The inherited class-level `note = discord.ui.TextInput(...)` works in a SUBCLASS because `Modal.__init_subclass__` (`discord/ui/modal.py:111–121`) walks the full `__mro__` collecting `Item`s, and `_init_children` (`:123–129`) `deepcopy`s each one per instance — checked against the installed library (checklist 29), pinned by `tests/test_panels.py::test_two_note_modals_do_not_share_one_field`. That same `deepcopy` is why a test must compare `note.style` with `==` and not `is`. This discord.py version deprecates the `.label` setter in favour of `discord.ui.Label` (Components V2); it still works, and the migration stays for whoever adopts Components V2 wholesale. |
| `black_bloc/cogs/community/requests.py:916` (`NoteModal`) | The requests note modal is now a **thin subclass** of the library's, keeping its `(cog, request_id, kind, previous)` signature and its `NOTE_TITLES`/`NOTE_LABELS`/`NOTE_LIMITS` lookups, so `Requests.note_submit`, `CardMoveButton` and all six existing tests are untouched. ⚠️ **The `self.cog`/`self.request_id`/`self.kind`/`self.previous` assignments come BEFORE `super().__init__(...)` on purpose**: the `on_submit=self.note_submit` bound method it hands up is called later, but it must exist as an argument now, and the state it closes over is this instance's. The bound method is named `note_submit` to match the cog method it forwards to. |
| `black_bloc/cogs/community/requests.py:13` (the two `panels` imports) — wave 0 | ⚠️ **`retire`, `answer`, `db_ready` and `still_staff` are imported to MODULE level in the cog on purpose, not called as `panels.retire(...)`.** The cog's own code resolves them through its module namespace, so `requests_cog.retire(view)` in `tests/cogs/community/test_requests.py` and any future `monkeypatch.setattr(requests_cog, "still_staff", …)` keep working exactly as they did — the same call-time name-lookup reasoning as `MOVE_FUNCS`'s lambdas one section above. `NoteModal` is imported `as PanelNoteModal` because the cog exports its own `NoteModal` under that name. |

# Requests, sixth pass — "Ask them to check" (2026-09-03)

> Keyed against `feat/requests-check` at its fourth commit. This section is NEW; nothing above
> it was re-keyed, because the sixth pass only INSERTS into `requests.py` and the cog — but the
> insertions shift every `Requests — third pass` and `Requests, fourth pass` key below the
> insertion point, so re-key those two sections at the merge (the standing rule). The design and
> its 11 deviations are [`requests-check-design.md`](requests-check-design.md).

| Key | Note |
|---|---|
| `black_bloc/cogs/community/requests.py:805` (`BackButton`, `row=1`) | ⚠️ **A review card is now FIVE moves wide, which is exactly Discord's action-row cap — Back had to leave row 0, and this is a crash fix, not a layout preference.** `discord/ui/view.py:193` raises `ValueError: item would not fit at row 0 (6 > 5 width)` while the view is being BUILT, so every review card in the panel would have failed to render, not merely looked wrong. Found by running the parametrised `test_the_card_renders_exactly_the_buttons_the_table_says[review]`, which is now also the guard: it asserts `len([item for item in view.children if item.row == 0]) <= 5` and that Back is on row 1, so the sixth move somebody adds later fails in a test instead of in production. Deviation 1. |
| `black_bloc/requests.py:47` (`LOOKS`) / `:46` (`CHECK_ASKED`) | ⚠️ **`check_asked` is a LOOK, never a status, and deliberately NOT in `DM_LOOKS`.** The states say who HOLDS a request (staff working / staff checking / waiting / finished); "the requester has been asked" is a fact ABOUT the review, so the row stays `review` and `TRANSITIONS` is untouched — §B of the design says why a `checking_by_requester` state would double every review button and every test for no new transition. It is out of `DM_LOOKS` because `DM_LOOKS` is the table `tell_person` (`cogs/community/requests.py:186`) reads on a MOVE; `ask_check` sends its own DM at a moment no move happened, and putting the look in that tuple would have made a `request_dm_on_decision=false` server silently skip the one DM this feature exists to send. Added LAST in `LOOKS` so `MOVE_LOOKS` (`cogs/community/requests.py:109`) and `channel_moves` (`:534`) pick it up with no edit. |
| `black_bloc/requests.py:676` (`request_embed`, the `if look == CHECK_ASKED`) / `:191` (`CHECK_ASKED_DESCRIPTION`) | The ONE `if` in an otherwise table-driven builder. Title, colour and fields stay in `EMBED_TITLES`/`EMBED_COLOURS`/`EMBED_FIELDS`; only this look sets `embed.description`, and a fourth table with one populated row would cost more than the branch. `mention(check_asked_by) or "staff"` — a row whose asker is unknown (an auto-ask by a bot actor with no id) still reads as a sentence rather than as `Try it and tell  how it went`. |
| `black_bloc/requests.py:623` (`asked_stamp`) / `:273` (`FIELD_LABELS["asked"]`) | The "Asked to check · @who · 3 minutes ago" field on review and done cards. ⚠️ **An empty string is the whole point**: `request_embed` skips a field whose value is empty, so a card nobody has asked on simply has no such field — no "not asked yet" line to read past. An unparseable `check_asked_at` falls back to the mention alone rather than raising, the same shape `moment` (`:612`) already uses; a stamp is cosmetic and must never be able to stop a card rendering. |
| `black_bloc/requests.py:165` (`NOT_READY_TO_CHECK`) | ⚠️ **Its last sentence was stale from the moment the fourth pass landed** — it recommended `/request ready {id}`, a subcommand that build deleted, and nothing caught it because no test asserted the wording. It now names the **Ready to check** button on the card, which is what actually puts a request there from either surface. The mock carries the same sentence (`site/mock/server.mjs`, `REQUEST_NOT_READY`) and was fixed in the same commit; it also gained a `{doing}` placeholder, because two routes refuse with it now. |
| `black_bloc/cogs/community/requests.py:461` (`ask_check`) | The one shared path, same `(what to say, row or None)` contract as `send_back` and `resume_request`, so the panel button and `POST /api/requests/{id}/check` differ only in the `via` they pass (checklist 34: `kind_via` builds the head, the route `note()`s nothing). ⚠️ **The order matters**: the fields are stamped BEFORE the DM is built, so the card the requester receives already carries its own "Asked by" line, and `told` is decided by what actually happened rather than by what was attempted. `request.dm_failed` on a refused DM is a SECOND row, not a substitute for the action row — today's contract for every look, not a new one. |
| `black_bloc/cogs/community/requests.py:236` (`post_line`, the `ping=` keyword) | ⚠️ **The only place this cog mentions anybody.** Every other send in the file is `AllowedMentions.none()` (checklist 11); this one narrows to exactly one user — `AllowedMentions(users=[discord.Object(id=ping)])` plus a `content` of that mention — so an `@everyone` in a `what` field still cannot ping, and neither can a role. Everything else in `post_line` (the guard check, the missing-channel warning, `NOTIFY_SKIPPED_KIND`, `NOTIFY_FAILED_KIND`) is unchanged and still applies to the fallback post, which is why test mode cannot leak one. |
| `black_bloc/cogs/community/requests.py:382` (`mark_ready`, the auto-ask) | `mark_ready` is no longer a pure delegation to `apply_decision`: with `request_check_on_ready` on it asks straight afterwards, and the ask is its OWN log row because it is its own thing that happened. ⚠️ **It returns `ask_check`'s fresh row, not `apply_decision`'s** — `finish_card` re-renders from the returned row, and the pre-ask row has no `check_asked_at`, so the new field would have been missing from the card until somebody refreshed. Deviation 6. The reply text stays `READY_SAID`; the ask's outcome is on the card, not in a second sentence. |
| `black_bloc/cogs/community/requests.py:525` (`MOVE_FUNCS["check"]`) | A lambda like its three neighbours, for the same reason the wave-0 note above gives: a lambda body looks the name up in the module's globals at CALL time, so `monkeypatch.setattr(requests_cog, "ask_check", fake)` in `test_a_direct_move_button_calls_its_shared_function_and_leaves_via_alone[review-Ask them to check-ask_check]` reaches it. `CardMoveButton.callback` needed no change at all — `check` is neither `ready` nor `needs_modal`, so it falls through to `run_move`, which already does `still_staff` → defer → `db_ready` → `MOVE_FUNCS` → `finish_card`. |
| `black_bloc/settings_store.py:871` (`REQUEST_CARD_DEFAULT`) / `:1324` (`default`) | `check_asked` is the eighth `request_channel_moves` CHOICE and, like `done`, is excluded from the DEFAULT — the DM is the point, and a channel copy is opt-in. Two tuples again, not one edited: `KEY_CHOICES` must keep both or the Settings checkbox and `/settings set-value` would refuse them. `request_check_fallback_channel` defaults **on** (a closed DM should not silently mean nobody was told) and `request_check_on_ready` **off** (asking is a judgement about whether the work is worth a person's time). Both are `bool` registry keys, which is all checklist 33 needs. |
| `black_bloc/api/tools/requests.py:537` (`request_check`) | The `/accept` route's shape exactly, with `ask_check` behind it and `via=VIA_WEBSITE`; `fresh is None` is a 409 carrying the shared function's own sentence, so the site says the same words the panel does. ⚠️ **No `note()`** — checklist 34, guarded by `tests/test_logkinds.py::test_a_route_never_notes_an_event_its_shared_path_already_logged`, an AST walk of `api/tools/*.py`. |
| `site/public/assets/page-requests.js:650` (`askedChip`) | Added to the shared `headBlock` rather than copied onto the done card, because `readyChip` — the thing the design said to mirror — is not used in the finished list at all: `shutCard`, `boardCard`, `openCard` and `heldCard` all render `headBlock`. One call covers every card, and rows nobody has asked on return `null` from it. The text is "asked **by** X", not "asked X": `check_asked_by_name` is the staffer who asked, and without the preposition it reads as the name of the person being asked. Deviations 7 and 8. |
| `site/mock/check.mjs` (the review walk) | The action-kinds walk is the one pass that does NOT re-seed between calls, so its order is a state machine: `ready → check → sendback → ready → accept`. `check` goes FIRST of the four on the review row because it is the only one that leaves the row where it is — put anywhere else it would be asking a row that had already moved on. The contract table (`contract.json`) re-seeds per route, so its `/check` entry is order-independent. |

# Applications, no-role pass

> Keyed against `feat/applications-no-role` at the build's third commit (`df99135`), 2026-09-03.
> 3414 tests pass, ruff clean, `check.mjs` 17 pages / 141 routes. The design is
> [`applications-no-role-design.md`](applications-no-role-design.md); its `## Deviations` foot
> lists the five places the build departed from it. ⚠️ **Re-key this section after the merge**
> — the sibling `feat/requests-check` build touches `settings_store.py` and `storage/db.py`
> and will shift these numbers (standing merge-order rule in `CLAUDE.md`).

| Key | Note |
|---|---|
| `black_bloc/storage/db.py:728` (`_loosen_application_form_roles`) | 🔴 **The one place in this build where a mistake destroys owner data, and the reason it is called at `:697` — BEFORE `PRAGMA foreign_keys=ON` at `:699` rather than beside the other migration steps.** `application_questions.form_id` carries `REFERENCES application_forms(id) ON DELETE CASCADE` (`db.py:595`). With foreign keys ON, `DROP TABLE application_forms` performs an implicit `DELETE FROM application_forms`, which cascades — **every question on every form would vanish silently**, and a form with no questions cannot be applied for. Run before the pragma, the drop is inert. `tests/storage/test_db.py::test_a_schema_26_file_lets_a_form_drop_its_role_and_keeps_every_question` builds a 26-shape file by hand (two forms, three questions, one application) and asserts all three questions survive. |
| `black_bloc/storage/db.py:743` (the `commit()` before the log line) | ⚠️ **Without it `PRAGMA foreign_keys=ON` two lines later becomes a SILENT no-op.** SQLite refuses to change `foreign_keys` inside a transaction and does not raise — it just ignores the pragma. The rebuild's `INSERT … SELECT` opens one under Python's legacy transaction control, so the connection is mid-transaction when `connect()` reaches the pragma. Found by running the test, not by reading the code: the first version passed every row-and-question assertion and failed only `PRAGMA foreign_keys == 1`. That assertion is the reason the test keeps it. |
| `black_bloc/storage/db.py:13` (`APPLICATION_FORMS_COLUMNS`) / `:34` (`APPLICATION_FORMS_LOOSENED`) | One fact, one home (checklist 15): the column list is a module constant that the `SCHEMA` f-string interpolates AND the rebuild's `CREATE TABLE … ` reuses, so the copy cannot drift from the live shape and `INSERT INTO … SELECT *` cannot fall out of column order. ⚠️ `SCHEMA` became an **f-string** for this — it contains no other `{` or `}`, which was checked before the change; any SQL added to it that needs a literal brace must double it. |
| `black_bloc/storage/db.py:728` vs `:747` (`_set_aside_mod_cases_with_a_required_user`) | ⚠️ **Deliberately NOT the mod_cases rename-then-restore shape.** Renaming the LIVE table would make SQLite ≥ 3.26 rewrite `application_questions`' foreign key to point at the renamed copy (unless `legacy_alter_table` is on), leaving the fresh `application_forms` unreferenced. Create-copy-drop-rename renames only the new table, which nothing references, so the existing FK text keeps naming `application_forms` and resolves again the moment the rename lands. |
| `black_bloc/applications.py:371` (`role_of`) | The ONE reader of a form's role. Returns `None` for missing, `NULL`, `0` and unparseable, so no caller ever formats `<@&None>` or `<@&0>` — the sentinel-zero design was rejected in §B5 precisely because `holds_role(member, 0)` is a silent lie. Every `form["role_id"]` read in the tree now goes through it (`render_card`, `decision_lines`, `_approve`, `remove`, `/applications list`, `form_row`). |
| `black_bloc/applications.py:35` (`NO_ROLE`) with `:606` and `:626` (`update_form`) | ⚠️ **`role_id=None` means "leave it alone" and `role_id=NO_ROLE` (0) means "clear it" — they are NOT interchangeable, and the difference is invisible at the call site.** `update_form` drops `None` values (that is how every optional slash argument spells "unchanged"), so clearing needs its own word. `:606` turns the sentinel back into `None` AFTER that filter, and `:626` builds `SET role_id = NULL` as literal SQL for any surviving `None` rather than binding it — the parameter form works too, but the literal is what makes "this column is being cleared on purpose" visible in the statement. ⚠️ The same trick would fix the standing inability to clear `review_channel_id`/`approver_role_id`/`owner_user_id`; §E of the design keeps that OUT of scope, so do not widen it here without an item. |
| `black_bloc/applications.py:469` (`decision_lines(granted=…)`) | ⚠️ **Three-valued on purpose: `True` handed over, `False` Discord refused, `None` there was nothing to hand over.** The old `if not granted` treated "nothing to hand over" as "the grant failed" and would have pasted "Discord refused to add the role — hand it over with `/role grant`" onto every no-role approval. Every test of it is `is False`, never falsy. `_approve` (`cogs/community/applications.py:451`) and `is_live` guard the same way. |
| `black_bloc/applications.py:193` (`APPROVED_ON_RECORD`) | The no-role half of `APPROVED_SAID`. It names the form's HEADING rather than a role, and takes no `{extra}`: `EXPIRES_EXTRA` says "the role runs out …", which is a lie when there is no role, so the expiry line is suppressed on a list form even when `expires_days` is set. |
| `black_bloc/applications.py:856` (`remove_application`) | `APPROVED → REMOVED`, the owner's "staff always get the final say" rule (`CLAUDE.md`, 2026-09-03) applied to a list. ⚠️ **`status = 'approved'` is in the WHERE clause, not checked before it** — same one-writer-wins shape as `decide_application`, so two staff pressing the button at once produce one removal and one "not approved" sentence. The reason reuses the `deny_reason` column: the row's `status` says which kind of no it was, and a second column would have been two homes for one fact. |
| `black_bloc/applications.py:779` (`last_decision`) | ⚠️ **`REMOVED` was added to the status list here, which is what makes `retry_days` bite after a removal.** Without it `cooling_until` would see no decision and somebody taken off the list could re-apply the same minute — the design says the wait applies "as it does after a denial", and this line is where that is true. |
| `black_bloc/applications.py:786` (`twitch_logins_for`) | One `IN (…)` query for the whole roster, never one per row — `tests/test_applications.py::test_the_whole_roster_s_twitch_logins_come_back_in_one_query` counts the executes. De-duplicates ids before building the placeholders, and drops rows whose `twitch_login` is empty so the caller's `.get()` is the only emptiness check. Reads `golive_links`, which is per MEMBER and not per guild (`db.py:99`) — the roster is therefore as accurate as `/twitch link`, and §D3 keeps a hand-typed login out of scope. |
| `black_bloc/cogs/community/applications.py:440` (`_approve`) | `until` is computed only when there IS a role, so a list form with `expires_days` set neither writes an expiry into the log nor mentions one in the DM. `details["role_id"]` is `None` rather than absent — the Logs page reads details as a flat map and a missing key would render as though the form had never had one. `_hand_over` is not called at all, so `application.granted`, `application.would_grant` and `application.grant_failed` simply never fire; `application.approved` with `granted: null` is the whole record (design §C6). |
| `black_bloc/cogs/community/applications.py:560` (`remove`) | The web route and the button share this one function, which is why it takes `via` and logs `kind_via("application.removed", via)` itself — checklist 34, one write leaves ONE `action_log` row. `api/tools/applications.py:470` passes `via=VIA_WEBSITE` and does NOT `note()` on top of it. ⚠️ Refuses a form that HAS a role in words (`REMOVE_IS_FOR_LISTS`, `:180`) rather than removing the record and leaving the role on them: for a role form the role's own end IS the way off, and `/role revoke` is the sentence's answer. |
| `black_bloc/cogs/community/applications.py:158` (`can_decide`) | ⚠️ **`may_decide` ANSWERS the interaction when it refuses, so it cannot be used to decide whether to draw a button.** `can_decide` is the same question with no side effect, used by `show_panel` at render time; the button's own callback still calls `may_decide` (`:812`), so a staffer demoted while the panel is open is refused in words at click time (checklist / invariant P8). |
| `black_bloc/cogs/community/applications.py:838` (`show_panel`) | Returns `None` — and `/applications show` then sends the bare embed exactly as it always did — unless the application is approved AND the form keeps a list AND the caller may decide. Owner rule 2026-09-03: no new slash command; the move is a button that renders only when it is valid. `view.message` is set from `interaction.original_response()` after the send (`:1462`) so `Panel.on_timeout` has something to disable; without it the footer is never written. |
| `black_bloc/cogs/community/applications.py:1405` (`/applications list`) | The header line prints `list` instead of `<@&None>` — the old code interpolated `one['role_id']` raw, which is what a nullable column turns into a visible bug. `listed` is the set of no-role form ids, computed once, so the Twitch suffix is appended only to approved rows on those forms and `twitch_logins_for` is called once for the page. |
| `black_bloc/cogs/community/applications.py:38` (`PANEL_MINUTES_KEY`) | `applications_panel_minutes`, the fourth `<feature>_panel_minutes` key (invariant P13 / checklist 33). Deviation 5 in the design doc: §C4 asked for ONE new setting and this is a second, because `Panel` takes a timeout and a hard-coded 10 would be a decided default the owner cannot reach. Its help text carries the same 15-minute warning `request_panel_minutes` does. |
| `black_bloc/api/tools/applications.py:85` (`form_row`) | ⚠️ **`resolve_one(guild, None)` must never be called** — it would name whatever the guild resolves for a null id (`@everyone` in practice) and the forms table would show a role chip for a form that has none. `role_name` is resolved inside the `if role` branch, and `tests/api/tools/test_applications.py::test_a_form_can_be_made_with_no_role_and_never_resolves_one` monkeypatches `resolve_one` and asserts it is never asked about `None`. |
| `black_bloc/api/tools/applications.py:268` (the PATCH `role_id` branch) | A blank `role_id` in the payload becomes `forms.NO_ROLE`, not `None` — see the `update_form` note above; `None` here would mean "the page did not send a role" and the clear would be dropped. The page sends `''` on PATCH and `null` on create for exactly this reason (`page-rolemenus.js:723`). |
| `black_bloc/api/tools/applications.py:367` (`GET /roster`) | Declared BEFORE `GET /{application_id}` so `/roster` is not eaten by the id route. 404s in words when the form is unknown or belongs to another guild. Honours `applications_roster_shows_left` (default true, §D1): somebody who has left is listed with `in_server: false` rather than hidden, because the official team page may still list them and staff decide. |
| `site/public/assets/page-rolemenus.js:694` (the blank role option) | Deviation 1: `roleSelect` ALREADY prepends a blank `not set` option, so the design's `allowNone: true` was not needed — the option is relabelled **"No role — keep a list"** in place. `:780` (`syncRole`) hides the whole **Role lasts, days** field while the select is blank, using `el.hidden` (the artifact reset and the site's CSS both honour it) rather than `style.display`. |
| `site/public/assets/page-rolemenus.js:1008` (`rosterFoldout`) | The roster is fetched **on the fold's first open**, not with the page — a server with a dozen forms would otherwise make a dozen requests nobody asked for. The `toggle` listener is guarded by `asked` so re-opening does not re-fetch (press the page's own refresh for fresh rows). The count in the summary starts from the approved rows the section ALREADY has from `/api/applications` and is replaced with the roster's own length once it lands, because `applications_roster_shows_left` can make the two differ. |
| `site/public/assets/page-rolemenus.js:103` (`ROSTER_LINE`) / `:936` (`copyLines`) | §D4: one line per member, `Name — twitch.tv/login`, `no Twitch linked` where there is none and ` (left the server)` appended where they have gone — the bridge to the official team page, which is not ours to write (§E). ⚠️ `navigator.clipboard.writeText` rejects in a page without clipboard permission, so the failure is caught and answered with a sentence telling the reader to select the rows by hand — never a silent no-op. |
| `site/mock/server.mjs` (`GONE_FROM_GUILD`) | The mock's one seeded member who is on a list but no longer in the server, so `in_server: false` and the roster's "left the server" flag are exercised by `check.mjs` rather than only by pytest. `nameFor` still knows them: the record outlives the membership, which is the whole point of the flag. |
| `site/mock/check.mjs` (`checkRoster`) | Deviation 4: the design asked for a DOM walk (open the fold, click the button, count rows). `check.mjs` has no DOM — it is a route-contract runner — so the same walk is done over the routes: roster lists 3, one carries a `twitch_login`, one is flagged as having left, a removal with a reason succeeds, a removal with a blank reason is a 400, the roster is then 2, and the removed row appears under `?status=removed`. |

# Birthdays panel (wave 1)

> Added 2026-09-03 by the birthdays panel build, branch `worktree-agent-a19bdce15408f8243`, keyed
> against that branch's tip. `/birthday` is ONE command that opens an ephemeral panel;
> the `birthday` `Group`, the nested `birthday role` group and all twelve subcommands are
> gone. The design is [`birthdays-panel-design.md`](birthdays-panel-design.md); the
> invariants it is built against are [`panels-program.md`](panels-program.md) §2.
>
> ⚠️ **The Phase 5 section above was re-keyed by ANCHOR in the same commit** — 30 keys in
> `## black_bloc/cogs/community/birthdays.py — F6`, every one checked against the
> construct it names. Two of them now point at `black_bloc/birthdays.py` because the code
> moved there (`chunked`, and the confirmation sentence as `stored_sentence`), and one
> (`:250`) says out loud that the `/birthday import` command it was written for has not
> existed since 2026-08-27. The design doc's header called this drift out before the
> build: the old `:308` named `loop_health`, which was at `:300`, and the old `:624` lock
> note was at `:673`.
>
> NOT verified: anything against live Discord. No panel has been opened by a person; no
> boot was run (this build has no bot token). `commands synced` was read from
> `tests/test_bot.py::test_the_command_tree_stays_inside_discords_limits`, which measures
> the real tree with every cog loaded: **44, unchanged** — a `Group` already held one
> top-level slot, so retiring it frees nothing (requests deviation 7).

## `black_bloc/birthdays.py` — the panel's pure half

| Key | Note |
|---|---|
| `black_bloc/birthdays.py:14` (the `panels` import) | The pure module now imports the wave-0 library for `panel_minutes` alone. That is the same edge `black_bloc/requests.py:508` has and it is not a cycle: `panels` imports `command_errors` and `settings_store`, neither of which imports this module. It does mean `black_bloc/birthdays.py` pulls `discord` in, which it did not before — the module is still free of Discord *objects*, which is what "no Discord in them" was ever about. |
| `black_bloc/birthdays.py:37` (`MODE_WARNINGS`) | A member who sets a birthday deserves to know nothing will be posted, and the feature ships in `shadow`. Keyed on the mode string so `on` simply has no entry and the line disappears — an absent key, not an `if mode == …` chain that a fourth mode would silently fall through. |
| `black_bloc/birthdays.py:418` (`DATE_PARTS`) | `-`, `/`, `.` and space, collapsed as one run, because people type `09-15`, `09/15` and `9 15` and none of them is wrong. Splitting on a character class rather than parsing a format string is what makes all four separators one rule instead of four. |
| `black_bloc/birthdays.py:421` (`parse_birthday_input`) | ⚠️ **It never guesses a year.** Two parts means no year, full stop — a birthday typed `09-15` must not silently acquire one, because a stored year is what puts somebody's age on a public embed. Anything that is not two or three runs of digits is `None`, and the caller answers `DATE_UNREADABLE`; the month/day/year values themselves are NOT checked here, so `date_problem` and `year_problem` keep their existing sentences and their existing home. |
| `black_bloc/birthdays.py:432` (`date_modal_title`) | Discord refuses a modal whose title is over 45 characters, and a display name is arbitrary attacker-controlled text — a nickname of 90 characters would have made `Set their birthday` fail with an HTTP error and no explanation. Only the "theirs" title is clamped; the "mine" one is a constant. |
| `black_bloc/birthdays.py:439` (`stored_prefill`) | Zero-padded so the prefill round-trips through `parse_birthday_input` — the modal offers back exactly the shape it accepts, which is what makes "change my birthday" an edit rather than a re-type. |
| `black_bloc/birthdays.py:459` (`PanelButton`) / `:473` (`PANEL_BUTTONS`) / `:480` (`panel_buttons`) | The button table is DATA (invariant P3), the shape of `requests.MoveButton`/`CARD_BUTTONS`/`card_buttons`. ⚠️ `(False, True)` is **unreachable** — `opted_out` is defined as `has_date and not opted_in` — so `panel_buttons` falls back to the nothing-stored row rather than raising: a hand-edited row or a future caller must not be able to crash the panel, and the parametrised test pins the fallback. |
| `black_bloc/birthdays.py:485` (`panel_minutes`) / `:489` (`panel_shows_next`) / `:493` (`panel_allows_lookup`) | One helper per key, beside the key name, so no caller ever spells a settings key as a literal. Same shape as `requests.panel_minutes`/`panel_shows_own_list`. |
| `black_bloc/birthdays.py:509` (`card_lines`) | ⚠️ **`mine` changes two things, and both matter.** The stored birth **year** is shown in brackets only on your own block — `birthday_show_age` already decides whether an *age* is public, and the year is the raw fact behind it, so somebody else's card renders exactly what `/birthday show` rendered before. And the opted-out sentence reads "You are" rather than "They are", because the same builder writes your panel and their card. |
| `black_bloc/birthdays.py:557` (`stored_line`) | ONE home for the stored/opted-in/imported/self sentence, which the staff panel and `status_lines` both show. It was inline in `/birthday status` before and would otherwise have been typed twice, which is checklist 15. |
| `black_bloc/birthdays.py:564` (`status_lines`) | The cog gathers the values and this only words them, so the whole of the Status button's output is testable without a bot, a database or an interaction. The dict is deliberately flat and named — a positional signature of thirteen arguments is not readable at the call site. |

## `black_bloc/cogs/community/birthdays.py` — the panel

| Key | Note |
|---|---|
| `black_bloc/cogs/community/birthdays.py:79` (`NOT_STORED`) etc. | Five strings named a command that no longer exists (`NOT_STORED`, `NOT_STORED_FOR`, `OPTED_OUT`, `NOBODY_YET`, and two lines inside `report_lines`). They name **buttons** now. ⚠️ `report_lines` is rendered by the WEBSITE too (`api/tools/birthdays.py:121`), so its two replacements name neither a button nor a slash command: "until the person corrects their own birthday", and "set these by hand, from the Birthdays page or the `/birthday` panel". `ROLE_CLEARED`/`ROLE_NOT_SET` were left alone — `/settings set-role birthday_role_id` still exists. |
| `black_bloc/cogs/community/birthdays.py:322` (`dm`) | Staff forgetting somebody else's birthday is the one move in this feature that affects a person who is not in the room, so they are told. A closed DM is logged at info and the move still succeeds — the row is gone either way, and failing the removal because a DM bounced would be worse. |
| `black_bloc/cogs/community/birthdays.py:335` (`store_birthday`) | The one path a birthday is written by, for the member's own modal and staff's alike (P4). It returns the **sentence to show**, so the refusal and the confirmation come back the same way and the caller never decides wording. `date_problem`/`year_problem` run here rather than in the modal, so the two paths cannot drift. ⚠️ The retired `set` option was `Range[int, 1900, 2200]` while `year_problem` refuses anything after this year — the modal keeps `year_problem`, so the effective rule is unchanged and the dead 1901-year gap in the Range is gone. |
| `black_bloc/cogs/community/birthdays.py:366` (`forget_birthday`) | Takes an optional `member`, which the design's `(cog, guild, actor)` signature did not: the person card's **Forget their birthday** is the Discord path to a move the website already had (`DELETE /api/birthdays/{user_id}`), and the owner's staff-final-say rule of 2026-09-03 requires it to exist somewhere in Discord. `source` rides into the log row's details so a staff removal is distinguishable from a member's own. The role still goes back before the row is deleted (Phase 5 review finding 3) and the whole thing is inside the per-user lock. |
| `black_bloc/cogs/community/birthdays.py:397` (`change_opt`) | ⚠️ **Staff cannot opt somebody else out, and no surface offers it.** Opting out is the member's own privacy control; the card gives staff *set* and *forget*, which are both about the stored fact, never about somebody's choice. |
| `black_bloc/cogs/community/birthdays.py:476` (`build_panel`) | One command, two panels, split on `store.is_staff` (P2). Every block is a LINE, never a dead control (P9): a member with the coming-up list switched off is told it is for staff rather than shown an empty heading. The colour is the same `parse_color(birthday_color)` the wish embed uses, so the panel and the post are one look. |
| `black_bloc/cogs/community/birthdays.py:518` (`build_card`) | The person card. **Forget their birthday** renders only when there is something to forget (P3), so staff never press a button that would answer "there is nothing stored". A non-staff caller gets `Back` and nothing else, which is the whole difference between looking somebody up and acting on them. |
| `black_bloc/cogs/community/birthdays.py:587` (`open_confirm`) | Three confirms — remove, forget-theirs, clear-the-role — share one builder because they differ only in their sentence and their two buttons. Each is a re-render in place with `retire(previous)`, so the confirm screen is the panel, not a second message. |
| `black_bloc/cogs/community/birthdays.py:710` (`send_month`) / `:724` (`send_status`) | The two staff reads answer NEW ephemeral messages and never edit the panel, so the panel is still there when the list is read — the same shape as the Logs button (P11). They still `defer()` first, which is a `deferred_message_update` on a component: it acknowledges without changing anything. |
| `black_bloc/cogs/community/birthdays.py:765` (`MoveButton`) | Row 0 is built from `panel_buttons`, and the button's `action` decides the shape of the click: `set` opens a modal (so it must NOT defer), `remove` opens the confirm, `refresh` re-renders, and the two opt actions run the shared function. One class for four moves rather than four near-identical classes. |
| `black_bloc/cogs/community/birthdays.py:783` (`LookupSelect`) | A `UserSelect` rather than a name-matching select: it is not option-capped by us, so `capped_placeholder` (P10) has nothing to bite on here and its absence is deliberate, not an omission. `self.values[0]` is a real member object, so the card never has to resolve an id. |
| `black_bloc/cogs/community/birthdays.py:791` (`MonthSelect`) | 13 options — `Every month` plus the twelve names — against Discord's 25, so no cap and no placeholder. Value `"0"` means every month; the alternative (a separate "all" button) would have been a sixth control on a panel that already fills all five rows. |
| `black_bloc/cogs/community/birthdays.py:805` (`ModeSelect`) | The current mode is marked `default=True` so the select reads as a state, not a menu of two spellings of the same move (P3). It is rebuilt on every render, which is why flipping the mode makes the warning line and the select's own default change together. |
| `black_bloc/cogs/community/birthdays.py:925` (`DateModal`) | ONE modal, one field, for both the self path and the staff path — the title and the `mine` flag are the only difference, and `mine` also decides whether the submit re-renders the panel or the person card. |
| `black_bloc/cogs/community/birthdays.py:952` (`open_date_modal`) | ⚠️ **A modal cannot follow a `defer()`**, so the stored row is read BEFORE anything acknowledges the interaction, and a database that is down is answered in words here rather than raising. This is the one click in the panel that does not defer first, and the reason is Discord's, not ours. |
| `black_bloc/cogs/community/birthdays.py:1286` (`/birthday`) | The whole command: `_ready` (guild + database), build, send ephemerally, keep the message on the view so `on_timeout` can write the gone-quiet footer. `MEMBER_COMMANDS` still contains `birthday` — the panel is a member command that grows a staff half, not a staff command. |
| `black_bloc/cogs/community/birthdays.py:1298` (`date_submit`) | ⚠️ **`still_staff` runs BEFORE the defer** on the staff path (requests deviation 11): `require_staff` writes through `interaction.response.send_message` and cannot be used after one. A staffer demoted between opening the modal and submitting it moves nothing. |

## `settings_store.py` and `tests/test_bot.py` — what the panel changed

| Key | Note |
|---|---|
| `black_bloc/settings_store.py:932` (the birthdays panel block) | Appended as its OWN `KEY_TYPES.update` / `KEY_HELP.update` pair rather than added to the existing `birthday_*` rows, so the three wave-1 branches merge textually instead of fighting over one dict literal. The `default()` branches go beside `birthday_show_age`, which is a different file region again. |
| `black_bloc/settings_store.py:1322` (`birthday_panel_minutes` default) | **10, not 15.** The "gone quiet" footer can only be written while Discord's 15-minute interaction window is open (KI-20), so 15 or more means the buttons stop with nothing to explain it. Nothing is clamped — the help text is the warning, exactly as `request_panel_minutes` does it. |
| `black_bloc/settings_store.py:1324` / `:1326` (the two member-visibility defaults) | Both **True**, which is today's behaviour: `/birthday next` and `/birthday show @user` were open to every member. That is fork **F-B1**, decided by the owner 2026-09-03 12:20 — the coming-up list already shows the next five to everyone and `Opt out` is the member's own control, so the lookup reveals nothing somebody did not choose to store. The owner made the analogous call the other way on requests; either way the key exists and the Settings page flips it. |
| `tests/test_bot.py:11` (`LOGS_GROUPS`) | Lost its `"birthday"` entry: `/birthday` is no longer a `Group` with a `logs` child, so both loops that walk this dict would `KeyError`. `LOG_LEVEL_COMMANDS["birthday"]` in `settings_store.py` was deliberately NOT touched — it still names a real command, and `/request` is carried the same way after its own panel landed. |
