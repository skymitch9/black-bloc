...
> Last verified: **2026-09-02 — KI-14 and KI-15 added by the Phase 18 (F19, raid
> trains) build, from reading its own code rather than from an incident; both
> describe a branch that has never met live Discord. Before that the same day,
...
## KI-14 — A raid-train slot keeps the Twitch name it was claimed with — `ACCEPTED`
...
## KI-15 — Raid-train check-in sees only what go-live sees — `ACCEPTED`
