...
| Events | `community/events.py` | `/event` | 0 | 1 | member + staff | ✅ **BUILT** (`feat/events-panel`, wave 1) — `/event` opens the panel; the `event` group and the whole `timezone` group are retired, so this is the program's first real `commands synced` drop (44 → 43). [`events-panel-design.md`](events-panel-design.md) |
| Polls | `community/polls.py` | `/poll` (logs, create, end, cancel, results, list, settings, `recur create|list|pause|delete`) | 11 | 1 | staff | `/poll`: open polls → pick → card (End, Cancel, Results); Create modal; Recurring tab (pick → Pause/Resume/Delete); Settings; Logs |
| Birthdays | `community/birthdays.py` | `/birthday` (logs, set, set-for, remove, optout, optin, show, next, list, mode, status, `role clear`) | 12 | 1 | member + staff | `/birthday`: mine (Set modal, Remove, Opt out/in) + Next; staff adds list, Set for… (UserSelect + modal), Mode, Role clear, Status, Logs |
| Applications | `community/applications.py` | `/apply` (start, status, withdraw), `/applications` (mode, create, edit, delete, panel, list, show, 2 unnamed, logs, `question add|edit|remove|list`) | 17 | 2 | member (apply) + staff | `/applications`: member — pick a form → Start (modal), my status, Withdraw; staff — forms → pick → card (Edit, Delete, Post panel, Questions sub-panel with add/edit/remove), open applications → pick → Approve/Deny; Mode; Logs. **2 groups → 1 command** |
...
| YouTube | `content/youtube.py` | `/youtube` (link, unlink, status), `/uploads` (logs, mode, setup, link-for, unlink-for, list) | 9 | 2 | member (youtube) + staff | `/youtube`: my channel (Link modal, Unlink, Status); staff adds list → pick → Unlink for, Link for (UserSelect + modal), Setup, Mode, Logs. **2 groups → 1 command** |
...
| Chat memory | `content/chat_memory.py` | `/memory` (show, forget, forget-this, off, on) | 5 | 1 | member | `/memory`: my profile card (Forget all, Forget this via select, Off/On) |
