...
| `black_bloc/settings_store.py:1304` (`default`) | `request_panel_minutes` follows the exact shape `request_review_by_other` already set: `KEY_TYPES["request_panel_minutes"] = "int"`, a `KEY_HELP` sentence, and a `default()` branch returning **10** — no `KEY_MIN`/`KEY_MAX` bound, matching every other undecided-bound `int` key in this registry (`honeypot_purge_days`, `poll_reminder_minutes`) rather than inventing a ceiling nobody asked for. ⚠️ **10, not the 15 this was first built with**: review finding F2 measured that the "gone quiet" footer can only be written while Discord's 15-minute interaction token is still valid, so 15+ silently loses it. The value is still NOT clamped — the `KEY_HELP` sentence is the warning, and KI-20 carries the same one. |
...
| `black_bloc/requests.py:477` (`panel_shows_own_list`) / `:49` (`PANEL_OWN_LIST_KEY`) | The helper and its key sit beside `panel_minutes`/`PANEL_MINUTES_KEY` rather than in the cog, so the cog holds no `store.get(..., "request_panel_own_list")` string of its own — the same shape `channel_moves`, `review_by_other` and `panel_minutes` already use, and the reason `tests/test_requests.py` can pin the default (`Store()` with no value at all → `False`) without building a `SettingsStore`. `bool(store.get(...))` rather than `is True`, because the registry coerces the key to a real bool before it is ever stored and a `None` from an unconfigured guild must read as off. Registered in the three places `request_panel_minutes` is: `settings_store.py:876` (`KEY_TYPES`, `"bool"`), `:897` (`KEY_HELP`) and `:1306` (`default()` → `False`) — which is all checklist 33 needs, since `VALUE_KEYS` (`cogs/core.py:27`) derives the `/settings set-value` list from `KEY_TYPES` and the Settings page reads the same registry. |
