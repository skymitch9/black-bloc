...
> Last verified: **2026-09-26 08:5x** — the HEADER and the new v172 entry only, at the v172 docs ritual. The newest entry is **2026-09-26 (v172, one merge: marathon-feeds-oengus)**, ONE TODO item moved whole into it; the last line of [`deploys.log`](deploys.log) is v172 (171 lines), which matches. ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual met Discord by hand or a browser (the live proof is the action log, read through the operator token).
> Before that, **2026-09-25 22:3x** — the HEADER and the new v171 entry only, at the v171 docs ritual. The newest entry is **2026-09-25 (v171, one merge: marathon-notice-when)**, ONE TODO item moved whole into it; the last line of [`deploys.log`](deploys.log) is v171 (170 lines), which matches. ⚠️ **NOT checked:** any older entry's facts (the v167–v170 entries were written at their own rituals); nothing in the ritual met Discord by hand or a browser.
> Before that, **2026-09-25 19:3x** — the HEADER and the new v166 entry only, at the v166 docs ritual. The newest entry is **2026-09-25 (v166, six merges: marathon-ux, shadow-home-per-feature, marathon-people, event-drawer, marathon-drawer-lite, event-links, plus two small changes on `main`)**, NINE TODO items moved whole into it; the last line of [`deploys.log`](deploys.log) is v166 (165 lines), which matches. ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual met Discord by hand or a browser. The *newest entry* sentence in the 16:2x paragraph below is history.
> Before that, **2026-09-25 16:2x** — the HEADER and the new v165 entry only, at the v165 docs ritual. The newest entry is **2026-09-25 (v165, three merges: marathon-feeds, marathon-event-modes, ci-linux-hang)**, three TODO items moved whole into it; the last line of [`deploys.log`](deploys.log) is v165 (164 lines), which matches. ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual met Discord by hand or a browser. The *newest entry* sentence in the 14:2x paragraph below is history.
> Before that, **2026-09-25 14:2x** — the HEADER and the new v164 entry only, at the v164 docs ritual. The newest entry is **2026-09-25 (v164, four merges: spotlight-ping-windows, marathon-schedule, marathon-next-event, marathon-events-page, plus the Channels row off the nav)**, seven TODO items moved whole into it; the last line of [`deploys.log`](deploys.log) is v164 (163 lines), which matches. ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual met Discord by hand or a browser. The *newest entry* sentence in the 18:3x paragraph below is history.
> Before that, **2026-09-23 18:3x** — the HEADER and one appended line in the v162 entry, at the v163 docs ritual. The newest entry is **2026-09-23 (v163, one merge: greeting-tone)**; the last line of [`deploys.log`](deploys.log) is v163 (162 lines), which matches. The v162 entry gained one ↳ line (the owner verified `CB-a` live at 16:3x, from `access/sweeps.md` commit `2d5defa0`). ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual met Discord by hand or a browser.
> Before that, **2026-09-23 16:2x** — the HEADER only, at the v162 docs ritual. The newest entry is **2026-09-23 (v162, three merges: chat-banter, mention-names, chat-review-loop)**; the last line of [`deploys.log`](deploys.log) is v162 (161 lines), which matches. ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual met Discord by hand or a browser. The *newest entry* sentence in the 15:1x paragraph below is history.
> Before that, **2026-09-23 15:1x** — the HEADER only, at the v160 + v161 docs ritual. The newest entry is
> **2026-09-23 (v161, the channel-visibility merge)** and the one under it is **2026-09-23 (v160, the `foldout` hotfix)**;
> the last line of [`deploys.log`](deploys.log) is v161 (160 lines), which matches. ⚠️ **NOT checked:** any older entry's
> facts; nothing in the ritual met Discord by hand or a browser. The *newest entry* sentence in the 14:1x paragraph below is history.
> Before that, **2026-09-23 14:1x** — the HEADER only, at the v158 + v159 docs ritual. The newest entry is
> **2026-09-23 (v159, four merges: channel-catalog, birthday-post-today, channels-page, personality-tones)** and the one
> under it is **2026-09-23 (v158, the birthday Change button)**; the last line of [`deploys.log`](deploys.log) is v159
> (158 lines), which matches. ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual met Discord by hand or a
> browser. The *newest entry* sentence in the 04:3x paragraph below is history.
> Before that, **2026-09-23 04:3x** — the HEADER only, at the v157 docs ritual. The newest entry is
> **2026-09-23 (v157, the four code-health merges)** and the one under it is **2026-09-22 (v156)**; the last line of
> [`deploys.log`](deploys.log) is v157, which matches. ⚠️ **NOT checked:** any older entry's facts; nothing in the ritual
> met Discord by hand or a browser. The *newest entry* sentence in the 2026-09-22 21:2x paragraph below is history.
> Before that, **2026-09-22 21:2x** — the HEADER only, at the v156 docs ritual. The newest entry is
> **2026-09-22 (v156, three spotlight merges)** and the one under it is **2026-09-22 (v155)**; the last line of
> [`deploys.log`](deploys.log) is v156, which matches. ⚠️ **NOT checked:** any older entry's facts; nothing
> in the ritual met Discord by hand or a browser. The *newest entry* sentence in the 2026-09-19 paragraph below is history.
> Before that, **2026-09-19** — the HEADER only, by the docs staleness pass after the
...
- 📡 **SS4C — the Oengus feed (owner, 2026-09-25 15:2x, verbatim: *"can we scrape speed stuff for charity"*).** Yes: Oengus's v1 marathon record carries the channel's `twitch` login and `scheduleDone`; `for-home` lists live / next / submissions-open marathons months ahead — so a feed reads `for-home`, checks each new id's record once, and keeps the ones whose Twitch login is the channel row's; runs come from the `for-slug` lines endpoint the feeds build found (Deviation 25). Design: [`info/marathon-feeds-design.md`](info/marathon-feeds-design.md) §H. Status: ✅ **LIVE as v172 08:50** (merged to `main` 2026-09-26 08:4x as `ca5ec825`; 3 commits, 347k Opus tokens; gate on the branch 8,467 passed + 3 skipped, `check.mjs` 22 pages / 252 routes, 7 node test files; schema 68 = `marathon_feeds.seen`, keys still 492; 17 deviations in the design doc — the reader raises `unpublished` like a GDQ event, an Oengus feed's `feed_ref` is the channel login, Look again clears `seen`, Forget does not; **Speed Stuff 4 LHS 2026 is LIVE on Oengus now** (2026-09-26 15:00Z → 09-28 02:50Z, schedule published) so the first boot after a deploy adds it AND posts its staff notice at once (shadow home while `marathon_mode` is shadow); deployed 08:50 on the owner's word — ↳ **LIVE PROOF:** the whole path fired within 8 s of login: seeded → checked (found 1, new 1, added 1) → *Speed Stuff 4 LHS 2026* added as marathon #6 → 17 runs read → one BaF run matched (run 1, *A Link to the Past*) → the ping window set → the staff notice posted to the shadow home with `because=published` → the board rehearsed). NOT verified: nothing met Discord or Fly; the bot's aiohttp client never read oengus.io (the KI-30 datacenter wall untested for it); sweeps `MO-a…d`. Dispatched 2026-09-25 22:4x (owner: *"Do it"*) — one Opus agent on branch `marathon-feeds-oengus` off `main` `69750bd7` (worktree `C:/lcw/bb-marathon-feeds-oengus`); scope: `oengus` source + reader (schedules → for-slug lines), `oengus` feed reading `for-home` and keeping marathons whose v1 `twitch` is the channel login, `marathon_feeds.seen` (schema 68), the `speedstuff4charity` seed, the Oengus pick on Add a feed, fixtures captured live; sweeps `MO-a…d`. Before-read: session 8% / weekly 84% / Fable 89% at 22:34.

## 2026-09-25 — v171: a feed-found marathon is pre-loaded quietly; the staff notice posts once its schedule is out

Deployed **22:29 Phoenix** (release commit `329ea08e`, merge `083a626a`; gate 8,426 passed + 3 skipped; boot `database: added marathons.noticed_at` 05:29:21Z, logged in 05:29:25Z, no Traceback; `/health` 62 ms; pre-deploy snapshot `backup-2026-09-25-pre-v171.sqlite3` 3.70 MB). Schema **67**, keys **492** (`marathon_feed_notice_when`, default `published`). The four live GDQ marathons carry no `noticed_at`, so each posts its staff notice once, on the first read that yields runs. Nothing met Discord; sweep `MF-f` is the owner's. Design: [`info/marathon-feeds-design.md`](info/marathon-feeds-design.md) (top line).

- 🔔 **Prompt staff about a marathon when its schedule is POSTED, not when the feed first sees it (owner, 2026-09-25 22:1x Phoenix, verbatim: *"I only want to be prompted with a marathon in the event category once the schedule is posted i think"*).** Today the feed's *added* notice posts at discovery (AGDQ 2027 was noticed months early with no runs). Change: `marathon_feed_notice_when` (enum `published` / `added`, default **published**) + `marathons.noticed_at` (nullable): the marathon row still appears on the site at discovery (quiet, `marathon.feed_added` stays IMPORTANT in the log); the staff notice (embed + buttons) posts on the first read that yields runs, with real dates and counts; in `added` mode it posts as today. The four live GDQ marathons carry no `noticed_at` → each gets its notice when its schedule publishes. The next-event suggestion is unchanged. Status: ✅ **LIVE as v171 22:29** (merged to `main` 2026-09-25 22:2x, branch `marathon-notice-when`, 2 commits) — gate: 8,426 passed + 3 skipped both orders, ruff, 20 node tests, `check.mjs` 22 pages / 252 routes; schema **67** (`marathons.noticed_at`, backfilled = `added_at` for rows without a feed), keys **492** (`marathon_feed_notice_when`, default `published`); `claim_notice` stamps before the post (one conditional UPDATE); the notice fires from the read call sites (`refresh_marathon`, `tick_marathon`, the add's own first read) so the embed shows the right Event field; `off` leaves the notice unclaimed; a shadow rehearsal uses the one notice. The four live GDQ rows (feed 1, `noticed_at` NULL) each notice once when their schedule publishes. Nothing has met Discord; sweep `MF-f`.
...
**Verified:** boot log `database ready` 04:54:37Z, `synced 33 app commands`, logged in 04:54:40Z, no Traceback; `guide.seed_refreshed count=3` at 04:54; `/health` ready 65 ms; `release.json` v168.

**NOT verified / noted:** nothing met Discord by hand (sweep `GP-a`); the v167 boot's `guide.shots_stale count=3` means three guide pictures (the `/mod`, `/rolemenu`, `/settings` cards) are stale again — queued on TODO with the seven guides that have no picture.

- 📖 **Guides: several guides per command (owner, 2026-09-25 20:3x Phoenix, verbatim: *"why does marathon have an empty event category"*).** The two marathon guides (and `golive-channels`, `ping-windows`) were saved with NO command because the store allows one published guide per command and audience, and `/event` / `/golive` already had theirs — so their hub cards read *· 4 steps* with no `/command`. Since one command now hosts several features (`/event` ▸ Marathons…, `/golive` ▸ Channels…), the rule is wrong: allow several published guides per command and audience (the index becomes non-unique or keyed by slug; `/help` lists every guide for the command; the hub card keeps the command line), then set `/event` on `marathons-follow` / `marathons-manage` and `/golive` on `golive-channels` / `ping-windows` (site + seed). Status: ✅ **LIVE as v168 21:54 (release commit `a90bbe14`; boot: `guide.seed_refreshed count=3` — the three commands filled)** — was: MERGED to `main` 20:5x (branch `guides-per-command`, 2 commits) — gate: 8,415 passed + 3 skipped both orders, ruff, 20 node tests, `check.mjs` 22 pages / 252 routes; schema **66** (the partial unique index `guides_one_published_command` retired via `RETIRED_INDEXES` + `DROP INDEX IF EXISTS` at boot, a plain `guides_by_command` index instead); `published_for` returns a list, `/help` names each guide on the command's heading line (`NAMED_GUIDE_CLAUSE`), the editor's command refusal removed; the seed sets `/event` on the two marathon guides and `/golive` on `ping-windows` (`golive-channels` already had it); the boot seed refresh fills a NULL live command only (never wording). Nothing has met Discord; sweep `GP-a`.
  ↳ **LIVE as v168 21:54** as above.

## 2026-09-25 — v167: `/rolemenu` hidden while role menus are off, the timed-grants console gets a `/mod` door

One merge over v166, deployed **20:22 Phoenix** (release commit `1cfdc0f7`, `release.json` v167 at `1b296a60`; gate 8,409 passed + 3 skipped, ruff clean, check.mjs 22 pages / 252 routes, green on the first run; no schema, keys 491). The guides session's seed sync and eight new guides (`7a3cdff1`) rode in the same image. A Sonnet pass first STOPPED (20:1x) because `rolemenu` was on `NEVER_HIDDEN` for a measured reason (v116 `57ce7cb`: with menus off the command still hosted the only door to approval-gated menus and timed grants); the owner chose **B** — `/mod` ▸ **Role grants…** as a second door onto the same console (`role_menus.open_grants_from`, one implementation, Back walks home to `/mod`), then `rolemenu` under `HIDDEN_WHEN_OFF["rolemenu_mode"]` (17 entries), `settings_panel.EXTRA_MODES` 4 → 3, the mode's help reworded, +10 tests.

**Verified:** boot log `database ready` 03:22:19Z, `synced 33 app commands`, **`command visibility: /rolemenu hidden in guild`** 03:22:20Z beside `/minutes`, logged in 03:22:23Z, no Traceback; `/health` ready 63 ms; `release.json` v167.

**NOT verified / left:** nothing met Discord by hand (sweeps `RM-a`, `RM-b`); with menus off the per-menu *Hand roles out…* and the *Waiting on staff* list stay reachable only via `/rolemenu` (no new requests arrive while menus are off; the approval cards' own buttons still work); the mock's copy of the old `rolemenu_mode` help sentence (`server.mjs:491`) is stale (JS untouched); no snapshot was taken (no schema change).

- 🙈 **Hide `/rolemenu` while role menus are off (owner, 2026-09-25 20:0x Phoenix, verbatim: *"Yes hide /rolemenu"* — the guides audit's third call).** Role menus have been off since the 2026-09-16 retirement but the command never left the list; the hidden-when-off rule exists for 16 other commands (`black_bloc/command_visibility.py`). Status: ⏸️ **STOPPED by the builder 20:1x, WAITING ON THE OWNER** — `rolemenu` is on `NEVER_HIDDEN` on purpose (v116 fix `57ce7cb`, `pings-remake-design.md` §C6 deviation 16): with `rolemenu_mode` off the command still hosts the staff console for approval-gated menus and timed grants (Grants… / Hand roles out…, `rolemenus.py:194` renders Grants unconditionally), and hiding the command once hid the only door — a measured regression. **Owner 20:1x: "B"** — move Grants… / Hand roles out… (the approval-gated and timed-grant staff console) into `/mod`, then hide `/rolemenu` while `rolemenu_mode` is off. ✅ **LIVE as v167 20:22 (release commit `1cfdc0f7`; boot log: `command visibility: /rolemenu hidden in guild` 03:22:20Z)** — was: MERGED to `main` 20:1x (branch `hide-rolemenu`, 3 commits) — gate: 8,409 passed + 3 skipped both orders, ruff clean, +10 tests; `/mod` ▸ **Role grants…** opens the same timed-grants console (one implementation, `role_menus.open_grants_from`, Back walks home to `/mod`); `rolemenu` left `NEVER_HIDDEN`, `HIDDEN_WHEN_OFF["rolemenu_mode"]` added (17 entries), `settings_panel.EXTRA_MODES` 4 → 3, the `rolemenu_mode` help reworded. Finding: with menus off, the per-menu *Hand roles out…* and the *Waiting on staff* list stay reachable only via `/rolemenu` (no new requests arrive while menus are off; the approval cards' own buttons still work) — sweep `RM-b`. The mock's copy of the old help sentence (`server.mjs:491`) is stale, JS untouched. The hide shows after the deploy's command sync.
  ↳ **LIVE as v167 20:22** as above.

## 2026-09-25 — the guides, audited and re-shot after v166 (no deploy)

Owner, 17:2x: *"once everything is done for the day, we need to redo the stale guides after the most recent set of changes plus old changes weve made too. maybe do a guide audit now so we can fix that"*. Two Opus passes: the AUDIT (`d81047dc`, `info/guides-audit-2026-09-25.md`: 12 stale shots, 11 of 80 steps drifted + 10 fault answers, 8 missing guides drafted, the trap that a guide's live text is a DB row) and the RE-SHOOT SESSION (commits `7a3cdff1` + `aee55007`, 19:3x–20:0x): every live guide brought to the seed text plus the audit's fixes through `PUT /api/guides/<slug>` (the live pages had never received the seed rewrites since v111 — seeding runs once), Pawpette's live `apply-form` goal kept, the pings guide made a member guide, seven test-mode fault answers rewritten, **eight new guides** created on the site and in the seed (`marathons-follow`, `marathons-manage`, `golive-channels`, `ping-windows`, `chat-panel`, `request-check`, `apply-review`, `posts-welcome`; three carry no command because one published guide per command and audience is the rule), self-test run **#69** (131 ok, 25 cards, `selftest_purge_minutes` 1 → 30 → 1), **eleven fresh v166 captures** (`media/42`–`52`) and **three redrawn mocks** (`media/53`–`55`: golive-announce, modmail-ticket, birthday-set — the real cards name members), nothing pressed in Discord.

**Verified:** `/api/guides/stale` **0** at 19:59 (was 12); `media/53` answers 200; the event-propose page seen rendered with its new picture and wording; the seed change's seven hard-coded test counts updated and the full suite green (8,399 passed + 3 skipped).

**NOT verified / left:** the seed and test edits are on `main` and NOT deployed (live guides were edited directly, so live is consistent; the seed matters for a fresh server); six older pictures (apply-form, mod-case, automod-arm, honeypot-set, rolemenu-post at v121, voice-room at v127) were not stale and not re-shot; `minutes-take` and six new staff guides have no picture (no self-test card exists for them); hiding `/rolemenu` while role menus are off is a code change still on TODO.

- 📖 **GUIDES — redo the stale ones at the end of the day (owner, 2026-09-25 17:2x Phoenix, verbatim: *"once everything is done for the day, we need to redo the stale guides after the most recent set of changes plus old changes weve made too. maybe do a guide audit now so we can fix that"*).** Two halves: (1) an AUDIT now — the bot's own `/api/guides/stale` list plus a read of every guide's wording against what the commands and panels do today (v165 + the merged, undeployed builds); (2) the RE-SHOOT after v166 deploys, per [`access/guides-capture.md`](access/guides-capture.md) (the operator token reads the stale list; the self-test posts the cards; the owner's Discord tab is where they are shot, pressing nothing). Status: ✅ audit DONE 17:4x — [`info/guides-audit-2026-09-25.md`](info/guides-audit-2026-09-25.md) (`d81047dc`): 12 stale shots (9 re-shot from 8 self-test cards, 3 mocks), 11 of 80 steps drifted + 10 fault answers, 8 missing guides drafted; ⚠️ live guide text is a DB row — fix on each guide's page AND in the seed, never press *Put the original back*. Three calls to make at the re-shoot unless the owner objects: pings-follow → audience member; the seven *test mode is on* fault answers rewritten; `/rolemenu` hidden while role menus are off. The re-shoot waits for v166 live. **→ UNBLOCKED 2026-09-25 19:30: v166 is LIVE (release commit `b06d4f1d`) — the re-shoot is next.**
  ↳ **DONE 2026-09-25 20:0x** as above.

## 2026-09-25 — v166: the Events page made plain, People, the event drawer, the lighter marathon drawer, rehearsal homes, BaF

**v166 (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`, `changed_features` birthday, events, golive, marathon, modmail, poll, posts; **8399 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 252 routes**; schema **64 → 65** (`marathon_spotlights`, a new table; `SCHEMA_VERSION` 65 read in `black_bloc/storage/db.py`), keys **482 → 491** (six shadow homes + three spotlight keys; 491 by importing `settings_store.KEY_TYPES` at HEAD), routes **250 → 252**, pages **22**, `TOP_LEVEL_NOW` **33**, cogs **24** (`len(bot.COGS)` by import), deploys log **165** lines).** SIX merges over v165: `marathon-ux` merge **`c86b9660`** 16:53 (5 commits, 343k Opus); `shadow-home-per-feature` merge **`0fecffa9`** 17:31 (4 commits, 334k Opus); `marathon-people` merge **`afba30b2`** 17:59 (3 commits, 490k Opus); `event-drawer` merge **`0eb2650d`** 18:58 (3 commits, 210k Opus); `marathon-drawer-lite` merge **`213fe142`** 19:12 (4 commits, 216k Opus); `event-links` merge **`b0401e0c`** 19:15 (3 commits, 112k Opus) — **1,705k Opus** for the six. Plus two small changes straight on `main`: the Go-live drawer's Announcements / Remove cards are buttons only (`b5427c62`) and the Marathons box's two waiting strips are gone (`4b37c659`). Commit counts by `git rev-list --count`, merge times by `git log`. Designs: [`info/marathon-ux-design.md`](info/marathon-ux-design.md), [`info/shadow-home-per-feature-design.md`](info/shadow-home-per-feature-design.md), [`info/marathon-people-design.md`](info/marathon-people-design.md), [`info/event-drawer-design.md`](info/event-drawer-design.md), [`info/marathon-drawer-lite-design.md`](info/marathon-drawer-lite-design.md). Sweeps `MX`, `SH`, `MP`, `ED`, `ML`. Shipped 19:30 Phoenix (02:30Z). Pre-deploy DB snapshot `backup-2026-09-25-pre-v166.sqlite3` (3.59 MB, 19:28).

The launch waited on a flyctl re-login: the token vanished at 19:14 during the pre-deploy snapshot's `flyctl ssh console`, and the owner re-authorised remotely at 19:2x with a token on his side (the 🔴 item below; the gotcha is now in [`access/deploy.md`](access/deploy.md)). After the boot, the conductor set the three rehearsal-home keys through the site at 19:3x, as the owner asked at 17:0x: `shadow_channel_id` → #blackbloc-logs, `frontdoor_shadow_channel_id` and `posts_shadow_channel_id` → #welcome-test — each answered 200 and read back. `marathon_mode` stays **shadow**, so marathon rehearsals now land in #blackbloc-logs.

Verified (the conductor's reads, recorded in [`deploys.log`](deploys.log)): boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**; `release.json` v166 at `9b84b06e` (re-read by this ritual from `b06d4f1d`). No `database: added` line is expected — the one schema change is a new table. The retiring process's `asyncio: Unclosed client session` line (KI-10) was NOT read — `flyctl logs --no-tail` at 19:3x starts at 02:32:34Z, after the boot.

⚠️ **NOT verified:** nothing has met Discord by hand — sweeps `MX`, `MP`, `ML`, `ED`, `SH` are the owner's; no rehearsal copy has yet been seen in its new home; no browser opened the LIVE site (every render and press above was on the mock); no live link in the event drawer was clicked; `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's); the three key values were not re-read by this ritual (the conductor's read-back); this ritual made no live API call of its own; `TEST_MODE` was not touched.

**The TODO items, moved whole** (each with its ↳ line):

- 🎨 **THE EVENTS PAGE, MADE PLAIN (owner, 2026-09-25 16:2x–16:3x Phoenix, verbatim: *"the ui is confusing now in the modal, and i dont see where we're scanning for schedules."* → *"i think the whole events page and the marathon modal are confusing to look at."* → *"and for marathon highlights don't say ours say BaF as the category"*).** Looked at in Chrome on the mock: six sections, two settings, two logs, two things called *Next up*, eight controls per feed row, the marathon drawer leading with a seven-line Event explanation while the schedule reading was a grey header fragment. Design: [`info/marathon-ux-design.md`](info/marathon-ux-design.md) — three sections (Events · Marathons · Settings and logs), the marathon table first with the feeds in a shut *Where marathons come from* foldout, ONE Next up, the drawer led by a **Schedule** card (source · last read · next read · runs · the re-read field · Read it now), the Event card down to one line + one select, a Posts line, Pause/Remove last; **BaF** everywhere a person read *ours* (chips, columns, the member view, the board/reminder/shout DEFAULT templates); `/event` ▸ Marathons… mirrors the order. No schema, no keys, no routes. Status: ✅ **MERGED to `main` 2026-09-25 17:0x (merge `c86b9660` of branch `marathon-ux`, 5 commits, 343k Opus), NOT deployed** — gate: 8,324 passed + 3 skipped both orders, ruff clean, 14 node tests (a new `marathon-words.test.mjs`, wired into `ci.yml` and `deploy.ps1`), `check.mjs` 22 pages / 250 routes, headless renders of the page, the drawer and the feed drawers at 1400 and 390 px with zero console errors; one row field added (`next_read_at`, one helper shared with `fetch_due`). Looked at by Fable in Chrome on the mock 17:0x: three sections in the rail, the marathon table first, the Sources foldout, the AGDQ 2027 drawer opening on **Schedule** (source · last read · next read · 11 runs · 3 BaF · the interval field · Read it now), then Runs with the BaF / All chips. Three template DEFAULTS changed to BaF (board head, board empty line, event description); a stored value is left alone. 10 deviations; sweeps `MX-a…MX-f`. Nothing has met Discord; the panel order is tests-only. Owner's check: http://localhost:8797/events.html and `#marathon-1`.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; merge `c86b9660` 16:53, 5 commits by `git rev-list`; no schema, keys or routes of its own). ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE Events page; the three BaF template DEFAULTS have not been seen in a posted board, reminder or event description; the panel order is still tests-only; sweeps `MX-a`…`MX-f` are the owner's.
- 🎭 **A REHEARSAL HOME PER FEATURE (owner, 2026-09-25 17:0x Phoenix, verbatim: *"you posted the rehersal stuff in the welcome test, should have gone to lackblocl logs"* → *"for now make post go to welcome test, and everything else go to logs channel"*).** One global `shadow_channel_id` (#welcome-test) cannot say that. Design: [`info/shadow-home-per-feature-design.md`](info/shadow-home-per-feature-design.md) — a `<feature>_shadow_channel_id` override per rehearsing feature, blank = the global, read by the one helper in `shadow.py`; after the deploy the conductor sets the global to #blackbloc-logs and the front door's to #welcome-test through the site. **Plus §E (owner 17:0x: *"also pause it and remove it, thats not enough control over the schedule or enough detail"*): the feed / next-event notice becomes an embed (When · Read from · Channel · Schedule · Event · Found by) with Pause it / Remove it / Read it now, an Event-mode select, and Manage… (opens the marathon card ephemeral) + Open on the site.** No schema. Status: ✅ **MERGED to `main` 2026-09-25 17:3x (merge of branch `shadow-home-per-feature`, 4 commits, 334k Opus), NOT deployed** — gate: 8,358 passed + 3 skipped both orders, ruff clean, 14 node tests, `check.mjs` 22 pages / 250 routes; keys **488** (SIX overrides, not eleven — modmail's ticket follows the door's home, minutes has no shadow, tempvoice/honeypot/automod post no copy; go-live's covers spotlights); rehearsal rows carry `shadow_home`; the feed-added notice is the embed with three rows (the suggestion / next-event notices keep Add it / Not this one — no marathon exists yet). ⚠️ After the deploy the conductor sets: `shadow_channel_id` → #blackbloc-logs, `frontdoor_shadow_channel_id` AND `posts_shadow_channel_id` → #welcome-test (the door looks for the rules post's rehearsal copy in its own home, so the pair stays together — Deviation 14). Nothing has met Discord; sweeps `SH-a…SH-c`. Rides as v166.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; merge `0fecffa9` 17:31, 4 commits by `git rev-list`; the six `<feature>_shadow_channel_id` keys are part of the registry's **482 → 491**, no schema). ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. **The three keys SET through the site at 19:3x** by the conductor, as the owner asked at 17:0x (*"for now make post go to welcome test, and everything else go to logs channel"*): `shadow_channel_id` → #blackbloc-logs, `frontdoor_shadow_channel_id` and `posts_shadow_channel_id` → #welcome-test — each answered 200 and read back (the conductor's report, recorded in the moved 🔴 bullet; `deploys.log`'s v166 line was written before they were set and says so). ⚠️ **NOT verified:** no rehearsal copy has yet been seen landing in #blackbloc-logs, nor a front-door / posts copy in #welcome-test under the new keys; the feed-added notice embed (Pause / Remove / Read it now, the event select, Manage…, Open on the site) has not been seen in Discord; sweeps `SH-a`…`SH-c` are the owner's.
- 👥 **MARATHON PEOPLE — the source as a column, and a marathon opens on its people, BaF first (owner, 2026-09-25 17:1x Phoenix, verbatim: *"hm for the events page maybe we need to get rid of there where marathon comes from and just put that as a column in the marathons setion. we also need to be able to click into it and see which runners are running in a list with the top section of that list set aside for BaF members. that way we can manually click other runners to link them and mark them as BaF runners or just spotlight other runners"*).** Design: [`info/marathon-people-design.md`](info/marathon-people-design.md) — the *Where marathons come from* foldout goes, **Source** is a table column, the feeds' controls sit behind a *Sources…* button and the Schedule line's *found by* link; the drawer opens **Schedule · People · Runs …** where People lists everyone on the schedule in two blocks — **BaF** (matched members) and **Everyone else** — each row with **Link to a member…** (the pairing, moves them up and makes their runs BaF) and **Spotlight…** (a channel-only spotlight row through the existing path, dated to the marathon, remembered so it can be stopped); `/event` ▸ Marathons… ▸ People…. One small table, three keys. **17:2x: the schedule block is by DAY (foldouts, today open) and by SLOT (a race shares a row; opening a slot lists its people to link or spotlight), the Runs card retires into it, six folded suggestions (§B2); owner: *"build it"*.** Status: ✅ **MERGED to `main` 2026-09-25 18:0x (merge of branch `marathon-people`, 3 commits, 490k Opus; seven conflicts with `shadow-home-per-feature` resolved by hand — the People… button placed on the notice's Manage row), NOT deployed** — gate on the merged tree by the conductor: **8,394 passed + 3 skipped**, ruff clean, 18 node tests, `check.mjs` 22 pages / 252 routes; schema **65** (`marathon_spotlights`), keys **491**, +2 routes; all six §B2 items landed (near-miss *looks like @name — Link?*, day headers count BaF, a race shouts once, the member view is the BaF block, the notice's People…, spotlight from a slot uses that run's window); Who is who retired into People; the free-text Pair form is gone (the panel keeps Pair a runner…). Renders: BaF block on top, today open, a 4-runner race as one row with five lines inside. Nothing has met Discord; sweeps `MP-a…MP-f`. Two ideas NOT built unless asked: a daily day-board post; a member's runs on their Go-live row. Rides as v166.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; merge `afba30b2` 17:59, 3 commits by `git rev-list`). Schema **64 → 65** — `marathon_spotlights` is a NEW TABLE, so the boot log has no `database: added` line for it (it names added COLUMNS only); `SCHEMA_VERSION` **65** read in `black_bloc/storage/db.py` at HEAD, keys **491** by importing `settings_store.KEY_TYPES`, routes **250 → 252**. ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. ⚠️ **NOT verified:** nothing has met Discord by hand; no runner has been linked or spotlighted from People on the live site; the four live GDQ marathons still have unpublished schedules, so no live People list has been read; sweeps `MP-a`…`MP-f` are the owner's.
- 🪟 **AN EVENT OPENS IN A DRAWER (owner, 2026-09-25 18:0x Phoenix, verbatim: *"the outer page looks good … now we need to work on the modal that opens when you click on an event"*).** Looked at: Open grows a SECTION with sixteen fact rows (seven of them `—` / `no` for a pending event), the decision buttons only on the queue row, the whole edit form open below. Design: [`info/event-drawer-design.md`](info/event-drawer-design.md) — a drawer like the marathons: header (status · asked by · when), **The event** (only the facts that are set, Where as a link with Spotlight this stream when it applies, Review, Announced, Discord event), **Decide** (Approve / Deny… / Cancel… by the allowed-transition rule, or the decision made), **Marathon** when linked, **Change it** as a shut foldout whose Save keeps the drawer open, a quiet foot. No routes, keys or schema. Status: ✅ **MERGED to `main` 2026-09-25 18:2x (merge of branch `event-drawer`, 3 commits, 210k Opus), NOT deployed** — gate: 8,394 passed + 3 skipped, ruff clean, 18 node tests, `check.mjs` 22 pages / 252 routes, renders at 1400 and 390 px with zero console errors, Approve pressed and the drawer redrew; a new `event-drawer.js` (463 lines), `page-events.js` 401 → 188 lines; deep link `#event-<id>` (`#detail=` rewritten). Deviations to know: Announced / Discord event / forum post are WORDS not links (the row carries no ids the site can link — a route change if wanted); a decided event shows *Decided by…* AND the moves still allowed; the mock's spotlight route has no contract entry; two `code-notes` anchors flagged stale for the next re-key. Nothing has met Discord; sweeps `ED-a…ED-d`. Rides as v166.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; merge `0eb2650d` 18:58 by `git log` — the status above says 18:2x, the merge commit's own time is 18:58; 3 commits by `git rev-list`; no routes, keys or schema). ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. Deviation 1 (the words-not-links) was closed by the 🔗 *event-links* item below, which shipped in the same release. ⚠️ **NOT verified:** no browser opened the LIVE Events page or any event's drawer; Approve / Deny / Cancel have not been pressed live; sweeps `ED-a`…`ED-d` are the owner's.
- 🔗 **Event drawer: Announced, Discord event and the review post as LINKS (owner, 2026-09-25 19:0x Phoenix, verbatim: *"make them links"*, answering the event-drawer build's Deviation 1).** The event row gains `announce_url`, `scheduled_event_url` and `review_url` built server-side from the guild id and the stored ids (`https://discord.com/channels/<guild>/<channel>/<message>`, `https://discord.com/events/<guild>/<event>`, the forum post's thread URL); the drawer renders them as links; contract + mock rows. Status: ✅ **MERGED to `main` 2026-09-25 19:2x (merge of branch `event-links`, 3 commits, 112k Opus), NOT deployed** — gate: 8,399 passed + 3 skipped, ruff clean, 18 node tests, `check.mjs` 22 pages / 252 routes; one helper `api/tools/events.py:event_links` feeds both routes: `announce_url` (the announce CHANNEL is the `events_announce_channel_id` setting read at request time — the row stores only the message id — so changing that setting after an announcement breaks the old link, sweep `ED-e`), `scheduled_event_url`, `review_url` (room or forum thread, same shape). No URL was clicked. Rides as v166.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; merge `b0401e0c` 19:15, 3 commits by `git rev-list`; the rows gain three read-only fields, no schema or keys). ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. ⚠️ **NOT verified:** no live link has been clicked — Announced, the Discord event and the review post; the stale-announce-link case (changing `events_announce_channel_id` after an announcement) is sweep `ED-e`, the owner's.
- 🪶 **THE MARATHON DRAWER, LIGHTENED (owner, 2026-09-25 18:1x Phoenix, verbatim: *"The whole modal for a marathon is a lot, can we rework it"*).** Looked at: three screens — a seven-line Schedule card, three lines per BaF person, then Event / The channel / Posts cards with help paragraphs. Design: [`info/marathon-drawer-lite-design.md`](info/marathon-drawer-lite-design.md) — a two-line header (state · dates · source ↗ · feed / reading · counts · event · channel), People as the body with ONE line per BaF person that opens to its moves, the day foldouts unchanged, ONE shut *Settings for this marathon* foldout (Event, Airs on, Re-read every, one Save), Posts as one quiet line, the moves bar last (Read it now · Pause · Remove). Fixed chrome ~40 lines → ~8. No keys, routes or schema; `/event` ▸ Marathons… mirrors the header. Status: ✅ **MERGED to `main` 2026-09-25 19:1x (merge of branch `marathon-drawer-lite`, 4 commits, 216k Opus), NOT deployed** — gate: 8,394 passed + 3 skipped, ruff clean, 20 node tests, `check.mjs` 22 pages / 252 routes, renders clean; the drawer is **41% shorter** at 1400 px (2177 → 1290) and 27% at 390; Save and Spotlight… pressed on the mock. Deviations to know: Save sends only changed fields and the Event select no longer saves on change; the route applies fields one after another (a refused later field leaves an earlier one saved); the panel card lost its Runs/Event/Channel heads and the *Event mode:* line; `code-notes` rows naming the retired `scheduleCard` / `drawerCards` are flagged for the re-key. Nothing has met Discord; sweeps `ML-a…ML-d`. Rides as v166.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; merge `213fe142` 19:12, 4 commits by `git rev-list`; no keys, routes or schema). ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. ⚠️ **NOT verified:** no browser opened the LIVE marathon drawer (the 41% figure is the mock's, at 1400 px); Save and Spotlight… were pressed on the mock only; the `code-notes` rows naming `scheduleCard` / `drawerCards` were not re-keyed by this ritual; sweeps `ML-a`…`ML-d` are the owner's.
- ✂️ **Go-live channel drawer: Announcements and Remove cards are buttons only (owner, 2026-09-25 17:1x Phoenix, verbatim: *"in golive we dont need the text in remove section and annoucement section just the button"*).** ✅ **DONE on `main` 17:1x, NOT deployed** — `page-golive.js` builds both cards from their move bars alone; the state sentence helper and the *Nobody here* constant went with it. Verified in Chrome on the mock: Announcements = *Opt out of announcements* + *Marathons: on / off*; Remove = *Remove this channel*. `check.mjs` 22 pages / 250 routes. Rides with v166.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; the change is `b5427c62` 17:09 on `main`, no merge). ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. ⚠️ **NOT verified:** looked at on the mock only — no browser opened the LIVE Go-live drawer.
- ✂️ **Marathons box: the two waiting strips are gone (owner, 2026-09-25 18:0x Phoenix, verbatim: *"the 1 marathon has a next event waiting: Halo Fest / 1 new event from a source is waiting for staff: Sources… at the bottom of the marathon box is undeeded"*).** ✅ **DONE on `main` 18:0x, NOT deployed** — both strip builders and their two constants removed from `marathons-section.js`; the facts still live in the marathon drawer's Schedule card and in the Sources drawer. `check.mjs` 22 pages / 252 routes. Rides with v166.
  ↳ **LIVE as v166 19:30** (release commit `b06d4f1d`; `release.json` says `v166` at `9b84b06e`; the change is `4b37c659` 18:42 on `main`, no merge). ✅ **VERIFIED at boot:** then boot log `database ready` **02:30:22Z**, `synced 33 app commands`, `logged in as Black_Bloc` **02:30:27Z**, no Traceback; `/health` ready=true, latency **65 ms**. ⚠️ **NOT verified:** no browser opened the LIVE Marathons box.
- 🔴 **v166 DEPLOY BLOCKED — flyctl signed out (2026-09-25 19:14 Phoenix).** `flyctl auth whoami` answers *no access token available* since 19:14:49 (`~/.fly/config.yml` was rewritten at that second, during the pre-v166 snapshot's `ssh console`; the v165 deploy at 16:12 had worked). Only the owner can `flyctl auth login` (a real terminal + the browser). Everything for v166 is MERGED and GREEN on `main` `b0401e0c` (8,399 passed + 3 skipped, ruff, 20 node tests, `check.mjs` 22 pages / 252 routes, all re-run on the merged tree): the Events page rework, BaF wording, the People card, the lighter marathon drawer, the event drawer with links, the go-live drawer trim, the notice embed, the rehearsal homes. The pre-v166 DB snapshot was NOT taken (the same login loss) — take it right after the login, before the deploy. After boot: set `shadow_channel_id` → #blackbloc-logs, `frontdoor_shadow_channel_id` + `posts_shadow_channel_id` → #welcome-test through the site. Owner notified 19:2x. **→ RESOLVED 19:2x: the owner re-authorised flyctl remotely (a token on his side); v166 DEPLOYED 19:30 (release commit `b06d4f1d`, boot verified: database ready 02:30:22Z, 33 commands, logged in 02:30:27Z, /health 65 ms, release.json v166); the pre-v166 snapshot WAS taken first (3.59 MB, 19:28); the three rehearsal-home keys SET through the site at 19:3x (`shadow_channel_id` → #blackbloc-logs, `frontdoor_shadow_channel_id` and `posts_shadow_channel_id` → #welcome-test, each answered 200 and read back). Docs ritual next.**
  ↳ **CLOSED by v166 19:30** (release commit `b06d4f1d`; the block was recorded at `9b84b06e` 19:18). `deploys.log`'s v166 line: *the launch itself waited on a flyctl re-login - the token vanished at 19:14 during the pre-deploy snapshot, the owner re-authorised remotely 19:2x*; gate green on the FIRST run; snapshot `backup-2026-09-25-pre-v166.sqlite3` (3.59 MB, 19:28), taken before the launch. The gotcha is now one line in [`access/deploy.md`](access/deploy.md) (*flyctl can lose its login mid-day*). ⚠️ **NOT known:** WHY the token vanished — only that `~/.fly/config.yml` was rewritten at 19:14:49 during `flyctl ssh console`; not reproduced.

## 2026-09-25 — v165: marathon feeds (GDQ + RPGLB), event modes, ESA opted out, CI green again

**v165 (release commit `2351311a`; `release.json` says `v165` at `e121d984`, `changed_features` events, golive, marathon; **8322 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 250 routes**; schema **62 → 64** (seven added columns + two new tables; `SCHEMA_VERSION` 64 read in `black_bloc/storage/db.py`), keys **469 → 482** (by importing `settings_store.KEY_TYPES` at HEAD), routes **237 → 250**, pages **22**, `TOP_LEVEL_NOW` **33**, cogs **24** (`len(bot.COGS)` by import), deploys log **164** lines).** THREE merges over v164: `marathon-feeds` merge **`fafdb5e4`** 15:21 (9 commits, 552k Opus, 28 deviations; SS4C parked in Deviation 25, now answered by the design's §H — a queued follow-up, NOT shipped); `marathon-event-modes` merge **`ba513bb9`** 16:11 (4 commits, 609k Opus, 16 deviations; also closed feeds Deviation 21); `ci-linux-hang` merge **`fe81a137`** 15:54 (4 commits, 155k Opus; tests only). Commit counts by `git rev-list --count`. Designs: [`info/marathon-feeds-design.md`](info/marathon-feeds-design.md), [`info/marathon-event-modes-design.md`](info/marathon-event-modes-design.md), [`info/ci-linux-hang.md`](info/ci-linux-hang.md). Sweeps `MF-a`…`MF-e`, `MV-a`…`MV-f`. Shipped 16:15 Phoenix (23:15Z). Pre-deploy DB snapshot `backup-2026-09-25-pre-v165.sqlite3` (3.54 MB, 16:12).

First boot (action rows through the operator token, recorded in [`deploys.log`](deploys.log)): `golive.channel_marathons_set esamarathon from=True to=False` at 23:15:07Z — **ESA opted out of marathons**; `marathon.feed_seeded feeds=['GDQ','RPG Limit Break']`; the GDQ feed ADDED four marathons (Games Done Hitless 2026-10-23, Games Done Quick Express 2026 2026-11-13, Halo Fest, Awesome Games Done Quick 2027), each schedule unpublished (`marathon.fetch_failed reason=unpublished`, a 404 for the runs, expected), each with a notice to `#welcome-test` under shadow (`marathon_mode` is shadow); `marathon.feed_checked` for both feeds; **no event wish made** (`event_mode` none).

CI: every run on `main` from 2026-09-22 21:24 Phoenix to the fix was red — the conductor's count is **41**; `gh run list` at this ritual counts **45** `failure` runs on `main` between the last green (`2026-09-23T04:24:40Z`) and `36198821056` (the difference not reconciled). Two causes: first the `tests.loopback` plugin import (`No module named 'tests'` — the `pytest` console script does not put the repo root on `sys.path`), fixed in `e8d57319` (14:17, `python -m pytest`); then the shared per-module aiosqlite connection killed by a query answered after a test's loop closed — KI-26's D8 shape — fixed by `ci-linux-hang`. **CI GREEN at `fc267114`, run `36198821056`** (15:54), and green again on the next two pushes.

Verified (the conductor's reads, recorded in [`deploys.log`](deploys.log); the boot-log lines re-read by this ritual through `flyctl logs --no-tail`): `database: added marathons.feed_id` / `marathons.event_mode` / `marathons.held_by_channel` / `marathon_runs.event_id` / `marathon_feeds.event_mode` / `marathon_feeds.held_by_channel` / `spotlight_channels.marathons` at **23:15:04Z**, `database ready`, `loaded cog marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **23:15:07Z**, no Traceback; `/health` ready=true, latency **64 ms**. The retiring process's `asyncio: Unclosed client session` line (KI-10) was NOT read — Fly's log buffer at 16:1x starts at 23:15:00Z with the new machine.

⚠️ **NOT verified:** nothing has met Discord by hand — sweeps `MF-a`…`MF-e` and `MV-a`…`MV-f` are the owner's; no run event has been approved against Discord; the staff notice has not been seen in forum mode (live is shadow — the notices went to `#welcome-test`, and nobody looked at them); no browser opened the LIVE site; `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's); this ritual made no live API call of its own; `TEST_MODE` was not touched.

**The TODO items, moved whole** (each with its ↳ line):

- 📡 **MARATHON FEEDS — the bot polls GDQ (and ESA on horaro.net) for events itself (owner, 2026-09-25 14:2x Phoenix, verbatim: *"lets add a feature where constantly poll gdq to get events, check if we can do that for any of the other channel only ones we have. if not GDQ is prio"* → 14:3x: *"only do marathons we have in channel though, not random ones"*).** Checked live 14:2x: GDQ's tracker events list is one JSON page; **ESA's whole archive is on horaro.net** (`esamarathon.com/schedule` says so) with a JSON list per event and a JSON per schedule — so ESA gets a feed too, through the Horaro reader parked in the schedule design, with the honest limit that ESA names players instead of linking them (name matching + pairings only). **14:4x, owner: *"check retro and rpglimit too"* — checked: RPG Limit Break runs the SAME tracker software as GDQ (`tracker.rpglimitbreak.com/api/v2/events/`, identical shape, event 21 `rpglb2026`) so it is the GDQ reader with a different base URL — free; Speed Stuff 4 Charity is on oengus.io (lines endpoint still unverified — last, only if cheap); RetroGamingLiveTV has NO readable schedule source anyone could find (site does not resolve, Twitch panels script-rendered, nothing on horaro/oengus) — no feed. Design: [`info/marathon-feeds-design.md`](info/marathon-feeds-design.md) — a `marathon_feeds` row PER CHANNEL ROW we already watch (never a free-standing source; seeded for GamesDoneQuick and ESAMarathon), checked every 6 h, each new event added as a marathon (or suggested, per the feed's action) with a staff notice carrying Pause it / Remove it, a removed marathon remembered so it is not re-added, the Feeds card on the Events page's Marathons section and `/event` ▸ Marathons… ▸ Feeds…. Status: ✅ **MERGED to `main` 2026-09-25 15:1x (merge `fafdb5e4` of branch `marathon-feeds`, 9 commits, 552k Opus), NOT deployed** — gate on the branch: 8,259 passed + 3 skipped both orders, ruff clean, `check.mjs` 22 pages / 246 routes; schema **63** (`marathon_feeds`, `marathon_feed_seeds`, `marathons.feed_id`), keys **475**, +8 routes, +20 kinds. Shipped: GDQ + RPGLB tracker feeds (seeded at boot, one per channel, once ever), the horaro.net reader (no ESA seed — opted out), a Discord-username match step in `match_people` (Deviation 17); SS4C/Oengus PARKED (the lines endpoint exists — `…/schedules/for-slug/1` — but no way to list a channel's marathons, Deviation 25); RGL nothing. ⚠️ **First boot will add the four GDQ events still ahead** (Games Done Hitless, GDQx 2026, Halo Fest, AGDQ 2027) with a staff notice each — and, under the live `marathon_makes_event = true`, an event wish each — so this DEPLOYS TOGETHER WITH `marathon-event-modes` (default: no event), never alone. Gaps for that build: a feed's name/channel is PATCH-only (Deviation 21). 28 deviations; sweeps `MF-a…MF-e`. Nothing has met Discord.
  ↳ **LIVE as v165 16:15** (release commit `2351311a`; `release.json` says `v165` at `e121d984`; merge `fafdb5e4` 15:21, 9 commits by `git rev-list`). ✅ **VERIFIED at boot:** `database: added marathons.feed_id` at **23:15:04Z** (`marathon_feeds` + `marathon_feed_seeds` are the deploy's two new tables; the boot log names added COLUMNS only), then boot log `database ready`, `loaded cog marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **23:15:07Z**, no Traceback; `/health` ready=true, latency **64 ms**. First boot, read through the operator token (action rows in `deploys.log`): `marathon.feed_seeded feeds=['GDQ','RPG Limit Break'] action=add`; the GDQ feed ADDED four marathons — (1) Games Done Hitless 2026-10-23, (2) Games Done Quick Express 2026 2026-11-13, (3) Halo Fest, (4) Awesome Games Done Quick 2027 — each `marathon.fetch_failed reason=unpublished` (404 for the runs, failures=0, expected), each `marathon.would_feed_add` + `marathon.notice_posted home=shadow` into `#welcome-test` (`marathon_mode` is shadow), then `marathon.feed_checked` for both feeds; no ESA feed (opted out). SS4C/Oengus is NOT shipped — its follow-up is the design's §H, still on [`TODO.md`](TODO.md) as branch `marathon-feeds-oengus`. ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site; the four shadow notices in `#welcome-test` were not looked at; no schedule has published yet, so no run has been read from a feed-added marathon; sweeps `MF-a`…`MF-e` are the owner's.
- 🗓️ **A MARATHON'S EVENT MODE + the staff notice in Events (owner, 2026-09-25 14:4x Phoenix, verbatim: *"post it in events but dont make an event by default for a marathon, add a few options, 1 being make an event, 2 being make an event just for BaF runner (then update this event with the schedule api so it stays current), 3 skip event tracking. does that cover all use cases?"*).** Answer: yes, plus a free fourth (*both*) and one collision to settle — a run with its own event is ALSO announced by the events feature when it starts, so the marathon shoutout for that run is skipped by default (a key turns both on). Design: [`info/marathon-event-modes-design.md`](info/marathon-event-modes-design.md) — `event_mode` per marathon (and per feed): `none` (the new default) / `marathon` / `runs` (an approved event per run of ours, re-dated as the schedule moves, cancelled when the run drops) / `both`; `marathon_makes_event` retired for `marathon_event_mode_default`; the staff notice becomes a post in the Events forum (`marathon_notice_home` = events; staff channel in room mode). **Owner 14:5x: *"lets opt esam out of events"* → 15:0x: *"yes have esam opted out for marathons, make sure any channel only can opt out of marathons"* — §A2: a `marathons` on/off toggle on every channel row (Go-live drawer + `/golive` ▸ Channels…), ESA seeded OFF, no feed and no marathon on an opted-out row; the running feeds build was told (a narrowing) not to seed the ESA feed.** Status: ✅ **MERGED to `main` 2026-09-25 16:0x (merge `ba513bb9` of branch `marathon-event-modes`, 4 commits, 609k Opus), NOT deployed** — gate on the branch: 8,322 passed + 3 skipped both orders, ruff clean, `check.mjs` 22 pages / 250 routes; schema **64** (six columns + a once-only backfill: a marathon that had an event becomes `marathon`), keys **482** (`marathon_makes_event` retired), +2 routes, +10 kinds; feeds Deviation 21 closed (Rename… / Move to channel… on a feed). Deviations that matter: run events skip the review only while `marathon_mode = on` (live is shadow → they go through the review for now); `event_id = 0` marks a run staff unlinked; the forum tag word `marathon` is a constant (flagged against the every-word rule). First boot: opts the live ESA row out (holding anything on it), backfills, and from then on no marathon makes an event unless its mode says so. Nothing has met Discord; sweeps `MV-a…MV-f`. Deploys WITH `marathon-feeds` as v165.
  ↳ **LIVE as v165 16:15** (release commit `2351311a`; `release.json` says `v165` at `e121d984`; merge `ba513bb9` 16:11, 4 commits by `git rev-list`). ✅ **VERIFIED at boot:** `database: added marathons.event_mode` / `marathons.held_by_channel` / `marathon_runs.event_id` / `marathon_feeds.event_mode` / `marathon_feeds.held_by_channel` / `spotlight_channels.marathons` at **23:15:04Z** (schema **64**, `SCHEMA_VERSION` read at HEAD; keys **482** by import), then boot log `database ready`, `loaded cog marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **23:15:07Z**, no Traceback; `/health` ready=true, latency **64 ms**. First boot: `golive.channel_marathons_set esamarathon from=True to=False` at **23:15:07Z** — ESA opted out of marathons; no event wish made (`event_mode` none). ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site; no run event has been made or approved against Discord; the staff notice has not been seen in forum mode (live is shadow — the four notices went to `#welcome-test`); sweeps `MV-a`…`MV-f` are the owner's.
- 🟢 **CI green again? (owner, 2026-09-25 14:1x Phoenix, verbatim: *"i keep getting spammed with emails, probably from before, of ci fails. we gotta fix this. it happens everytime we make code"*).** Every CI run since 2026-09-22 21:24 failed with `ImportError: Error importing plugin "tests.loopback": No module named 'tests'` — the `pytest` console script does not put the repo root on `sys.path` — **40 red runs**, each one e-mailed to the owner. Fix pushed: `e8d57319` 14:17 (`.github/workflows/ci.yml` runs `python -m pytest -q -n auto`). Status: **still RED, but a NEW failure** — run `36190704964` (read 14:23 by `gh run view`): the `tests.loopback` ImportError is GONE and the suite ran (**8166 passed, 1 skipped, 2 failed** in 5m22s), the two failures being pytest-xdist worker crashes (`node down: Not properly terminated`) in `tests/cogs/content/test_spotlight.py::test_a_cog_load_with_no_guilds_yet_leaves_the_boot_to_on_ready` (gw3) and `tests/cogs/moderation/test_modmail.py::test_two_reconciles_at_boot_post_exactly_one_ticket_button` (gw4) — the local deploy gate passed both; cause not investigated by this ritual, so the owner is still getting a failure e-mail per push. The e-mails stop only when a run on `main` is green; if it is still red, read the failing step before anything else. Also worth a look once green: whether GitHub's *failed workflow* e-mail setting should stay on (the owner's account setting, his call).
  ↳ **CI GREEN** run `36198821056` at `fc267114`, 15:54 (read by `gh run list` 16:1x: `success`, 3m27s); the fix is tests-only and shipped inside v165's image (merge `fe81a137` of branch `ci-linux-hang`, 4 commits by `git rev-list`: the `db` fixture in `tests/conftest.py` settles the module connection before a test's loop closes; CI runs `--timeout-method=signal`; account in [`info/ci-linux-hang.md`](info/ci-linux-hang.md); KI-26 stays `WATCHING`). The next two pushes also ran green (`36200070117` at `e121d984`, `36200184603` at `2351311a`). ⚠️ **NOT verified:** the run for `963db247` (`36200380794`) was still in progress at this ritual; whether the owner's failure e-mails actually stopped was not asked.

## 2026-09-25 — v164: spotlight ping windows, GDQ marathon schedules (request #12), the next-event suggestion, marathons on the Events page

**v164 (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce`, `changed_features` core, events, golive, guides, marathon, selftest; **8166 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 237 routes**; schema **58 → 62** (five added columns + four new tables; `SCHEMA_VERSION` 62 read in `black_bloc/storage/db.py`), keys **419 → 469** (measured by importing `settings_store.KEY_TYPES` at HEAD), routes **216 → 237**, pages **22 → 22** (`marathons.html` came and went; `channels.html` off the nav, still on disk), `TOP_LEVEL_NOW` **33**, cogs **24** (`len(bot.COGS)` by import), deploys log **163** lines).** FOUR merges over v163, plus fixes on `main`: `spotlight-ping-windows` (9 commits `7682bfa4..69198341`, landed LINEAR — the pre-push `git pull --rebase` flattened merge `f25561ee`, see [`access/deploy.md`](access/deploy.md); 426k Opus) with the owner's two Go-live drawer fixes `57a5994a` + `fd881363`; `marathon-schedule` merge **`fb0e683e`** (9 commits, 592k Opus; request #12, moved to *in progress* by the owner 11:2x); `marathon-next-event` merge **`80553014`** (8 commits, 392k Opus); `marathon-events-page` merge **`56130106`** (5 commits, 469k Opus); the Channels row off the main nav `642ea8ce`. Commit counts by `git rev-list --count`. Designs: [`info/spotlight-ping-windows-design.md`](info/spotlight-ping-windows-design.md), [`info/marathon-schedule-design.md`](info/marathon-schedule-design.md), [`info/marathon-next-event-design.md`](info/marathon-next-event-design.md), [`info/marathon-events-page-design.md`](info/marathon-events-page-design.md). Sweeps `PW-a`…`PW-e`, `MS-a`…`MS-f`, `MN-a`…`MN-d`, `ME-a`…`ME-d`. Shipped 14:16 Phoenix (21:16Z). Pre-deploy DB snapshot `backup-2026-09-25-pre-v164.sqlite3` (3.49 MB, 14:13).

Verified (the conductor's reads, recorded in [`deploys.log`](deploys.log)): boot log `database: added spotlight_channels.ping_mode` / `spotlight_sessions.pinging_last` / `marathons.suggested_next` / `marathons.event_id` / `marathons.event_wanted` at **21:15:56Z**, `database ready`, `loaded cog spotlight` + `marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **21:16:01Z**, no Traceback; `/health` ready=true, latency **67 ms**. The retiring process logged `asyncio: Unclosed client session` at 21:15:49Z during the swap ([`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) ▸ KI-10, known). `marathon_mode` ships **shadow**.

Also today (a live fact, no item to move): the conductor, with the owner's word, linked **nine** members to their GDQ Twitch logins through the Go-live page (**41** links, was 32) and linked anarchy's YouTube channel. And on `main` but not a feature: the CI fix **`e8d57319`** 14:17 — `.github/workflows/ci.yml` runs `python -m pytest -q -n auto`; every CI run since 2026-09-22 21:24 had failed with `ImportError: Error importing plugin "tests.loopback": No module named 'tests'` because the `pytest` console script does not put the repo root on `sys.path` (40 red runs, each e-mailed to the owner). CI outcome: **still RED, but a NEW failure** — run `36190704964` (read 14:23 by `gh run view`): the `tests.loopback` ImportError is GONE and the suite ran (**8166 passed, 1 skipped, 2 failed** in 5m22s), the two failures being pytest-xdist worker crashes (`node down: Not properly terminated`) in `tests/cogs/content/test_spotlight.py::test_a_cog_load_with_no_guilds_yet_leaves_the_boot_to_on_ready` (gw3) and `tests/cogs/moderation/test_modmail.py::test_two_reconciles_at_boot_post_exactly_one_ticket_button` (gw4) — the local deploy gate passed both; cause not investigated by this ritual, so the owner is still getting a failure e-mail per push. Followed on [`TODO.md`](TODO.md) ▸ 🟢 *CI green again?*.

⚠️ **NOT verified:** nothing has met Discord by hand — the four sweep sets are the owner's; no browser opened the LIVE site; no ping window, marathon board, reminder, shoutout, next-event suggestion or marathon-made event has fired live; the nine Twitch links were not re-read by this ritual. `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's); this ritual made no live API call of its own; `TEST_MODE` was not touched.

**The TODO items, moved whole** (each with its ↳ LIVE line):

- 🔔 **SPOTLIGHT SPLIT FROM ITS PING ROLE — pings only during events (owner, 2026-09-25 10:4x Phoenix, verbatim: *"I want to have a spotlight split from its ping role, for example I want GDQ to always be spotlighted, but I only want it to ping the marathon role (which I will set later) during events. I want this to be customizable, its a lot of moving parts."*).** Design: [`info/spotlight-ping-windows-design.md`](info/spotlight-ping-windows-design.md) — a channel row gains a **ping mode** (`always` / `never` / `events`) and a list of **ping windows** (staff-typed date ranges; a marathon's schedule opens one automatically once the item below lands); outside a window an `events` row announces and bumps silently, and a window opening on an already-live channel posts one reminder WITH the pings. Status: ✅ **MERGED to `main` 2026-09-25 11:5x (branch `spotlight-ping-windows`, 9 commits `7682bfa4..69198341` — landed LINEAR, not as a merge commit: the pre-push `git pull --rebase` flattened merge `f25561ee`; content identical, see the `deploy.md` gotcha), 426k Opus, NOT deployed** — gate on the branch: 7,872 passed + 3 skipped both orders, ruff clean, `check.mjs` 22 pages / 219 routes, six node tests; schema **59**, keys **428**; Fable review closed deviation 5 (the window-open reminder needs spotlight ON). Sixteen deviations at the design's foot; sweeps `PW-a…PW-e`. Nothing has met Discord. Owner's check: the local mock, GamesDoneQuick row ▸ drawer ▸ **Pings** card; Settings ▸ Go-live ▸ *When a channel pings*.
  ↳ **LIVE as v164 14:16** (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce`; the 9 commits `7682bfa4..69198341`, landed linear). ✅ **VERIFIED at boot:** `database: added spotlight_channels.ping_mode` and `spotlight_sessions.pinging_last` at **21:15:56Z** (schema 59's columns; the windows table is one of the four new tables), then boot log `database ready`, `loaded cog spotlight` + `marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **21:16:01Z**, no Traceback; `/health` ready=true, latency **67 ms**. Final gate over all four merges: **8166 passed, 3 skipped**, ruff clean, `check.mjs` 22 pages / 237 routes, green on the FIRST run. ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site at this ritual — no ping window has opened live and no `events` row has announced silently yet; sweeps `PW-a`…`PW-e` are the owner's.
- 🐛 **Pings card on the Go-live page — two site bugs (owner, 2026-09-25 11:5x Phoenix, verbatim: *"for the golive menu when i change the ping from any status ex during events to never, it closes the modal. this is annoying keep it open. also the highlighting for the during events is extending the whole column and not just the button box"*).** (1) Changing the ping mode closes the row drawer — it must stay open and show the new state; (2) the *During events* segment's pressed highlight fills the whole column, not the button. Status: ✅ **FIXED on `main` 2026-09-25 12:1x, NOT deployed** — `page-golive.js:redrawRow` re-reads the channel row and draws the drawer again with the same notice (mode change, Add a window, Remove all go through it); `site.css` `.card-body > .seg { justify-self: start }`. Verified in Chrome against the local mock: Never → the drawer stayed open reading *Pings: never*, the pressed highlight sits on the button only. `check.mjs` 22 pages / 219 routes, join test green. Rides with the ping-windows deploy.
  ↳ **LIVE as v164 14:16** (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce`; the fix is `57a5994a` on `main`, 12:03). The deploy's `check.mjs` read 22 pages / 237 routes green. ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site at this ritual — the drawer-stays-open and the segment highlight were verified in Chrome against the local mock only (above), not on the live site.
- 🎨 **Go-live page — the channel drawer's Spotlight card (owner, 2026-09-25 12:0x Phoenix, verbatim: *"for spotlight remove the text at the top of the modal and make the dates part seem like a sub section of spotlight not like its own section so its not confusing"*).** (1) Drop the state paragraph at the top of the Spotlight group (*Spotlighted — pinned while it streams … the kept / expires / bump / pin moves are the spotlight's …*); (2) Dates becomes a sub-section INSIDE the Spotlight group, not a card of its own. Status: ✅ **FIXED on `main` 2026-09-25 12:2x, NOT deployed** — the Spotlight card is now moves → a hairline → a small *Dates* label → the range and pickers (`page-golive.js:datesCard` returns a `.card-sub`, both row branches); the paragraph and its four constants are deleted (`spotlightQuiet` keeps only the note and the event line, which are facts, not explanation); the Spotlight moves and Save dates keep the drawer open through `redrawRow` like the Pings card. Verified in Chrome against the mock through the accessibility tree (Spotlight heading → four moves → Dates → Save dates); `check.mjs` 22 pages / 219 routes. Rides with the ping-windows deploy.
  ↳ **LIVE as v164 14:16** (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce`; the fix is `fd881363` on `main`, 12:11). The deploy's `check.mjs` read 22 pages / 237 routes green. ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site at this ritual — the Spotlight card was read in Chrome against the local mock only (above), not on the live site.
- 🗓️ **MARATHON SCHEDULES — request #12 "Dynamic Scheduling Shoutouts" (Sky, 2026-09-21; owner, 2026-09-25 10:4x Phoenix, verbatim: *"building the last request for highlighting when BaF members are on a marathon stream. We will need to be able to scrape a site that has the marathon schedule for twitch/username and add those to a dynamic schedule. The other caveat is that the schedules sometimes get delayed or mixed up, so we need to be constantly checking every maybe 30 - 1h?"*).** Design: [`info/marathon-schedule-design.md`](info/marathon-schedule-design.md) — a `marathons` row (name, schedule URL, the channel it airs on), readers for the GDQ tracker, Horaro and Oengus, a re-fetch every `marathon_poll_minutes` (30) that diffs the runs, runners matched to members through their Twitch links (staff can pair by hand), one edited **board** post per marathon, reminders before each member's run, and a **shoutout** when the run is on — confirmed by the channel's live title so a late schedule does not fire early. Depends on the ping-windows item above (a running marathon opens its channel's ping window). **Owner, 11:1x: *"Lets start with the GDQ since thats the biggest one. The rest we can look at in the future"* — the GDQ tracker reader only (verified live: `tracker.gamesdonequick.com/tracker/api/v2/runs/?event=<id>`, 500 a page, runners carry `name` + `stream`); horaro.net and oengus.io are parked in the design's §E.** Status: ✅ **MERGED to `main` 2026-09-25 12:3x (merge `fb0e683e` of branch `marathon-schedule`, 9 commits, 592k Opus), NOT deployed** — gate on the branch: 8,069 passed + 3 skipped both orders, ruff clean, `check.mjs` 23 pages / 231 routes, six node tests; schema **60**, keys **464**, cog **24**, `/marathon`, page `marathons.html`, log feature `marathon`; `marathon_mode` ships **shadow**. ⚠️ Two build findings: the tracker's runs route is `/api/v2/events/<id>/runs/` (the design's `?event=` form returned every GDQ run since 2011), and **AGDQ 2027's schedule is not published yet** (events 71–74 are drafts; adding it is accepted and re-read until GDQ publishes). 21 deviations at the design's foot; sweeps `MS-a…MS-f`; two flagged gaps (a `poll_minutes` door; a done run's move back) folded into the next-event follow-up (§G). Nothing has met Discord. Owner's check: the local mock, http://localhost:8797/marathons.html. Request #12 moved to *in progress* by the owner 2026-09-25 11:2x.
  ↳ **LIVE as v164 14:16** (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce`; merge `fb0e683e` 12:43, 9 commits by `git rev-list`). ✅ **VERIFIED at boot:** boot log `database ready`, `loaded cog spotlight` + `marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **21:16:01Z**, no Traceback; `/health` ready=true, latency **67 ms**. Schema 60's three tables (`marathons`, `marathon_runs`, `marathon_people`) are among the four new tables the deploy counted (schema 58 → 62); the boot log's `database: added …` lines name added COLUMNS only, so no boot line names these tables by itself. `marathon_mode` ships **shadow**. Superseded by the two merges below before it shipped: `marathons.html` is DELETED (a Marathons section on `events.html`) and `/marathon` is retired into `/event` ▸ Marathons…. ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site at this ritual; no GDQ tracker read by the live bot was read back; AGDQ 2027 is still unpublished on the tracker (per the build); sweeps `MS-a`…`MS-f` are the owner's.
- 🗓️ **MARATHONS — suggest the NEXT GDQ event when one ends (owner, 2026-09-25 12:0x Phoenix, verbatim: *"for the during marathon, especially for GDQ we should use the api to grab the next event and suggest that once the marathon build ends"*).** The GDQ tracker's `/api/v2/events/` lists every event with `id`, `short`, `name`, `datetime`; when a GDQ-sourced marathon goes over, read the list, pick the soonest event whose `datetime` is ahead, and SUGGEST it — a card on the Marathons page (*AGDQ 2027 is over — the next GDQ event is SGDQ 2027, 5 Jul. Add it?* with **Add it**) and one staff-channel notice with the same button; never added by itself (staff final say). Status: ✅ **MERGED to `main` 2026-09-25 13:2x (merge of branch `marathon-next-event`, 8 commits, 392k Opus), NOT deployed** — gate on the branch: 8,125 passed + 3 skipped both orders, ruff clean, `check.mjs` 23 pages / 235 routes; schema **61** (`marathons.suggested_next`), keys **468**; §G landed whole (a *Re-read every N minutes* field + `/marathon` modal; **Mark it upcoming** / **Mark it live** on a run, page + panel, with a staff HOLD the title match never undoes). Live facts from the build: the tracker lists 70 events newest-first, every event still ahead is `draft: true` (so drafts are KEPT, only `archived` dropped), `datetime` is the first run's start; today's *next* for any GDQ marathon is 71 *Games Done Hitless*. 23 deviations at the design's foot; sweeps `MN-a…`. Nothing has met Discord. Owner's check: the mock, http://localhost:8797/marathons.html — Halo Fest (#2) carries the Next up card — it expands that build's scope, so it is NOT sent to the running agent (mid-flight messages narrow, never expand). Design: ✅ written 12:2x — [`info/marathon-next-event-design.md`](info/marathon-next-event-design.md) (one `suggested_next` column, a pure `next_gdq_event` picker, a Next up card + staff notice with Add it / Not this one / Look again, four keys).
  ↳ **LIVE as v164 14:16** (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce`; merge `80553014` 13:20, 8 commits by `git rev-list`). ✅ **VERIFIED at boot:** `database: added marathons.suggested_next` at **21:15:56Z**, then boot log `database ready`, `loaded cog spotlight` + `marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **21:16:01Z**, no Traceback; `/health` ready=true, latency **67 ms**. ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site at this ritual; no marathon has gone over live, so no suggestion or staff notice has been posted; sweeps `MN-a`…`MN-d` are the owner's.
- 🗓️ **MARATHONS SHARE THE EVENTS PAGE (owner, 2026-09-25 12:5x Phoenix, verbatim: *"i think marathons and events can share a page since a marathon will be an event"*).** One surface, one home: `marathons.html` retires into a **Marathons** section on `events.html` (the same list, drawer, Add form and Next up card; the nav row and `FEATURE_PAGES['marathon']` move; `site.md` back to 23 → 22 pages), and, following the raid-train precedent (*a train that makes an event*, `raidtrain-page-design.md`), **adding a marathon also makes an Events row** — a scheduled event named after it, dated from the schedule, Where = the channel's Twitch URL, re-dated when the schedule's start/end move — behind an *Also make it an event* toggle on the Add form (default on, key `marathon_makes_event`); the event row links back to the marathon and vice versa. `/marathon` folds into `/event`'s panel as a **Marathons…** button (minimise slash commands), the command retired the way `/twitch` folded into `/golive`. Status: ✅ **MERGED to `main` 2026-09-25 14:1x (merge of branch `marathon-events-page`, 5 commits, 469k Opus), NOT deployed** — gate on the branch: 8,166 passed + 3 skipped both orders, ruff clean, `check.mjs` **22 pages** / 237 routes, six node tests, `events.html` rendered headless with zero console errors; schema **62** (`marathons.event_id`, `event_wanted`), keys **469** (`marathon_panel_minutes` retired), `TOP_LEVEL_NOW` **33** (`/marathon` gone), `marathons.html` deleted → `assets/marathons-section.js` on `events.html#marathon-<id>`. ⚠️ One design claim corrected by the build: `PUT /api/events/{id}` does NOT move the Discord scheduled event; a new `events.move_scheduled_event` does it for the marathon re-date. 23 deviations at the design's foot; sweeps `ME-a…ME-d`. Nothing has met Discord. Owner's check: the mock, http://localhost:8797/events.html (Marathons section under the Queue; AGDQ 2027 ↔ event #5).
  ↳ **LIVE as v164 14:16** (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce`; merge `56130106` 14:06, 5 commits by `git rev-list`). ✅ **VERIFIED at boot:** `database: added marathons.event_id` and `marathons.event_wanted` at **21:15:56Z**, then boot log `database ready`, `loaded cog spotlight` + `marathon`, `synced 33 app commands`, `logged in as Black_Bloc` **21:16:01Z**, no Traceback; `/health` ready=true, latency **67 ms** (33 = `tests/test_bot.py:TOP_LEVEL_NOW`, `/marathon` gone). Live figures from `deploys.log`: schema **62**, registry **469**, routes **237**, pages **22** (`ls site/public/*.html` at HEAD returns 22, no `marathons.html`). ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site at this ritual; no marathon has filed an Events row live; sweeps `ME-a`…`ME-d` are the owner's.
- 🙈 **Channels page off the main nav (owner, 2026-09-25 13:5x Phoenix, verbatim: *"we're done with the channels audit page, that can get hidden now, dont rm it but it doesnt need to be on the main nav"*).** `channels.html` stays (reachable by URL and from the Chat page's link if one exists); its row leaves `shell.js:GROUPS`. Status: ✅ **DONE on `main` 2026-09-25 14:2x, NOT deployed** — the row left `shell.js:GROUPS` (`channels.html` stays, reachable by URL); side effect fixed for free: the Overview's feature → label map had `chat` reading *Channels* because the Channels row shared the `chat` feature and came last — it reads *Chat* now. `node --test` layout + labels green.
  ↳ **LIVE as v164 14:16** (release commit `7787b10c`; `release.json` says `v164` at `642ea8ce` — `642ea8ce` IS the commit `release.json` names). `channels.html` still on disk (pages **22**). ⚠️ **NOT verified:** nothing has met Discord by hand; no browser opened the LIVE site at this ritual — the nav was not looked at on the live site.

## 2026-09-23 — v163 (18:32): a hello is answered by the model in the member's tone, unless the voice is cookout

**v163 (release commit `8acfcde2`; `release.json` says `v163` at `ddf90db6`, `changed_features` chat; **7807 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 216 routes**; schema **58** unchanged (`SCHEMA_VERSION` 58 read in `black_bloc/storage/db.py`), keys **418 → 419** live (`chat_greeting_via_model`; the chat namespace **128**; measured by importing `settings_store.KEY_TYPES` at HEAD), pages **22** (unchanged), deploys log **162** lines).** ONE merge over v162: `greeting-tone` merge **`9f49581d`** (2 commits: `da04ac4b` code + tests, `9983a0be` docs). Merge time by `git log` (committer, Phoenix): **18:24** — the moved bullet says 16:5x. Design: [`info/phase14-design.md`](info/phase14-design.md) ▸ *Follow-up 2026-09-23 — greetings through the model*. Sweep `CB-c`. Shipped 18:32 Phoenix (2026-09-24 01:32Z).

Verified (the conductor's reads, recorded in [`deploys.log`](deploys.log)): boot log `database ready` **01:32:49Z**, `synced 33 app commands`, `logged in as Black_Bloc` **01:32:53Z**. The retiring process logged `asyncio: Unclosed client session` at 01:32:43Z during the swap ([`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) ▸ KI-10, known).

⚠️ **NOT verified:** no live greeting under a tone has been read — sweep `CB-c` is the owner's check; no fallback row (`path: canned, fallback: canned`) seen live; no Discord, no browser. `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's); this ritual made no live API call of its own; `TEST_MODE` was not touched.

**The TODO item, moved whole** (its status line updated):

- 👋 **GREETINGS THROUGH THE MODEL** (owner, 2026-09-23 16:3x: *"yes route greetings through the model too"* — decided; was undecided at the v162 ritual). Today a greeting-intent hit (hi / whats good / sup …) is answered by the canned `greeting` lines and never reaches the model, so it carries no tone. Build (`greeting-tone`, Opus): when the voice is not `cookout` and the new key `chat_greeting_via_model` (on/off, default **on**) is on, a greeting goes down the SIMPLE banter path (zero notes, the banter hint, the member's tone) instead of the canned line; `chat_greeting_reaction` (wave instead) still wins first; voice `cookout` or the key off → the canned lines as today; the reply log says which path; a model failure falls back to a canned line. **Built by Opus on branch `greeting-tone` (2 commits, 172k, 12 min) — MERGED to main `9f49581d` 2026-09-23 16:5x, NOT DEPLOYED**: key `chat_greeting_via_model` (on/off, default on; registry 418 → **419**, chat 128); order: wave first (`chat.would_wave`, one check for both doors) → canned when `chat_llm_mode` off / key off / voice cookout (`chat_llm.greets_by_model`) → else `conversational_reply(greeting=True)` forces SIMPLE with no note search, the banter hint and the member's tone; fallback to a canned line on nothing/throw/cap/no key with `path: canned, fallback: canned` on the log row; every model reply row now carries `path`; guard: a greeting never opens a review item even when the tier ladder climbs (Groq down) — `chat_review.answered` skips the ungrounded reason for `greeting`; cost: one quick-tier turn per greeting (Groq priced $0 in `llm.PRICES`, dollars only on the Haiku fallback); +23 tests; phase14 follow-up, sweep `CB-c`. Gate on merged main: ruff clean, **7807 passed / 3 skipped** (80 s). Not verified: no live model or Discord. Status: ~~MERGED, waiting on the owner's word to deploy.~~ ✅ **LIVE as v163** (the line under).
  ↳ **LIVE as v163 18:32** (release commit `8acfcde2`; `release.json` says `v163` at `ddf90db6`). ✅ **VERIFIED LIVE at boot:** `database ready` 01:32:49Z, `synced 33 app commands`, `logged in` 01:32:53Z; the old process logged `asyncio: Unclosed client session` at 01:32:43Z (KI-10). ⚠️ **NOT verified:** no live greeting under a tone has been read (`CB-c` is the owner's check); no Discord, no browser. ⚠️ Times: `git log` puts the merge `9f49581d` at **18:24** (committer, Phoenix; `da04ac4b` 18:21, `9983a0be` 18:22), not the *16:5x* written above.

## 2026-09-23 — v162 (16:20): banter gets banter, a popup-picked mention matches its note like a typed name, and weak replies land in a review queue staff approve with one click

**v162 (release commit `4e6e4e4c`; `release.json` says `v162` at `d6a2ea45`, `changed_features` chat; **7790 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 216 routes**; schema **57 → 58** live (`chat_review`; `SCHEMA_VERSION` 58 read in `black_bloc/storage/db.py`), keys **366 → 418** live (the two banter keys, then 6 behaviour + 44 wording review keys; the chat group **127**; measured by importing `settings_store.KEY_TYPES` at HEAD), pages **22** (unchanged), deploys log **161** lines).** THREE merges over v161: `chat-banter` merge **`d8278877`** (4 commits: `1ee6e618`, `1b9d5345`, `df235a7c`, `d27af8ed`), `mention-names` merge **`654570e9`** (1 commit: `144b8045`), `chat-review-loop` merge **`737e660c`** (8 commits: `559aae15` … `3dd12149`) plus the conductor's count fix **`ed42d7e3`** (`tests/test_settings_store.py`, registry 368 + 50 = 418). Merge times by `git log` (committer, Phoenix): 15:44, 16:05, 16:14 — the moved bullets say 16:0x, 16:2x and 16:4x. Designs: [`info/phase14-design.md`](info/phase14-design.md) ▸ *Follow-up 2026-09-23 — banter gets banter* (and its mention-id line), [`info/chat-review-loop-design.md`](info/chat-review-loop-design.md). Sweeps `CB-a`, `CB-b`, `RL-a`…`RL-g` (still lettered). Shipped 16:20 Phoenix (23:19Z). ⚠️ A first launch of `deploy.ps1` minutes earlier was REFUSED dirty-tree — the conductor edited `TODO.md` seconds after starting it; nothing reached Fly (gotcha in [`access/deploy.md`](access/deploy.md)).

Verified (the conductor's reads, recorded in [`deploys.log`](deploys.log)): boot log `database ready` **23:19:53Z**, `synced 33 app commands`, `logged in as Black_Bloc` **23:19:57Z**; `GET /api/chat/review` at 16:2x answers `counts {open 0, approved 0, changed 0, dismissed 0, untagged 0}`, `items []`, with `filter`, `intents`, `notes`, `reasons`, `statuses`, `tagging` — the route is live and the queue is empty (nothing weak has happened since boot). The retiring process logged `asyncio: Unclosed client session` at 23:19:48Z during the swap ([`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) ▸ KI-10, known).

⚠️ **NOT verified:** no live model reply under the new matching; nobody has said *what up* to the bot since v162 (`CB-a`); no popup-picked mention tried (`CB-b`); no 👎 pressed and no follow-up tried (`RL-*`); no daily digest posted yet; the Chat page's **Review queue** section not opened in a browser after v162; Groq's tag shape and cost still unseen. A live observation that is NOT a v162 check: at **16:17** the owner said *"whats good"* and got the CANNED greeting-intent reply (log `chat: answered … (greeting)`) — intents answer before any model, so a greeting never carries the tone; 16:17 was under v161, three minutes before the v162 boot, and the v162 merges did not touch the intent path. Whether greetings should go through the model when the voice is not `cookout` is open in TODO ▸ 🔧 (👋, owner undecided). `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's); this ritual made no live API call of its own; `TEST_MODE` was not touched.

↳ **2026-09-23 16:3x, owner: `CB-a` VERIFIED LIVE** (appended at the v163 ritual) — he said *what up* to the bot on v162 and got the short banter reply, no quoted notes (*"it worked"*; recorded in [`access/sweeps.md`](access/sweeps.md) by `2d5defa0`, 16:33). The `CB-a` clause above is history.

**The TODO items, moved whole** (the learning-loop bullet whole, its status line updated; loop 2 stays in TODO as its own 🅿️ PARKED bullet):

- 💬 **"WHAT UP" GOT A KNOWLEDGE DUMP — banter must get banter** (owner, 2026-09-23 15:2x Phoenix, pasted the exchange: *@Black_Bloc What up* → *"Smoke was thick that night, cousin. > #upcoming-events: … > Who has the Tech Support role: … > #knuck-up: … Check #upcoming-events for the next cookout, ping Raelcun if tech's acting up, and swing by #knuck-up … Holler if you need anything else."* — *"this response was too much"*). Measured 15:2x: log row 10199 `chat.llm_reply tier=simple, trope=noir` (voice is `pool`, so a tone drew — fine); the three `> ` lines are KNOWLEDGE HITS quoted back verbatim by the simple model — `hits_for` matched "what up" against *upcoming-events*, *knuck-up* and the Tech Support role (substring/short-token matching), `user_turn` attached them via `grounding(hits)`, and nothing told the model not to echo them. Build (`chat-banter`, Opus): (1) `hits_for` matches whole words of ≥ 3 letters after a stop list (what/up/hey/yo/sup/lol…) and only STRONG hits count on a SIMPLE turn — "what up" yields zero; (2) a SIMPLE-tier turn with no real question carries NO grounding at all (a greeting gets a greeting: one or two lines), and the grounding header the model sees says *use these silently, never quote or list them*; (3) a keyed length hint for banter (`chat_banter_style`, default *one or two lines, no lists, no channel tour*) in the core; (4) tests: "What up" → simple, no hits, no grounding, prompt carries the banter hint; a real question keeps its hits. **Built by Opus on branch `chat-banter` (4 commits, 204k) — MERGED to main `d8278877` 2026-09-23 16:0x, NOT DEPLOYED**: `knowledge.STOP_WORDS` (one frozen home, incl. who/has/where), `TOKEN_MIN` 3, whole-word `occurrences`/`score`, `Hit.landed` (distinct tokens) ranks first, `name_of` (the `#channel` / role a note names), `is_strong` = ≥ 2 distinct tokens land OR one token that names the top note's channel/role (so "post my PBs" still hits `#speed-and-pbs`); `chat_llm.grounded(tier, text, hits)` drops notes ONLY for a SIMPLE non-question with weak hits — two conductor rounds narrowed it: `is_a_question` = a `?` anywhere OR the first word in `QUESTION_OPENERS` (who what when where why how which can could would should does do did is are am was were any anyone anybody), so "when is the cookout" keeps its note without a `?`, "What up" is a question with zero hits (clean), "sup fam, cookout vibes" carries none; side effect: a 13+-word opener question with no `?` is IMPORTANT now; keys `chat_grounding_note` (the silent-use header) + `chat_banter_style` (in the cached core after the cookout sheet), both in `PROMPT_WORDS` (600-char cap), editable on the Chat page's Personality section (chat group 75 → 77, registry 366 → **368**); trade-offs: the staff Knowledge filter is whole-word too ("cook" no longer finds Cookout); real Discord mentions `<#id>` are not resolved to names. Gate on merged main: ruff clean, **7659 passed / 3 skipped** (70 s). Not verified: no live model; nobody has said "what up" since (sweep `CB-a`). Status: ✅ **LIVE as v162**.
  ↳ **LIVE as v162 16:20** (release commit `4e6e4e4c`; `release.json` says `v162` at `d6a2ea45`). ✅ **VERIFIED LIVE at boot:** `database ready` 23:19:53Z, `synced 33 app commands`, `logged in` 23:19:57Z. ⚠️ **NOT verified:** nobody has said *what up* since v162 (`CB-a`); no live model reply under the new matching read.
- 🔗 **RESOLVE DISCORD MENTION IDS TO NAMES BEFORE MATCHING** (owner, 2026-09-23 16:1x Phoenix: *"fix the mention ids too"* — trade-off 2 of the banter build). A channel/role/member picked from Discord's popup arrives as `<#id>` / `<@&id>` / `<@id>`; the knowledge matcher sees digits and matches nothing on that token. Build: one helper that rewrites those tokens to `#name` / `@Role` / the member's display name from the guild cache before `hits_for` tokenises (and before the model sees the text? NO — the model already gets Discord's rendering; only the MATCHING text is rewritten), so "what goes in <#…knuck-up…>" hits the knuck-up note; unknown ids stay as-is. Built by Opus on branch `mention-names` (1 commit `144b8045`, 89k) — **MERGED to main `654570e9` 2026-09-23 16:2x, NOT DEPLOYED**: new `black_bloc/mentions.py:named(guild, text)` rewrites `<#id>` → `#name`, `<@&id>` → `@role`, `<@id>`/`<@!id>` → display name from the guild cache (unknown ids untouched; the bot's own mention left for `spoken` to strip); `hits_for(db, guild_id, text, guild=)` searches `spoken(named(guild, text))` — the model and the logs still see the raw mention; `who_they_named` deliberately NOT rewritten (it finds people BY `<@id>`); a typed or resolved `#knuck-up` is one token `knuck-up` (the tokeniser keeps inner hyphens) and `is_strong`'s name rule fires for both; behaviour change: a picked member becomes their display name before the search, so a note naming that person can match. +10 tests; sweep `CB-b`. Gate: ruff clean, 7669 passed on the branch; on main the mentions/chat_llm/knowledge files give 144 passed. Status: ✅ **LIVE as v162**.
  ↳ **LIVE as v162 16:20** (release commit `4e6e4e4c`; `release.json` says `v162` at `d6a2ea45`). ✅ **VERIFIED LIVE at boot:** `database ready` 23:19:53Z, `synced 33 app commands`, `logged in` 23:19:57Z. ⚠️ **NOT verified:** no popup-picked mention tried (`CB-b`).
- 🔁 **AN AUTOMATED LEARNING LOOP FOR THE CHAT BOT** (owner, 2026-09-23 15:3x Phoenix, verbatim: *"is there a way to automate the training loop? like can we scrape conversations from the whole discord and see how people talk, assign intentions to them and then audit those intentions periodically and feed that data to the bot to help it better understand real world scenarios?"*). Assessment given: the models are hosted (no fine-tuning); what the bot learns is intents, knowledge, channel notes and the voice. Two loops proposed, in order: **(1) learn from conversations WITH the bot** — flag weak replies automatically (no knowledge hits, a re-ask within a minute, a 👎 reaction, "not what I meant"), a review queue on the Chat page where staff assign an existing intent / write one / add a knowledge line / dismiss, fed back immediately; no scraping, no per-message model cost, `llm_ledger` + `chat_window` already hold the data. **(2) learn how people talk server-wide** — a weekly per-channel SAMPLE through the cheap model, aggregated and never stored per person, producing a proposals list (intent phrases, channel-note sharpening, cookout vocabulary) staff approve or drop; needs a data policy first (member opt-out like `/memory`, a retention window, the ignore list excluding the Basement and tickets), spend under the existing monthly cap. Owner 15:3x: *"start with loop 1, park loop 2, we would want an automated way to tag intent. maybe we need to feed it into a doc that i can set up a schedule for and have claude work review it and assign intent or something. I want to automate it somehow"*. DECIDED: **loop 1 builds now, loop 2 PARKED** (needs the data policy first — stays in this section as its own line). Automation shape: the bot itself tags — every weak conversation is sent once through the cheap model, which picks the closest EXISTING intent (or proposes a new phrase / a knowledge line) as a SUGGESTION; staff approve or change it with one click on the Chat page or `/chat`; an approved suggestion writes the intent phrase / knowledge line immediately; a daily digest tells staff how many wait; `GET /api/chat/review` + a markdown export exist so a scheduled Claude routine (read-only, operator token) can do a second-pass review into a doc if the owner sets one up. **Built by Opus on branch `chat-review-loop` (8 commits, 499k) — MERGED to main `737e660c` 2026-09-23 16:4x** (four conflicts: two append-only docs kept both; `test_core` / `test_settings_panel` chat count 77 + 50 = **127**; `test_settings_store` registry 368 + 50 = **418**, fixed in `ed42d7e3`): schema **58** (`chat_review`, unique on `reply_id`); new `black_bloc/chat_review.py` — the four reasons (IMPORTANT reply with zero hits / a re-ask inside `chat_review_reask_seconds` that is not an ack per `chat_review_ack_phrases` / the `chat_review_downvote_emoji` reaction / a `chat_review_not_it_phrases` follow-up), the memory opt-out honoured (no row at all), messages capped at 1,000 chars, DMs never queued; tagging = one Groq call per item in the background under tier `review` in `llm_ledger`, stops at `chat_monthly_cap_usd`, malformed → `kind: none`; teaching writes a trigger phrase / a NEW intent with one switched-OFF placeholder line / a fact into a staff note or *From review*; one write path in `chat_panel.py` (approve / change / dismiss / **reopen** for dismissed only); `GET /api/chat/review`, `GET /api/chat/review.md` (reader gate — the operator token can read it, its approve is 403), `POST …/{id}/approve|dismiss|reopen`, `PUT …/{id}`; Chat page **Review queue** section (stats bar, filters, Download as markdown, cards with Approve / Dismiss / Change… foldout); `/chat` ▸ **Review queue…** (5 a page); a separate 10-minute `_review` loop tags leftovers in batches of 5 and posts the daily digest (`chat_review_digest_hour`, `chat_review_mode` on by default) to `log_channel_id`; **6** behaviour keys + **44** wording keys (chat group → 127, registry → 418), 7 `chat.review_*` kinds ROUTINE; 42 + tests; design `chat-review-loop-design.md`, sweeps `RL-a`…`RL-g`. Gate on merged main: ruff clean, **7789 passed / 3 skipped** after the count fix, check.mjs ok 22 pages / **216** routes. Not verified: live Discord listeners, Groq's JSON shape and cost (Groq priced $0 in `llm.PRICES`), the digest post, re-ask noise; 👎/follow-up state is in-memory (forgets across a restart, accepted). Status: ✅ **LIVE as v162** (deployed with `chat-banter` + `mention-names` on the owner's word 16:2x, *"merge at the same time"*). Loop 2 PARKED — it stays in TODO ▸ ⏳ Waiting on the owner as its own 🅿️ bullet (the data policy first).
  ↳ **LIVE as v162 16:20** (release commit `4e6e4e4c`; `release.json` says `v162` at `d6a2ea45`). ✅ **VERIFIED LIVE at boot:** `database ready` 23:19:53Z, `synced 33 app commands`, `logged in` 23:19:57Z. `GET /api/chat/review` at 16:2x answers every count 0, `items []` (route live, queue empty). ⚠️ **NOT verified:** no 👎 pressed, no follow-up tried (`RL-*`); no digest posted yet; the Review queue section not opened in a browser after v162.

## 2026-09-23 — v161 (15:03): a channel members reach through a role they pick for themselves is told to the bot, staff can tell it or hide it per channel, and an ignored category always wins

**v161 (release commit `c8d2403d`; `release.json` says `v161` at `034ab624`, `changed_features` chat; **7637 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 211 routes**; schema **56 → 57** live (`channel_reach`; `SCHEMA_VERSION` 57 read in `black_bloc/storage/db.py`), keys **361 → 366** live (five `chat_channel_reach_*` sentences; measured by importing `settings_store.KEY_TYPES` at HEAD), pages **22** (unchanged), deploys log **160** lines).** ONE merge over v160: `channel-visibility` merge **`cbdcb6a3`** (3 commits: `16b69f1a` the rule, the table, the API; `3f700cd6` the page, the mock, the contract; `c3831f1a` docs). Merge time by `git log` (committer, Phoenix): 14:55 — the moved bullet says 15:0x. Kinds `chat.channel_reach_set` / `_cleared`; `PUT/DELETE /api/chat/channels/{id}/reach`. Design: [`info/channel-catalog-design.md`](info/channel-catalog-design.md) ▸ *Follow-up — channel reach*. Sweeps `CV-a`…`CV-e` (still lettered). Shipped 15:03 Phoenix (22:03Z). A settings change rode beside it, not code: The Basement category (`1094708810813280326`) on `chat_ignore_categories` LIVE at 14:4x.

Verified (the conductor's read through the operator token, 15:05 Phoenix, recorded in [`deploys.log`](deploys.log)): boot log `database ready` **22:03:50Z**, `synced 33 app commands`, `logged in` **22:03:53Z**. **The opt-in-role theory is PROVEN** — `GET /api/chat/channels` reads #landing via *Ask my pronouns*, #qotw via *QOTW*, #shows-and-movies via *Shows*, #sports-ball via *Sports*, #music-recommendations via *Musichead*, #recipes-and-food-pics via *Foodie*, #knuck-up via *Squads*, #rpg via *RPGer*; all **13** *The Hole in the Wall* and all **6** *Gaming* channels told; told **31 of 94** (was 24 under v159/v160); The Basement told **0 of 9** (`ignored_category`); review **48/94** before AND after the deploy — Pawpette's 48 reviewed rows intact (the seed's `INSERT OR IGNORE`, pinned by `test_the_seed_never_overwrites_a_decided_row`).

⚠️ **NOT verified:** the Channels page not opened in a browser after v161; no Reach button (*Tell the bot anyway / Hide from the bot / Back to the rule*) pressed by a human; no Discord-side door for the override exists yet (TODO ▸ 🔧 the 🚪 bullet, checklist 33); sweeps `CV-a`…`CV-e` unwalked by a person. `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's, off [`deploys.log`](deploys.log)); this ritual made no live API call of its own; `TEST_MODE` was not touched.

**The TODO item, moved whole** (the *mock fix in flight* sentence struck as the false alarm it was — a stale mock process on the port; gotcha in [`access/testing.md`](access/testing.md)):

- 👁️ **THE CHANNELS PAGE HIDES CHANNELS MEMBERS CAN REACH** (owner, 2026-09-23 14:3x Phoenix, verbatim: *"The channels page seems to think the Chat Category #Hole-in-the-wall isn't visible my members, it is fix that"*). Measured 14:3x through the operator token: `chat_visibility_role_id` IS the Member role (`1073741054563602532`), yet 6 of the 13 *The Hole in the Wall* channels read `not_visible` (landing, qotw, shows-and-movies, sports-ball, music-recommendations, recipes-and-food-pics) and 65 of 94 overall; `directory.everyone_sees` asks `channel.permissions_for(<Member role>)`, which applies only @everyone's and that one role's overwrites — a channel members reach through an OPT-IN role (the role-menu interest roles) or any second role reads as private. Build (`channel-visibility`, Opus): (1) a channel is visible when the Member role OR any self-assignable role (the roles the role menus hand out, `rolemenus`) can read it — one helper, used by the directory, the knowledge ingest and the Channels page badge, which then says *told (via the X role)*; (2) staff final say — a per-channel **Tell the bot anyway / Hide from the bot** override on the Channels page (stored, logged, shown as *overridden*), so a channel the rule still misses can be switched on by hand; (3) the badge names the reason in words. Owner 14:4x: *"Make sure the basement is still not referenced"* → The Basement category (`1094708810813280326`, the staff area) put on `chat_ignore_categories` LIVE 14:4x (all 9 channels read `ignored_category`, the directory block mentions none); owner 14:5x: *"members can see qotw too"* (one of the six). **Built by Opus on branch `channel-visibility` (3 commits, 291k, +1372 lines) — MERGED to main `cbdcb6a3` 15:0x, NOT DEPLOYED**: one rule `directory.reach_in` in this order — ignored/ticket category (hidden, beats everything, an override there is refused 409) → staff override (`channel_reach` table, schema **57**) → archive → the Member role reads it → any self-assignable role reads it (`channel_reach.openers_for`: options of every non-staff role menu; `rolegrants` hands out none) → hidden; `refresh` rereads openers + overrides on each ingest tick, each reply, each page read; `PUT/DELETE /api/chat/channels/{id}/reach`; the page badge says why in words and names the role, one Reach button per card (Tell the bot anyway / Hide from the bot / Back to the rule); **5** `chat_` keys (chat group 70 → 75, registry 361 → 366), kinds `chat.channel_reach_set/_cleared`; tests incl. `test_the_basement_is_never_referenced_even_when_only_an_opt_in_role_reads_it` and the staff-refusal twin; the agent's own undefined-identifier script would have caught the v160 `foldout`. Gate on merged main: ruff clean, **7637 passed / 3 skipped** (67 s); ~~⚠️ check.mjs on merged main reports the MOCK's draft-move answers (`none`/`reset`, maybe `use`/PUT) missing `counts` and `channel.reach` — the real API matches the contract (pytest green); a small Opus fix on main is IN FLIGHT.~~ **FALSE ALARM (15:01):** a stale mock process (started 13:38, pre-merge code) was still listening on the port check.mjs called; the mock code on main was already right, no fix and no commit resulted, the process was killed 15:01 — the gotcha is now in [`access/testing.md`](access/testing.md) ▸ *Gotchas*. Not verified: the opt-in-role explanation is unproven until the deployed page shows qotw / shows-and-movies / sports-ball / music-recommendations / recipes-and-food-pics / rpg as *told … through the X role*; `#landing` and `#knuck-up` match no menu role and will likely still read hidden — *Tell the bot anyway* is the answer there. Follow-up noted: no Discord-side door for the override yet (checklist 33). Status: MERGED, ~~mock fix in flight,~~ then waiting on the owner's word to deploy.
  ↳ **LIVE as v161 15:03** (release commit `c8d2403d`; `release.json` says `v161` at `034ab624`; merge `cbdcb6a3`, 3 commits `16b69f1a`, `3f700cd6`, `c3831f1a`). Owner 15:0x, before the deploy: *"some channels were already confirmed, make sure they stay confirmed and arent overwritten"* → verified: review **48/94** before AND after the deploy (Pawpette's reviewed rows intact; the seed is `INSERT OR IGNORE`, pinned by `test_the_seed_never_overwrites_a_decided_row`). ✅ **VERIFIED LIVE after the v161 boot** (operator token, 15:05): boot log `database ready` 22:03:50Z, `synced 33 app commands`, `logged in` 22:03:53Z; **the opt-in-role theory is PROVEN** — `GET /api/chat/channels` reads #landing via *Ask my pronouns*, #qotw via *QOTW*, #shows-and-movies via *Shows*, #sports-ball via *Sports*, #music-recommendations via *Musichead*, #recipes-and-food-pics via *Foodie*, #knuck-up via *Squads*, #rpg via *RPGer* (the bullet's guess that #landing and #knuck-up would still read hidden was WRONG — both are opened by a menu role); all 13 *The Hole in the Wall* and all 6 *Gaming* channels told; told **31 of 94** (was 24); The Basement told **0 of 9** (`ignored_category`). ⚠️ **NOT verified:** the Channels page not opened in a browser after v161; no Reach button pressed by a human; no Discord-side door for the override exists (its own 🔧 bullet now, 🚪); sweeps `CV-a`…`CV-e` unwalked by a person (`CV-a`'s expectation is met by the API read above).

## 2026-09-23 — v160 (14:20): HOTFIX — the Chat page renders again (`page-chat.js` imports `foldout`)

**v160 (release commit `d7cda292`; `release.json` says `v160` at `b20a5aed`, `changed_features` chat; **7604 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 209 routes**; schema **56** and keys **361** unchanged; site only; deploys log **159** lines at that deploy).** ONE commit over v159: **`b20a5aed`** (committed 14:18 Phoenix; `site/public/assets/page-chat.js`, +1 line — the missing `foldout` import from `ui.js`). Shipped 14:20 Phoenix (21:20Z).

Verified (recorded in [`deploys.log`](deploys.log)): the Chat page opened in a browser after the deploy (14:2x) — no error banner, every section rendered (Try it, Intents 21, Knowledge 51, Channel directory, Personality 11 with the cookout sheet + tone clause editors, Who hears what).

⚠️ **NOT verified / still open:** the gate still cannot catch an undefined identifier that fails only at render — the ES-module parse succeeds and check.mjs calls routes, not pages. That half of the bullet stays OPEN in TODO ▸ 🔧 (the 🧪 bullet). `pytest` and `check.mjs` were not re-run after the deploy.

**The TODO item, moved — its hotfix half only** (the *GAP … Fix to build* half is still open work and stays in TODO):

- 🩹 **v160 HOTFIX 14:20 — the Chat page died on render under v159** (owner, 14:1x: *"The chat page isn't working"*): `page-chat.js` used `foldout` from `ui.js` without importing it (the personality-tones build); the page showed *Something is wrong at the bot — foldout is not defined*; the Channels page was fine. One-line import, release `d7cda292`, gate green first run.
  ↳ **LIVE as v160 14:20** (release commit `d7cda292`; `release.json` says `v160` at `b20a5aed`; one commit `b20a5aed`, `site/public/assets/page-chat.js` +1 line, site only). ✅ **VERIFIED:** the Chat page opened in a browser after the deploy (14:2x) — no error banner, every section rendered (Try it, Intents 21, Knowledge 51, Channel directory, Personality 11 with the cookout sheet + tone clause editors, Who hears what), per [`deploys.log`](deploys.log). The gate gap it exposed stays open as its own 🔧 bullet (🧪 *The gate cannot see an undefined identifier*).

## 2026-09-23 — v159 (14:07): every channel can carry a staff note the bot reads first and staff review the 94 drafts on a Channels page, staff post today's birthday wishes by hand, and the cookout is the base voice with every mood a tone on it that follows the person

**v159 (release commit `c54f14a8`; `release.json` says `v159` at `105e5b59`, `changed_features` birthday + chat; **7604 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **22 pages / 209 routes**; schema **53 → 56** live (main stepped 53 → 54 `channel_notes` → 56 `channel_drafts`, `chat_voice`, `llm_ledger.trope`, `personality_tropes.voice_edited_by/at`; `SCHEMA_VERSION` 56 read in `black_bloc/storage/db.py`), keys **308 → 361** live (main stepped 308 → 319 → 330 → 334 → 361; measured by importing `settings_store.KEY_TYPES` at HEAD and at the v158 release commit — the `deploys.log` line's *54 -> 56, 334 -> 361* are the last merge's steps, not the live ones), pages **21 → 22** (`channels.html`), deploys log **158** lines).** FOUR merges over v158: `channel-catalog` merge **`3acc8a83`** (6 commits), `birthday-post-today` merge **`b67e96e4`** (4 commits), `channels-page` merge **`654be0c8`** (5 commits), `personality-tones` merge **`c1f0148d`** (12 commits incl. two merges of main). Merge times by `git log` (committer, Phoenix): 12:56, 13:08, 13:31, 13:37 — ⚠️ the moved bullets below say 13:1x / 13:5x / 14:1x / 14:3x; git is the authority (a 14:3x merge cannot precede the 14:06 release commit). Designs: [`info/channel-catalog-design.md`](info/channel-catalog-design.md) (and its Channels page follow-up), [`info/channel-catalog.md`](info/channel-catalog.md), [`info/birthdays-panel-design.md`](info/birthdays-panel-design.md) ▸ *Follow-up 2026-09-23*, [`info/personality-tones-design.md`](info/personality-tones-design.md), the survey [`info/personality-per-person-survey.md`](info/personality-per-person-survey.md). Sweeps `CC-a`…`CC-d`, `BT-a`…`BT-d`, `CP-a`…`CP-e`, `PT-a`…`PT-e` (still lettered). Shipped 14:07 Phoenix (21:07Z).

Verified: boot log **21:07:23Z** `database: added personality_tropes.voice_edited_by`, `database: added personality_tropes.voice_edited_at`, `database: added llm_ledger.trope`, `database ready`, `personas: pool v1 synced — 0 inserted, 11 updated, 0 retired` (the eleven rewritten tone bodies landed in the live table), `synced 33 app commands`, `logged in as Black_Bloc` **21:07:27Z** (the conductor's read of the boot log). Re-read at the ritual (14:1x Phoenix) through the operator token: action **10103** `chat.channel_drafts_seeded` *count=94, notes=2* at **21:07:27Z**, via boot (then 10104 `chat.knowledge_ingested sections=51`); `GET /api/chat/channels` answers `review {total: 94, reviewed: 2, drafts_left: 92}`, `budget {used: 1454, cap: 4096, trimmed: []}`, `counts {total: 94, shown: 24, noted: 2}`, and the two owner notes are live — #general-chat and #speed-and-pbs, both status `used`, `decided_by` empty (seeded, not pressed); **23** of the 94 channels carry a Discord topic; the 70 left out of the directory are 65 `not_visible`, 4 `archive`, 1 `ignored_category` (the catalog's two open questions — the live topics and what the Member view reads — are now answerable on the page). `GET /api/chat/voices` answers setting **`cookout`**, 11 tones, `counts {total: 0, pinned: 0, active: 0}`; `/health` says `personality_pool_version: 1`.

⚠️ **NOT verified:** no live model reply under a tone has been read — and none CAN be yet: the live voice is `cookout`, which by the precedence turns tones off for everyone until staff set **pool** or a named mood; no Discord button pressed (`/chat` ▸ **Channel notes…**, `/chat` ▸ Personality ▸ **Who hears what…**, `/birthday` ▸ **Post today's wishes**); no browser opened the Channels page or the Chat page's cookout-voice / tone-wording / Who hears what sections; the post-today door not pressed (no `birthday.posted_now` row among the last 200 actions). `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's, off [`deploys.log`](deploys.log)); `TEST_MODE` was not touched.

**The TODO items, moved whole:**

- 🗂️ **CHANNEL CATALOG — every channel described, so the bot knows what each is for** (owner, 2026-09-23 12:3x Phoenix, verbatim: *"the bot thinks speed-and-pbs is the general chat, its not, that the channel for talking about speed running records. #general-chat is the general chat"* → *"lets catalog every channel in the discord and the description to help the bot get a better feel for each channel"*). Measured 12:2x: the chat model learns channels ONLY from Discord channel names + topics (`directory.py:rows_for`, 160 chars of topic, 4 KB budget), and `chat_home_channel_id` (the fallback when an answer names a channel that does not exist) pointed at #speed-and-pbs — set to #general-chat 12:25 through the site. Build: a per-channel NOTE the bot owns (`channel_notes`, schema 54) that beats the Discord topic in `directory_block`; `GET/PUT/DELETE /api/chat/channels[/{id}]` listing every text channel (name, category, topic, note, whether the directory shows it and why not); a *Channel directory* section on the Chat page with an editable note per channel and a *what the bot sees* preview; a `/chat` ▸ Channel notes… door; `docs/info/channel-catalog.md` — the catalog itself, one row per channel with a DRAFT description for the owner to approve. Built by Opus on branch `channel-catalog` (6 commits, 386k, +1844 lines) — **MERGED to main `3acc8a83` 2026-09-23 13:1x, NOT DEPLOYED** (two append-only docs conflicts, both kept): schema **53 → 54** (`channel_notes`), new `black_bloc/channel_notes.py` (`notes_for` / `notes_or_nothing` never raises / `set_note` / `clear_note`, `NOTE_CHARS` 240), `directory.py` `description_of` (note → topic → name), `why_hidden`, `directory_preview` (block, bytes, cap, trimmed); the note feeds `knowledge.channel_sections` too; one write door `chat_panel.save_channel_note` / `clear_channel_note` for the API (`GET/PUT/DELETE /api/chat/channels[/{id}]`, 422 in words over 240) and the `/chat` ▸ **Channel notes…** card (Discord's own channel select → prefilled form); Chat page **Channel directory** section with the *What the bot sees* card (block, 4096-byte meter, trimmed names) and one row per channel (badge: told / why left out; topic dim; inline note Save/Clear); **11** new `chat_` word keys (`settings_store.CHANNEL_NOTE_WORDS`, registry 308 → 319, chat group 28 → 39); kinds `chat.channel_note_set/_cleared` ROUTINE; **the catalog `docs/info/channel-catalog.md`: 94 live text channels in 14 categories (+5 forums, left out), 2 final rows (general-chat, speed-and-pbs), 92 ⚠️ DRAFT**; design doc `channel-catalog-design.md`; sweeps `CC-a`…`CC-d`; mock: two channels + the two notes + two DRAFT notes. Gate on merged main: ruff clean, **7427 passed / 3 skipped** (`-n 16`, 53 s); check.mjs on the branch 21 pages / **201** routes. Not verified: nothing met Discord; the live topics are unknown (the operator API does not expose them); which channels the Member role sees is unknown. Status: MERGED, waiting on the owner's word to deploy; the 92 drafts wait on the owner's review (question open: review in the doc, or write fresh).
  ↳ **LIVE as v159 14:07** (release commit `c54f14a8`; `release.json` says `v159` at `105e5b59`; merge `3acc8a83`). ✅ Schema 56 ran on the live database at 21:07:23Z; the 92 drafts question was answered by the Channels page (next bullet) — the drafts were seeded, not written into notes. ⚠️ `/chat` ▸ **Channel notes…** not pressed; sweeps `CC-b`…`CC-d` unwalked (`CC-a` is superseded by `CP-a`).
- 🎭 **PERSONALITY PER PERSON + COOKOUT AS THE BASE VOICE** (owner, 2026-09-23 12:5x Phoenix, verbatim: *"i want to see what personality is connected to each person for the bot, is that something we have implemented"* → *"look in the Heygabi workspace, which is catalogplatform book buddy gamebuddy and maybe one other, find out how they do the personality mapping per person. I also want the cookout personality to be the main personality and all other personality are tones upon the main personalities lingo and mannerisms"*). Measured 12:5x: NOT implemented — `chat_personality` is cookout / pool / one mood; in pool mode `pick_trope(key=window_key(channel_id, user_id), turns)` draws a mood deterministically per channel+member 30-minute window and drifts a step every `DRIFT_EVERY_TURNS`; nothing stores a mood per person, the mood used is not on the reply's log row, and no page or panel shows who hears what. The stack today (`personas.py:system_blocks`) is core + FEATURES + `COOKOUT_VOICE` (cached) → channels → `TROPE_BLOCK(voice, REGISTER, INVARIANT)` — the cookout voice is already always present; whether a mood REPLACES its lingo or sits on top is what the prompt text decides. Step 1 (IN FLIGHT, Opus read-only survey, branch none): how `catalog-platform` (GABI), `bookbuddy`, `boardbuddy` and any other estate bot map a personality to a PERSON — findings to `docs/info/personality-per-person-survey.md`. Step 2 (after the survey): the design — cookout as the base, moods as tones (the prompt stack + manifest), the mood on every reply log row, a *who hears what* view on the Chat page + `/chat`, per-member pins (staff, clearable) if the owner keeps them (question 1 of 1 pending: all three, or without pins). **Survey DONE 13:2x** (`74c719e3`, [`info/personality-per-person-survey.md`](info/personality-per-person-survey.md), 220 lines, six repos read): GABI (`catalog-platform` gateway) is the ONLY estate bot with a per-person mapping — keyed by Discord user id alone, stored in the gateway Durable Object with no expiry, weighted-random first mood then a step every 4 exchanges at 25%, members set/clear their own with a hidden phrase, devops pin/clear by @mention + a devops roster; the mood block is appended after the core and says *a mood, not a different person… still GABI*; nobody logs the mood used. **Black Bloc can already let a mood override cookout's lingo:** `COOKOUT_VOICE` describes a mood ("favourite uncle at the grill") but carries no lingo or mannerisms, `TROPE_BLOCK` opens `## How you sound right now` (competes with the base's `## How you sound`, later in the prompt), and `REGISTER`/`INVARIANT` guard facts and PG-13, not the voice; most mood bodies are GABI's with the book references removed. **Design (dispatched 13:2x, Opus, branch `personality-tones`):** (a) cookout as the BASE — `COOKOUT_VOICE` becomes a real lingo-and-mannerisms sheet, an editable setting `chat_cookout_voice` (default = the rewritten sheet), always in the cached core; the mood block is reworded as *a TONE on the cookout voice above — keep its words, names and mannerisms; the tone changes only energy, pace and attitude*; the eleven mood bodies rewritten as cookout-flavoured tones (no manifest bump), each editable on the Chat page; tests that the whole base is in every prompt. (b) per person — table `chat_voice(guild_id, user_id, trope, turns, since, pinned, pinned_by, pinned_at)` separate from `chat_profiles` (so `/memory` forget never clears a staff pin); precedence: server `cookout` turns tones off for all → a member's pin → a named server mood → the pool roll, which now FOLLOWS THE PERSON across channels (the 30-minute window still re-rolls when empty, drift inside it); the mood used goes on every reply's ledger/log row; a *Who hears what* roster on the Chat page + `/chat`, staff **Set a member's tone…** / **Clear** (one log row each). Decision taken (owner can flip): NO member self-serve door for now — GABI hides hers on the owner's orders. **Built by Opus on branch `personality-tones` (12 commits incl. two merges of main, 452k, +2974 lines) — MERGED to main `c1f0148d` 2026-09-23 14:3x, NOT DEPLOYED** (clean merge; the branch already carried main): `COOKOUT_VOICE` is a 22-line lingo-and-mannerisms sheet — key `chat_cookout_voice` (⚠️ the builder's draft, editable on the Chat page) — always in the cached core; the mood block is `TONE_HEADING` + body + `chat_tone_clause` (*This is a TONE on the cookout voice above, not a different voice…*) + REGISTER + INVARIANT; all eleven bodies rewritten as cookout tones with an example line, each editable (`PUT /api/chat/personality/{name}` `voice`, a staff edit survives the boot sync via `voice_edited_by/at`, blank restores the shipped text); new `black_bloc/chat_voice.py` — table `chat_voice(guild_id, user_id, trope, turns, since, pinned, pinned_by, pinned_at)`, precedence server cookout → member pin → named server mood → the roll keyed `guild:user:since` (follows the person across channels; re-rolls on an empty 30-minute window; drift unchanged), DMs keep the old roll; `llm_ledger.trope` on every row and `trope` on the `chat.llm_reply` log details; `GET/PUT/DELETE /api/chat/voices[/{user_id}]` (422 in words for unknown/off/cookout/pool, 404 for a non-member or a bot); Chat page: the cookout-voice card, a wording editor per tone, **Who hears what** (roster, tone select + Pin/Clear, member picker); `/chat` ▸ Personality ▸ **Who hears what…** (25 a page, member picker → tone select + Clear the pin, Try again); **27** keys (chat group → 70, registry 334 → **361**), kinds `chat.voice_pinned/_cleared`, `chat.tone_edited` ROUTINE; schema additions under the merged **56** (`chat_voice`, `llm_ledger.trope`, `personality_tropes.voice_edited_by/at`; the branch's own 55 test kept as history); decisions recorded as reversible: no member self-serve, no DM on a pin, a tone edit applies to every server; sweeps `PT-a`…`PT-e`; `personality-pool-design.md` + `access/personality-pool.md` carry superseding notes. **Gate on merged main 14:3x: ruff clean, 7604 passed / 3 skipped (`-n 16`, 63 s), check.mjs ok 22 pages / 209 routes.** Not verified: no live model has read the new prompt (tests prove the sheet and clause are present, not that the model follows them — read replies under noir, scholar and deadpan after the deploy); no Discord, no browser; the rewritten bodies reach the live table at the next boot sync while `personality_pool_sync` is on. Status: MERGED — ALL FOUR held builds are on main; waiting on the owner's word to deploy (v159 = channel-catalog, birthday-post-today, channels-page, personality-tones over v158).
  ↳ **LIVE as v159 14:07** (release commit `c54f14a8`; merge `c1f0148d`). ✅ **VERIFIED LIVE:** the boot log added `personality_tropes.voice_edited_by`, `…voice_edited_at` and `llm_ledger.trope` and read `personas: pool v1 synced — 0 inserted, 11 updated, 0 retired` at 21:07:23Z — the eleven rewritten tone bodies are in the live table; `GET /api/chat/voices` answers (setting `cookout`, 11 tones, 0 voices, 0 pins). ⚠️ **NOT verified:** no live model reply under a tone has been read, and none can be while the server voice is `cookout`; the Chat page's new sections not opened in a browser; `/chat` ▸ **Who hears what…** not pressed; sweeps `PT-a`…`PT-e` unwalked.
- 📄 **CHANNELS PAGE — staff review the 92 draft descriptions on the site, editable** (owner, 2026-09-23 13:3x Phoenix, verbatim: *"make that a page on the website and i will have the staff do it"* → *"make it editable too so they can adjust the text"*). Build: the Channel directory MOVES from the Chat page to its own **Channels** page (`channels.html`, one home — the Chat page keeps a link card); the 92 drafts from `info/channel-catalog.md` are seeded into the bot (`channel_drafts_seed.json` → a drafts table at boot, never a live note) and each row shows the draft as a proposal with **Use this** / an editable box + **Save my wording** / **No note** / **Reset to the draft**, a progress line (*reviewed N of 94*), the real Discord topic beside it, and the *What the bot sees* card at the top; staff-gated; one write path with the existing note helpers; log kinds for the review moves. Built by Opus on branch `channels-page` (5 commits, 293k, +2401 lines) — **MERGED to main `654be0c8` 2026-09-23 14:1x, NOT DEPLOYED** (two append-only docs conflicts, both kept): schema **56** (`channel_drafts`, CREATE IF NOT EXISTS; 55 left free for `personality-tones`), new `black_bloc/channel_drafts.py` (seed from `channel_drafts_seed.json` — **94 rows**, the 2 finals seed as `used` and become real notes on the first boot only when no note exists; `use_draft` / `save_wording` / `no_note` / `reset_draft`, status derived from the stored note by `effective()`, every move through `channel_notes` + one log row), boot seed on the ingest tick (`chat.py:seed_drafts`, kind `chat.channel_drafts_seeded`), the `/chat` modal saves through `save_wording` (one write path), `GET /api/chat/channels` rows gain draft/status/decided_by/decided_at + `review {total, reviewed, drafts_left}`, `POST …/{id}/use|none|reset`; **new page `channels.html`** (nav after Chat under *Runs the cookout*): *Reviewed N of 94 · M drafts left*, Next draft, filters, cards with the editable draft box and Use this / Save my wording / No note / Reset to the draft + a 240 counter, the *What the bot sees* card; the Chat page's section is a link card; **4** `chat_` keys (`chat_channel_draft_used/_none/_reset/_missing`, chat group 39 → 43, registry 330 → 334), kinds `chat.channel_draft_used/_rewritten/_none/_reset`; `site.md` 21 → **22** pages; sweeps `CP-a`…`CP-e`. Gate on merged main: ruff clean, **7511 passed / 3 skipped** (`-n 16`, 60 s); on the branch check.mjs ok **22 pages / 204 routes**, headless Chrome at desktop and 400 px (no horizontal scroll; Use this moved the count 4 → 5 of 94). Not verified: nothing met Discord or the live db — the boot seed against the real 94, the finals landing as notes, real staff names. Status: MERGED, waiting on the owner's word to deploy (held until `personality-tones` lands).
  ↳ **LIVE as v159 14:07** (release commit `c54f14a8`; merge `654be0c8`). ✅ **VERIFIED LIVE:** action **10103** `chat.channel_drafts_seeded count=94, notes=2` at 21:07:27Z; `GET /api/chat/channels` answers `review {total: 94, reviewed: 2, drafts_left: 92}` and `budget {used: 1454, cap: 4096, trimmed: []}`; the two finals (#general-chat, #speed-and-pbs) landed as real notes, status `used`. ⚠️ No browser has opened <https://blackbloc.heygabi.ai/channels.html>; no staff review move made yet; real staff names in the chips unseen; sweeps `CP-a`…`CP-e` unwalked.
- 🎂 **POST TODAY'S BIRTHDAYS ON DEMAND** (owner, 2026-09-23 12:4x Phoenix, verbatim: *"can we add a birthday command to post the days birthdays? that way they can be manually triggered"*). Built by Opus on branch `birthday-post-today` (4 commits, 282k, +1089 lines) — **MERGED to main `b67e96e4` 13:5x, NOT DEPLOYED** (two append-only docs conflicts, both kept): one door `post_today(bot, guild, actor, again=, via=)` runs the sweep's own `_celebrate` per member under the member lock — *Post the ones not sent yet* (skips anyone wished today) / *Post them all again*; mode off refuses in words and still logs; shadow posts a rehearsal copy to the shadow home (the sweep's shadow is unchanged: it posts nowhere); `/birthday` ▸ staff **Post today's wishes** button → a three-button confirm; the Birthdays page has the same control in the page-head aside; `POST /api/birthdays/post-today {again}` (staff, 409 when off, 503 without the cog); **11** `birthday_` word keys (`settings_store.BIRTHDAY_POST_WORDS`, group 10 → 21, registry 319 → 330), kind `birthday.posted_now` ROUTINE; +27 tests; design follow-up in `birthdays-panel-design.md`, sweeps `BT-a`…`BT-d`. Gate on merged main: ruff clean, **7468 passed / 3 skipped** (`-n 16`, 47 s). Not verified: nothing met Discord or the live site. Status: MERGED, waiting on the owner's word to deploy.
  ↳ **LIVE as v159 14:07** (release commit `c54f14a8`; merge `b67e96e4`). ⚠️ **NOT verified live** — the door has not been pressed from Discord or the site (no `birthday.posted_now` row among the last 200 actions); sweeps `BT-a`…`BT-d` unwalked. The five-minute sweep itself kept posting under v158 (Mathcat, action 10087).

## 2026-09-23 — v158 (12:51): a Change button on every Birthdays row, and the form's Saved. notice survives the refresh

**v158 (release commit `3dec2518`; `release.json` says `v158` at `203274c6`, `changed_features` birthday; **7379 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **21 pages / 198 routes**; schema **53** (unchanged) and keys **308** (unchanged), both measured by importing the package at `3dec2518`; site only; deploys log **157** lines at that deploy).** ONE merge over v157: `birthday-change-button` merge **`477844da`** (2 commits; Merge times by `git log` (committer, Phoenix): 12:44 — the moved bullet says 13:0x). Sweep `BC-a` (still lettered). Shipped 12:51 Phoenix (19:51Z).

Verified: boot log `database ready` **19:51:01Z**, `synced 33 app commands`, `logged in as Black_Bloc` **19:51:06Z** (the conductor's read, recorded in [`deploys.log`](deploys.log)); re-read at the ritual through the operator token: action **10087** `birthday.announce` *mode=on, date=September 23, text=Happy Birthday **Mathcat**!* at **19:51:06Z** — the first sweep after boot. Anarchy's wish, the 12:3x ask, posted under v157 before this deploy: action **10075** `birthday.announce` *Happy Birthday **anarchy**!* at **19:31:48Z**.

⚠️ **NOT verified:** the **Change** button has not been pressed on the live site (sweep `BC-a`); a member the bot does not know by name (the picker would show the id). `pytest` and `check.mjs` were not re-run after the deploy (the figures are the gate's).

**The TODO items, moved whole:**

- ✏️ **Birthdays page: a Change button per row (ride-along)** (owner, 2026-09-23 12:4x: *"is it worth adding an edit for a birthday or is that too niche?"* — answer: worth it, cheap, not its own deploy). Today the *Add or change one* form already overwrites a stored date and `/birthday` prefills its modal, but no row says *change this*, so people clear-then-set (Pawpette, 12:14 today: set 23 → clear → set 24, which also resets `last_announced_on`). Build: a **Change** button on each row that fills the form with that member and date and scrolls to it; no API, no key, no schema; one node test. Fold into the next Birthdays page touch or the UX-audit pass for that page. Owner 12:4x: *"yes add a change button next to the remove button"* — built by Opus on branch `birthday-change-button` (2 commits, 94k) — **MERGED to main `477844da` 2026-09-23 13:0x, NOT DEPLOYED**: a quiet-toned **Change** before **Remove** on every row; `setCard()` returns `{node, fill}` and `fill(row)` sets the picker (new `memberPicker.set({id,name})` in `ui.js`), month/day/year, scrolls and focuses Day; found and fixed on the way that the form's *Saved.* notice was wiped by the page refresh (`keepSaying('birthdays.set')`). No API, key or schema change; no node DOM tests exist for pages, so it was exercised in headless Chrome against the mock (rows read *yes, Change, Remove*; Change filled Dax, save overwrote, one row not two); check.mjs ok, ruff clean, the page loads as an ES module. Sweep `BC-a`. Not verified: a member the bot does not know by name (the picker would show the id). Status: MERGED, waiting on the owner's word to deploy.
  ↳ **LIVE as v158 12:51** (release commit `3dec2518`; `release.json` says `v158` at `203274c6`; merge `477844da`). ⚠️ **NOT verified live** — nobody has pressed **Change** on the live Birthdays page; sweep `BC-a` unwalked.
- 🎂 **Anarchy's birthday is the 23rd** (owner, 12:3x: *"prio saying Anarchy's birthday message also set it for the 23rd today not the 24th tomorrow"*): set to September 23 on the Birthdays page 12:3x (Pawpette had set 23 → cleared → 24 at 12:14); the five-minute sweep posts the wish in #general-chat (`birthday_channel_id`) — watcher running. Status: SET, waiting on the sweep.
  ↳ **DONE** — the sweep posted it: action **10075** `birthday.announce` *mode=on, date=September 23, local_date=2026-09-23, text=Happy Birthday **anarchy**!* at **19:31:48Z** (12:31 Phoenix, under v157), read through the operator token at the v158/v159 ritual.

## 2026-09-23 — v157 (04:21): the code-health round — a walled YouTube channel row says so and searches, the suite clears its own environment, a click on a gone panel is answered in words, and the gate names what failed

**v157 (release commit `e9ecd0d0`; `release.json` says `v157` at `8f1dfc6d`, `changed_features` golive + youtube; **7379 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **21 pages / 198 routes** — the first gate under `-n 16` + `pytest-timeout` + the junit file, whose file (read at the ritual) says `tests=7382 failures=0 errors=0 skipped=3`, 44.8 s; schema **53** (unchanged), keys **307 → 308** (one new core key, `panel_expired_text`, measured by importing `settings_store.KEY_TYPES`), deploys log **156** lines).** FOUR merges over v156, all from the owner's code-health round of 2026-09-22 21:3x: `youtube-walled` merge **`d2b4b9b7`** (5 commits), `env-proof-tests` merge **`4d5bd916`** (2 commits), `panels-survive-restart` merge **`5f0e94ab`** (2 commits), `gate-names` merge **`01309d43`** (8 commits incl. its own merge of main). Designs and runbooks: [`info/youtube-live-design.md`](info/youtube-live-design.md) ▸ *The walled channel rows*, [`info/panels-orphaned-click-design.md`](info/panels-orphaned-click-design.md), [`access/deploy.md`](access/deploy.md) ▸ *The gate's pytest step*, [`access/testing.md`](access/testing.md). Shipped 04:21 Phoenix (11:21Z).

Verified: boot log `database ready` **11:21:44Z**, `loaded cog black_bloc.cogs.content.youtube`, `synced 33 app commands`, `logged in as Black_Bloc` **11:21:47Z** (the conductor's read of the boot log); and, re-read at the ritual through the operator token (`/api/actions`), **one second after boot** the YouTube sweep wrote BOTH new shapes for ESAMarathon (spotlight_id 4, channel `UC3Oe-jfrIqEGygxYBYyN6jQ`) at **11:21:48Z**: action **10027** `youtube.live_seen … video_id=None, botcheck=True, spotlight_id=4, announced=False, because=opted_out` — the row that until v156 said `announced=True` on every walled read (the one before it, 10018 at 07:03:00Z under v156, had found the id on an unwalled read and says `announced=True`) — and action **10028** `youtube.probe_walled … live=True, video_id=None, keyed=True`, the wall as its own outcome with the live signal surviving it.

**v156, seen live overnight (same read, recorded here because DONE is never edited):** GamesDoneQuick's pinned announcement (message 1552167740410957925) was re-worded at **05:02:59Z** (action 10017) and **08:12:59Z** (10020) to game *Games Done Quick* with `[Replay] …` titles; the four-hourly reminder fired at **08:02:59Z** (10019 `golive.spotlight_bumped`, bump=1, its details carrying the card `embed` with the current game and box art); and the stream end at **11:12:59Z** unpinned the post (10022 `golive.spotlight_unpinned because=ended`). So the v156 entry's *reminder card* is now seen live, and the pinned refresh twice more. ⚠️ The v156 **retro-pin** path (Spotlight ON pressed on a live unpinned post) is STILL unverified — sweep `RP-a`.

⚠️ **NOT verified:** an orphaned panel click (`panel.expired_click` has **0** rows — it needs a member to click a panel opened before a deploy; sweep `PX-a`); the Go-live card's walled rows in a browser (`YW-a`…`YW-d`); the gate's `FAILED TESTS:` block on a real RED gate (the v157 gate was green; the block was exercised against synthetic junit files only); how much RAM `-n 16` saves (KI-37). `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's, off [`deploys.log`](deploys.log)); `TEST_MODE` was not touched. KNOWN_ISSUES at the ritual: **KI-32 CLOSED**, **KI-35 CLOSED** (cannot recur), KI-20 / KI-26 / KI-30 / KI-37 WATCHING with a v157 note each.

**The TODO item, moved whole:**

- 🩺 **CODE-HEALTH ROUND (owner, 2026-09-22 21:3x Phoenix, verbatim: *"lets work on fixing this stuff, at least the worth fixing stuff"* — the four "worth fixing soon" items from the 21:2x code-health read). Four Opus builds, one worktree each, dispatched 21:3x:** (1) **`env-proof-tests`** — the suite no longer depends on the shell: an autouse session fixture in `tests/conftest.py` blanks every name `config.py` declares (+ `DEV_GUILD_ID`), so the nine "no key" tests and the seven presence tests pass with real keys in the env; the deploy script's hand-clearing block becomes belt-and-braces. (2) **`gate-names`** — the gate keeps the names of what failed (junit xml + a failed-names block in `deploy.log`), pins the worker count, adds a per-test timeout so a hang (KI-26) ends as a named failure instead of idle workers; measured over three gate runs. (3) **`youtube-walled`** — KI-30 (a): a walled `/live` page that drops `isLive` must not read as a quiet offline channel; investigate why ESA's `youtube.live_seen` rows say `video_id=None, botcheck=True` every few hours with `YOUTUBE_API_KEY` SET on Fly (measured `flyctl secrets list` 21:3x, names only), fix what is found, surface it on the Go-live page. (4) **`panels-survive-restart`** — KI-20: a click on a panel whose view died with the process answers in words ("this panel expired, run /x again") instead of Discord's "This interaction failed"; the sentence is a settings key. Status: (3) **`youtube-walled` MERGED to main `d2b4b9b7` 2026-09-22 22:0x — NOT DEPLOYED** (5 commits, 199k Opus, +552 lines, 7358 passed on the branch; on main the youtube + logkinds files give 311 passed, ruff clean, `page-golive.js` executes as an ES module up to `document`). ⚠️ **FOUND: the channel-row path (`_channel_live`, v151) never called the 100-unit id search at all** — only the member path did — so a walled ESA page could only ever link `/live`; and `announced=True` on those rows was FALSE every time because ESA's announce cell is off and `announce_info` returned silently. Now: opted-out → `because: opted_out`, no search, 0 units; open Twitch session → `joined_session`; otherwise search + confirm once per broadcast like a member; the wall is its own outcome (`Probe.walled`, `wall_reading` True/False/None, ONE `youtube.probe_walled` row per state change, ROUTINE); `/api/youtube/status` gains `walled` + `id_unknown`, the Go-live card three rows (Behind the bot check now / Video id / What a post links), `/youtube` staff lines to match; sweeps `YW-a`…`YW-d` (Fly only); KI-30 dated note, still WATCHING. Deviation kept: the new staff readouts are constants like `BOT_CHECKED`, not keys. Not verified: nothing met YouTube or Discord; the walled card state was never rendered. Gotcha: `node --check` passes a broken `.js` page module — only an `.mjs` copy or `--input-type=module` catches it. (1) **`env-proof-tests` MERGED to main `4d5bd916` 22:1x — NOT DEPLOYED** (2 commits, 114k Opus): a session-scoped autouse fixture in `tests/conftest.py` pops every `Settings.model_fields` name (derived, not typed) plus `DEV_GUILD_ID`/`TEST_MODE`/`TEST_CHANNEL_ID`/`BLACK_BLOC_*` and sets `Settings.model_config['env_file'] = None`; the three per-fixture `delenv` lists are folded into it. Proof on the branch: 7339 passed with dummy names SET and with them unset (one unset run gave 22 failures under heavy CPU contention from the parallel builds, green on rerun — not diagnosed). **Verified on main WITH the shell's real keys present: the eight formerly env-sensitive files give 683 passed.** The presence guard (Residue (a)) was NOT needed — those seven never failed in this tree with `DEV_GUILD_ID` set; Residue (a) can close. (4) **`panels-survive-restart` MERGED to main `5f0e94ab` 22:4x — NOT DEPLOYED** (2 commits, 221k Opus; two append-only docs conflicts, both sides kept): new `black_bloc/orphaned.py` — `BlackBlocBot.dispatch` override MARKS a component/modal interaction whose custom_id no live view, dynamic item or modal owns (the same three `ViewStore` lookups discord.py 2.7.1 makes, read through `bot._connection._view_store`), BEFORE any handler task starts; the `on_interaction` listener answers a marked, unanswered one ephemerally with `panel_expired_text` (new core key, `{command}` from `Message._interaction.name`), logs `panel.expired_click` (new head `panel`, ROUTINE). Measured: the naive listener-only version double-responded on two tests; the dispatch version passes all 18. The core group is now 26 keys → `/settings` ▸ core shows *Find a setting*. The panel message itself is left as it is (one interaction, one response). +18 tests + 1 dispatch test; `panels-orphaned-click-design.md` (NOT yet in `info/README.md`), KI-20 dated note (still WATCHING, 0 reports), sweep `PX-a`. On main: orphaned/bot/logkinds/settings/youtube files give 759 passed, ruff clean. ⚠️ Relies on discord.py 2.7.1 internals (named in the design doc); an upgrade re-checks them — the failure mode is today's red banner, never a double answer. Not verified: nothing met Discord. (2) **`gate-names` MERGED to main `01309d43` 22:5x — NOT DEPLOYED** (8 commits incl. its own merge of main, 230k Opus): 🔴 **KI-26 ROOT CAUSE FOUND — Windows loopback port exhaustion.** Every asyncio loop's self-pipe is a TCP socketpair on 127.0.0.1 and each close leaves one `TIME_WAIT`; 32 workers × loops per test ate the 16,384 dynamic ports (13,565 `TIME_WAIT` measured mid-run) and every worker blocked in `accept()` for ever. Fix: `tests/loopback.py` (`-p tests.loopback` in `addopts`) sets `SO_LINGER 0` on both ends on Windows → 300 loops leave 0 `TIME_WAIT`, a full suite ~50. **KI-32 ROOT CAUSE: the api tests' `web_settings_now()` never set `database_path`**, so every xdist worker shared one guides media folder and media id 1 — `web_settings_at(path)` gives each test bot its own; 0 of 8 runs red after. Gate: `-rfE --junitxml=%TEMP%black-bloc-gate\gate-junit.xml`, a `FAILED TESTS:` block before the refusal (KI-35), `pytest-timeout>=2.3` with `timeout = 120`, `timeout_method = thread` (a hang ends as `worker gwN crashed while running <nodeid>`), **`-n 16`** (measured table in `access/deploy.md`: auto 45 s / 16 51 s / 8 ~72 s; 16 halves the processes for KI-37). All four KIs stay WATCHING with numbers. One NEW single-test hang seen 2 in 21 runs after the fix (`test_two_reconciles_at_boot_post_exactly_one_ticket_button`, async fixture setup, loop idle) — recorded under KI-26, a third sighting gets its own entry. **Verified on merged main 22:5x, real keys in the shell, after killing 99 stalled worker processes the builds had left: ruff clean, `pytest -n 16` → 7379 passed / 3 skipped in 46 s.** ALL FOUR MERGED; waiting on the owner's word to deploy (v157 = four merges over v156: youtube-walled, env-proof-tests, panels-survive-restart, gate-names).
  ↳ **(3) `youtube-walled` LIVE as v157 04:21** (release commit `e9ecd0d0`; merge `d2b4b9b7`). ✅ **VERIFIED LIVE:** action 10027 `youtube.live_seen … announced=False, because=opted_out` and action 10028 `youtube.probe_walled … live=True, video_id=None, keyed=True` for ESAMarathon at 11:21:48Z, one second after boot. ⚠️ The Go-live card's three new rows and `/youtube`'s staff lines have not been looked at; sweeps `YW-a`…`YW-d` unwalked; KI-30 stays WATCHING (the proof it waits on is a `live: null` walled row for a channel that was demonstrably live — 10028 read `live=True`).
  ↳ **(1) `env-proof-tests` LIVE as v157 04:21** (merge `4d5bd916`). The v157 gate ran the suite with this fixture and was green on the first run (7379 passed / 3 skipped). Residue (a) in TODO is struck as not needed.
  ↳ **(4) `panels-survive-restart` LIVE as v157 04:21** (merge `5f0e94ab`; `panel_expired_text` is key **308**, namespace `core`, by import). ⚠️ **NOT verified live** — `panel.expired_click` has 0 rows; sweep `PX-a` is unwalked. KI-20 stays WATCHING (0 reports).
  ↳ **(2) `gate-names` LIVE as v157 04:21** (merge `01309d43`). The v157 gate was the first real gate on the new script: `-n 16`, green on the first run, no `crashed while running` line, and its junit file (`%TEMP%\black-bloc-gate\gate-junit.xml`, 04:19:41 local) holds all **7382** cases, `failures=0`. KI-32 → CLOSED (its closing condition, a green landing gate, is met); KI-35 → CLOSED as cannot-recur; KI-26 at 1 of 10 green gates toward its closing condition; KI-37 WATCHING. ⚠️ The `FAILED TESTS:` block has not printed on a real red gate.

## 2026-09-22 — v156 (21:18): Spotlight ON settles the pin, the reminder names the game being played now with the go-live card, and the pinned announcement follows the game

**v156 (release commit `4fb69e00`; `release.json` says `v156` at `beb75348`; **7339 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **21 pages / 198 routes**; schema **53** (unchanged), keys **307** (unchanged), no new settings keys, deploys log **155** lines).** THREE merges over v155, all spotlight, off three owner asks the same evening (20:3x, 20:4x, 21:1x Phoenix): `spotlight-retro-pin` merge **`0ef4534c`** (3 commits), `spotlight-bump-preview` merge **`bde90469`** (2 commits), `spotlight-pinned-refresh` merge **`cc5a6de1`** (2 commits). Designs: [`info/channel-streamers-design.md`](info/channel-streamers-design.md) ▸ *Follow-up 2026-09-22: Spotlight ON settles the pin*; [`info/spotlight-design.md`](info/spotlight-design.md) ▸ *Follow-up 2026-09-22* and *Follow-up 2026-09-22 (2)*. Sweeps `RP-a`, `BP-a`, `PR-a` (still lettered).

Verified: boot log `database ready` **04:17:54Z**, `loaded cog black_bloc.cogs.content.spotlight`, `synced 33 app commands`, `logged in as Black_Bloc` **04:17:58Z**; `release.json` answers **`v156`** at `beb75348`; and **two seconds after boot** the first poll re-worded GamesDoneQuick's PINNED announcement — action row `golive.spotlight_announcement_refreshed` at **04:18:00Z** (spotlight_id 3, session_id 5, message_id 1552167740410957925), game *Nicktoons: Attack of the Toybots*; that post had been announced at 04:00:27Z as *Viewfinder* and pinned at 04:00:28Z by the OLD v155 code's normal announce-time pin, after the owner's delete-and-repost through the opt-out door (`golive_channel_optout_post` set to `delete`, opted out and back in, then set back to `end`). That action row is the live proof that the pinned-refresh code shipped and that Discord accepts the bot editing its own pinned post.

**Owner decision, 2026-09-22 21:2x, verbatim:** *"i think gdq will always be log noisy during marathons, lets let it be and deal with it then"* — said of every TITLE change also editing the pinned post, so a marathon is edited on many polls. Recorded as ACCEPTED behaviour in [`info/spotlight-design.md`](info/spotlight-design.md) ▸ *Follow-up 2026-09-22 (2)* and as [`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) KI-39.

⚠️ **NOT verified:** the retro-pin path (nobody has pressed **Spotlight on** against a live unpinned post since v156), the reminder card (no bump has fired under v156 yet), how often a real marathon's edits land (edits per hour not measured); no browser or Discord client was opened by a session. `node site/mock/check.mjs` and `pytest` were not re-run after the deploy (the figures are the gate's, off [`deploys.log`](deploys.log)); `TEST_MODE` was not touched and `spotlight_mode` was not flipped.

**The TODO items, moved whole:**

- 📌 **SPOTLIGHT ON MID-STREAM MUST PIN THE LIVE POST** (owner, 2026-09-22 20:3x Phoenix, verbatim: *"quick fix, when something gets tagged for a spotlight do a check to see if the channel is pinned, if its live and not pinned, pin it. if its not live and pinned un pin it, when its live again its pinned again"*). Measured live 20:23: GDQ row 3 had spotlight turned OFF 2026-09-21 16:53Z, its Fatal Frame stream was `golive.channel_announced` unpinned 2026-09-22 22:55Z, spotlight turned back ON 03:17Z mid-stream — no `spotlight_pinned` / `spotlight_pin_failed` row, because `settle_open_session` (`cogs/content/spotlight.py:1329`) settles only opt-out and spotlight 1→0; 0→1 settles nothing, which `channel-streamers-design.md` and sweep 721 recorded as a decision while the bot's own words (`SPOTLIT_SAID`, `SPOTLIT_STATE`) promise a pin. Build: spotlight 0→1 with an open session and the row's `pin` on → fetch the announcement, `_pin` it (`golive.spotlight_pinned`, `because: spotlight_on`), tell staff *pinned now*; with NO open session and the last announcement still pinned (a failed unpin at the end) → `_unpin` it; announce-time pinning unchanged, so the next stream pins itself. A manual unpin by staff during a stream is NOT fought (toggle time only, never the poller). Built by Opus on branch `spotlight-retro-pin` (3 commits, 129k), **MERGED to main `0ef4534c` 2026-09-22 21:0x — NOT DEPLOYED**: `settle_open_session` gains `brightened` → `_brighten` (live + unpinned → `_pin` with `because: spotlight_on`, answer `PINNED_NOW`; no session + stale pin → `_unpin`, answer `UNPINNED_ENDED_NOW`; poller untouched); +6 cog tests, +1 words test; mock mirrors it; design doc bullets struck + follow-up section; sweep 721 and 731 reworded, new sweep `RP-a`; code-notes 8 entries. Gate: ruff clean, **7323 passed / 3 skipped** with the shell's secrets unset (9 no-key tests fail when real keys are in the env — harness quirk, not the code). Nothing met Discord. Status: MERGED, waiting on the owner's word to deploy.
  ↳ **LIVE as v156 21:18** (release commit `4fb69e00`; `release.json` says `v156` at `beb75348`; merge `0ef4534c`). The spotlight cog loaded at boot (boot log `database ready` **04:17:54Z**, `loaded cog black_bloc.cogs.content.spotlight`, `synced 33 app commands`, `logged in as Black_Bloc` **04:17:58Z**). ⚠️ The retro-pin path itself is **NOT verified live** — nobody has pressed **Spotlight on** against a live unpinned post since v156; sweep `RP-a` is unwalked.
- 🎮 **THE "STILL LIVE" REMINDER CARRIES THE CURRENT GAME AND ITS PREVIEW** (owner, 2026-09-22 20:4x Phoenix, verbatim: *"also when we post a spotlight channel is still live make sure we still include the game they're playing"* → *"and do a game preview like a normal go live post"*). Measured: the default `spotlight_bump_template` already reads `**{name}** is still live — **{game}**, {duration} so far. {url}` and the live 03:20Z GDQ bump said *BUBSY 4D*; but (a) the poller's bump uses the fresh Helix stream while the manual **Bump now** (`cogs/content/spotlight.py:bump`, `info=None`) falls back to the session's ANNOUNCE-TIME game, stale for a marathon that changes games; (b) the bump posts `embed=None` — no box-art preview, unlike the announcement (`announcement_embed` with `_box_art`). Build: every bump asks Helix for the channel's current stream first (fallback: session info), the session row's game/title are refreshed when they changed, and the bump carries the same announcement embed + box art the go-live post does when `spotlight_embed` (the announce embed key) is on. Built by Opus on branch `spotlight-bump-preview` (2 commits, 127k), **MERGED to main 2026-09-22 21:1x — NOT DEPLOYED** (two append-only docs conflicts in `sweeps.md` / `code-notes.md`, both sides kept): `bump()` asks Helix for the channel's current stream when none is passed (`_current`, stored words on every failure, Helix skipped for a YouTube session), `_refresh` writes `game`/`title` back to the session row when they changed (poller path too, so Recent streams stays current), the reminder carries `announcement_embed` + box art under the existing `golive_embed` key (no new key), log rows gain `game`/`title`/`refreshed`/`embed`; +6 tests; design follow-up in `spotlight-design.md`, sweep `BP-a`, code-notes 5 rows. Gate on the branch: ruff clean, **7322 passed / 3 skipped** with the shell's secrets unset (same 9 no-key harness failures otherwise; ~half the full runs saw xdist workers die at ~99% — flake, cause not found). Nothing met Discord or Helix. Status: MERGED, waiting on the owner's word to deploy.
  ↳ **LIVE as v156 21:18** (release commit `4fb69e00`; `release.json` says `v156` at `beb75348`; merge `bde90469`). ⚠️ **NOT verified live** — no reminder has fired under v156 yet, so the Helix-fresh game and the go-live card on a bump have not been seen; sweep `BP-a` is unwalked.
- ✏️ **THE PINNED ANNOUNCEMENT IS EDITED TO THE CURRENT GAME AT EVERY REMINDER — never re-pinned** (owner, 2026-09-22 21:1x Phoenix, verbatim: *"also when it repost lets make sure to edit the pinned post to be the most current game, i would say pin the new post but i dont want to spam with pins and un pins"*). Today (post-merge main): the announcement is posted and pinned once with the announce-time game and stays worded that way until the end rewrites it to past tense; the reminder (now Helix-fresh) names the current game but the pinned post above it still names the old one. Build: when a bump finds the game or title changed (the `_refresh` result), the announcement message is EDITED in place — text re-rendered from the announce template with the current info, embed re-drawn with the current box art — and the log row says so (`golive.spotlight_announcement_refreshed` or a detail on `spotlight_bumped`); the pin is never touched, no new pin, no unpin. Built by Opus on branch `spotlight-pinned-refresh` (2 commits, 145k), **MERGED to main `cc5a6de1` 2026-09-22 21:3x — NOT DEPLOYED**: `_seen` runs `_follow` on every poller tick for an open Twitch session — a changed game/title re-words the pinned announcement at once (`_refresh_announcement`: `again_render` keeps the original mention text, the card is redrawn with the new box art and keeps its original timestamp, `allowed_mentions` none) and the reminder that is due on the same tick reuses the result; **Bump now** does the same; the pin is never touched (tested). Kind `golive.spotlight_announcement_refreshed` ROUTINE, its `_failed` twin IMPORTANT. Decisions: a spotlight-OFF channel post follows the game too; YouTube-sweep sessions are left alone. +10 tests, code-notes re-keyed for the cog. Gate on the branch: ruff clean, **7339 passed / 3 skipped**; on merged main the spotlight + logkinds files give 215 passed, ruff clean. Live risk noted: a stream that retitles often is edited every poll. Nothing met Discord. Status: MERGED, waiting on the owner's word to deploy (three spotlight merges over v155: retro-pin, bump-preview, pinned-refresh).
  ↳ **LIVE as v156 21:18** (release commit `4fb69e00`; `release.json` says `v156` at `beb75348`; merge `cc5a6de1`). ✅ **VERIFIED LIVE:** **two seconds after boot** the first poll re-worded GamesDoneQuick's PINNED announcement — action row `golive.spotlight_announcement_refreshed` at **04:18:00Z** (spotlight_id 3, session_id 5, message_id 1552167740410957925), game *Nicktoons: Attack of the Toybots*; that post had been announced at 04:00:27Z as *Viewfinder* and pinned at 04:00:28Z by the OLD v155 code's normal announce-time pin, after the owner's delete-and-repost through the opt-out door (`golive_channel_optout_post` set to `delete`, opted out and back in, then set back to `end`). Owner 21:2x, verbatim, on the edit rate: *"i think gdq will always be log noisy during marathons, lets let it be and deal with it then"* — ACCEPTED ([`KNOWN_ISSUES.md`](KNOWN_ISSUES.md) KI-39); edits per hour on a marathon not measured. Sweep `PR-a` has not been walked by the owner.

## 2026-09-22 — v155 (13:35): a spotlight is a DATE RANGE, a start as well as an end — a scheduled row stays quiet until its start passes

**v155 (release commit `a285afc7`; `release.json` says `v155` at `6d5f5841`; **7316 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **21 pages / 198 routes**, six node tests green; schema **52 → 53**, keys **299 → 307**, routes **198** (unchanged), pages **21** (unchanged), setting groups **24** (unchanged), cogs **23** (unchanged), deploys log **154** lines).** ONE merge off ONE owner ask from the same morning (10:29 Phoenix) — built, merged into `main` and put on the local mock for his check before he said *deploy it* at ~11:12.

- **A spotlight is a DATE RANGE — a start as well as an end** (owner, 2026-09-22 10:29 Phoenix, verbatim: *“in golive when i set a spotlight i can extend a week or keep forever. let me set a date range for start and end time”*, design [`info/spotlight-dates-design.md`](info/spotlight-dates-design.md), Opus on branch `spotlight-dates`, merge **`fad98c9e`**, **5 commits**, **+63** tests, sweeps `SD-a`…`SD-d`, review-checklist item **38**): a `spotlight_channels` row had **`expires_at`** only (NULL = kept for ever) and the moves were *Extend a week* / *Keep for ever* / *Let it expire*, so a row added early started being announced the moment the channel next went live. One nullable column — **`starts_at`** (TEXT ISO UTC, NULL = *started already*, which every existing row is) — makes the row a RANGE and adds one state: a row whose start is still ahead is **scheduled**, and the poller announces, pins, bumps and reminds nothing of it until the start has passed, at which point it logs `golive.spotlight_started` (ROUTINE). Both doors set it: the `/golive` ▸ **Channels…** panel’s **Add a channel** modal takes `Starts` / `Ends` and every picked row gains a **Set dates…** button pre-filled with what is stored; the Go-live page’s row drawer gains a **Dates** card with two `whenField` pickers and a **Save dates** button, and the **Spotlight a channel** form gains the same two pickers (which is what turned its *Days to keep it* number box into a real picker — the site when-picker audit’s row **#12**). `GET/POST/PATCH /api/golive/spotlight` carry `starts_at` and a `scheduled` flag and answer **422 in words** on a backwards range or an unreadable date, on both surfaces. Eight new word keys, all `golive`-namespaced, in their own Settings drawer — registry **299 → 307**, go-live page namespace keys **54 → 62**.
  ↳ **LIVE as v155 13:35** (release commit `a285afc7`; `release.json` says `v155` at `6d5f5841`; merge `fad98c9e`). The migration ran itself on boot: `database: added spotlight_channels.starts_at` at **20:35:23Z**, `SCHEMA_VERSION` **52 → 53**, one nullable column through `ADDED_COLUMNS`, no backfill and no rewrite — every existing row reads as *started already*. ⚠️ Nothing in it has met Discord: no modal has been opened, no **Set dates…** pressed, and no scheduled row has yet reached its start. `spotlight_mode` still ships **shadow**.

⚠️ **What met Discord at this deploy: only the boot.** Nothing in the build has met Discord live — no **Set dates…** modal has been opened on a real server, no scheduled row has reached its start, and `golive.spotlight_started` has never been logged for real. `spotlight_mode` still ships **shadow**, so rows `SD-b` and `SD-c` would land in `shadow_channel_id` (`#welcome-test`) rather than the go-live channel unless the owner turns it on first — that is the point of the mode, not a defect. Sweeps `SD-a`…`SD-d` are unwalked.

Verified: boot log — `database: added spotlight_channels.starts_at` **20:35:23Z**, `database ready` **20:35:23Z**, `synced 33 app commands` and `logged in as Black_Bloc` **20:35:26Z**, no Traceback (the one `settings ignored: youtube_mode` WARNING is the known normal state, [`access/runbook.md`](access/runbook.md)); `/health` ready (`ok true, ready true, guilds 1`, read 20:35:58Z); `release.json` answers **`v155`** at `6d5f5841`; and the **operator read** of `/api/golive/spotlight` ([`scripts/read.ps1`](access/operator-read.md), 20:36:11Z) answers `starts_at` on all **four** live rows — all `null`, i.e. *started already* — beside the new derived `scheduled` (false on all four) and `range` fields, which is the live proof that both the migration and the route change shipped. The gate was **7316 passed, 3 skipped** on the FIRST run, ruff clean, `check.mjs` ok **21 pages / 198 routes**, six node tests green. Re-measured by import at this ritual: keys **307** (`len(settings_store.KEY_TYPES)`), `SCHEMA_VERSION` **53**, cogs **23**, setting groups **24**, features **21**.

⚠️ **NOT verified:** nothing met Discord (above); no browser rendered the live Go-live page, its row drawer’s **Dates** card, the **Spotlight a channel** form or the new Settings drawer against production — the headless renders in the design doc were against the LOCAL MOCK, on the branch, before the deploy; `node site/mock/check.mjs` was **not re-run after the deploy**, so **21 pages / 198 routes** is the gate’s own figure; no backwards range was refused against production and no `starts_at` was ever WRITTEN live (the operator read is read-only, and all four rows still carry `null`); `TEST_MODE` was not touched and `spotlight_mode` was not flipped. The build’s own NOT-verified foot is in [`info/spotlight-dates-design.md`](info/spotlight-dates-design.md).

**The TODO item, moved whole:**

- 🔨 **A SPOTLIGHT IS A DATE RANGE — a start as well as an end** (owner, 2026-09-22 10:29 Phoenix, verbatim: *"in golive when i set a spotlight i can extend a week or keep forever. let me set a date range for start and end time"*). Today a `spotlight_channels` row has `expires_at` only (NULL = kept for ever) and the moves are Extend a week / Keep for ever / Let it expire. Adding a nullable `starts_at` (TEXT ISO UTC, NULL = started already) makes the row a RANGE: a row whose start is still ahead is **scheduled** — the poller does not announce it, no session opens, nothing is pinned or bumped, until the start has passed. Both doors get it: the `/golive` ▸ **Channels…** sub-panel's Add modal takes Starts + Ends and every row gets a **Set dates…** button, and the Go-live page's row drawer gets two `whenField` pickers with a **Save dates** button. `GET/POST/PATCH /api/golive/spotlight` carry `starts_at` and a `scheduled` flag; end-before-start is refused in words on both surfaces. Every new word is a settings key. Design: [`info/spotlight-dates-design.md`](info/spotlight-dates-design.md). Branch `spotlight-dates`, off `main` `f560ffa0`. Status: 🔨 **BUILT + MERGED, NOT DEPLOYED** — merge **`fad98c9e`** on `main` (five branch commits: `fa1918b9` this item, `6bae04fe` storage + helpers + keys, `5371768f` the cog, `7b2aa027` routes + mock + site, `509517c5` docs). **Measured, not guessed:** `pytest -n auto` **7,316 passed + 3 skipped** on the branch and again on `main` after the merge (collected **7,256 → 7,319**, so **+63** tests); `ruff check black_bloc tests site` clean; `node site/mock/check.mjs` **ok — 21 pages, 198 routes, 24 core settings, all keys present**; all six node tests green; `SCHEMA_VERSION` **52 → 53** (one nullable `spotlight_channels.starts_at` through `ADDED_COLUMNS`); registry **299 → 307** keys, the eight new ones all `golive`-namespaced with help, labels, mock rows and a contract entry; go-live page namespace keys **54 → 62**; the three `/api/golive/spotlight` routes gained fields, none was added or removed; one new log kind, `golive.spotlight_started` (ROUTINE). The Go-live and Settings pages were RENDERED headless against the local mock — the Dates card, both pickers, the Scheduled chip and the end-to-end worded refusal were all seen, zero console errors. ⚠️ **NOT deployed, and nothing has met Discord, Helix, the live site or a real database.** `TEST_MODE`, `release.json` and `deploys.log` untouched; `spotlight_mode` still ships **shadow**. Sweeps `SD-a`…`SD-d` are the owner's. Four follow-ups are named in the design doc — chiefly that **event-created spotlights still keep a NULL start**, which is a decision for the owner rather than an oversight.
  ↳ **LIVE as v155 13:35** (release commit `a285afc7`; `release.json` says `v155` at `6d5f5841`; merge `fad98c9e`). The four follow-ups the design doc names — chiefly that **event-created spotlights still keep a NULL start** — are the owner’s decisions, not oversights, and none of them was closed by this deploy.

## 2026-09-21 — v154 (20:08): the mock drops its preview door, a channel announcement stops looking like a spotlight, the logs channel goes quiet for channel/spotlight events, and the go-live settings page gets seven named, described drawers

**v154 (release commit `52c48235`; `release.json` says `v154` at `ef7c5fd5`; **7253 passed, 3 skipped** on the FIRST gate run, ruff clean, check.mjs ok **21 pages / 198 routes**, six node tests green; schema **52** (unchanged), keys **299** (unchanged), routes **198** (unchanged), pages **21** (unchanged), setting groups **24** (unchanged), deploys log **153** lines).** FIVE merges off four owner asks from the evening of 2026-09-21 (16:5x–17:3x) — all merged into `main` and put on the local mock for his check before he said deploy.

- **No more preview pages** (owner, 12:2x, verbatim: *"We don't need the preview pages anymore"*, Opus on branch `no-previews`, merge `4dd9f649`, **2 commits**): the mock serves the WORKING TREE at the normal URLs — `server.mjs`'s `/preview/` door and its `LIVE_ROOT` mirror of the deployed release are deleted (−40 lines), the `C:/lcw/bb-live` worktree and the move-the-mirror ritual retire, no Python changed.
- **A channel announcement is not a spotlight** (owner defect report 16:5x, verbatim: *"why is gdq being spotlighted in the logs channel? its spot light isnt on"*, design [`info/channel-streamers-design.md`](info/channel-streamers-design.md) § *Follow-up 2026-09-21 (16:5x)*, Opus on branch `channel-kinds`, merge `6017db05`, **3 commits + one conductor line**, **7 new tests**, sweep **736**): measured live — GDQ's channel row had spotlight OFF, announce ON, and its 22:53Z stream was announced in `#live-now` unpinned exactly as designed; the log embed the owner read was the IMPORTANT `golive.spotlight_announced` kind, reused for a non-spotlit channel with `spotlight: false` buried in the details — the behaviour was right and the title lied. A channel with spotlight OFF now logs `golive.channel_announced` / `golive.channel_ended` (`golive.would_channel_announce` in shadow mode), never a `spotlight_*` kind; the conductor made `channel_ended` ROUTINE like every other end (`golive.spotlight_ended` and `golive.end` both already were).
- **No log embeds for channel linking, spotlight or announce** (owner, 17:2x, verbatim: *"i dont want log messages appearing in black bloc logs for channel linking or channel spotlight or channel annouce"*, Opus on branch `quiet-channel-kinds`, merge `78aed68a`, **1 commit**, sweep **737**): six kinds moved from IMPORTANT to ROUTINE — `golive.channel_announced`, `golive.history_swept`, `golive.spotlight_added/announced/expired/removed` — six more (`golive.link`, `youtube.link`, and the rest of the spotlight family) were routine already; the rows still land on the Logs page under the **golive** chip, and `golive_log_level = all` restores the Discord mirror with no deploy; `golive.costream_added` was left IMPORTANT as a judgement call because it rewrites a member's post.
- **Recent streams drops the Mode-then column** (owner, 17:2x–17:4x, asked what the *Mode then* column meant, then verbatim: *"yes drop the mode column too"*, Sonnet on branch `mode-column`, merge `f3f16dd0`, **1 commit**, sweep **738**, rendered headless): the column read `on` for every row since test mode lifted and was noise; the table is back to six columns and the Ended cell carries a `rehearsal` badge for any row not posted for real.
- **Go-live ▸ Everything else — duplicates and unclear help** (owner, 17:3x, verbatim: *"in the everything else section of golive there are duplicate settings it seems, also what each of the settings does isnt clear"*, design [`info/golive-page-design.md`](info/golive-page-design.md) § *Follow-up 2026-09-21: the settings read right*, branch `golive-settings-help`, merge `ef7c5fd5`, **4 commits**, sweep **739**): the "duplicates" were eight keys in the Everything else catch-all differing only by feature prefix (`golive_panel_minutes` / `youtube_panel_minutes` / `pings_panel_minutes` and the matching `*_log_level` trio); the fix regrouped all 54 golive/pings/youtube keys into seven named, described drawers (Who gets announced, Where the announcement goes, How a stream is spotted, Spotlighted channels, Two platforms at once, Ping roles, Slash panels and log lines), so Everything else renders nothing; every one of the 54 keys got a `KEY_HELP` line of 10+ words, pinned equal across the registry, `contract.json` and the mock by tests. ⚠️ **Started by an Opus agent that was dropped FIVE times by Anthropic-side `529`/`500` errors during a minor outage (17:5x–18:3x)** — it checkpoint-committed after every step and lost zero work; a Sonnet agent finished the last three commits from the same worktree. Now **KI-38**, `WATCHING`.

⚠️ **What met Discord at this deploy: only the boot.** Nothing in any of the five builds has met Discord live — no channel-off announcement has logged under its new kind on a real server, no embed has been suppressed in `#blackbloc-logs` for a real event, and no staff member has opened the redrawn Recent streams table or a named settings drawer against production. Sweeps **736–739** are unwalked.

Verified: boot log — `database ready` **03:08:42Z** and `logged in as Black_Bloc` **03:08:45Z**; `/health` ready; `release.json` answers **`v154`** at `ef7c5fd5`. The gate was **7253 passed, 3 skipped** on the FIRST run, ruff clean, `check.mjs` ok **21 pages / 198 routes**, six node tests green. Registry keys re-measured by import at this ritual: **299** (`len(settings_store.KEY_TYPES)`), unchanged from v153.

⚠️ **NOT verified:** nothing met Discord (above); `node site/mock/check.mjs` was **not re-run after the deploy**, so **198 routes** is the gate's own figure, not a post-deploy one; no browser rendered the live Logs page or any of the seven new go-live settings drawers — only `/health`, `release.json` and the boot log were read against production. The five builds' own docs carry their own NOT-verified feet (`info/channel-streamers-design.md`, `info/golive-page-design.md`, `info/autolink-design.md`'s quiet-kinds note).

**The four TODO items, moved whole:**

- 🧭 **GO-LIVE ▸ EVERYTHING ELSE — duplicates and unclear help** (owner, 2026-09-21 17:3x, verbatim: *"in the everything else section of golive there are duplicate settings it seems, also what each of the settings does isnt clear"*). Build: measure the Everything else bucket on the mock (`page-golive.js:1515`, `golive-join.js:placeSettings`), find the duplicates (same key drawn twice, or two keys with one label), give every golive/pings/youtube key a help line that says what it does and when it applies in plain words (the site AND `/settings` read `KEY_HELP`), move stragglers into the named drawers so Everything else is small or gone. Also asked 17:2x about the *Mode then* column on Recent streams: it is the go-live mode at announce time (on / shadow / off); every row reads on since test mode lifted — recommended dropping the column for a rehearsal badge on non-on rows; Owner 17:4x: *"yes drop the mode column too"* → dispatched to Sonnet as branch `mode-column` (drop *Mode then*; a *rehearsal* badge on the Announced cell for any row not posted for real). Settings-help dispatched to Opus as branch `golive-settings-help`. **Mode column LANDED on main 17:5x (1 commit; rehearsal badge on the Ended cell; rendered headless; sweep 738).** Settings-help: its Opus agent died on an API 529 mid-build, was resumed, and died again on a 500 — Anthropic server errors, 17:5x; retried once the API answers; the worktree keeps its 10 edited files. **Settings-help LANDED on main 18:5x (finished by a Sonnet agent after five Opus 529 drops; 4 commits; the 'duplicates' were eight keys in Everything else differing only by feature prefix — panel_minutes / log_level per feature; now seven named drawers with descriptions, all 54 keys placed, Everything else renders nothing; all 54 help lines rewritten; registry ↔ contract ↔ mock help pinned equal by tests; 7251 tests; sweep 739; rendered headless).** Status: both waiting on the owner's word to deploy.
  ↳ **LIVE as v154 20:08** (release commit `52c48235`; `release.json` says `v154` at `ef7c5fd5`). Mode column shipped as merge `f3f16dd0`; Settings-help shipped as merge `ef7c5fd5`, sweeps **738–739**. ⚠️ Nothing in either has met Discord or been opened in a browser against production.

- 🔕 **NO LOG EMBEDS IN #blackbloc-logs FOR CHANNEL LINKING, CHANNEL SPOTLIGHT OR CHANNEL ANNOUNCE** (owner, 2026-09-21 17:2x, verbatim: *"i dont want log messages appearing in black bloc logs for channel linking or channel spotlight or channel annouce"*). Today `actionlog.should_post` posts every IMPORTANT kind as an embed to `log_channel_id`. Build: reclassify to ROUTINE `golive.link`, `youtube.link`, `golive.history_swept`, `golive.spotlight_added/announced/expired/removed/updated/pinned/bumped/unpinned`, `golive.channel_announced` (the `_failed` twins, `role_stuck` and the opt-out kinds stay IMPORTANT); the rows still land on the Logs page; `golive_log_level` already lets staff turn the mirror up or down. Dispatched to Opus as branch `quiet-channel-kinds`; **LANDED on main 17:4x (1 commit; six kinds moved to routine, six already were; `golive_log_level = all` brings the embeds back; `golive.costream_added` left loud as a judgement call — it rewrites a member's post; 7246 tests; sweep 737; nothing met Discord).** Status: waiting on the owner's word to deploy.
  ↳ **LIVE as v154 20:08** (release commit `52c48235`; `release.json` says `v154` at `ef7c5fd5`; merge `78aed68a`). ⚠️ Nothing in it has met Discord — no embed has been suppressed for a real event.

- 🏷️ **A CHANNEL ANNOUNCEMENT IS NOT A SPOTLIGHT — the log kind must say so** (owner, 2026-09-21 16:5x, verbatim: *"why is gdq being spotlighted in the logs channel? its spot light isnt on"*). MEASURED on live: GDQ row 3 has spotlight OFF, announce ON; its stream opened session 3 at 22:53Z and was announced in `#live-now` (`golive_channel_id`), unpinned, as designed. What the owner saw in `#blackbloc-logs` is the IMPORTANT log embed for kind `golive.spotlight_announced`, which the channel-streamers build REUSED for a non-spotlit channel (its Deviation 2, with `spotlight: false` in the details) — the embed's title says *spotlight announced* for a channel whose spotlight is off. Fix: a channel row's announcement with spotlight OFF logs `golive.channel_announced` (IMPORTANT) and its end `golive.channel_ended`, worded *channel announced / channel ended*; the `spotlight_*` family means the spotlight only; the Logs page filter, `logkinds.py`, the mock seed and the kind guards follow. Dispatched to Opus as branch `channel-kinds`; **LANDED on main 17:2x (3 commits + one conductor line: `golive.channel_ended` is ROUTINE like `spotlight_ended` and `golive.end`, so an end never reaches the logs channel; `channel_announced` IMPORTANT; 7 tests; sweep 736; old rows keep their old spelling — no migration; nothing met Discord).** Status: waiting on the owner's word to deploy.
  ↳ **LIVE as v154 20:08** (release commit `52c48235`; `release.json` says `v154` at `ef7c5fd5`; merge `6017db05`, sweep **736**). ⚠️ No existing `golive.spotlight_announced` row is rewritten — there is no migration — and nothing has met Discord since the deploy.

- 🧹 **NO MORE PREVIEW PAGES** (owner, 2026-09-21 12:2x, verbatim: *"We don't need the preview pages anymore"*). The live `/preview/` pages went at v150; what remains is the local mock's `/preview/<page>.html` door (`site/mock/server.mjs` — `LIVE_ROOT`, `realChanged`, `whereFrom`, the `/preview/assets/` rewrite) that served the working tree's page beside the deployed release. Build: the mock serves the WORKING TREE at the normal URLs, the `/preview/` route and the `LIVE_ROOT` mirror go, the `C:/lcw/bb-live` worktree and the move-the-mirror ritual retire, docs follow (`access/deploy.md`, `access/site.md`, `info/architecture.md`, the design docs that name `/preview/`). Dispatched to Opus as branch `no-previews`; **LANDED on main 12:4x (2 commits, server.mjs −40 lines, no Python changed; the mock on 8797 now runs from main without `LIVE_ROOT`; `C:/lcw/bb-live` removed).** Status: waiting on the owner's word to deploy (mock-only change; nothing on live moves).
  ↳ **LIVE as v154 20:08** (release commit `52c48235`; `release.json` says `v154` at `ef7c5fd5`; merge `4dd9f649`). Mock-only change — nothing on the live bot moved because of it, and it shipped inside the v154 release image alongside the other four.


## 2026-09-21 — v153 (12:13): a member's opt-out ends the announcement that is already out, and a YouTube video link can name the channel (shipped OFF)

**v153 (release commit `3d75254`; `release.json` says `v153` at `0b3653d`; **7237 passed, 3 skipped** on the THIRD gate run, schema **52** (unchanged — neither build added a column), keys 297 → **299**, routes **198** (unchanged), pages **21** (unchanged), setting groups **24** (unchanged), cogs **23** and top-level commands **33** (unchanged), deploys log **152** lines).** TWO builds off the owner's 10:4x and 10:5x asks — both merged into `main` and put on the local mock for his check before he said deploy.

- **A member's opt-out ends the announcement that is already out** (owner ask 10:4x, verbatim: *"opt out should end their annoucement"*, design [`info/golive-panel-design.md`](info/golive-panel-design.md) § *Follow-up 2026-09-21*, Opus on branch `member-optout`, merge `1deb296`, **3 commits**, **eight deviations**, **15 new tests**, sweeps **732–733** (were `MO-a` / `MO-b`)): `opt_out` no longer only writes the `golive_optout` row and drops the fan role — a member with an OPEN `golive_sessions` row now runs the existing end path at once (unpin if pinned, the one end wording, the live role off, the session closed). The new key **`golive_member_optout_post`** (enum, group `golive`, default **`end`**) mirrors the channel's `golive_channel_optout_post` and decides what becomes of the post that is already out — `end` unpins and edits it to the one end wording, `delete` removes it outright, `leave` leaves the words alone and takes only the pin. Two new ROUTINE kinds, **`golive.post_deleted`** and **`golive.unpinned`**, each with an IMPORTANT `_failed` twin. The same sentence comes back from `/golive` ▸ **Stop announcing my streams**, the member drawer's **Opt out** and `POST /api/golive/optouts`. ⚠️ **The deviation worth knowing:** the brief named a design doc that **does not exist**, so the follow-up was written into [`info/golive-panel-design.md`](info/golive-panel-design.md) instead — that is where this build's design and its eight deviations live. ⚠️ **And a finding that outlives the build:** the builder's three "deadlocks" at 89/98/93 % of pytest were a SECOND 8-worker gate that the PowerShell tool had BACKGROUNDED after a timeout — not a deadlock at all. It is now part of **KI-26**.
- **A YouTube video link can name the channel — shipped OFF** (owner question 09:5x, go-ahead 10:5x verbatim: *"Let's build it but keep it off for now"*, design [`info/autolink-design.md`](info/autolink-design.md) § *Follow-up 2026-09-21*, Opus on branch `youtube-video-link`, merge `0b3653d`, **3 commits**, **eight deviations**, **24 new tests**, sweeps **734–735** (were `YV-a` / `YV-b`)): `YouTubeClient.resolve_video_channel` and `video_id_in` read the channel behind a `watch?v=` address the keyless way `resolve_without_key` reads a channel page, wired into `link_from_url` behind the new key **`golive_autolink_youtube_video`** (bool, group `golive`, **default false**) — with it ON, a watch URL links the channel both at go-live time and in the history sweep; with it OFF, which is today and is what is on live, nothing changes at all. `youtube.link` gains `because` / `via_video` when it fires. 🔴 **The parser is the thing to read:** it rests on the watch page's **player JSON `channelId`**, measured on two real watch pages and one channel page — the `<meta itemprop="channelId">` tag everybody reaches for first was **ABSENT on both watch pages**, so there is no second source to fall back on and a YouTube markup change breaks it silently. ⚠️ Its two sweep rows need a **YouTube presence this server has produced ZERO of in 166 live sessions**, so they may stay unwalked indefinitely.

⚠️ **The gate took THREE runs, and only the third reached Fly.** Run 1 was **REFUSED** on `tests/api/tools/test_guides.py::test_a_picture_is_uploaded_against_a_step_and_served_back_to_a_member`, with **7236 passed** beside it — **KI-32**'s SECOND sighting, and ⚠️ a *different* test in the same file from the one that entry is titled for, which widens its symptom to the guides media-file family. Run 2 was **killed by the OS for low memory at 99 % of pytest** (`[gw6] node down: Not properly terminated`) while a **12 GB whisper transcription job outside this project** held the machine — now **KI-37**, `WATCHING`. **Neither reached Fly.** Run 3 shipped.

⚠️ **The join fixture collided again, and this is the second time.** Both branches bumped the every-key-lands-once join fixture (`site/mock/golive-join.test.mjs`, the golive/pings/youtube namespaces) **52 → 53** independently; merged, the honest number is **54** — measured. The first collision was the `autolink` build's **Deviation 16**. A merge that resolves such a conflict by taking one side passes the count and silently drops a key. Filed as **KI-36**, `WATCHING`; the fix is one line — count the registry instead of typing the number.

⚠️ **What met Discord at this deploy: only the boot.** Nothing in either build has met Discord or YouTube **live** — no announcement has been ended by an opt-out, nothing has been deleted, no pin has come off, and no watch URL has been resolved from Fly. Sweeps **732–735** are unwalked.

Verified: boot log — `database ready` **19:13:07Z** (schema **52**, no migration: this release added no column) and `logged in as Black_Bloc` **19:13:11Z**; `/health` ready; `release.json` answers **`v153`** at `0b3653d`. The gate on its third run was **7237 passed, 3 skipped**, ruff clean, `check.mjs` ok **21 pages / 198 routes**, six node tests green. ✅ **The one live measurement this build could make was made:** the deployed `/api/settings` shows **`golive_autolink_youtube_video` — type `bool`, value `false`, default `false`**, so the brake is confirmed ON in production. Registry keys re-measured by import 2026-09-21 12:2x: **299** (`len(settings_store.KEY_TYPES)`), 24 namespaces, the golive/pings/youtube join set **54** — ⚠️ **the dispatch brief for this ritual said 298 and was one short**; 297 at v152, plus the two new keys, is 299.

⚠️ **NOT verified:** nothing met Discord, Helix or YouTube from Fly, so neither `end` nor `delete` nor `leave` has ever been applied to a real announcement, and `resolve_video_channel` has never run in production — it cannot, the key is off. The parser was measured on two real watch pages **from this machine**, never from Fly's datacenter address, and **KI-30** already records that YouTube serves a bot check to datacenter addresses — so the one measurement that decides whether this works in production is the one that is missing. `node site/mock/check.mjs` ran in the gate but was **not re-run after the deploy**, so **198 routes** is the gate's figure and not a post-deploy one. No browser rendered the new Settings row or the member drawer's sentence; the site half is proved by `check.mjs` and the suite, not by a page anybody looked at. No human has opened a dashboard page since the deploy and no Discord button has been pressed. And the run-1 and run-2 gate failures were **retried, not diagnosed**. One more thing worth recording because it nearly cost a doc: the `youtube-video-link` builder **truncated `access/sweeps.md` to 0 bytes with a bad open** and restored it from git — the incident behind this ritual's write-to-temp rule.

**The two TODO items, moved whole:**

- 🔇 **MEMBER OPT-OUT ENDS THEIR OPEN ANNOUNCEMENT** (owner, 2026-09-21 10:4x, verbatim: *"opt out should end their annoucement"* — the answer to the v152 asymmetry). `cogs/content/golive.py:opt_out` (:655) only writes the `golive_optout` row; a member's open `golive_sessions` row and its posted announcement stay until the stream ends. Build: opt-out with an open session runs the existing `end_live` path now (unpin if pinned, the one end wording, live role off, session closed, `golive.end` with `reason: opted_out`), a key `golive_member_optout_post` (end/delete/leave, default end) mirroring the channel's; the same sentence on `/golive` ▸ Stop announcing my streams, the member drawer's Opt out and `POST /api/golive/optouts`. Dispatched to Opus as branch `member-optout`; **LANDED on main 11:5x (`1deb296`, 3 commits; key `golive_member_optout_post`; 15 tests; sweeps 732–733; nothing met Discord; the follow-up lives in `info/golive-panel-design.md`) — on the local mock at `http://127.0.0.1:8797/preview/golive.html` ▸ Casey (live) ▸ Opt out.** Status: waiting on the owner's word to deploy.
  ↳ **LIVE as v153 12:13** (release commit `3d75254`; `release.json` says `v153` at `0b3653d`). The key shipped as **`golive_member_optout_post`** (enum `end`/`delete`/`leave`, default `end`), registry keys 297 → **299** with the YouTube key beside it, **15** new tests, sweeps **732–733**, and two new ROUTINE kinds `golive.post_deleted` / `golive.unpinned` with IMPORTANT `_failed` twins. ⚠️ **Nothing in it met Discord** — no opt-out has ended a real announcement. The follow-up doc is [`info/golive-panel-design.md`](info/golive-panel-design.md) § *Follow-up 2026-09-21*, not the doc the brief named, which does not exist. This closes the v152 asymmetry on both halves, so the open sentence it rode on has come OFF the residue line of [`TODO.md`](TODO.md).

- ▶️ **YOUTUBE PRESENCE — how a YouTube go-live can link the channel** (owner question, 2026-09-21 09:5x, verbatim: *"what can we do about youtube presence? can we just wait for a go live and then get the channel from it or whats the best way? people have go live messages for youtube?"*). MEASURED on live 09:5x: 166 go-live sessions in history, 159 presence/Twitch, 5 Twitch poller, 2 presence with no platform, **zero YouTube addresses ever**. Discord sets the Streaming status itself only for Twitch; a YouTube presence comes only from a third-party rich-presence app and carries a `watch?v=` URL. Owner 10:5x, verbatim: *"Let's build it but keep it off for now"* → a keyless `resolve_video_channel(video_id)` reading the watch page's channel id the way `resolve_without_key` reads a channel page, wired into `link_from_url` behind a NEW key `golive_autolink_youtube_video` (bool, **default false**), so with it on a watch URL links the channel at go-live time and in the history sweep, and with it off (today) nothing changes. Dispatched to Opus as branch `youtube-video-link`; **LANDED on main 12:0x (3 commits; key `golive_autolink_youtube_video` default false; 24 tests; sweeps 734–735, which need a YouTube presence this server has never produced; parser relies on the watch page's player JSON `channelId`, measured on two real pages; nothing met Discord or YouTube from Fly) — on the local mock at `http://127.0.0.1:8797/preview/settings.html` ▸ Go-live ▸ How streams are spotted.** Status: waiting on the owner's word to deploy (with member-optout).
  ↳ **LIVE as v153 12:13** (release commit `3d75254`; `release.json` says `v153` at `0b3653d`) — and ✅ **shipped OFF, measured on live**: the deployed `/api/settings` shows `golive_autolink_youtube_video`, type `bool`, value **`false`**, default **`false`**. **24** new tests, sweeps **734–735**, which still need the YouTube presence this server has produced **zero** of in **166** live sessions. 🔴 The parser rests on the watch page's player JSON `channelId` — the `<meta itemprop="channelId">` tag was **absent on both** real watch pages measured — and it has never run from Fly, where **KI-30** says YouTube answers a datacenter address with a bot check.

## 2026-09-21 — v152 (10:35): opting a live channel out ends the announcement that is already out, and spotlight off unpins now

**v152 (release commit `32d3c0b`, which IS the deployed commit; `release.json` says `v152` at `8dcb52f`; **7189 passed, 3 skipped** in both orders, schema **52** (unchanged), keys 296 → **297**, routes **198** (unchanged), pages **21** (unchanged), setting groups **24** (unchanged), cogs **23** and top-level commands **33** (unchanged), deploys log **151** lines).** ONE build, off the owner's 09:5x defect report at the v151 landing — merged into `main` and put on the local mock for his check before he said deploy.

- **Channel opt-out ends the announcement that is already out** (owner defect report 09:5x, design [`info/channel-streamers-design.md`](info/channel-streamers-design.md) § *Follow-up 2026-09-21* 10:0x, Opus on branch `channel-optout`, merge `8dcb52f`, **2 commits** (`d8f5654` the build, `249deca` the docs), **six follow-up deviations**, **10 new tests**, sweeps **730–731** (were `CO-a` / `CO-b`)): `change_spotlight` funnels through `changed_spotlight`, which returns `(row, settled)` and settles an OPEN session **under the row's own lock**. `announce` 1 → 0 with a session open runs the existing `_end` path with the reason word **`opted_out`** — the session is closed, a `golive.spotlight_ended` row is written carrying `reason: opted_out` **and a `post` detail** saying which treatment ran, and no reminder follows. `spotlight` 1 → 0 with a session open **only unpins** (`golive.spotlight_unpinned` with `because: spotlight_off`); the session stays open and the post stays live-worded, because the stream still is, and it is edited to past tense when the stream really ends. Turning either back ON settles nothing. The new key **`golive_channel_optout_post`** (enum, group `golive`, default **`end`**) decides what becomes of the post that is out — `end` unpins and edits it to the ONE end wording, `delete` removes it outright (the new routine kind **`golive.spotlight_post_deleted`**, its failure reusing `golive.spotlight_post_failed` with `what: "delete"`), `leave` leaves the words alone and takes only the pin — and `spotlight.announce_said` / `spotlight_said` is the ONE sentence both doors return, so `/golive` ▸ **Channels…** ▸ **Opt out of announcements** and `PATCH /api/golive/spotlight/{id}` say the same thing. ⚠️ **The deviation that matters:** the end row spells its why **`reason`**, not `because` as the brief asked, because every other end row in the family already spells it that way; `because` was instead ADDED to `golive.spotlight_unpinned` / `_unpin_failed`, which had no why field at all.
- **Why the key exists at all:** the owner's ESA row runs reruns around the clock, so the only thing that ends a spotlight session — `spotlight_end_misses` quiet Helix polls — never happens, and the pin was permanent. This is the case the `channel-streamers` build never considered; its **Deviation 5** asserted the opposite and is struck in place (checklist 35).

⚠️ **What met Discord at this deploy: only the boot.** Nothing in this build has met Discord or Helix **live** — no pin has come off a real message, nothing has been deleted, and no announcement has been edited by this code. Sweeps **730–731** are unwalked.

Verified: boot log — `database ready` (schema **52**, no migration: this release added no column) and `logged in as Black_Bloc`; `/health` ready; `release.json` answers **`v152`** at `8dcb52f`. The gate was **7189 passed, 3 skipped**, **both orders**. Registry keys re-measured by import 2026-09-21 10:38: **297** (`len(settings_store.KEY_TYPES)`), the golive/pings/youtube join set the key was filed against reading **52**. One boot line worth knowing about: `spotlight: could not read message 1551636949834662019 — NotFound` — ESA's announced post, **deleted by hand by the owner before this deploy**; harmless, the code carries on. Applied on live through the site after boot: ESA (row id **4**) was opted back **in** and **out** once to settle its open session 2 — the step the TODO item warned would be needed, because the settle fires only on the on→off flip and ESA was already opted out when the deploy landed.

⚠️ **NOT verified:** **the result of that ESA settle was not captured in this doc** — nobody wrote down whether the pin came off or what the post then read, so the one real-world exercise of this build is unrecorded. Nothing met Discord or Helix otherwise. `delete` was never exercised against a message Discord refused to delete. No browser rendered the drawer's new sentence; the site half is proved by `check.mjs` and the suite, not by a page anybody looked at, and **the mock's settle is a hand-written mirror** (`site/mock/server.mjs` — `settleOpenSession`, `OPTED_OUT_POST_SAID`, `UNPINNED_NOW`), so if the Python wording changes and the mock's does not, nothing fails. `node site/mock/check.mjs` was not re-run after the deploy, so **198 routes** is still read off `site/mock/contract.json`. And ⚠️ **one unexplained random-order flake**: 2 tests failed on ONE of six gate runs and **the two names were lost**; five later runs were clean. Filed as **KI-35**, `WATCHING`.

**The TODO item, moved whole:**

- 🔇 **CHANNEL OPT-OUT MUST END AN OPEN SESSION** (owner, 2026-09-21 09:5x, verbatim: *"the bot went live and annouced esam and pinned it, it should have been in the list silent. i opted out of the spotlight for it and opted out of notifications"*). Measured: ESA's spotlight session 2 opened 16:51:16Z, the same second the bot logged in — the boot poll announced and pinned it with the migration's `announce = 1` default, before the opt-out could be applied. Then `change_spotlight` only updates the row: the open session stays open and PINNED until the stream ends, and a 24/7 rerun channel never ends. Fix dispatched to Opus as branch `channel-optout`: announce off with an open session ends it now (unpin + the one end wording, or delete the post per a new key), spotlight off with an open session unpins now. **LANDED on main 10:1x (2 commits; key `golive_channel_optout_post` end/delete/leave; 10 tests; sweeps 730–731; nothing met Discord) — on the local mock at `http://127.0.0.1:8797/preview/golive.html` ▸ ESA Marathon ▸ Announcements.** ⚠️ After its deploy ESA is ALREADY opted out with session 2 open and pinned, and the settle fires only on the on→off flip: opt ESA back in and out once through the site (or unpin by hand). Status: waiting on the owner's word to deploy.
  ↳ **LIVE as v152 10:35** (release commit `32d3c0b`, which is also the deployed commit; `release.json` says `v152` at `8dcb52f`). The after-the-deploy step the item named WAS done: ESA was opted back in and out once through the site to settle its open session 2 — ⚠️ its result was not captured. The key shipped as **`golive_channel_optout_post`** (enum `end`/`delete`/`leave`, default `end`), registry keys 296 → **297**, 10 new tests, sweeps **730–731**. ⚠️ **The MEMBER opt-out was deliberately NOT changed to match** (follow-up Deviation 5): a member's open session ends by itself within the hour, a 24/7 rerun channel's never does. The owner has been told and has **not answered** — the open sentence rides on the residue line of [`TODO.md`](TODO.md), not as an item of its own.

## 2026-09-21 — v151 (09:51): a channel is a persistent row of its own, and the go-live history links people to the channel they streamed from

**v151 (release `c1b83f0`, deployed commit `c7ef8f1`, `release.json` says `v151` at `9ebf15b`; **7178 passed, 3 skipped**, schema 51 → **52**, keys 294 → **296**, routes 197 → **198**, pages **21** (unchanged), setting groups **24** (unchanged), cogs **23** and top-level commands **33** (unchanged), deploys log **150** lines).** Two builds off the owner's morning asks, both merged into `main` and put on the local mock for his check before he said deploy.

- **Channel streamers** (owner ask 08:1x, design `info/channel-streamers-design.md` 08:2x, Opus on branch `channel-streamers`, merge `e3f873a`, **7 commits**, **nine deviations**, **22 new tests**, sweeps **719–725**): the spotlight row becomes the **persistent channel record** — `spotlight`, `announce`, `youtube_channel_id` and `youtube_handle` are on/off (or set) toggles on the row, so removing a channel from Spotlight no longer deletes the channel, its ping role and its sessions with it, which is what the owner hit. A channel going live is announced like a member's link **whether or not spotlight is on** (spotlight adds the pin and the four-hourly bumps); the YouTube live sweep now walks channel rows as well as member links; **Add a streamer** takes a channel with no member behind it (a Twitch login and/or a YouTube handle); **Remove** asks first and says the role goes too; `/golive` ▸ *Spotlight…* became **Channels…**; new key `golive_channel_spotlight_default`. ⚠️ **Deviation 1:** `twitch_login` was NOT relaxed, so a YouTube-only channel is refused **in words** for now. The migration is **purely additive — four columns, no table rebuild** (schema **52**), which is why no backup was needed for this one.
- **Auto-link** (owner ask 08:3x, design `info/autolink-design.md` 08:4x, Opus on branch `autolink`, merge `731519e`, **4 commits**, **sixteen deviations**, **27 new tests**, sweeps **726–729**): `link_from_history` sweeps `golive_sessions` and links every member with no channel to the newest Twitch or YouTube address their go-live carried — through the ONE `link_channel` path, idempotent, with opted-out, departed and already-linked members skipped, another member's channel refused by name, and one IMPORTANT `golive.history_swept` row carrying the counts. Doors: **Link from history** in the Streamers toolbar behind an `ask()`, `POST /api/golive/links/sweep`, and a staff button on `/golive`. Plus `golive_autolink_presence` (**true** by default): a presence go-live for an unlinked member writes the link at announce time, refusals logged ROUTINE as `golive.autolink_refused`. 🔴 **Deviation 2 is the one to read:** the already-linked check has to run BEFORE the readable check, or a clean server is told *1 could not be read* every time.
- **Rode along:** (1) `discordmock.test.mjs`'s embed-stamp fixture was widened (`59716fe`) — the fixture was dated **2026-09-20**, so from the next day the stamp read *Yesterday at* and the unchanged test went red **on `main` AND on the v150 release commit alike**; a date-dependent fixture, not a regression, now filed as **KI-33**. (2) The sweep report keeps its own notice (`d56e899`) — `load()` hands ONE `say` node to both `streamersSection` and `recentSection`, and a DOM node lives in one place, so the *Link from history* report landed inside the **collapsed** Recent streams: in the accessibility tree, out of the pixels. Fixed for that door with `sayAgain('golive.sweep', notice())`; the five other notices that still share the node are **KI-34**.

⚠️ **What met Discord at this deploy: only the boot.** Nothing in either build has met Discord, Helix or YouTube **live** — no channel has gone live since, no presence go-live has been auto-linked, no announcement has been posted from a channel row. Sweeps **719–729** are all unwalked.

Verified: boot log — `database: added spotlight_channels.spotlight`, `database: added spotlight_channels.announce`, `database: added spotlight_channels.youtube_channel_id`, `database: added spotlight_channels.youtube_handle`, then `database ready` (schema **52**, four ADDED columns, **no rebuild**), logged in as **Black_Bloc**, `/health` ready. Applied on live **through the site** after boot (09:5x): GDQ (row id **3**) linked to `@GamesDoneQuick` — `UCI3DTtB-a3fJPjKtQ5kYHfA`, title *Games Done Quick*; ESA (row id **4**) linked to `@esamarathon` — `UC3Oe-jfrIqEGygxYBYyN6jQ`, title *ESA Speedrunning* — and its **announce set OFF**; `rpglimitbreak` added as row id **5**, kept, spotlight **on**, announce **OFF**; and **Link from history run once** (the links count on live afterwards read **29**). Earlier, 08:30 on live: the member `nadia_hooligan_` opted out of go-live announcements. Observed at boot and worth knowing: `spotlight: could not read message 1551613305230069853 — NotFound` — a pinned spotlight message that had been deleted by hand; harmless, the code carries on. ⚠️ **observed 09:5x: ESA was announced and pinned at boot before its opt-out landed — see the 🔇 item on [`TODO.md`](TODO.md)**, dispatched as branch `channel-optout`.

⚠️ **NOT verified:** the **Link from history** report SENTENCE was not captured (only the resulting links count, 29, was read) — what it said about kept / skipped / unreadable is unknown; `node site/mock/check.mjs` was **not** re-run after the deploy, so **198 routes** is read off `site/mock/contract.json`, not off a live check; no human opened a dashboard page after the deploy; no Discord button was pressed; `/golive` ▸ **Channels…** has never been opened by a person; the YouTube sweep has never walked a channel row against YouTube; and `golive_autolink_presence` has never fired, because nobody has gone live.

**The two TODO items, moved whole:**

- 🔗 **AUTO-LINK — a sweep of the go-live history links people to the Twitch/YouTube channel they streamed from, and a presence go-live links them from then on** (owner, 2026-09-21 08:3x, verbatim: *"can we do a sweep of the golive channel and link people to their twitch channel and or YouTube channel? Can we also perform the link for each user when they go live with presence?"* — plus *"It seems like you need to be linked to be spotlighted. While this is preferred I don't think this should be mandatory."* → told the running `channel-streamers` build: spotlight never requires a link). Design `info/autolink-design.md`: (1) a one-off staff sweep (site button + `/golive` ▸ Link from history) over `golive_sessions` — every presence-detected session with a twitch.tv / youtube URL whose member has no link gets one, logged per person, opted-out members skipped; (2) `golive_autolink_presence` (bool, default ON): a presence go-live for an unlinked member writes the link at announce time. **Designed 08:4x (`info/autolink-design.md`: a staff sweep over `golive_sessions` through the one `link_channel` path, idempotent, opted-out skipped, a report in words; `golive_autolink_presence` ON by default — a presence go-live links the member at announce time; `golive.history_swept` / `golive.autolink_refused`).** Dispatched to Opus as branch `autolink` 08:31; **LANDED on main 09:2x (`731519e`, 4 commits incl. a merge of main; 27 tests; sweeps 726–729; nothing met Discord; open question: whether a YouTube presence carries a watch URL or a channel address — row 727 settles it) — on the local mock at `http://127.0.0.1:8797/preview/golive.html` ▸ Streamers ▸ Link from history.** Also 08:4x, owner: *"once built let's keep esam in the list but opt them out of notifications"* → an `announce` on/off on the channel row, added to the running `channel-streamers` build (row `CS-g`). Also 08:3x, owner: *"Also link rpglimitbreak but also opt them out so we have them in the list but no notifications from them"* → after the channel build deploys: add `rpglimitbreak` as a channel row (kept), `announce` off; opt-out of the member nadia_hooligan_ DONE on live 08:30. Status: building.
  ↳ **LIVE as v151 09:51** (release `c1b83f0`, deployed commit `c7ef8f1`). The `channel-streamers` follow-ups named in the item are DONE on live: `rpglimitbreak` added kept with announce off, `nadia_hooligan_` opted out at 08:30. **Link from history** was run once on live (links afterwards: 29; the report sentence was not captured). The open question it names — whether a YouTube presence carries a watch URL or a channel address — was MEASURED at 09:5x and is now its own open item: **zero** YouTube addresses in **166** live sessions.

- 📺 **CHANNEL STREAMERS — an org channel is a persistent row like any linked member; spotlight and the ping role are on/off toggles on it; a YouTube channel can be linked too** (owner, 2026-09-21 08:1x, verbatim: *"I removed them from Spotlight as a test and they vanished from the whole list. I want them linked like any other stream so they're persistent. I then want to be able to spotlight or role them like any other stream on and off. Also find YouTube channels for both of them if possible to link"*). Today a spotlight row IS the only record of a channel with no member, so Remove deletes the whole row. Design `info/channel-streamers-design.md`: the spotlight row becomes the persistent channel record (`spotlight` on/off flag + `youtube_channel_id`), a channel going live is announced like a member's link whether or not spotlight is on (spotlight adds the pin and the bumps), the ping role stays attached to the row, the YouTube sweep watches a channel row's YouTube channel, Add a streamer takes a channel with no member (Twitch login and/or a YouTube handle), Remove asks and says the role goes too. GDQ and ESA re-added as kept spotlight rows meanwhile (08:1x). **Designed 08:2x (`info/channel-streamers-design.md`: `spotlight_channels` gains `spotlight` on/off + `youtube_channel_id`/`youtube_handle`, schema 52; a channel going live is announced like a member's link whether or not spotlight is on; the YouTube sweep walks channel rows; Add a streamer takes a channel with no member; `/golive` ▸ Channels…; key `golive_channel_spotlight_default`) and dispatched to Opus as branch `channel-streamers`.** **LANDED on main 09:0x (`e3f873a`, 7 commits, schema 52 additive, 22 tests, sweeps 719–725; YouTube ids resolved: GDQ `UCI3DTtB-a3fJPjKtQ5kYHfA`, ESA `UC3Oe-jfrIqEGygxYBYyN6jQ`; nothing met Discord/Helix/YouTube live) — on the local mock at `http://127.0.0.1:8797/preview/golive.html` for the owner's check.** Status: waiting on the owner's word to deploy; after the deploy: link both YouTube channels, add `rpglimitbreak` (announce off), set ESA announce off.
  ↳ **LIVE as v151 09:51** (release `c1b83f0`, deployed commit `c7ef8f1`); the migration RAN at this boot — four ADDED columns, schema **52**, no rebuild. Every after-the-deploy step in the item was done through the site at 09:5x: both YouTube channels linked (GDQ `UCI3DTtB-a3fJPjKtQ5kYHfA`, ESA `UC3Oe-jfrIqEGygxYBYyN6jQ`), `rpglimitbreak` added as a kept channel row with announce off, ESA's announce set off. ⚠️ **But ESA had already been announced and pinned at the boot itself**, before the opt-out could land — the 🔇 **CHANNEL OPT-OUT MUST END AN OPEN SESSION** item on [`TODO.md`](TODO.md), dispatched as branch `channel-optout`.

## 2026-09-20 — v150 (23:32): the previews became the real pages — Posts, Requests and Raid trains, the Discord mock, one end-of-stream box, spotlight ping roles, and every preview deleted

**v150 (release `575dde2`, 7118 tests + 3 skipped, schema 49 → **51**, keys 293 → **294**, routes 192 → **197**, pages 20 → **21**, setting groups 25 → **24**, deploys log **149** lines).** Thirteen builds and a night of the owner's asks in ONE release. Everything below had been merged into `main` and put on the local mock under his standing rule (*"keeping sending builds to local as they finish for me to check, dont stop until everything is done and its just waiting for me to say deploy"*); it all shipped the moment he said *"okay deploy it all, the previews can become the real things on live. also rm the previews pages on live, we dont need them anymore"*. Sweeps **666–718** were numbered at this merge.

- **The real Posts page** (stage 3, Opus 257k, merge `77c9deb`, fourteen deviations, all six §C tests met): the approved preview IS the page, live against the real API — **The posts** (open) with the mode chip, a search box, four chips and **New post** right-aligned, one `button.grid-row` per post (Post · Status · Channel · Last saved), a row opening `openDrawer` with the whole editor, the moves and the shared Versions foldout; **Settings and logs** (shut) with the real `logsSection('posts')`. Both sections `data-span="full"` so `mountColumns` cannot demote the list. Deviation that matters: `saveBar` is NOT used in the drawer (it docks at the page foot behind a modal backdrop) — Save Changes / Discard / *N changes pending* live inside it, and Save with nothing changed is a refusal in words. The static preview and its module are deleted. Sweeps **666–671**.
- **Four small fixes** (Opus 261k, one commit per fix): (1) every page's columns keep **source order** — new pure `columns.js:columnSplit` plus `site/mock/layout.test.mjs` in the gate, so the real Requests page no longer put **Open** in the right column under Settings; (2) the **400 on every page** was not `per_page` — the rail badge asked `/api/requests?status=pending`, a word that route never had; it asks `status=open` now and `per_page` is honoured (`wanted_per_page`); (3) **`deploy.ps1` writes `release.json` AFTER the gates**, right before the push, so a refused gate leaves no stray `Release vN` commit — ⚠️ **measured on THIS deploy**: the release commit `575dde2` (23:31:16) is the tip, after the gate and after the last content commit (23:29:32); (4) **Restart the bot** on the Overview page (`POST /api/bot/restart`, Manage Server, one IMPORTANT `core.restart_requested` row, a clean close then `os._exit(4)` so Fly's `on-failure` policy restarts it; the site goes down with it for ~15 s, because the dashboard is served by the bot process). ⚠️ Nothing in fix 4 has met Fly — the real 15 s is still unmeasured. Sweeps **672–674**.
- **The Discord mock** (Opus 421k, `discord-mock-design.md`, fourteen deviations): `black_bloc/preview.py` with **15 renderers over 20 settings keys**, each calling the bot's OWN render function, behind `POST /api/preview/message` + `GET /api/preview/features`; `assets/discordmock.js` draws Discord's dark-theme chrome (avatar, BOT tag, embed colour bar, author, fields, footer, button rows, mention pills) from a pure `messageTree`. Mounted on the settings ROW, so Go-live, Posts, Birthdays, Modmail, Minutes, Polls and the Settings page all draw it lazily; keys with no renderer correctly draw nothing. 🔴 Sweep `DM-b` (a real go-live beside the mock) is the ONLY row that can prove the mock is honest, and nobody has walked it. Sweeps **675–678**.
- **One box for the end-of-stream wording** (Opus 289k + 255k for §C2, `end-wording-design.md`, eight + six deviations): `golive_end_template` gained `{live}`; `golive_end_suffix` and `golive_end_mode` are RETIRED with a one-shot boot migration. 🔴 **Deviation 1 — the design's *"nobody's wording changes"* was FALSE**: `golive_end_template` already carried a full-rewrite default, so the conductor ruled that **today's wording stays the default**, and the live migration row proves nothing moved (`carried_suffix=False, dropped_mode=edit`). §C2: `golive_live_author` (default = today's words), ONE card *The wording* with a Starting / Ending segment in its head, and `GET /api/golive/preview` deleted with its nine imports, mock route, contract row and eight test call sites. Sweeps **679–684**.
- **A ping role for a spotlight channel** (Opus 500k, `spotlight-pings-design.md`, thirteen deviations): a fan role keyed by `spotlight_id` as well as by member, so GamesDoneQuick can have *GDQ pings*; `spotlight_bump_pings` (default off). ⚠️ **`golive_fan_roles` was REBUILT at this boot, not widened** — SQLite cannot drop NOT NULL from a PK column, so the two live rows were copied out and back; this is why a backup was taken at 23:30. `pings_mode` is still off, so `/pings` cannot list channels yet. Sweeps **685–689**.
- **The drawer became a centred modal**, the editor went two-column, and **Pin it became a switch** (Sonnet 178k + 144k + 192k): CSS only for the modal (`dialog.drawer` centred at `min(64rem, 96vw)`, `90vh`, edge-to-edge under 600 px — `openDrawer` needed no change); `.drawer-body` is a container and `.postgrid` goes two columns at 56 rem (measured 479 px + 479 px side by side); Pin it moved to the foot bar beside Save / Post it and the editor is **not one pixel taller** (921 px before and after). The site-wide audit the owner asked for found **NOTHING to convert** — three dialogs exist site-wide (ask, palette, drawer) and every one measured centred on six pages. Sweeps **690–693**.
- **Events page trims** (Sonnet 249k): the **Remove its room** staff move is gone (the room goes when the event ends or is denied) and the standalone Events forum card folded into Settings — *Make the forum* now sits inside the `events_forum_channel_id` row while the key is blank. Sweeps **694–695**.
- **Raid trains are page 21** (Opus 472k, `raidtrain-page-design.md`, fourteen deviations): `raidtrain.html` under **Runs the cookout**, all six audit tests met — a `button.grid-row` list with the event as a COLUMN, the lineup in the centre modal, *Start a raid train* in the toolbar; the Events page is 211 lines lighter; **Also make an event** goes through the real review path (`events.propose_from`, which `submit_draft` now uses too); `raid_trains.event_id` is **schema 51**; key `raidtrain_event_default`. ⚠️ `raidtrain_mode` is OFF in production and sweep `RT-e` (the Discord doors) is unverified. Sweeps **696–700**.
- **The ping-role modal** (Opus 336k, `ping-role-modal-design.md`, fifteen deviations): `ui.js:askForm` — a dialog that **stays open on a refusal** — with *Make a new role* first, the name pre-filled and editable, a live duplicate warning that greys the confirm, and the server refusing a duplicate in words (`DUPLICATE_ROLE`, proved against the mock); **Rename…** beside Remove. A finding with no production change: the test fake's `FakeRole.delete` never took the role off the guild, and two tests had leaned on it. ⚠️ All five sweep rows **make or rename a REAL Discord role** when run. Sweeps **701–705**.
- **The Requests page** (stage 3, Opus 349k, fourteen deviations, all six tests met): ONE server-filtered list (Request · Who · Status · Assignee · Updated), six counted status chips + *On me* / *Nobody yet* + search + Export CSV + **File a request**, a row opening the centred modal with the card, the legal moves from `TRANSITIONS`, the one promise line and the Notes thread; the member shape is *Your requests*; the static preview deleted; no Python. ⚠️ **Requests are LIVE to members**, so sweep `RQ-b` DMs a real person the moment it runs. It also fixed `page-raidtrain.js:openTrain` dropping `location.search` from the hash. Sweeps **706–711**.
- **A date-time picker everywhere the site takes a date or time** (Opus 207k): one shared `ui.js:whenField` (+ `zoneSelect`, `localWhen`) — **nine fields changed** (raid-train start, the event's Starts with a pickable zone, polls' first slot / time of day / zone, the logs page's from/to, the two preview copies) and **nine left with reasons** (durations; spotlight's *days*, honest only once `POST /spotlight` takes `expires_at`; birthdays cannot be a native date because they carry no year; requests' due box waited for the requests page). No route changed; a past start is still refused in words. Sweeps **712–716**.
- **One `events` settings group, and the Birthday Bot import gone whole** (Opus 274k, eight deviations): `event_panel_minutes` and `event_panel_own_list` fold into `events` through `NAMESPACE_OVERRIDE` (the key NAMES stay, so nothing stored moved) — **24 groups**, and a new guard refuses any singular/plural twin. The Birthday Bot import went WHOLE: the card, `POST /api/birthdays/import`, the parser/matcher, the frozen 2026-08-05 data file **and the daily import loop** (deviation 1: the loop only fed the matcher from a file that can never change, so keeping it would have kept a promise the bot cannot honour); the `birthday.import` kind is removed and old rows still render. Sweeps **717–718**.
- **Rode along:** the events **spacing** fix (Sonnet 141k — it was the REAL page, not the preview: the always-open forum card was paired against the always-collapsed Settings card, 348 px vs 34 px; the forum card now spans full width); the go-live Wording card lost the *Just add the ending instead* button and the *Playing a game* switch (owner: confusing/redundant, and it would never be off); **every static preview, the hub and the `/previews/` redirect are DELETED** — the mock's `/preview/<page>` keeps working as the working tree's real page, so the local lower environment survives its own scaffolding.

⚠️ **What met Discord at this deploy: only the boot.** Two live rows were read afterwards and nothing else in this release has met a real client or a real member — the sweeps are the proof, and **666–718** are all unwalked. ⚠️ **Also NOT verified:** no process has been restarted by pressing **Restart the bot** (nothing has met Fly); `raidtrain_mode` and `pings_mode` are off, so those features are dark in production; the Discord mock's honesty (`DM-b`) is unproven; and no page was opened by a human in a browser after the deploy — the three 200s and the one 404 were fetched, not looked at.

Verified: boot log — `database: rebuilding golive_fan_roles so a role may belong to a channel` and `database: added raid_trains.event_id` (schema **51**; the `post_versions` backfill and `selftest_runs.keep_minutes` had run at earlier boots), every cog loaded, logged in, **no Traceback**; `/health` ready, latency **62 ms**; `/raidtrain.html`, `/requests.html` and `/posts.html` answer **200** on the live site and `/preview/posts.html` answers **404** (the previews are gone); a database backup was taken **23:30, before the deploy** (`~/black-bloc-backups/backup-2026-09-20.sqlite3`, 3,014,656 bytes) because of the `golive_fan_roles` rebuild; the live mirror worktree moved to `575dde2` and the mock restarted; and the two first-ever rows were read on the live Logs page 23:3x — **`golive.boot_swept`**: sessions_kept=0, sessions_closed=0, sessions_dropped=0, swept=True, members_walked=123, members_cached=True, presence_found=0, presence_announced=0, presence_skipped={} (nobody was live at 23:32), and **`golive.end_wording_migrated`**: carried_suffix=False, dropped_mode=`edit` — the live guild had `edit` stored, the default anyway, **so nothing changed for anyone**.

**The twenty-one TODO items, moved whole:**
...
- 🔁 **A "Restart the bot" button on the dashboard** (owner, 2026-09-18 10:4x, verbatim: *"is the website down when the bot is down? id like to have a bot restart button on the dhasboard if not"*). Measured: the dashboard is served BY the bot process (`start_api` runs inside `bot.setup_hook`, one Fly machine, one port), so when the process is down the site is down with it — a button on the dashboard can only reach a bot that is UP but stuck (a dead gateway, a wedged loop), never a crashed one; a crash is Fly's restart policy's job (measured below). Design (small): `POST /api/bot/restart`, Manage Server only, one IMPORTANT `core.restart_requested` row (who), the DND shutdown sentence (v134), a clean `bot.close()`, then the process exits so Fly starts it again; a **Restart the bot** button on the Overview page behind a confirm naming the ~15 s of downtime; refused in words while a deploy is mid-flight if that is detectable. The website-survives-a-crash case is a different build: the dashboard on its own machine + a Fly deploy token — bigger, and the custody item's (3) covers the deploy half. Owner 2026-09-18 13:2x: *"add it to the future todos"* — QUEUED, not now; goes after the Sunday reset and after the strings pass / custody items unless he says otherwise. Measured 13:21: the Fly machine's restart policy is `on-failure`, 10 retries — a crash restarts on its own, a clean exit does NOT, so the button must exit non-zero. **Dispatched 19:4x** as fix 4 of the `small-fixes` branch (Opus, `C:/lcw/bb-small-fixes`) — the owner: *"I dont see why we're waiting for something i just asked for"*. Status: building.
  ↳ **LIVE as v150 23:32** (release `575dde2`).
...
- 🚦 **STANDING FOR THIS STRETCH (owner, 2026-09-20 20:1x, verbatim: *"keeping sending builds to local as they finish for me to check, dont stop until everything is done and its just waiting for me to say deploy"*)** — every landed build is merged into `main` and put on the local mock (`/preview/<page>.html`) for his check; NO deploy until he says so (v150, already in flight, is allowed to land). **On the mock now (merged into `main`, NOT deployed):** the real Posts page (`/preview/posts.html`), the hub at `/preview/`, the go-live card trims, and the four small fixes (`/preview/requests.html` — Open first; `/preview/index.html` — Restart the bot; no 400 in any console). ⚠️ The v150 deploy that was already in flight when the rule arrived STALLED at 99 % for 14 minutes (KI-26, **sixteenth sighting**, three suites on the box) and was killed by its own tree; its stray `Release v150` commit was dropped unpushed; nothing shipped — so the live site is still v149. Builds still out: `spotlight-pings`, `discord-mock`, `end-wording` (+ §C2).
  ↳ **The stretch CLOSED as v150 23:32** (release `575dde2`) — the owner said *"okay deploy it all"* and every build that had been waiting on the mock shipped in one release. The local mock stays a lower environment; the runbook for keeping its mirror honest is [`access/deploy.md`](access/deploy.md) ▸ *After every deploy: move the live mirror*.

- 📌 **POSTS EDITOR — Pin it becomes a switch near the bottom; EVERY side panel on the site becomes a floating centre modal** (owner, 2026-09-20 21:4x, verbatim: *"pin it is in a weird place, its just kind of floating there taking a ton of space up. maybe make it an on off toggle near the bottom. also lets make all left side modals on the whole website floating middle modals."*). Dispatched to Sonnet 21:5x (branch `modal-sweep`): the Pin it checkbox moves to an on/off switch beside the editor's buttons; an audit of every side-docked panel/dialog outside `openDrawer` (which is already centred — the site has only three dialogs: ask, palette, drawer) with each converted to the centred construct. **Landed 21:5x (`01ff1c8`, 192k Sonnet): Pin it is a switch on the foot bar beside Save / Post it (the editor is not one pixel taller — 921 px both before and after; the old control had never matched the switch rule, so it was a plain checkbox); the audit found NOTHING to convert — three dialogs exist site-wide and every one measured centred on six pages. Merged into `main`, on the mock, NOT deployed.**
  ↳ **LIVE as v150 23:32** (release `575dde2`).

- 🏷️ **GO-LIVE ▸ ADD A PING ROLE — make a NAMED role from the modal, and refuse a duplicate name without closing it** (owner, 2026-09-20 21:3x, verbatim: *"in the golive i see the ability to set a ping but not to make a ping role. I should be able to make a role and set it in the golive menu, also if 2 roles have the same name when trying to make a new one give a warning so they know it already exist. dont close the modal in error though just inform the user and dont let them set a duplicate."*). **Designed 21:3x (`info/ping-role-modal-design.md`: a form dialog that stays open on a refusal — `ui.js:askForm` — with Make a new role / Use an existing role, an editable name pre-filled from the template, a live duplicate warning that greys the confirm, and a server-side `DUPLICATE_ROLE` refusal; `name` on `POST /api/pings/streamers`) and dispatched to Opus as branch `ping-role-modal`.** 21:3x, the owner refined it, verbatim: *"i see that i was partially wrong, leaving it blank sets an auto role. how about we don't do that and we have a new role button, the name is automatically generated in an editable text box so the end user can change the role name or hit confirm and take the role name we auto generated"* — which is the design as written (Make a new role, pre-filled editable name, no blank-means-auto path); the brief says so explicitly. 21:5x, the owner (on the mock's OLD modal): *"the ping role thing worked and i was able to set an auto generated one, i wasnt able to edit it on the go live screen where i made it, it should be editable right away"* → a **Rename…** move in the drawer's Ping-role group (same dialog, same duplicate check, `PATCH /api/pings/streamers/…` with `name`, `pings.fan_role_renamed`) added to the same build. **Landed 22:0x (`25f8db1`, 336k Opus, fifteen deviations; a FINDING: the test fake's `FakeRole.delete` never took the role off the guild — two tests had leaned on it, fixed, no production change): `askForm` (a dialog that stays open on a refusal), Make a new role first with the name pre-filled and editable, the live duplicate warning greying the confirm, the server refusing a duplicate in words (proved with the mock refusing for real), **Rename…** beside Remove, the template path reusing a same-named assignable role. Merged into `main`, on the mock, NOT deployed.** Sweeps `PR-a…e` (⚠️ all five make or rename a REAL role when run).
  ↳ **LIVE as v150 23:32** (release `575dde2`).
...
- 🗑️ **THE PREVIEW PAGES ARE GONE** (owner, 2026-09-20 23:1x, verbatim: *"okay deploy it all, the previews can become the real things on live. also rm the previews pages on live, we dont need them anymore"*): the eleven static previews under `site/public/preview/`, the hub, the `/previews/` redirect, `page-preview-*.js`, `page-previews.js` and `preview.js` are deleted on `main`; the mock's `/preview/<page>` keeps working as the working tree's real page (the lower environment stays). Ships in this deploy.
  ↳ **LIVE as v150 23:32** (release `575dde2`).

- 🧭 **ONE `events` SETTINGS GROUP (the stray `event` group folds in) + THE BIRTHDAY BOT IMPORT GOES** (owner, 2026-09-20 22:4x, verbatim: *"in the settings and i think everywhere there are 2 events areas, event and events. find all and combine them. in bvirthdays remove the import from birthday bot, its been kicked"*). Measured: the `event` group is exactly two keys, `event_panel_minutes` and `event_panel_own_list` (the panel-minutes family named its prefix by the command); `logkinds` already maps the `event.` head to the `events` feature, so the Logs page has one chip — the Settings page and the `/settings` group select are where the two groups show. Fix: `NAMESPACE_OVERRIDE` the two keys into `events` (the key NAMES stay, so nothing stored moves), the mock's groups follow, the Settings page shows one group. The Birthday Bot import (a card on the Birthdays page, `POST /api/birthdays/import`, `birthdays.py:parse_birthday_bot_table`, its cog door if any, its tests and mock route) is removed whole. Dispatched to Opus 22:4x; **landed 23:0x (`ef464a7`, 274k, eight deviations): the two keys fold into `events` (24 groups now, a new guard refuses any singular/plural twin), and the Birthday Bot import went WHOLE — the card, the route, the parser/matcher, the frozen 2026-08-05 data file, AND the daily import loop (deviation 1: the loop only fed the matcher from a file that can never change, so keeping it would have kept a promise the bot cannot honour); `birthday.import` kind removed (old rows still render); tests 7118. Merged into `main`, on the mock, NOT deployed — the owner confirms on `/preview/birthdays.html` and `/preview/settings.html`, then the deploy.**
  ↳ **LIVE as v150 23:32** (release `575dde2`).

- 📅 **A DATE-TIME PICKER EVERYWHERE THE SITE TAKES A DATE OR TIME** (owner, 2026-09-20 22:3x, verbatim: *"for raid train and events and anywhere else we set a date time, put a date time picker"*). One shared `ui.js:whenField` (a native `datetime-local` / `date` input + the zone select from `timezone_choices`, answering the same `start`/`tz` (or ISO) the routes already take) replacing every typed date/time box on the site — raid trains (start), events (propose / edit when + finishes), spotlight (until), polls (closes), birthdays (the date), minutes if any — with an audit listing every field found. Dispatched to Opus 22:3x; **landed 22:4x (`5ea8d44`, 207k): `ui.js:whenField` (+ `zoneSelect`, `localWhen`) — nine fields changed (raid-train start, the event's Starts with a pickable zone, polls' first slot / time of day / zone, the logs page's from/to, the two preview copies), nine left with reasons (durations; spotlight's *days* — honest only once `POST /spotlight` takes `expires_at`; birthdays can't be a native date because no-year; requests' due box waits for the requests-page build); no route changed; past starts still refused in words. Fast-forwarded into `main`, on the mock, NOT deployed.** Sweeps `WP-a…e`.
  ↳ **LIVE as v150 23:32** (release `575dde2`).

- 📋 **REQUESTS PAGE — one list with filters, click a row to open it** (owner, 2026-09-20 22:3x, verbatim: *"the request page is too much, we cant see that many request at once, it needs to show a list with filters and then click on one to open it"*). Stage 3 for Requests, the posts/raid-train shape: ONE `button.grid-row` list (Request · Who · Status · Assignee · Updated), status chips (Open · In progress · Ready to check · On hold · Closed · All) + search + On me, **File a request** in the toolbar, a row opens the request in the centre modal (the card, the legal-move buttons from `TRANSITIONS`, the why/hold/decline dialogs, comments, the DM-on-move rule as today); the member shape = the same list scoped to *Your requests*; Settings and logs shut; the static preview deleted when it ships. Dispatched to Opus 22:3x; **landed 22:5x (`bb0b097`, 349k, fourteen deviations, all six audit tests met): one server-filtered list (Request · Who · Status · Assignee · Updated), six counted status chips + On me / Nobody yet + search + Export CSV + File a request, a row opens the centred modal with the card, the legal moves, the one promise line and the Notes thread; the member shape is *Your requests*; the preview deleted; no Python. Merged into `main`, on the mock (`/preview/requests.html`), NOT deployed.** Two findings from it: (a) `page-raidtrain.js:openTrain` dropped `location.search` from the hash — fixed on `main` in the merge commit; (b) ⚠️ HARNESS: an agent whose env still carries `DEV_GUILD_ID` sees seven tests fail (`presence.status_guild` calls `bot.get_guild`, which the test `FakeBot` lacks) — the conductor's cleared-env gate passes them; a one-line `getattr` guard in `status_guild` would make the fixture honest either way (queued small).
  ↳ **LIVE as v150 23:32** (release `575dde2`).

- 🚂 **RAID TRAINS — their own page, and a raid train can make an event through the events set-up** (owner, 2026-09-20 21:1x, verbatim: *"raid train should be in its own page and it should have the option to make an event that ties to the event set up we use."*). Today raid trains are two sections at the foot of the Events page. **Designed 21:2x (`info/raidtrain-page-design.md`: page 21 under Runs the cookout, the two sections lifted off Events, *Also make an event* on Start a raid train → `events.create_event` through the events review path, `raidtrains.event_id` schema 51, key `raidtrain_event_default`, `/raidtrain`'s toggle) and dispatched to Opus as branch `raidtrain-page`.** **Landed 22:1x (`4010b32`, 472k Opus, fourteen deviations; KI-26 sighting twenty-two): page 21 `raidtrain.html` under Runs the cookout (all six audit tests met — a `button.grid-row` list with the event as a COLUMN, the lineup in the centre modal, Start a raid train in the toolbar, Settings and logs shut), the Events page 211 lines lighter with no raid-train section, *Also make an event* → `events.propose_from` (new, and `submit_draft` uses it too) so the event goes through the real review path, `raid_trains.event_id` (schema 50 → **51**), cancel cascading through `cancel_for` (re-read after, checklist 12), `/raidtrain`'s toggle + the card's Make an event, key `raidtrain_event_default`, two kinds. Merged into `main`, on the mock (`/preview/raidtrain.html`), NOT deployed.** ⚠️ `raidtrain_mode` is OFF in production; the migration meets the live database at the deploy; sweep `RT-e` (the Discord doors) is unverified. Mock note: `check.mjs` against the 8797 mirror answers 404 for `/raidtrain.html` because normal URLs there are v149 — not a defect; the deploy gate runs its own mock. 22:2x, the owner (on the v149 events page): *"for events page, have raid train be before settings and logs, its weird its on the bottom"* → moot on the coming version: raid trains left the Events page for their own page; told him.
  ↳ **LIVE as v150 23:32** (release `575dde2`).
