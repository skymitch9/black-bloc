...
> conductor numbers them at the merge. ⚠️ **`EW-b` is the one that could bite:** the end wording a server on DEFAULTS
> posts CHANGES with this branch — see the row, and Deviation 1 in the design. ⚠️ **One older row was CORRECTED in the
...
| **654** (was `BS-c`) | With a session open, have the streamer **stop while the bot is down** (end the stream, then redeploy — or redeploy and stop during it) | After boot the announcement is **edited** — on branch `end-wording` that means the posted sentence with ` — stream ended` on the end (⚠️ the old parenthetical here named `golive_end_mode`, which is retired; there is no off any more). The live role comes off. On the Logs page a `golive.end` row with `reason: reconciled_on_start`. ⚠️ **This is the row that proves the defect is fixed** — before this build nothing closed it until twelve hours had passed |
...
| **`EW-b`** | 🔴 **Read this one before the deploy, not after.** With the end wording box left **untouched**, have somebody stream and stop | The ended post reads **the live announcement with ` — stream ended` on the end** — e.g. *"REGULATORS! Mount up! **Ada** is currently streaming **Celeste**! Check it out: https://… — stream ended"*. ⚠️ **This is NOT what v149 posts.** Today a server on defaults gets a full rewrite — *"**Ada** was streaming **Celeste** — the stream has ended. https://…"* — because `golive_end_template`'s old default was that sentence. The design said the default was "unchanged in effect" and it was wrong; see **Deviation 1**. If the rewrite is the wording you want, paste it back into the box (it has no `{live}`, so it still rewrites) — or tell the conductor before the merge |
| **`EW-c`** | Put **`{live} (over)`** in the box, save, then have somebody stream and stop | The ended post is the posted sentence with ` (over)` on the end, and the **card's footer** reads *Black Bloc · via Twitch · (over)* — the one box drives both halves. Blank the box instead and the post keeps its sentence with **nothing** added and the footer reads *Black Bloc · via Twitch* |
