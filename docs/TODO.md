...
- **Owner 2026-08-31 ~10:13: "read all docs and global rules, lets get started with whats left on our todo list"** → the post-reset resume: audit leftovers **B4–B8** (temp-voice per-room actions, role-menu un-post / seed / staff-assign, event detail/edit — `info/site-feature-audit.md` §2). Status: **Opus builder dispatched 10:20 in a worktree, one commit per item.**
- **Owner 2026-08-31 ~10:16, verbatim: "also read all docs and verify if theyer stale or need updating too"** → full docs staleness audit vs measured repo/deploy state. Status: **Opus audit agent dispatched 10:20; edits land uncommitted for Fable review.**
