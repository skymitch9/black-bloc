...
> local-only until then). **Last verified: 2026-09-22** — item **38** added by the `spotlight-dates` build (a new precondition on a sweep gates the START of an effect, never the whole tick), **38 items**; nothing else on this page was re-checked at that pass. Before that, **2026-09-19** — the item COUNT was re-counted
...
> the code. Before that, **2026-09-17** — item **36** added (every surface a
> member can be on has a `render_again`, and every failure is an `error.*` row), **36 items**; nothing
> else on this page was re-checked at that pass. Before that, **2026-09-11 11:00** — item **35** added (a reversal edits the DECIDED bullet), **35 items**; 08:50 — docs-wide staleness pass.
...
35. **A change that REVERSES a decision edits the DECIDED bullet that recorded it, in the same commit.** A design doc's `DECIDED` / ✅ bullet is the doc a later session reads first; a reversal that lives only in a commit message or a `DONE.md` entry leaves the bullet asserting the opposite of what runs. Strike the old text (`~~…~~`), write the new decision beside it with the date and the commit that reversed it, and leave the reasoning — a bullet that says *why* it flipped is the only thing that stops the third session flipping it back. Traced to: the 2026-09-11 docs pass found fork **I-A2** and the automod / chat / birthdays / role-menus *"does not vanish when off"* bullets still DECIDED the pre-v78 way, eight days after `3429233` (v78, `HIDDEN_WHEN_OFF`) reversed all of them with the reversal recorded only in its commit message; slices D and F annotated them by hand. Not machine-guarded — a reviewer reads the design doc's DECIDED bullets against the diff.

36. **Every surface a member can be on has a `render_again`, so an error offers Try again — and every failure is an `error.*` action-log row.** A panel is built with `Panel(..., again=<a `Move`: (interaction, previous)>)`, a modal borrows its `previous`'s, and `command_errors.render_again_of` finds it; without one the member gets today's plain sentence and a stale card with nothing to press. The re-render goes through the interaction the member was ALREADY on (so `edit_original_response` reaches the card they were looking at, not the error message), which is why `error_retry_minutes` defaults to 10 and is capped at 30 — Discord closes that token at 15. The row half: `record()` writes `error.command` / `error.panel` / `error.modal` / `error.button` with `where` / `error` / `message` / `step` / `interaction`, all four IMPORTANT and headed `core`, and ⚠️ **never a member's words** — no modal field values, no message content, and an `HTTPException` reduced to Discord's own code and text. `record` must never raise; the Python traceback stays as the floor. Traced to: the zone-picker crash of 2026-09-17 17:15 (`panels.opened` deferred twice — fixed in v130), which fired for **every** member whose first Propose had to ask for a time zone: their zone WAS stored and a second Propose would have worked, nobody could know that, most started over, and there was **no row anywhere** for staff to see. Guarded by `tests/test_command_errors.py` and `tests/cogs/community/test_events.py::test_a_zone_picker_crash_offers_try_again_and_the_draft_comes_back_with_the_title`.

37. **A reconcile that POSTS runs under a lock, and re-reads its stored id AFTER taking it — `on_ready` and the loop's first tick must not both post.** A boot starts up to THREE of them (`cog_load` calls `reconcile()` then `_reconcile_loop.start()`; `on_ready` calls it again; the loop's own first tick is a third), so "it re-reads the stored message id before posting" is not enough — two runs can both read *nothing is posted* before either writes. Use `black_bloc/loops.py:Reconciler`: one `asyncio.Lock` per cog, the state read INSIDE it, `skip_if_recent=True` on the `on_ready` path only (the sweep's own tick never skips), and `stamp=False` for a single-guild re-post so it does not close the window on a sweep that never ran. The same rule covers a reconcile that WRITES rather than posts — a doubled row is the same bug with a cheaper symptom. Traced to: **2026-09-18 16:08 Phoenix**, the boot that followed the TEST_MODE lift — `frontdoor.posted` at 23:08:28.529Z *and* 23:08:29.352Z, `modmail.panel_posted` at 23:08:28.110Z *and* 23:08:28.589Z, one `frontdoor.ticket_button_hidden` for the first button only, and **two front doors plus an orphaned ticket button in `#welcome`** that the owner's conductor deleted by hand; the events reconcile was writing two `event.room_forgotten` rows per event from the same race. Guarded by `tests/test_loops.py` (the helper), and by `tests/cogs/community/test_frontdoor.py::test_two_reconciles_at_boot_post_exactly_one_door`, `tests/cogs/moderation/test_modmail.py::test_two_reconciles_at_boot_post_exactly_one_ticket_button` and `tests/cogs/community/test_events.py::test_two_reconciles_at_boot_forget_a_gone_room_once` — each fails with the incident's own doubled rows when the lock is removed. Design: [`front-door-design.md`](front-door-design.md) ▸ Deviations ▸ **14**.

38. **A new precondition on a sweep gates the START of an effect, never the whole tick — an effect already in flight must still be carried to its end.** Adding *"skip this row while X"* to a poll loop is one line and reads obviously correct; what it silently also skips is every LATER stage of something the loop already began — the end, the unpin, the past-tense edit, the cleanup. The row then holds an open session for ever and the post stays pinned in a public channel with nobody to close it. **The rule: put the guard after the state is read, and condition it on there being nothing in flight** (`if session is None and <new condition>: return`), so a row that is mid-effect is carried exactly as it was. Traced to: the `spotlight-dates` build, 2026-09-22 — a scheduled spotlight (its start still ahead) must not be ANNOUNCED, but staff can set a future start on a channel that is live right now, and the first draft's row-level skip would have stranded that announcement posted, pinned and never edited. Guarded by `tests/cogs/content/test_spotlight.py::test_a_start_set_mid_stream_never_strands_the_announcement_that_is_out`. The same shape is waiting in every other sweep that grew a condition: an opt-out, a mode going `off`, a pause switch. Item 3 is its cousin — *reversal must not depend on the mode at reversal time* — and this is the version for a loop rather than a stored flag.
