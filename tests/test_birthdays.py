...
async def test_the_zone_lookup_works_with_or_without_phase_four(db):
    assert await timezone_table_exists(db) is False
    ...
    assert await timezone_table_exists(db) is True
...
async def test_a_database_that_cannot_answer_still_gives_a_zone():
    ...
    assert await timezone_table_exists(Broken()) is False
